...
# تثبيت curl و lsb-release و gpg
RUN apt-get update && \
...
# تشغيل WARP مع قبول الشروط ثم تشغيل البوت
CMD warp-svc --accept-tos & sleep 3 && warp-cli register --accept-tos && warp-cli connect && python app.py