...
# تثبيت الاعتماديات الأساسية (curl, lsb-release, gpg, bsdutils)
RUN apt-get update && \
    apt-get install -y curl lsb-release gpg bsdutils
...
# ✅ الأمر النهائي الذي يعمل تلقائياً (بدون تدخل يدوي)
CMD warp-svc --accept-tos & sleep 5 && \
    script -q -c 'warp-cli registration new' /dev/null <<< $'y\n' && \
    sleep 2 && \
    warp-cli connect && \
    python app.py