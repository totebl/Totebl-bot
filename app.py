...
# خيط auto_recovery خفيف (بدون إعادة تحميل الأسواق) مع إضافة القفل المفقود
# تعديل مهلة جلب الرصيد قبل التحليل: max_retries=5, delay=5 (25 ثانية كحد أقصى)
# ========================================================================
...
CUSTOM_POSITION_PERCENT_NORMAL = 0.25   # قيمة افتراضية جديدة
...
def close_partial(symbol, percent, price, reason, pnl_usdt=None):
    ...
    with _global_state_lock:
        ...
        pos._closing = True
    try:
        # التحقق من min_cost لتجنب البيع الجزئي الصغير جداً
        ...
        if part_value < min_cost and percent < 1.0:
    ...
    try:
        ...
        if pending:
            ...
            for opp in pending:
                ...
                # التصحيح الأساسي: استخدم get_total_equity() مباشرة
