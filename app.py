...
# سيتم تعيين النماذج ديناميكياً
GEMINI_MODEL_FLASH = None
GEMINI_MODEL_FLASH_LITE = None
...
# دوال البحث عن أحدث نماذج Gemini (تم إضافتها)
def get_latest_flash_model():
    ...
    try:
        ...
        flash_models = [m.name for m in models if m.name.startswith('models/gemini-') and 'flash' in m.name.lower() and 'lite' not in m.name.lower()]
        if flash_models:
            flash_models.sort(reverse=True)
            return flash_models[0].replace('models/', '')
    except Exception as e:
        logger.error(f"خطأ في البحث عن نموذج Flash: {e}")
    return None

def get_latest_flash_lite_model():
    if not GEMINI_API_KEY or genai is None:
        return None
    try:
        genai.configure(api_key=GEMINI_API_KEY)
        models = genai.list_models()
        lite_models = [m.name for m in models if m.name.startswith('models/gemini-') and 'flash-lite' in m.name.lower()]
        if lite_models:
            lite_models.sort(reverse=True)
            return lite_models[0].replace('models/', '')
    except Exception as e:
        logger.error(f"خطأ في البحث عن نموذج Flash-Lite: {e}")
    return None

def refresh_gemini_models():
    global GEMINI_MODEL_FLASH, GEMINI_MODEL_FLASH_LITE
    GEMINI_MODEL_FLASH = get_latest_flash_model()
    GEMINI_MODEL_FLASH_LITE = get_latest_flash_lite_model()
    if GEMINI_MODEL_FLASH:
        logger.info(f"✅ تم العثور على نموذج Flash: {GEMINI_MODEL_FLASH}")
    else:
        logger.warning("⚠️ لم يتم العثور على نموذج Flash")
    if GEMINI_MODEL_FLASH_LITE:
        logger.info(f"✅ تم العثور على نموذج Flash-Lite: {GEMINI_MODEL_FLASH_LITE}")
    else:
        logger.warning("⚠️ لم يتم العثور على نموذج Flash-Lite")
...
# --------------------------- دالة اختبار مفتاح Gemini (محسنة) ---------------------------
def test_gemini_api_key():
    ...
    try:
        ...
        flash_ok = any(m.endswith(GEMINI_MODEL_FLASH) for m in model_names) if GEMINI_MODEL_FLASH else False
        lite_ok = any(m.endswith(GEMINI_MODEL_FLASH_LITE) for m in model_names) if GEMINI_MODEL_FLASH_LITE else False
...
# --------------------------- دالة التحقق من توفر نموذج معين ---------------------------
...
def _ask_ai_internal(symbol, df, current_price, sym_type, market_condition, ticker_data, threshold, desc):
    ...
    # تم حذف ذكر market_condition من النص
...
السؤال: بناءً على هذه البيانات، هل تنصح بشراء هذا الرمز الآن؟ أجب فقط بـ "نعم" أو "لا" ثم سبب مختصر جداً."""

    # استخدام النماذج الديناميكية
    if CURRENT_AI_MODEL == 'flash':
        model_name = GEMINI_MODEL_FLASH
    else:
        model_name = GEMINI_MODEL_FLASH_LITE
    if model_name is None:
        logger.error(f"❌ النموذج المطلوب ({CURRENT_AI_MODEL}) غير متاح. استخدم الفلاش باك.")
        model_name = GEMINI_MODEL_FLASH or GEMINI_MODEL_FLASH_LITE
        if model_name is None:
            logger.error("لا يوجد نموذج Gemini متاح على الإطلاق.")
            return AI_FALLBACK_ACTION == 'allow'

    # تم حذف السطر genai.configure لأنه تم تكوينه مسبقاً
    model = genai.GenerativeModel(model_name)

    # دالة الاستدعاء الفعلي
    def call_gemini():
    ...
    try:
        ...
        # الأخطاء المؤقتة: إعادة محاولة واحدة بعد 30 ثانية
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                                ...
                                if is_model_available(GEMINI_MODEL_FLASH_LITE):
                                    ...
                                    save_state()
                                    send_telegram(f"✅ تم التبديل إلى نموذج **Gemini Flash-Lite** ({GEMINI_MODEL_FLASH_LITE}).")
                                else:
