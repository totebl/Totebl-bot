...
# --------------------------- لجنة الشراء (محسّنة مع العضو الرابع والخامس) - تم تعديل الأوزان (التغيير الرابع) ---------------------------
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        # ============================================================
        # 1. التقطيع الثلاثي الديناميكي (يعتمد على العدد المتوفر)
        # ============================================================
        total_len = len(df_primary)
        
        if total_len < 20:
            # إذا كان العدد قليلاً جداً، نستخدم البيانات كاملة لجميع النوافذ
            df_full = df_primary
            df_half = df_primary
            df_quarter = df_primary
        else:
            df_full = df_primary
            n_half = max(20, total_len // 2)
            n_quarter = max(20, total_len // 4)
            df_half = df_primary.iloc[-n_half:]
            df_quarter = df_primary.iloc[-n_quarter:]

        # ============================================================
        # 2. دالة مساعدة لحساب درجات النماذج الخمسة على نافذة معينة
        # ============================================================
        def _get_scores(df):
            scores = {}
            # Rule
        ...
        # ============================================================
        # 3. حساب الدرجات على النوافذ الثلاث
        # ============================================================
        ...
        # ============================================================
        # 4. حساب المتوسط الحسابي لكل نموذج عبر النوافذ الثلاث
        # ============================================================
        avg_scores = {}
        ...
        # ============================================================
        # 🆕 التحقق من عتبة مجموع النموذجين (Rule+EMR)
        # ============================================================
        global RULE_EMR_SUM_THRESHOLD
        rule_score = avg_scores.get('rule', 0.5)
        emr_score = avg_scores.get('emr', 0.5)
        sum_rule_emr = rule_score + emr_score
        if sum_rule_emr > RULE_EMR_SUM_THRESHOLD:
            return 'neutral', 0, 0, f"❌ رفض: مجموع (Rule+EMR) = {sum_rule_emr:.2f} > {RULE_EMR_SUM_THRESHOLD:.2f} (عتبة الرفض)"

        # ============================================================
        # 5. جلب البيانات التأكيدية (5 دقائق و 1 ساعة) لحساب المضاعف
        # ============================================================
        ...
        if not df_confirm_1.empty and len(df_confirm_1) >= 20:
            emr_res_15m = self.models['emr'].update(symbol, df_confirm_1)
            if emr_res_15m:
                confirm_score_15m = emr_res_15m.get('buy', 0.5)
        ...
        if not df_confirm_2.empty and len(df_confirm_2) >= 20:
            emr_res_1h = self.models['emr'].update(symbol, df_confirm_2)
            if emr_res_1h:
                confirm_score_1h = emr_res_1h.get('buy', 0.5)
        ...
        # ============================================================
        # 6. تطبيق المضاعف على المتوسطات وتجميع القيم
        # ============================================================
        raw_values = [avg_scores['rule'], avg_scores['emr'], avg_scores['cfhm'], avg_scores['timing'], avg_scores['vwap_obv']]
        valid = [v for v in raw_values if v is not None]
        if not valid:
            return 'neutral', 0.0, 0.0, "⚠️ بيانات غير كافية من النماذج"

        adjusted = [max(0.0, min(1.0, v * multiplier)) for v in valid]

        # ============================================================
        # 7. الأوزان الجديدة (التغيير الرابع: خفض EMR إلى 15% ورفع CFHM إلى 40%)
        # ============================================================
        # تم التعديل: خفض rule إلى 15% ورفع timing إلى 15%
        if sym_type == 'meme':
        ...
        # ============================================================
        # 8. حساب المتوسط المرجح النهائي
        # ============================================================
        weighted_avg = 0.0
        weight_sum = 0.0
        for i, model_key in enumerate(['rule', 'emr', 'cfhm', 'timing', 'vwap_obv']):
            if i < len(adjusted):
                weighted_avg += adjusted[i] * weights[model_key]
                weight_sum += weights[model_key]
        if weight_sum > 0:
            avg = weighted_avg / weight_sum
        else:
            avg = sum(adjusted) / len(adjusted)

        # ============================================================
        # 9. العتبة والحد الأدنى للثقة
        # ============================================================
        thresh = cfg.get('thresh1', 0.45)
        min_conf = cfg['min_conf']

        # ============================================================
        # 10. بناء سبب القبول (يظهر الدرجات الخمس بعد تطبيق المضاعف)
        # ============================================================
        reason = (f"المتوسط={avg:.3f} | العتبة={thresh:.3f} | المضاعف={multiplier:.2f}x | "
                  f"الدرجات (متوسط 3 نوافذ): {', '.join([f'{v:.3f}' for v in adjusted])}")

        # ============================================================
        # 11. القرار النهائي
        # ============================================================
        if avg > thresh and avg >= min_conf:
            return 'buy', avg, avg, f"✅ موافقة: {reason}"
        else:
            if avg < min_conf:
            ...
            return 'neutral', avg, avg, f"❌ رفض: {main_reason} | {reason}"
...
def analyze_and_trade():
    ...
    global PAUSE_NEW_ENTRIES   # استخدم المتغير العام
    ...
    try:
        ...
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            if FILTER_MANIPULATION_ENABLED:
                ...
                if is_manipulated:
                    ...
                    continue
            dec, score, conf, reason = buy_comm.decide(df_primary, sym, sym_type, market_condition=market_cond)
            if dec != 'buy':
                # سجل الرفض (اختياري للإرسال)
                rejection_log.append((sym, reason, score))
                continue
            ...
            pending.append({'symbol': sym, 'score': extra, 'price': cur_price, 'atr': atr_val,
                            ...
                            'reason': reason})  # تخزين سبب القبول لاستخدامه لاحقاً
        if pending:
            ...
            if PAUSE_NEW_ENTRIES:
                ...
                # ننهي الدالة دون تنفيذ أي صفقة جديدة
                return
        ...
        # (اختياري) يمكن إرسال ملخص الرفض في نهاية الدورة، لكن حسب رغبتنا لا نرسل شيئاً عن الرفض.
    except TimeoutError:
...
# --------------------------- دالة telegram_polling المعدلة (مع إصلاح global) ---------------------------
def telegram_polling():
    global RULE_EMR_SUM_THRESHOLD  # <--- أول تصريح global
    global _last_telegram_update_id
                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ============================================================
                            # 🆕 16. أمر تعيين عتبة مجموع النموذجين (Rule+EMR)
                            # ============================================================
                                ...
                                if match:
                                    ...
                                    if 0.0 <= new_threshold <= 2.0:
                                        # لا حاجة لـ global هنا لأن التصريح العام في أعلى الدالة كافٍ
    ...
    try:
        ...
        data = {
            ...
            'filters': {
                ...
                'rule_emr_sum_threshold': RULE_EMR_SUM_THRESHOLD  # 🆕 حفظ العتبة الجديدة
            },
            'custom_settings': {
...
def load_state():
    ...
    global PAUSE_NEW_ENTRIES
    global COOLDOWN_WIN_HOURS, COOLDOWN_LOSS_MINUTES, _symbol_cooldown_until
    global RULE_EMR_SUM_THRESHOLD  # 🆕 استعادة العتبة الجديدة
