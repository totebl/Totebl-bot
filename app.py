...
# - إضافة أمر "رصيدي" لعرض الرصيد الحر + قيمة المراكز
# - تقرير يومي يعرض صافي الربح/الخسارة (net) بدلاً من الخسارة المطلقة
# - DAILY_LOSS_MODE = 'net' افتراضياً
# - إصلاح خطأ "فشل تحديث الوقف المسبق" في وضع المحاكاة الورقية
# - جميع الفلاتر السابقة: التلاعب، السيولة، القيمة السوقية، حجم 24h، تغير 24h، شمعة الساعة، شمعة 4 ساعات
...
DAILY_LOSS_MODE = os.environ.get('DAILY_LOSS_MODE', 'net')   # تغيير الوضع الافتراضي إلى net
...
# التعديل المطلوب: إضافة recvWindow و adjustForTimeDifference
...
def update_preemptive_stop_loss(symbol, pos, new_stop_price):
    # منع محاولة تحديث أوامر الوقف في وضع المحاكاة لتجنب رسائل الخطأ
    if _should_simulate():
        return
...
# --------------------------- لجنة الشراء (مع دعم التعديل الديناميكي) ---------------------------
class BuyingCommittee:
    ...
    def apply_multiplier(self, percent_multiplier):
        """تطبيق مضاعف على جميع العتبات (percent_multiplier: 1.0 = 100%)"""
