...
#   يضمن أن المرشحين المجمّعين يُشترون حتى لو انتهى التحليل بالتايم آوت
# ========================================================================
...
# ==========================================================================
MAX_UPPER_WICK_DISTANCE_PERCENT = float(os.environ.get('MAX_UPPER_WICK_DISTANCE_PERCENT', 0.0044))  # 0.44%
...
LIMIT_TO_MARKET_TIMEOUT = int(os.environ.get('LIMIT_TO_MARKET_TIMEOUT', 45))  # زيد إلى 45 ثانية
...
def analyze_and_trade():
    ...
    try:
        ...
        for sym in active:
            # 🆕 فحص وقت الدورة - إنهاء تعاوني (مرحلة التحليل فقط)
            if time.time() - cycle_start_time > MAX_ANALYSIS_CYCLE_SECONDS:
                logger.warning(f"⏱️ الدورة تجاوزت {MAX_ANALYSIS_CYCLE_SECONDS}s - إنهاء تعاوني (فحص {analyzed_count} رمز، {len(pending)} مرشح)")
                break
        ...
        if pending:
            ...
            for opp in pending:
                # 🆕 فحص وقت التنفيذ - فترة سماح منفصلة عن التحليل (MAX_ANALYSIS + EXECUTION_GRACE)
                max_total_cycle = MAX_ANALYSIS_CYCLE_SECONDS + EXECUTION_GRACE_SECONDS
                if time.time() - cycle_start_time > max_total_cycle:
                    logger.warning(f"⏱️ فترة التنفيذ تجاوزت {EXECUTION_GRACE_SECONDS}s - توقف عن معالجة المرشحين المتبقيين")
                    send_telegram(f"⏱️ تم إيقاف التنفيذ (تجاوز فترة السماح) - {len(pending)} مرشح، الباقي في الدورة القادمة")
                    break
                with _global_state_lock:
                ...
                # ⭐⭐⭐ فلتر الذيل العلوي - طلب لحظي قبل التنفيذ ⭐⭐⭐
                wick_ok, wick_msg, wick_distance = check_upper_wick_filter(opp['symbol'])
                if not wick_ok:
                    logger.info(f"⏭️ رفض نهائي {opp['symbol']}: {wick_msg}")
                    send_telegram(f"⏭️ رفض نهائي {opp['symbol']} (ذيل علوي لحظي): {wick_msg}")
                    continue
                # ⭐ تخزين المسافة في opp لتُمرَّر إلى execute_full_trade
                opp['upper_wick_distance'] = wick_distance
                # ⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐
                ...
                succ, actual_size = execute_full_trade(opp['symbol'], 'buy', sz, opp['price'], opp['atr'],
                                                       ...
                                                       upper_wick_distance=opp.get('upper_wick_distance'))
                if succ:
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ========== 🆕 أوامر فلتر الذيل العلوي ==========
                                ...
                                if match:
                                    ...
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين مسافة الذيل العلوي 0.44%`")
                                ...
                                send_telegram(
                                    ...
                                    f"⚙️ للتحكم:\n"
                                    f"• `تعيين مسافة الذيل العلوي 0.44%`\n"
                            ...
                            # ========== أمر إعادة الضبط ==========
                                ...
                                UPPER_WICK_FILTER_ENABLED = True
                                MAX_UPPER_WICK_DISTANCE_PERCENT = 0.0044
...
def load_state():
    ...
    try:
        ...
        MAX_UPPER_WICK_DISTANCE_PERCENT = filters.get('max_upper_wick_distance_percent', 0.0044)
...
if __name__ == '__main__':
    ...
    send_telegram(f"🚀 بوت v29.0 (النسخة المبسطة بعد التنظيف + التحديثات) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"🆕 تحديثات جديدة:\n"
                  f"   • أوامر Market فورية للخسائر والطوارئ\n"
                  f"   • Trailing Stop مخصص (تفعيل عند +2.2% | مسافة 1.4%)\n"
                  f"   • فحص فوري عند أول كسر لعتبة التتبع\n"
                  f"   • فحص الغبار وحذفه تلقائياً (< 5$)\n"
                  f"   • أمر `مزامنة` للمزامنة مع Binance\n"
                  f"   • Throttling لمنع حظر API\n"
                  f"   • 🪶 فلتر الذيل العلوي للشمعة الحالية 15د (طلب مستقل لحظي)\n"
                  f"   • 🪶 إظهار مسافة الذيل العلوي في رسالة الشراء (موجب/صفر/سالب)\n"
                  f"   • ⏱️ Cooperative Timeout: التحليل يُنهي نفسه بعد {MAX_ANALYSIS_CYCLE_SECONDS}s\n"
                  f"   • ➕ Execution Grace: فترة سماح {EXECUTION_GRACE_SECONDS}s للتنفيذ بعد التحليل\n"
                  f"   • 🧮 الميزانية الكلية للدورة: {MAX_ANALYSIS_CYCLE_SECONDS + EXECUTION_GRACE_SECONDS}s (آمن ضد التحرير القسري)\n"
                  f"   • 📊 إشعار تلغرام بعدد الرموز المفحوصة والناجحة ومدة الدورة\n"
                  f"   • ⚙️ أمر تلغرام للتحكم بمدة الدورة: `تعيين وقت انتهاء الدورة 180` (حتى 230s)")
