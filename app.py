                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                            # 16. أمر تعيين عتبة بيع 5 دقائق
                            # ------------------------------------------------------------
                            elif text.startswith('تعيين عتبة بيع 5 دقائق'):
                                match = re.search(r'(\d+(?:\.\d+)?)', text)
                                if match:
                                    new_val = float(match.group(1))
                            ...
                            # ------------------------------------------------------------
                                ...
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (الذكاء الاصطناعي معطل، النموذج الافتراضي Flash، نظام التبريد الذكي مفعّل بـ 12 ساعة للرابح و 5 دقائق للخاسر، وعتبات النموذجين: الأعلى 1.5 والأدنى 1.3، وتحديث الوقف مفعّل، نسبة تتبع الأرباح 1%، عتبة بيع 5 دقائق 0.40)")
                                save_state()
...
def load_state():
    ...
    try:
        ...
        # تم حذف قراءة SELL_DECISION_THRESHOLD
