# ========================================================================
# بوت التداول v28.1 – النسخة الذهبية مع الفلتر الكمي (MTF) و WebSocket المتقدم
# تم تعديله لإصلاح مشكلة توقف الفرص بعد عدة دورات
# التغييرات: إزالة حاجز ws_manager.initialized، إعادة استخدام get_cached_features،
# تفعيل sanitize_dataframe، زيادة TOP_CANDIDATES_COUNT، إصلاح last_access_time،
# زيادة MAX_WS_SYMBOLS، تعديل شرط التيكرات، تحسين get_cached_all_tickers،
# تقليل فاصل sync_preemptive_stop_orders، تحسين _safe_send، إصلاح التسخين الأولي،
# إصلاح آلية القفل (العودة إلى منطق v27.8)
# ========================================================================

import subprocess, sys, os, logging, time, warnings, threading, atexit, signal, importlib, re, traceback, io, random
...
# تثبيت مكتبة Google Gemini تلقائياً
try:
...
TOP_CANDIDATES_COUNT = 100  # تم التعديل من 30 إلى 100
...
# --------------------------- إعدادات Google Gemini (معدلة) ---------------------------
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY')
_detected_model = None  # سيتم اكتشافه تلقائياً
AI_ENABLED = os.environ.get('AI_ENABLED', 'false').lower() == 'true'
CURRENT_AI_MODEL = os.environ.get('CURRENT_AI_MODEL', 'auto')  # 'auto' للاكتشاف التلقائي
AI_TIMEOUT = 65
AI_FALLBACK_ACTION = os.environ.get('AI_FALLBACK_ACTION', 'allow')
AI_NOTIFICATION_COOLDOWN = 60
_last_ai_notification_time = 0
...
# --------------------------- متغيرات الوقف المسبق ---------------------------
PREPLACED_SL_TRIGGER_PERCENT = 0.017
PREPLACED_SL_LIMIT_PERCENT = 0.021
...
_ohlcv_cache_max = 100
_features_cache = OrderedDict()
_features_cache_max = 150
...
POSITION_MONITOR_INTERVAL = float(os.environ.get('POSITION_MONITOR_INTERVAL', 2.0))

# تعريف ws_manager كمتغير عام (في منتصف الكود، وليس في الأعلى)
ws_manager = None

# ========================== متغير فلتر الكم (MTF) ==========================
QUANTUM_THRESHOLD = float(os.environ.get('QUANTUM_THRESHOLD', 0.55))
FILTER_QUANTUM_ENABLED = os.environ.get('FILTER_QUANTUM_ENABLED', 'true').lower() == 'true'
...
# ========================== دالة تعقيم البيانات (معدلة: تم تفعيلها) ==========================
def sanitize_dataframe(df, required_cols=None):
    """تنظيف البيانات من القيم غير الصالحة (NaN, Inf)"""
    if df is None or df.empty:
        return None
    df = df.copy()
    # استبدال القيم اللانهائية بـ NaN
    df = df.replace([np.inf, -np.inf], np.nan)
    # تعبئة القيم المفقودة للأمام والخلف
    df = df.ffill().bfill()
    # إذا بقيت قيم NaN، استخدم interpolation
    df = df.interpolate(method='linear', limit_direction='both', limit_area='inside')
    # التحقق من وجود الأعمدة المطلوبة
    if required_cols:
        missing = [col for col in required_cols if col not in df.columns]
        if missing:
            logger.warning(f"أعمدة مفقودة: {missing}")
            return None
    return df
...
# ========================== كلاس Position (معدل لتأجيل حساب الزخم) ==========================
class Position:
    def __init__(self, symbol, side, size, entry, atr, sl, tp, sym_type, pred, conf, regime, tp_levels=None, ai_approved=False):
        ...
        self.momentum_calculated = False
    ...
    def _calc_initial_momentum(self):
        ...
        try:
            if ws_manager is not None:
                df = ws_manager.get_klines(self.symbol, '5m', limit=200)
                ...
                if df is not None and not df.empty and len(df) >= 15:
                    ...
                    self.momentum_calculated = True
        except Exception as e:
            logger.debug(f"تأجيل حساب الزخم لـ {self.symbol}: {e}")

    def calculate_momentum_later(self):
        if self.momentum_calculated:
            return
        try:
            if ws_manager is not None:
                df = ws_manager.get_klines(self.symbol, '5m', limit=200)
                ...
                if df is not None and not df.empty and len(df) >= 15:
                    ...
                    self.entry_momentum_time = datetime.now()
                    self.momentum_calculated = True
                    logger.debug(f"✅ تم حساب زخم {self.symbol} بتأخير")
        except Exception as e:
            logger.debug(f"فشل حساب الزخم المتأخر لـ {self.symbol}: {e}")
...
def _safe_fetch_balance_after_trade(attempts=15, delay=5.0, silent=True):
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            return bot_stats.last_balance
...
# ========================================================================
# AdvancedWebSocketManager – الإصدار الذهبي (يدعم التيكرات والشموع والدفتر والصفقات)
# ========================================================================
class AdvancedWebSocketManager:
    def __init__(self):
        ...
        # للشموع - المفتاح سيكون f"{symbol}_{timeframe}"
        self.klines_cache = {}
        ...
        self.max_symbols = 300  # تم التعديل من 100 إلى 300
        self.last_access_time = {}
        self.recently_removed = {}
        self.all_tickers_cache = {}
        ...
        self._last_fast_cleanup = 0
        self._last_medium_cleanup = 0
        self._last_slow_cleanup = 0

    def _get_next_id(self):
        self._id_counter += 1
        return self._id_counter

    def _fetch_rest_klines(self, symbol, timeframe='5m', limit=200):
        exchange = get_active_exchange()
        for attempt in range(3):
            try:
                ...
                logger.warning(f"REST جلب {symbol} {timeframe} محاولة {attempt+1} فشلت: {e}")
                time.sleep(2 ** attempt)
        return pd.DataFrame()

    def _safe_send(self, message):
        """إرسال رسالة WebSocket مع إعادة محاولة تلقائية"""
        with self.send_lock:
    ...
    def _safe_send_with_retry(self, message, max_retries=2, delay=1):
        """إرسال رسالة مع إعادة محاولة في حالة الفشل"""
        for attempt in range(max_retries):
            if self._safe_send(message):
                return True
            if attempt < max_retries - 1:
                logger.warning(f"⚠️ إعادة محاولة إرسال رسالة WebSocket (محاولة {attempt+2}/{max_retries})...")
                time.sleep(delay)
        logger.error(f"❌ فشل إرسال رسالة WebSocket بعد {max_retries} محاولات")
        return False

    def _initialize_or_repair(self, symbol, timeframe='5m', limit=200):
        try:
            logger.info(f"🔄 تهيئة/إصلاح {symbol} {timeframe} من REST...")
            df = self._fetch_rest_klines(symbol, timeframe, limit)
            if df is not None and not df.empty:
                cache_key = f"{symbol}_{timeframe}"
                with self.klines_lock:
                    self.klines_cache[cache_key] = df
                if timeframe == '5m' and len(df) >= 50:
                    self._update_features_async(symbol, df)
                return df
        except Exception as e:
            logger.error(f"فشل تهيئة {symbol} {timeframe}: {e}")
        return pd.DataFrame()

    def _check_and_fill_gap(self, symbol, timeframe='5m'):
        cache_key = f"{symbol}_{timeframe}"
        with self.klines_lock:
            df = self.klines_cache.get(cache_key)
            if df is None or df.empty:
                return
            last_time = df.index[-1]
            now = datetime.now(timezone.utc)
            if timeframe == '5m' and (now - last_time).total_seconds() > 360:
                logger.warning(f"⚠️ فجوة زمنية لـ {symbol} {timeframe}. جاري الإصلاح...")
                time.sleep(random.uniform(0.5, 3.0))
                missing = self._fetch_rest_klines(symbol, timeframe, limit=10)
                if missing is not None and not missing.empty:
                    df = pd.concat([df, missing])
                    df = df[~df.index.duplicated(keep='last')]
                    if len(df) > 200:
                        df = df.iloc[-200:]
                    df = sanitize_dataframe(df) or df
                    self.klines_cache[cache_key] = df
                    if timeframe == '5m':
                        self._update_features_async(symbol, df)
                    logger.info(f"✅ تم إصلاح فجوة {symbol} {timeframe}")

    def _update_features_async(self, symbol, df):
        def _update():
            try:
                if df is not None and not df.empty and len(df) >= 50:
                    df_copy = df.copy(deep=True)
                    df_copy = add_advanced_features(df_copy)
                    df_copy = sanitize_dataframe(df_copy) or df_copy
                    with self.klines_lock:
                        cache_key = f"{symbol}_5m"
                        self.klines_cache[cache_key] = df_copy
                        logger.debug(f"📊 تحديث مؤشرات {symbol} في الخلفية")
            except Exception as e:
                logger.error(f"فشل تحديث مؤشرات {symbol}: {e}")
    ...
    def _gap_checker(self):
        while self.running:
            ...
            time.sleep(60)
            with self.klines_lock:
                keys = list(self.klines_cache.keys())
            for key in keys:
                if '_' in key:
                    sym, tf = key.rsplit('_', 1)
                    if tf in ['5m', '1h', '4h']:
                        self._check_and_fill_gap(sym, tf)
                time.sleep(0.1)
            self._perform_cleanup(now)

    def _perform_cleanup(self, now):
        # كل 5 دقائق: تنظيف الصفقات والكاشات الاحتياطية
        if now - self._last_fast_cleanup > 300:
            self.cleanup_all_caches(
                max_trades=100,
                max_ohlcv=80,
                max_features=120,
                max_manipulation=200,
                max_market_cap=200
            )
            self._last_fast_cleanup = now

        # كل ساعة: تنظيف الشموع فقط
        if now - self._last_slow_cleanup > 3600:
            self.cleanup_all_caches(max_klines=200)
            self._last_slow_cleanup = now

    def cleanup_all_caches(self, max_klines=200, max_trades=100, max_ohlcv=80,
                           max_features=120, max_manipulation=200, max_market_cap=200):
        """تنظيف شامل حسب طبيعة كل كاش."""
        # الشموع
        with self.klines_lock:
            for key, df in list(self.klines_cache.items()):
                if len(df) > max_klines:
                    self.klines_cache[key] = df.iloc[-max_klines:].copy()

        # الصفقات
        with self.trades_lock:
            for symbol, entry in list(self.trades_cache.items()):
                if len(entry['trades']) > max_trades:
                    entry['trades'] = entry['trades'][-max_trades:]

        # كاش التلاعب
        with _cache_lock:
            if len(_manipulation_cache) > max_manipulation:
                items = sorted(_manipulation_cache.items(), key=lambda x: x[1]['timestamp'])
                for key, _ in items[:len(_manipulation_cache) - max_manipulation]:
                    del _manipulation_cache[key]

        # كاش القيمة السوقية
        with _market_cap_cache_lock:
            if len(_market_cap_cache) > max_market_cap:
                items = sorted(_market_cap_cache.items(), key=lambda x: x[1]['timestamp'])
                for coin, _ in items[:len(_market_cap_cache) - max_market_cap]:
                    del _market_cap_cache[coin]

        # كاش OHLCV
        with _cache_lock:
            if len(_ohlcv_cache) > max_ohlcv:
                items = list(_ohlcv_cache.items())
                items.sort(key=lambda x: x[1]['timestamp'])
                for key, _ in items[:len(_ohlcv_cache) - max_ohlcv]:
                    del _ohlcv_cache[key]

        # كاش المؤشرات
        with _features_cache_lock:
            if len(_features_cache) > max_features:
                items = list(_features_cache.items())
                items.sort(key=lambda x: x[1]['timestamp'])
                for key, _ in items[:len(_features_cache) - max_features]:
                    del _features_cache[key]

    def on_message(self, ws, message):
        if not self.initialized:
            self.initialized = True
            logger.info("✅ WebSocket جاهز (تم استلام أول رسالة)")
        try:
            data = json.loads(message)

            if 'stream' not in data and isinstance(data, list):
            ...
            if 'stream' in data and 'data' in data:
                ...
                if '@kline' in stream:
                    ...
                    symbol = symbol_raw.replace('USDT', '/USDT')
                    kline = data['data']['k']
                    tf = parts[1].replace('kline_', '') if len(parts) > 1 else '5m'
                    if tf not in ['5m', '1h', '4h']:
                        return
                    ...
                    with self.klines_lock:
                        df = self.klines_cache.get(cache_key)
                        if df is None:
                            df = self._fetch_rest_klines(symbol, tf, limit=200)
                            if df is None or df.empty:
                                return
                            self.klines_cache[cache_key] = df

                        last_time = df.index[-1].timestamp() * 1000
                        if kline['t'] <= last_time:
                            return
                        ...
                        if kline['x']:
                            ...
                            if tf == '5m':
                                self._update_features_async(symbol, df)
                            logger.debug(f"📊 شمعة جديدة مغلقة لـ {symbol} {tf}")
                    ...
                    logger.debug(f"📘 تحديث دفتر {symbol}")
                    ...
                    with self.trades_lock:
                        if symbol not in self.trades_cache:
                            self.trades_cache[symbol] = {'trades': [], 'timestamp': time.time()}
                        if isinstance(trade_data, list):
                            self.trades_cache[symbol]['trades'].extend(trade_data)
                        else:
                            self.trades_cache[symbol]['trades'].append(trade_data)
                        if len(self.trades_cache[symbol]['trades']) > 100:
                            self.trades_cache[symbol]['trades'] = self.trades_cache[symbol]['trades'][-100:]
                        self.trades_cache[symbol]['timestamp'] = time.time()
                    logger.debug(f"📊 تحديث صفقات {symbol}")
    ...
    def on_open(self, ws):
        logger.info("✅ WebSocket متصل بنجاح، إرسال جميع الاشتراكات...")
        self.reconnect_delay = 5
        self._safe_send_with_retry({"method": "SUBSCRIBE", "params": ["!ticker@arr"], "id": self._get_next_id()})
        if self.subscribed_symbols:
            params = []
            for sym in self.subscribed_symbols:
                base = sym.replace('/', '').lower()
                params.append(f"{base}@kline_5m")
                params.append(f"{base}@kline_1h")
                params.append(f"{base}@kline_4h")
                params.append(f"{base}@depth20")
                params.append(f"{base}@trade")
            if params:
                for i in range(0, len(params), 1024):
                    chunk = params[i:i+1024]
                    if not self._safe_send_with_retry({"method": "SUBSCRIBE", "params": chunk, "id": self._get_next_id()}):
                        logger.error(f"❌ فشل إرسال طلب الاشتراك للرموز: {chunk[:5]}...")
                    time.sleep(0.1)
            logger.info(f"✅ تم الاشتراك في الشموع (5م، 1س، 4س) والدفتر والصفقات لـ {len(self.subscribed_symbols)} عملة")
    ...
    def start(self, symbols=None):
        ...
        self.thread.start()
        threading.Thread(target=self._gap_checker, daemon=True).start()
        logger.info("🌐 بدء تشغيل AdvancedWebSocketManager (يدعم الشموع، الدفتر، الصفقات، والتيكرات)")
    ...
    def stop(self):
        ...
        logger.info("🛑 تم إيقاف AdvancedWebSocketManager")
    ...
    # ===== التعديل الأساسي: استخدام all_tickers_cache بدلاً من ticker_cache =====
    def get_ticker(self, symbol):
        with self.all_tickers_lock:
            data = self.all_tickers_cache.get(symbol)
    ...
    def get_all_tickers(self):
        with self.all_tickers_lock:
            result = {}
            now = time.time()
            for sym, data in self.all_tickers_cache.items():
                if sym != '_timestamp' and now - data.get('timestamp', 0) < 30:
                    result[sym] = {
                        'last': data['last'],
    ...
    # ===== دالة مساعدة للحصول على وقت الوصول الصحيح =====
    def get_last_access_time(self, symbol):
        """البحث عن أحدث وقت وصول لأي إطار زمني لهذا الرمز"""
        last_time = 0
        for tf in ['5m', '1h', '4h']:
            key = f"{symbol}_{tf}"
            if key in self.last_access_time:
                last_time = max(last_time, self.last_access_time[key])
        return last_time

    # ---------- تعديل get_klines لجلب REST تلقائياً عند نقص البيانات ----------
    def get_klines(self, symbol, timeframe='5m', limit=None):
        if symbol in self.recently_removed and (time.time() - self.recently_removed[symbol]) < 600:
            logger.debug(f"⏳ تجاهل {symbol} لأنه أُزيل حديثاً")
            return pd.DataFrame()
        ...
        with self.klines_lock:
            df = self.klines_cache.get(cache_key)
            min_required = limit if limit is not None else 50

            # إذا كانت البيانات موجودة ولكن غير كافية، نطلب المزيد من REST
            if df is not None and not df.empty and len(df) < min_required:
                logger.info(f"🔄 بيانات {symbol} {timeframe} غير كافية ({len(df)} شمعة)، جلب من REST لتكملة البيانات...")
                new_df = self._fetch_rest_klines(symbol, timeframe, limit=200)
                if new_df is not None and not new_df.empty:
                    # دمج مع البيانات الحالية (تجنب التكرار)
                    combined = pd.concat([df, new_df])
                    combined = combined[~combined.index.duplicated(keep='last')]
                    if len(combined) > 200:
                        combined = combined.iloc[-200:]
                    self.klines_cache[cache_key] = combined
                    if timeframe == '5m' and len(combined) >= 50:
                        self._update_features_async(symbol, combined)
                    df = combined
                else:
                    logger.warning(f"⚠️ فشل جلب بيانات إضافية لـ {symbol} {timeframe} من REST، استمرار بالبيانات المتاحة ({len(df)} شمعة)")

            # إذا كان الكاش فارغاً، نطلب البيانات من REST (كما كان)
            elif df is None or df.empty:
                df = self._initialize_or_repair(symbol, timeframe, limit=200 if limit is None else limit)
                if df is None or df.empty:
                    return pd.DataFrame()
    ...
    def update_subscriptions(self, new_symbols):
        protected = set(BASE_SYMBOLS) | set(open_positions.keys())
        new_set = set(new_symbols) | protected

        if len(new_set) > self.max_symbols:
            removable = new_set - protected
            sorted_removable = sorted(
                removable,
                key=lambda s: (scanner.scores.get(s, 0), -self.get_last_access_time(s))  # استخدام الدالة المساعدة
            )
            to_remove_extra = set(sorted_removable[:len(new_set) - self.max_symbols])
            new_set -= to_remove_extra
            for s in to_remove_extra:
                self.recently_removed[s] = time.time()
                logger.info(f"🗑️ رمز {s} أُزيل للحد الأقصى، مُمنع من إعادة التهيئة لمدة 10 دقائق")
            logger.info(f"⚠️ تم إزالة {len(to_remove_extra)} رمز للحد الأقصى ({self.max_symbols})")

        to_add = new_set - self.subscribed_symbols
        to_remove = self.subscribed_symbols - new_set

        if to_remove:
            unsub_params = []
            for sym in to_remove:
                base = sym.replace('/', '').lower()
                unsub_params.append(f"{base}@kline_5m")
                unsub_params.append(f"{base}@kline_1h")
                unsub_params.append(f"{base}@kline_4h")
                unsub_params.append(f"{base}@depth20")
                unsub_params.append(f"{base}@trade")
            for i in range(0, len(unsub_params), 1024):
                chunk = unsub_params[i:i+1024]
                self._safe_send_with_retry({"method": "UNSUBSCRIBE", "params": chunk, "id": self._get_next_id()})
                time.sleep(0.1)
            with self.klines_lock:
                for sym in to_remove:
                    for tf in ['5m', '1h', '4h']:
                        key = f"{sym}_{tf}"
                        self.klines_cache.pop(key, None)
                    logger.debug(f"🗑️ حذف كاش الشموع لـ {sym}")
            with _cache_lock:
                keys_to_delete = [k for k in _ohlcv_cache.keys() if any(k.startswith(f"{sym}_") for sym in to_remove)]
                for k in keys_to_delete:
                    del _ohlcv_cache[k]
            with _features_cache_lock:
                keys_to_delete = [k for k in _features_cache.keys() if any(k.startswith(f"{sym}_") for sym in to_remove)]
                for k in keys_to_delete:
                    del _features_cache[k]
            with self.orderbook_lock:
                for sym in to_remove:
                    self.orderbook_cache.pop(sym, None)
            with self.trades_lock:
                for sym in to_remove:
                    self.trades_cache.pop(sym, None)
            self.subscribed_symbols = new_set
            logger.info(f"🗑️ إلغاء اشتراك {len(to_remove)} رمز")

        if to_add:
            for sym in to_add:
                for tf in ['5m', '1h', '4h']:
                    self._initialize_or_repair(sym, tf, 200)
            sub_params = []
            for sym in to_add:
                base = sym.replace('/', '').lower()
                sub_params.append(f"{base}@kline_5m")
                sub_params.append(f"{base}@kline_1h")
                sub_params.append(f"{base}@kline_4h")
                sub_params.append(f"{base}@depth20")
                sub_params.append(f"{base}@trade")
            for i in range(0, len(sub_params), 1024):
                chunk = sub_params[i:i+1024]
                self._safe_send_with_retry({"method": "SUBSCRIBE", "params": chunk, "id": self._get_next_id()})
                time.sleep(0.1)
            self.subscribed_symbols = new_set
            logger.info(f"✅ اشتراك في {len(to_add)} رمز جديد (شموع 5م، 1س، 4س + دفتر + صفقات)")

    def get_cached_all_tickers(self, max_age=5):
        """جلب التيكرات المخزنة مع التحقق من عمر كل تيكر على حدة"""
        with self.all_tickers_lock:
            result = {}
            now = time.time()
            for sym, data in self.all_tickers_cache.items():
    ...
    def get_cached_orderbook(self, symbol, max_age=2):
        with self.orderbook_lock:
            entry = self.orderbook_cache.get(symbol)
            if entry and (time.time() - entry.get('timestamp', 0)) <= max_age:
                return {'bids': entry.get('bids', []), 'asks': entry.get('asks', [])}
        return None

    def get_cached_trades(self, symbol, max_age=5, limit=50):
        with self.trades_lock:
            entry = self.trades_cache.get(symbol)
            if entry and (time.time() - entry.get('timestamp', 0)) <= max_age:
                return entry.get('trades', [])[:limit]
        return None
...
def fetch_tickers_with_retry(max_retries=2):
    if ws_manager is not None:
        all_tickers = ws_manager.get_cached_all_tickers(max_age=5)
        if all_tickers and len(all_tickers) > 10:  # تم التعديل من 50 إلى 10
            with _global_state_lock:
                bot_stats.last_valid_tickers = dict(all_tickers)
...
# ========================== دوال جلب OHLCV (معدلة) ==========================
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=200, max_retries=4):
    time.sleep(0.1)
    exchange = get_active_exchange()
    last_err = ""
    attempt = 0
...
def fetch_ohlcv_retry(symbol, timeframe='5m', limit=200, max_retries=2, ttl_seconds=60):
    if timeframe == '5m':
        if ws_manager is not None:
            df = ws_manager.get_klines(symbol, timeframe, limit=limit)
            if df is not None and not df.empty:
                return df
        return fetch_ohlcv_retry_raw(symbol, timeframe, limit, max_retries)
    ...
    if df is not None and not df.empty:
        with _cache_lock:
            ...
            if len(_ohlcv_cache) > _ohlcv_cache_max:
                items = list(_ohlcv_cache.items())
                for key, _ in items[:int(_ohlcv_cache_max * 0.2)]:
                    if key in _ohlcv_cache:
                        del _ohlcv_cache[key]
    return df if df is not None else pd.DataFrame()

# ======================= دالة add_advanced_features المعدلة =======================
def add_advanced_features(df):
    ...
    df = df.replace([np.inf, -np.inf], np.nan)
    df = df.ffill().bfill()
    # استخدام interpolation بدلاً من fillna(0) لتجنب تشويه المؤشرات
    df = df.interpolate(method='linear', limit_direction='both', limit_area='inside')
    df = df.fillna(0)  # كملاذ أخير
    return df.astype('float64')

# ========================== دالة الفلتر الكمي (MTF) ==========================
def calculate_mtf_consensus_score(symbol, df_5m, df_1h, df_4h):
    scores = []
    weights = {'5m': 0.2, '1h': 0.35, '4h': 0.45}

    if df_5m is not None and not df_5m.empty and len(df_5m) >= 20:
        mom = (df_5m['close'].iloc[-1] - df_5m['close'].iloc[-4]) / df_5m['close'].iloc[-4] if len(df_5m) >= 4 else 0
        rsi = df_5m['rsi'].iloc[-1] if 'rsi' in df_5m and not pd.isna(df_5m['rsi'].iloc[-1]) else 50
        macd = df_5m['macd_diff'].iloc[-1] if 'macd_diff' in df_5m and not pd.isna(df_5m['macd_diff'].iloc[-1]) else 0
        macd_signal = df_5m['macd_signal'].iloc[-1] if 'macd_signal' in df_5m and not pd.isna(df_5m['macd_signal'].iloc[-1]) else 0
        mom_score = max(0, min(1, (mom * 20) + 0.5))
        rsi_score = max(0, min(1, (rsi - 30) / 40))
        macd_score = 0.5 + (0.5 * (macd - macd_signal) / (abs(macd_signal) + 0.001))
        macd_score = max(0, min(1, macd_score))
        tf_score = (mom_score * 0.4) + (rsi_score * 0.3) + (macd_score * 0.3)
        scores.append(('5m', tf_score))

    if df_1h is not None and not df_1h.empty and len(df_1h) >= 20:
        close = df_1h['close'].iloc[-1]
        sma50 = df_1h['sma_50'].iloc[-1] if 'sma_50' in df_1h and not pd.isna(df_1h['sma_50'].iloc[-1]) else close
        adx = df_1h['adx'].iloc[-1] if 'adx' in df_1h and not pd.isna(df_1h['adx'].iloc[-1]) else 25
        mom = (df_1h['close'].iloc[-1] - df_1h['close'].iloc[-6]) / df_1h['close'].iloc[-6] if len(df_1h) >= 6 else 0
        price_score = max(0, min(1, ((close - sma50) / sma50) * 5 + 0.5))
        adx_score = max(0, min(1, (adx - 20) / 40))
        mom_score = max(0, min(1, (mom * 15) + 0.5))
        tf_score = (price_score * 0.4) + (adx_score * 0.3) + (mom_score * 0.3)
        scores.append(('1h', tf_score))

    if df_4h is not None and not df_4h.empty and len(df_4h) >= 20:
        close = df_4h['close'].iloc[-1]
        sma200 = df_4h['sma_200'].iloc[-1] if 'sma_200' in df_4h and not pd.isna(df_4h['sma_200'].iloc[-1]) else close
        rsi = df_4h['rsi'].iloc[-1] if 'rsi' in df_4h and not pd.isna(df_4h['rsi'].iloc[-1]) else 50
        change = (df_4h['close'].iloc[-1] - df_4h['close'].iloc[-2]) / df_4h['close'].iloc[-2] if len(df_4h) >= 2 else 0
        price_score = 0.5 + (0.5 * (close - sma200) / (sma200 + 0.001))
        price_score = max(0, min(1, price_score))
        rsi_score = max(0, min(1, (rsi - 30) / 40))
        change_score = max(0, min(1, (change * 30) + 0.5))
        tf_score = (price_score * 0.4) + (rsi_score * 0.3) + (change_score * 0.3)
        scores.append(('4h', tf_score))
...
<head><meta charset="UTF-8"><title>بوت v28.1 (MTF + Full WebSocket)</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v28.1 (مع الفلتر الكمي MTF)</h1>
<div id="stats"></div>
...
def api_stats():
    with _global_state_lock:
        return jsonify({
            ...
            'quantum_enabled': FILTER_QUANTUM_ENABLED
        })
...
def test_gemini_api_key():
    ...
    try:
        ...
        if not model_names:
            logger.warning("⚠️ لم يتم العثور على أي نماذج Gemini.")
            return False
        ...
        for p in preferred:
            if any(p in name for name in model_names):
                _detected_model = p
                logger.info(f"✅ تم اكتشاف النموذج: {p}")
                return True
        
        _detected_model = model_names[0].split('/')[-1]
        logger.info(f"✅ تم استخدام النموذج الافتراضي: {_detected_model}")
        return True
    except Exception as e:
...
def _ask_ai_internal(symbol, df, current_price, sym_type, market_condition, ticker_data, threshold, desc):
    ...
    prompt = f"""أنت خبير تداول. قم بتحليل الرمز {symbol} (نوع: {sym_type}) بناءً على البيانات الفنية فقط.
...
السؤال: بناءً على هذه البيانات فقط (دون النظر إلى حالة السوق العامة)، هل تنصح بشراء هذا الرمز الآن؟ أجب فقط بـ "نعم" أو "لا" ثم سبب مختصر جداً."""

    global _detected_model
    if not _detected_model:
        _detected_model = 'gemini-pro'

    model = genai.GenerativeModel(_detected_model)
    ...
    if now - _last_ai_notification_time > AI_NOTIFICATION_COOLDOWN:
        if approved:
            send_telegram(f"🤖 Gemini ({_detected_model}) ✅ **وافق** على شراء {symbol}\nالسبب: {reason[:200]}")
        else:
            send_telegram(f"🤖 Gemini ({_detected_model}) ❌ **رفض** شراء {symbol}\nالسبب: {reason[:200]}")
        _last_ai_notification_time = now

    return approved

# ========================== دوال التصنيف والظروف (معدلة) ==========================
def classify_symbol(symbol, df=None, ticker_data=None):
    ...
    if current_price < 1.0:
        ...
        return 'meme'
    if df is None:
        df = ws_manager.get_klines(symbol, '1h', limit=200)
        df = sanitize_dataframe(df) or df
    if df.empty or len(df) < 24:
        return 'normal'
    df_f = add_advanced_features(df)
    df_f = sanitize_dataframe(df_f) or df_f
...
def execute_limit_order(symbol, side, size, price_ref, sym_type='normal'):
    ...
    slippage = 0.01 if sym_type == 'meme' else LIMIT_ORDER_SLIPPAGE
    if side == 'buy':
        adjusted_price = price_ref * (1 + slippage)
    else:
        adjusted_price = price_ref * (1 - slippage)
...
def place_preemptive_stop_loss(symbol, size, entry_price, sym_type, stop_price_override=None):
    ...
    if stop_price_override is not None:
        ...
        stop_price = entry_price * (1 - PREPLACED_SL_TRIGGER_PERCENT)
    if stop_price <= 0:
    ...
    try:
        ...
        logger.info(f"✅ أمر وقف مسبق (stop_loss_limit) لـ {symbol} | تفعيل عند {stop_price:.8f} | حد السعر {limit_price:.8f} | ID: {oid}")
        return oid
    except Exception as e:
...
# ========================== النماذج الأساسية (معدلة للتعقيم) ==========================
class RuleBasedModel:
    ...
    def get_signal_percent(self, df):
        ...
        if close > ema9:
            buy_score += 0.2
        if rsi > 55:
            buy_score += min(0.2, (rsi - 55) / 150)
        if mom > 0.002:
            buy_score += min(0.2, mom * 100)
        if vol_ratio > 1.2:
            buy_score += min(0.1, (vol_ratio - 1.2) / 10)
        if close / high12 > 0.97:
            buy_score += 0.1
        if stoch_k < 30:
            buy_score += 0.1
        sell_score = 0.0
        ...
        if close < ema9:
            sell_score += 0.2
        if rsi < 45:
            sell_score += min(0.2, (45 - rsi) / 150)
        if mom < -0.002:
            sell_score += min(0.2, -mom * 100)
        if vol_ratio > 1.2:
            sell_score += min(0.1, (vol_ratio - 1.2) / 10)
        if close / low12 < 1.03:
            sell_score += 0.1
        if stoch_k > 70:
            sell_score += 0.1
...
class EMRModel:
    ...
    def update(self, symbol, df):
        ...
        df = sanitize_dataframe(df) or df
        if df is None or df.empty or len(df) < self.window:
            result = None
        else:
            ...
            if len(prices) < self.window:
                ...
                if freq is None:
                    ...
                    if len(self.symbol_phases) > self._max_cache_size:
                        self._cleanup_old_entries(now)
        ...
        with self._cache_lock:
            ...
            if len(self._result_cache) > 200:
                ...
                for sym, _ in items[:50]:
                    del self._result_cache[sym]
        return result
    ...
    def _cleanup_old_entries(self, now, ttl=3600):
        ...
        for sym in list(self._phase_timestamps.keys()):
            if self._phase_timestamps[sym] < threshold:
                if sym in self.symbol_phases:
                    del self.symbol_phases[sym]
...
class CFHMModel:
    ...
    def calculate_scores(self, df):
        df = sanitize_dataframe(df) or df
        if df is None or len(df) < max(self.flow_win, self.mom_win):
            return None, None
        ...
        if atr > 0:
            hidden = ret_std / (atr / df['close'].iloc[-1])
        else:
            hidden = 0.0
...
class OverboughtModel:
    def get_sell_vote_percent(self, df, cur_price, pos):
        ...
        if rsi > 75 and stoch_k > 80 and vol_ratio < 0.8:
            return 0.7
...
class ExtremeATRModel:
    def get_sell_vote_percent(self, df, cur_price, pos):
        df = sanitize_dataframe(df) or df
        if df is None or 'atr' not in df.columns or df['atr'].isna().all():
            return 0.0
        atr = df['atr'].iloc[-1]
        if atr <= 0:
            return 0.0
...
class MomentumDecayModel:
    def get_sell_vote(self, df, pos, cur_price):
        df = sanitize_dataframe(df) or df
        if df is None or pos.initial_momentum is None or pos.entry_momentum_time is None or pos.initial_momentum == 0:
            return 0.0
        ...
        if elapsed_momentum_min < pos.momentum_check_minutes:
            return 0.0
        if len(df) < 15:
            return 0.0
        ...
        if len(df) > lookback:
            ...
            return 0.0
        if abs(pos.initial_momentum) < 1e-6:
            return 0.0
...
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        df_primary = sanitize_dataframe(df_primary) or df_primary
        if df_primary is None or df_primary.empty or len(df_primary) < 20:
        ...
        df_confirm_1 = ws_manager.get_klines(symbol, '15m', limit=200)
        df_confirm_2 = ws_manager.get_klines(symbol, '1h', limit=200)
        ...
        if not df_confirm_1.empty and len(df_confirm_1) >= 20:
            ...
            if confirm_score_15m is not None:
                if confirm_score_15m < 0.4:
                    multiplier -= cfg['weak_penalty_15m']
                elif confirm_score_15m > 0.6:
                    multiplier += cfg['strong_bonus_15m']
        if not df_confirm_2.empty and len(df_confirm_2) >= 20:
            ...
            if confirm_score_1h is not None:
                if confirm_score_1h < 0.35:
                    multiplier -= cfg['weak_penalty_1h']
                elif confirm_score_1h > 0.55:
                    multiplier += cfg['strong_bonus_1h']
        ...
        if avg > thresh and avg >= cfg['min_conf']:
            return 'buy', avg, avg
        else:
            return 'neutral', avg, avg
...
class SellingCommittee:
    ...
    def decide_sell(self, df_primary, symbol, pos, cur_price, market_condition='normal'):
        ...
        if pos.side == 'buy':
            ...
            profit = (pos.entry_price - cur_price) / pos.entry_price
        ...
        df_confirm_1 = ws_manager.get_klines(symbol, '15m', limit=200)
        df_confirm_2 = ws_manager.get_klines(symbol, '1h', limit=200)
        if market_condition == 'bad':
            weak_penalty_15m, strong_bonus_15m = 0.15, 0.05
            weak_penalty_1h, strong_bonus_1h = 0.15, 0.05
        elif market_condition == 'good':
            weak_penalty_15m, strong_bonus_15m = 0.05, 0.15
            weak_penalty_1h, strong_bonus_1h = 0.05, 0.15
        else:
            weak_penalty_15m, strong_bonus_15m = 0.10, 0.10
            weak_penalty_1h, strong_bonus_1h = 0.10, 0.10
        multiplier = 1.0
        if not df_confirm_1.empty and len(df_confirm_1) >= 20:
            _, sell_15m = self.models['rule'].get_signal_percent(df_confirm_1)
            if sell_15m is not None:
                if sell_15m < 0.3:
                    multiplier -= weak_penalty_15m
                elif sell_15m > 0.5:
                    multiplier += strong_bonus_15m
        if not df_confirm_2.empty and len(df_confirm_2) >= 20:
            _, sell_1h = self.models['rule'].get_signal_percent(df_confirm_2)
            if sell_1h is not None:
                if sell_1h < 0.25:
                    multiplier -= weak_penalty_1h
                elif sell_1h > 0.45:
                    multiplier += strong_bonus_1h
...
def fetch_orderbook_with_cache(symbol, limit=LIQUIDITY_CHECK_DEPTH, ttl=2):
    if ws_manager is not None:
        ob = ws_manager.get_cached_orderbook(symbol, max_age=ttl)
        if ob:
...
def fetch_recent_trades(symbol, limit=50, ttl=10):
    if ws_manager is not None:
        trades = ws_manager.get_cached_trades(symbol, max_age=ttl, limit=limit)
        if trades:
            return trades
    now = time.time()
...
class OrderBookManipulationDetector:
    ...
    def cleanup_snapshots(self, max_age_seconds=300):
        now = time.time()
        stale = [s for s, d in self.order_snapshots.items() if now - d.get('timestamp', 0) > max_age_seconds]
        for s in stale:
            del self.order_snapshots[s]

    def _cleanup_old_entries(self, now, ttl=300):
        if now - self.last_cleanup < 60:
            return
        self.last_cleanup = now
        self.cleanup_snapshots(ttl)
    ...
    def detect_spoofing(self, symbol, current_ob, sym_type='normal', window_seconds=10, min_orders=5):
        if current_ob is None or not isinstance(current_ob, dict) or 'bids' not in current_ob or 'asks' not in current_ob:
            return False, 0.0
        if not SPOOFING_DETECTION_ENABLED:
            return False, 0.0
        now = time.time()
        self._cleanup_old_entries(now)
        if symbol not in self.order_snapshots:
            self.order_snapshots[symbol] = {'timestamp': now, 'bids': current_ob.get('bids', [])[:20],
                                            'asks': current_ob.get('asks', [])[:20]}
            return False, 0.0
        ...
        for a in prev.get('asks', []):
            if a[0] * a[1] > large_order_threshold:
                ...
                if a in prev_asks_set and a not in curr_asks_set:
                    large_orders_cancelled += 1
        for b in prev.get('bids', []):
            if b[0] * b[1] > large_order_threshold:
                total_large_orders += 1
                if b in prev_bids_set and b not in curr_bids_set:
                    large_orders_cancelled += 1
        cancel_ratio = large_orders_cancelled / total_large_orders if total_large_orders > 0 else 0
        self.spoofing_history[symbol].append(
            {'time': now, 'cancel_ratio': cancel_ratio, 'large_orders': total_large_orders, 'cancelled': large_orders_cancelled})
    ...
    def detect_fake_liquidity(self, symbol, current_ob, sym_type='normal'):
        ...
        vanished_bids = sum(1 for price, size in prev_large_bids if
                            not any(is_same_price_level(price, cp, 0.005) for cp, cs in curr_large_bids))
        vanished_asks = sum(1 for price, size in prev_large_asks if
                            not any(is_same_price_level(price, cp, 0.005) for cp, cs in curr_large_asks))
...
# ===== تعديل: إضافة last_update_time في WhaleManipulationDetector =====
class WhaleManipulationDetector:
    def __init__(self):
        ...
        self.last_update_time = {}  # لتتبع آخر استخدام لكل رمز

    def detect_whale_trade(self, symbol, sym_type='normal', current_ob=None):
        self.last_update_time[symbol] = time.time()  # تحديث وقت الاستخدام
        if not WHALE_DETECTION_ENABLED:
            return False, 0.0, "no_whale"
        now = time.time()
        trades = fetch_recent_trades(symbol, limit=50)
        if not trades:
            return False, 0.0, "no_trades"
        ...
        for trade in trades:
            ...
            if amount_usd >= min_whale_usd:
                recent_whale_trades.append({'time': trade['timestamp'] / 1000 if isinstance(trade['timestamp'],
                                                                                             (int, float)) else now,
                                            'amount_usd': amount_usd, 'price': trade['price'],
        ...
        if recent_whale_trades:
            ...
            if whale_score > 0.3:
                logger.info(f"🐋 [WHALE DETECTION] {symbol}: {len(recent_whales_in_window)} صفقة حوت في 5 دقائق، درجة {whale_score:.2f}")
        ...
        if current_ob and recent_whale_trades:
            ...
            if price_impact > WHALE_TRADE_IMPACT_THRESHOLD:
                ...
                logger.info(
                    f"🐋 [WHALE IMPACT] {symbol}: تأثير سعري {price_impact:.2%} من صفقة بقيمة ${last_whale['amount_usd']:,.0f} (درجة {impact_score:.2f})")
    ...
    def detect_volume_spike(self, symbol, sym_type='normal'):
        self.last_update_time[symbol] = time.time()  # تحديث وقت الاستخدام
        try:
            df = fetch_ohlcv_retry(symbol, '5m', limit=70)
            if df.empty or len(df) < 20:
                return False, 0.0
            recent_volume = df['volume'].iloc[-5:].sum()
            avg_volume_per_5m = df['volume'].iloc[-60:].mean() if len(df) >= 60 else df['volume'].mean()
            if avg_volume_per_5m <= 0:
                return False, 0.0
            expected_volume = avg_volume_per_5m * 5
            ...
            if spike_ratio > threshold:
                logger.warning(
                    f"📊 [VOLUME SPIKE] {symbol}: حجم {recent_volume:,.0f} (متوسط {expected_volume / 5:,.0f}/5د) نسبة {spike_ratio:.1f}x")
    ...
    try:
        ...
        if not ob:
            ...
            if is_spoofing:
                ...
                if is_fake_liq:
                    ...
                    is_whale, whale_score, whale_type = whale_detector.detect_whale_trade(symbol, sym_type,
                                                                                          current_ob=ob)
        ...
        with _cache_lock:
            ...
            if len(_manipulation_cache) > 500:
                for k in list(_manipulation_cache.keys())[:100]:
                    del _manipulation_cache[k]
        return result
    except Exception as e:
...
def log_trade(symbol, side, entry, exit, size, pnl, reason, pred, conf, regime, sym_type, ai_approved=False):
    trade_logger.info(
        f"{symbol},{side},{entry},{exit},{size},{pnl},{reason},{pred},{conf},{regime},{sym_type},{ai_approved}")
    global _daily_biggest_win, _daily_biggest_loss, _daily_most_traded
    with _global_state_lock:
        if pnl > 0:
            if pnl > _daily_biggest_win:
                _daily_biggest_win = pnl
            bot_stats.weekly_wins += 1
            bot_stats.weekly_pnl += pnl
        else:
            if pnl < _daily_biggest_loss:
                _daily_biggest_loss = pnl
    ...
    try:
        if percent < 1.0:
            ...
            if part_value < min_cost:
                logger.info(
                    f"⚠️ قيمة البيع الجزئي لـ {symbol} = ${part_value:.2f} أقل من min_cost (${min_cost:.2f}). سيتم بيع المركز بالكامل بدلاً من {percent * 100:.0f}%.")
                percent = 1.0
        ...
        try:
            if not is_sim:
                ...
                for attempt in range(MAX_RETRIES_CLOSE):
                    ...
                    success, fill_price, filled_size, _ = execute_limit_close(symbol, close_side, close_size, curr,
                                                                              pos.symbol_type,
                                                                              extra_attempt=(attempt == MAX_RETRIES_CLOSE - 1))
                    if success:
                        break
                ...
                if not success:
                    try:
                        ...
                        exchange = get_active_exchange()
                        if pos.side == 'buy':
                            order = exchange.create_market_sell_order(symbol, close_size)
                        else:
                            order = exchange.create_market_buy_order(symbol, close_size)
                        ...
                        if slippage > max_slippage:
                            send_telegram(f"⚠️ انزلاق كبير ({slippage:.2%}) عند إغلاق {symbol}")
                    except Exception as e:
            ...
            if not is_sim:
                new_balance = _safe_fetch_balance_after_trade(attempts=15, delay=5.0, silent=True)
                if new_balance is not None:
                    with _global_state_lock:
                        bot_stats.last_balance = new_balance
                else:
                    logger.error(f"⚠️ فشل تحديث الرصيد بعد بيع {symbol} - سيتم استخدام القيمة القديمة")
            else:
                ...
                with _global_state_lock:
                    bot_stats.last_balance += sale_proceeds
            ...
            with _global_state_lock:
                if DAILY_LOSS_MODE == 'net':
                    _daily_loss_tracker = max(0, _daily_loss_tracker - pnl_net) if pnl_net > 0 else _daily_loss_tracker + abs(
                        pnl_net)
                else:
                    if pnl_net < 0:
                        _daily_loss_tracker += abs(pnl_net)
                pos.remaining_size -= filled_size
                pos.closed_pnl += pnl_net
                bot_stats.total_pnl_usdt += pnl_net
                bot_stats.daily_pnl += pnl_net
                log_trade(symbol, pos.side, pos.entry_price, fill_price, filled_size, pnl_net, reason, pos.pred,
                          pos.confidence, pos.regime, pos.symbol_type, pos.ai_approved)
                ...
                sell_msg = (f"{direction} <b>بيع جزئي</b> {symbol}\nالنسبة: {percent * 100:.0f}% | الحجم: {filled_size:.6f}\n"
                            f"السعر: {fill_price:.8f} | الربح الصافي: {pnl_net:+.4f} USDT ({pnl_percent:+.2f}%)\nالسبب: {reason}")
                ...
                if pos.remaining_size <= 1e-8:
                    ...
                    if not is_sim:
                        ...
                        if pos.trailing_activated and pos.trailing_stop is not None:
                            ...
                            new_stop = pos.entry_price * (1 - PREPLACED_SL_TRIGGER_PERCENT)
                        if new_stop:
                            update_preemptive_stop_loss(symbol, pos, new_stop)
                save_state()
        finally:
            with _global_state_lock:
                if symbol in open_positions:
                    open_positions[symbol]._closing = False
    except Exception as e:
...
def close_all_positions(reason="أمر إداري"):
    with _global_state_lock:
        if not open_positions:
            send_telegram("ℹ️ لا توجد صفقات مفتوحة")
            return
        ...
        if ticker and ticker.get('last'):
            ...
            with _global_state_lock:
                if sym in open_positions:
                    failed.append(sym)
    if failed:
        send_telegram(f"<b>⚠️ فشل إغلاق:</b> {', '.join(failed)}")
        generate_error_report("فشل_أمر", "أوامر", f"فشل إغلاق المراكز: {failed}")
    else:
        send_telegram(f"<b>🔒 تم إغلاق الكل.</b> السبب: {reason}")
...
def manual_resume():
    ...
    with _global_state_lock:
        ...
        daily_loss_cooldown_until = None
        warnings_list = []
        if _daily_loss_tracker > 0:
            warnings_list.append(f"⚠️ الخسارة اليومية (${_daily_loss_tracker:.2f}) لم تُمسح.")
        if warnings_list:
            send_telegram("<b>▶️ تم الاستئناف اليدوي</b>\n" + "\n".join(warnings_list))
        else:
            send_telegram("▶️ تم الاستئناف")
        save_state()
...
def retry_stuck_positions():
    while True:
        try:
            with _global_state_lock:
                stuck = {sym: pos for sym, pos in open_positions.items() if
                         pos.last_fail_time is not None and pos.retry_count < MAX_STUCK_RETRIES}
            for sym, pos in stuck.items():
                ...
                if elapsed < retry_minutes:
                    continue
                logger.info(
                    f"🔄 إعادة محاولة إغلاق {sym} ({pos.symbol_type}) بعد {elapsed:.1f}د (محاولة {pos.retry_count + 1}/{MAX_STUCK_RETRIES})")
                ticker = fetch_ticker_with_retry(sym)
                if not ticker:
                    continue
...
def get_current_momentum(symbol, lookback_minutes):
    try:
        df = ws_manager.get_klines(symbol, '5m', limit=200)
        df = sanitize_dataframe(df) or df
...
def monitor_positions():
    ...
    while True:
        try:
            with _global_state_lock:
                snapshot = list(open_positions.items())
            ...
            for sym, pos in snapshot:
                ...
                if cur_price is None or cur_price <= 0:
                    ...
                    logger.debug(f"سعر غير صالح لـ {sym}، استخدام سعر الدخول")
                tf = SCALP_TIMEFRAME if pos.symbol_type == 'meme' else TIMEFRAMES['primary']
                df = ws_manager.get_klines(sym, '5m', limit=200)
                df = sanitize_dataframe(df) or df
                if df is None or df.empty:
                    continue
                ...
                if profit < 0:
                    if profit <= -STOP_LOSS_PARTIAL_1_PERCENT and not pos.sold_at_15:
                        ...
                        sell_percent = 0.33
                        close_partial(sym, sell_percent, cur_price,
                ...
                if pos.crash_monitor_start is None:
                    ...
                    drop = (pos.entry_price - cur_price) / pos.entry_price if pos.side == 'buy' else (
                                                                                                                         cur_price - pos.entry_price) / pos.entry_price
                ...
                if sell_dec:
                    close_partial(sym, 1.0, cur_price,
                                  f"🗳️ قرار لجنة البيع (متوسط {avg:.2%}) [عتبة البيع {SELL_DECISION_THRESHOLD:.2%}]")
                    continue
                if pos.trailing_activated and pos.trailing_stop is not None:
                    ...
                    if pos.last_target_hit_index == -1:
                        ...
                        if next_target_idx < len(pos.take_profit_levels):
                            ...
                            if (now_time - pos.last_target_hit_time).total_seconds() > 3600 and cur_price < next_target:
                                close_partial(sym, 1.0, cur_price,
                                              f"🕒 انقضت ساعة بعد تحقيق الهدف {pos.last_target_hit_index + 1} دون الوصول للهدف التالي – جني أرباح")
                                continue
                tp_tolerance = 0.001
...
def execute_full_trade(symbol, side, size, price, atr, sl, tp, sym_type, pred, conf, regime, ai_approved=False):
    ...
    try:
        if symbol in open_positions:
            return False, 0
        if symbol in _local_pending_symbols or symbol in _exchange_pending_symbols:
            return False, 0
        _local_pending_symbols.add(symbol)
    finally:
        _global_state_lock.release()
    try:
        min_amt, max_amt, min_cost = get_amount_limits(symbol)
        if size < min_amt:
            return False, 0
        if max_amt and size > max_amt:
            size = max_amt
        if min_cost and size * price < min_cost:
            return False, 0
        if _should_simulate():
            if TEST_MODE or PAPER_TRADING:
                ...
                with _global_state_lock:
                    if bot_stats.last_balance < total_cost:
                        logger.warning(
                            f"⚠️ رصيد غير كافٍ في المحاكاة الورقية: {bot_stats.last_balance:.2f} < {total_cost:.2f}")
                        return False, 0
        ...
        if recovery_attempts_remaining > 0:
            ...
            if recovery_attempts_remaining == 0:
                ...
                try:
                    ...
                    rest_rate_limiter = RateLimiter(max_calls=60, period=60)
                    logger.info("تم إعادة تعيين RateLimiter (max_calls=60).")
...
# ========================== دالة التحليل الرئيسية (معدلة - إصلاح القفل والتسخين) ==========================
def analyze_and_trade():
    ...
    global QUANTUM_THRESHOLD, FILTER_QUANTUM_ENABLED

    lock_acquired = False
    try:
        # ===== استخدام lock.acquire مع timeout وعداد المحاولات (كما في v27.8) =====
        ...
        # ===== إصلاح التسخين الأولي: لا نستخدم return بعد التسخين =====
        if not _initial_warmup_done and elapsed_since_start < STARTUP_DELAY_SECONDS + 30:
            logger.info("🔥 تنظيف الكاشات القديمة وتسخين الرموز الأساسية...")
            with _cache_lock:
                _ohlcv_cache.clear()
            with _features_cache_lock:
                _features_cache.clear()
            for sym in BASE_SYMBOLS:
                ws_manager.get_klines(sym, '5m', limit=200)
                time.sleep(0.5)
            _initial_warmup_done = True
            logger.info("✅ تم التهيئة. متابعة التحليل...")
            # لا نستخدم return هنا، نستمر في التحليل

        # ===== إزالة شرط ws_manager.initialized =====
        if ws_manager is None:
            logger.info("⏳ انتظار تهيئة WebSocket...")
            return
        ...
        with _global_state_lock:
            if _daily_trades_date != today:
                if _daily_trades_count > 0:
                    ...
                    report = (
                        f"<b>📊 تقرير يومي مفصل</b> ({today})\n"
                        f"📈 عدد الصفقات: {_daily_trades_count} | ✅ رابحة: {_daily_winning_trades} | ❌ خاسرة: {_daily_losing_trades}\n"
                        f"🏆 نسبة النجاح: {wr:.1f}%\n"
                        f"💰 <b>صافي الربح/الخسارة اليومي:</b> ${bot_stats.daily_pnl:.2f}\n"
                        f"🌟 أكبر ربح: ${_daily_biggest_win:.2f} | 💀 أكبر خسارة: ${_daily_biggest_loss:.2f}\n"
                        f"🕒 متوسط زمن الاحتفاظ (رابح): {avg_holding_win:.1f} دقيقة | (خاسر): {avg_holding_loss:.1f} دقيقة\n"
                        f"🔄 أكثر عملة تداولاً: {most_traded[0]} ({most_traded[1]} صفقة)"
                    )
                    send_telegram(report)
        ...
        free_bal = get_real_balance_usdt(max_retries=5, delay=5, silent=True)
        ...
        if market_cond == 'good':
            cooldown_hours = COOLDOWN_HOURS_GOOD
        elif market_cond == 'bad':
            cooldown_hours = COOLDOWN_HOURS_BAD
        else:
        ...
        elif scanner.should_scan():
            scanner.scan()
        
        all_tickers_list = list(bot_stats.last_valid_tickers.keys())
        all_syms = list(set(BASE_SYMBOLS + all_tickers_list + list(open_positions.keys())))
        active = all_syms[:effective_limit]
        all_tickers = fetch_tickers_with_retry()
        if not all_tickers:
            with _global_state_lock:
                all_tickers = dict(bot_stats.last_valid_tickers)
            if not all_tickers:
        ...
        for sym in active:
            if sym in forbidden:
                continue
            if sym in ws_manager.recently_removed and (time.time() - ws_manager.recently_removed[sym]) < 600:
                continue
            if last_close_time.get(sym) and (datetime.now() - last_close_time[sym]).total_seconds() < cooldown_hours * 3600:
                continue
            regime = detect_market_regime(sym)
            if regime == 'trending_down':
                continue
            # ===== استخدام get_cached_features بدلاً من ws_manager.get_klines للحصول على المؤشرات =====
            df_primary = get_cached_features(sym, TIMEFRAMES['primary'], limit=200, ttl=30)
            df_primary = sanitize_dataframe(df_primary) or df_primary
            ...
            if FILTER_QUANTUM_ENABLED:
                df_1h = ws_manager.get_klines(sym, '1h', limit=200)
                df_4h = ws_manager.get_klines(sym, '4h', limit=200)
                if df_1h is not None and not df_1h.empty and len(df_1h) >= 20 and df_4h is not None and not df_4h.empty and len(df_4h) >= 20:
                    quantum_score = calculate_mtf_consensus_score(sym, df_primary, df_1h, df_4h)
                    if quantum_score < QUANTUM_THRESHOLD:
                        logger.info(f"📉 رفض {sym} بسبب الفلتر الكمي: الدرجة {quantum_score:.2f} < {QUANTUM_THRESHOLD:.2f}")
                        continue
                else:
                    logger.debug(f"⏳ بيانات غير كافية للأطر الكبيرة لـ {sym}، تخطي الفلتر الكمي")
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    manipulation_results[sym] = (False, 0.0, "timeout")
                except Exception:
                    manipulation_results[sym] = (False, 0.0, "error")
        pending = []
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            ...
            if dec != 'buy':
                continue
            if (sym_type == 'meme' and score < SCALP_MIN_PROFIT) or (sym_type != 'meme' and score < STRENGTH_THRESHOLD):
                continue
            ticker = all_tickers.get(sym)
            if not ticker:
                continue
            ...
            if FILTER_HOUR_CANDLE_ENABLED:
                ...
                if cur_price <= last_completed_close:
                    continue
            if FILTER_4H_CANDLE_ENABLED:
                ...
                if cur_price <= last_completed_close_4h:
                    continue
            ...
            if max_amt and size > max_amt:
                size = max_amt
            if min_cost and size * cur_price < min_cost:
                continue
            if curr_exp + size * cur_price > max_allowed_exposure:
                continue
            if free_bal < size * cur_price:
                continue
            ...
            pending.append({'symbol': sym, 'score': extra, 'price': cur_price, 'atr': atr_val,
                            ...
                            'pred': score, 'conf': conf, 'regime': regime, 'df_primary': df_primary})
        if pending:
            ...
            for opp in pending:
                with _global_state_lock:
                    if _daily_trades_count >= CUSTOM_MAX_DAILY_TRADES:
                        break
                    if opp['symbol'] in open_positions or opp['symbol'] in _local_pending_symbols or opp['symbol'] in _exchange_pending_symbols:
                        continue
                approved = True
                ...
                if sz < min_amt2:
                    continue
                if max_amt2 and sz > max_amt2:
                    sz = max_amt2
                if min_cost2 and sz * opp['price'] < min_cost2:
                    continue
                if exp + pos_usdt > max_exp2:
                    continue
                if rem_bal < pos_usdt:
                    continue
                ...
                if succ:
                    new_rem = _safe_fetch_balance_after_trade(attempts=15, delay=5.0, silent=True) if not PAPER_TRADING else bot_stats.last_balance
                    if new_rem is None:
        ...
        logger.warning("⚠️ مهلة الحصول على قفل التحليل (30 ثانية)، تخطي الدورة")
    except Exception as e:
        ...
        if lock_acquired:
            try:
                _processing_lock.release()
                logger.debug("🔓 تم تحرير قفل التحليل")
            except Exception as e:
    ...
    try:
        ...
        for order in orders:
            ...
            if isinstance(ts, (int, float)):
                order_time = datetime.fromtimestamp(ts / 1000)
    ...
    while True:
        ...
        if _balance_retry_count <= MAX_BALANCE_RETRIES:
            ...
            if bal is not None and bal > 0:
                _balance_retry_count = 0
                long_failure_cycles = 0
                logger.info("✅ تم استعادة الرصيد بعد محاولات متعددة.")
            else:
                time.sleep(120)
            continue
        long_failure_cycles += 1
        _balance_retry_count = 0
        if long_failure_cycles <= 3:
            logger.warning(f"⚠️ الدورة الطويلة #{long_failure_cycles}: فشل الرصيد لأكثر من 20 محاولة.")
            if long_failure_cycles == 1:
                send_telegram("⏳ فشل استعادة الرصيد لمدة طويلة. سأحاول مجدداً بعد ساعة.")
            time.sleep(3600)
        else:
...
# ===== تعديل: تحديث cache_cleanup_thread لإدارة الذاكرة المتقدمة =====
def cache_cleanup_thread():
    while True:
        time.sleep(300)  # كل 5 دقائق
        now = time.time()
        if ws_manager:
            ws_manager.cleanup_all_caches(
                max_trades=100,
                max_ohlcv=80,
                max_features=120,
                max_manipulation=200,
        ...
        # ========== تنظيف متقدم للكاشات الأخرى ==========
        # 1. الحصول على الرموز النشطة (المفتوحة أو المشتركة)
        active_symbols = set(BASE_SYMBOLS)
        with _global_state_lock:
            active_symbols.update(open_positions.keys())
        if ws_manager:
            active_symbols.update(ws_manager.subscribed_symbols)

        # 2. تنظيف all_tickers_cache (مع استثناء النشطة)
        if ws_manager:
            with ws_manager.all_tickers_lock:
                expired = []
                for sym, data in list(ws_manager.all_tickers_cache.items()):
                    if sym == '_timestamp':
                        continue
                    ...
                    if now - data.get('timestamp', 0) > 600:  # 10 دقائق
                        expired.append(sym)
                for sym in expired:
                    ws_manager.all_tickers_cache.pop(sym, None)
                # حد أقصى 500 رمز (مع استثناء النشطة)
                if len(ws_manager.all_tickers_cache) > 500:
                    non_active = [(k, v.get('timestamp', 0)) for k, v in ws_manager.all_tickers_cache.items() if k != '_timestamp' and k not in active_symbols]
                    if non_active:
                        sorted_items = sorted(non_active, key=lambda x: x[1])
                        to_remove = [k for k, _ in sorted_items[:len(ws_manager.all_tickers_cache) - 500]]
                        for k in to_remove:
                            ws_manager.all_tickers_cache.pop(k, None)

        # 3. تنظيف trades_cache (مع استثناء النشطة)
        if ws_manager:
            with ws_manager.trades_lock:
                expired = []
                for sym, entry in list(ws_manager.trades_cache.items()):
                    if sym in active_symbols:
                        continue
                    if now - entry.get('timestamp', 0) > 600:
                        expired.append(sym)
                for sym in expired:
                    ws_manager.trades_cache.pop(sym, None)
                if len(ws_manager.trades_cache) > 300:
                    non_active = [(k, v.get('timestamp', 0)) for k, v in ws_manager.trades_cache.items() if k not in active_symbols]
                    if non_active:
                        sorted_items = sorted(non_active, key=lambda x: x[1])
                        to_remove = [k for k, _ in sorted_items[:len(ws_manager.trades_cache) - 300]]
                        for k in to_remove:
                            ws_manager.trades_cache.pop(k, None)

        # 4. تنظيف orderbook_cache (مع استثناء النشطة)
        if ws_manager:
            with ws_manager.orderbook_lock:
                expired = []
                for sym, entry in list(ws_manager.orderbook_cache.items()):
                    if sym in active_symbols:
                        continue
                    if now - entry.get('timestamp', 0) > 600:
                        expired.append(sym)
                for sym in expired:
                    ws_manager.orderbook_cache.pop(sym, None)
                if len(ws_manager.orderbook_cache) > 300:
                    non_active = [(k, v.get('timestamp', 0)) for k, v in ws_manager.orderbook_cache.items() if k not in active_symbols]
                    if non_active:
                        sorted_items = sorted(non_active, key=lambda x: x[1])
                        to_remove = [k for k, _ in sorted_items[:len(ws_manager.orderbook_cache) - 300]]
                        for k in to_remove:
                            ws_manager.orderbook_cache.pop(k, None)

        # 5. تنظيف _amount_limits_cache
        with _cache_lock:
            expired = [k for k, t in list(_amount_limits_cache_time.items()) if k not in active_symbols and now - t > 3600]
            for k in expired:
                _amount_limits_cache.pop(k, None)
                _amount_limits_cache_time.pop(k, None)
            if len(_amount_limits_cache) > 500:
                non_active = [(k, t) for k, t in _amount_limits_cache_time.items() if k not in active_symbols]
                if non_active:
                    sorted_items = sorted(non_active, key=lambda x: x[1])
                    to_remove = [k for k, _ in sorted_items[:len(_amount_limits_cache) - 500]]
                    for k in to_remove:
                        _amount_limits_cache.pop(k, None)
                        _amount_limits_cache_time.pop(k, None)

        # 6. تنظيف whale_detector (باستخدام last_update_time)
        for sym in list(whale_detector.last_update_time.keys()):
            if sym in active_symbols:
                continue
            if now - whale_detector.last_update_time[sym] > 600:
                whale_detector.trade_history.pop(sym, None)
                whale_detector.whale_trades.pop(sym, None)
                whale_detector.volume_history.pop(sym, None)
                whale_detector.last_update_time.pop(sym, None)
        # حد أقصى 300 رمز
        if len(whale_detector.last_update_time) > 300:
            non_active = [(k, t) for k, t in whale_detector.last_update_time.items() if k not in active_symbols]
            if non_active:
                sorted_items = sorted(non_active, key=lambda x: x[1])
                to_remove = [k for k, _ in sorted_items[:len(whale_detector.last_update_time) - 300]]
                for k in to_remove:
                    whale_detector.trade_history.pop(k, None)
                    whale_detector.whale_trades.pop(k, None)
                    whale_detector.volume_history.pop(k, None)
                    whale_detector.last_update_time.pop(k, None)

        # 7. تنظيف _orderbook_cache
        with _orderbook_cache_lock:
            expired = [k for k, entry in list(_orderbook_cache.items()) if now - entry['timestamp'] > 600]
            for k in expired:
                _orderbook_cache.pop(k, None)
            if len(_orderbook_cache) > _ORDERBOOK_CACHE_MAX:
                sorted_items = sorted(_orderbook_cache.items(), key=lambda x: x[1]['timestamp'])
                to_remove = [k for k, _ in sorted_items[:len(_orderbook_cache) - _ORDERBOOK_CACHE_MAX]]
                for k in to_remove:
                    _orderbook_cache.pop(k, None)

        # 8. تنظيف _trades_cache
        with _trades_cache_lock:
            expired = [k for k, entry in list(_trades_cache.items()) if now - entry['timestamp'] > 600]
            for k in expired:
                _trades_cache.pop(k, None)
            if len(_trades_cache) > 200:
                sorted_items = sorted(_trades_cache.items(), key=lambda x: x[1]['timestamp'])
                to_remove = [k for k, _ in sorted_items[:len(_trades_cache) - 200]]
                for k in to_remove:
                    _trades_cache.pop(k, None)

        # 9. تنظيف _ohlcv_cache و _features_cache (بحدود)
        with _cache_lock:
            if len(_ohlcv_cache) > _ohlcv_cache_max:
                items = sorted(_ohlcv_cache.items(), key=lambda x: x[1]['timestamp'])
                for k, _ in items[:len(_ohlcv_cache) - _ohlcv_cache_max]:
                    del _ohlcv_cache[k]
        with _features_cache_lock:
            if len(_features_cache) > _features_cache_max:
                items = sorted(_features_cache.items(), key=lambda x: x[1]['timestamp'])
                for k, _ in items[:len(_features_cache) - _features_cache_max]:
                    del _features_cache[k]

        # 10. تنظيف _manipulation_cache
        with _cache_lock:
            if len(_manipulation_cache) > 200:
                items = sorted(_manipulation_cache.items(), key=lambda x: x[1]['timestamp'])
                for k, _ in items[:len(_manipulation_cache) - 200]:
                    del _manipulation_cache[k]

        # 11. تنظيف _market_cap_cache
        with _market_cap_cache_lock:
            if len(_market_cap_cache) > 300:
                items = sorted(_market_cap_cache.items(), key=lambda x: x[1]['timestamp'])
                for coin, _ in items[:len(_market_cap_cache) - 300]:
                    del _market_cap_cache[coin]

        # 12. تنظيف order_book_detector snapshots
        order_book_detector.cleanup_snapshots(max_age_seconds=300)

        # 13. تنظيف تقارير الأخطاء
    ...
    while True:
        ...
        try:
            ...
            if memory_mb > 1024:
                ...
                with _cache_lock:
                    for key in list(_ohlcv_cache.keys())[:30]:
                        del _ohlcv_cache[key]
                with _features_cache_lock:
                    for key in list(_features_cache.keys())[:30]:
                        del _features_cache[key]
            ...
            logger.debug(f"خطأ في مراقبة الذاكرة: {e}")
...
def sync_preemptive_stop_orders():
    while True:
        try:
            if TEST_MODE or PAPER_TRADING or not ENABLE_TRADING:
                time.sleep(60)  # تم التعديل من 300 إلى 60
                continue
            exchange = get_active_exchange()
            with _global_state_lock:
                symbols_to_check = list(open_positions.keys())
                    ...
                    with _global_state_lock:
                        ...
                        if current_oid and current_oid not in order_map.values():
                            ...
                            if current_coin_balance < pos.remaining_size * 0.01:
                                logger.info(
                                    f"✅ تم اكتشاف إغلاق تلقائي لـ {sym} (الرصيد {current_coin_balance} < {pos.remaining_size * 0.01}). جاري حساب الربح/الخسارة...")
                                ...
                                if total_sold_qty > 0:
                                    ...
                                    total_pnl = calculate_net_pnl(sym, pos.entry_price, avg_price, total_sold_qty,
                                                                  pos.side, pos.symbol_type)
                                    ...
                                    trade_logger.info(
                                        f"{sym},{pos.side},{pos.entry_price},{avg_price},{total_sold_qty},{total_pnl},auto_stop_loss,{pos.pred},{pos.confidence},{pos.regime},{pos.symbol_type}")

                                    direction = "🔴" if total_pnl < 0 else "🟢"
                                    send_telegram(
                                        f"{direction} <b>وقف مسبق تم تفعيله تلقائياً</b> {sym}\nالحجم المباع: {total_sold_qty:.6f}\nالسعر المتوسط: {avg_price:.8f}\nالربح الصافي الكلي: {total_pnl:+.4f} USDT")
                                    ...
                                    logger.warning(
                                        f"لم يتم العثور على صفقات بيع لـ {sym} رغم نقص الرصيد. حذف المركز دون تحديث الربح.")
                                    del open_positions[sym]
                                    continue

                            else:
                                logger.warning(
                                    f"⚠️ أمر الوقف {current_oid} مفقود لكن الرصيد لا يزال {current_coin_balance:.6f} مقابل {pos.remaining_size:.6f}. إعادة إنشاء الوقف.")
                                if pos.trailing_activated and pos.trailing_stop is not None:
                                    new_stop = pos.trailing_stop
                                else:
                                    new_stop = pos.entry_price * (1 - PREPLACED_SL_TRIGGER_PERCENT)
                                new_oid = place_preemptive_stop_loss(sym, pos.remaining_size, pos.entry_price,
                                                                     pos.symbol_type, stop_price_override=new_stop)
                                if new_oid:
                                    pos.preemptive_sl_order_id = new_oid
                                    pos.last_trailing_stop_sent = new_stop
    ...
    while True:
        try:
            ...
            report = (f"<b>📊 تقرير دوري (كل 4 ساعات)</b>\n🕒 الوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                      f"💰 الرصيد الكلي: ${total_equity:.2f}\n📈 إجمالي الربح الصافي: ${total_pnl:.2f}\n"
                      f"📊 صفقات اليوم: {daily_trades}\n🔓 مراكز مفتوحة: {positions_count}\n⏸️ متوقف: {'نعم' if paused else 'لا'}\n"
                      f"🎯 عتبة الاشتباه الحالية: {MANIPULATION_REJECT_THRESHOLD:.2%}\n"
                      f"🎯 عتبة البيع الحالية: {SELL_DECISION_THRESHOLD:.2%}\n"
                      f"🛑 عتبات الخسارة الوقائية:\n   - بيع 25% عند {STOP_LOSS_PARTIAL_1_PERCENT:.2%}\n"
                      f"   - بيع 33% عند {STOP_LOSS_PARTIAL_2_PERCENT:.2%}\n"
                      f"   - بيع كامل عند {STOP_LOSS_FULL_PERCENT:.2%}\n"
                      f"🤖 <b>الذكاء الاصطناعي (Gemini):</b> {'✅ مفعل' if AI_ENABLED else '❌ معطل'} | النموذج الحالي: {_detected_model if _detected_model else 'غير مكتشف'}\n"
                      f"⚛️ <b>الفلتر الكمي (MTF):</b> {'✅ مفعل' if FILTER_QUANTUM_ENABLED else '❌ معطل'} | العتبة: {QUANTUM_THRESHOLD:.2f}")
            if positions_count > 0:
                ...
                report += "لا توجد مراكز مفتوحة."
            send_telegram(report)
        except Exception as e:
            logger.error(f"خطأ في التقرير الدوري: {e}")
        time.sleep(14400)

# --------------------------- دالة telegram_polling المعدلة (بإضافة أوامر الذكاء الاصطناعي والتبديل بين النموذجين) ---------------------------
def telegram_polling():
    ...
    global CUSTOM_MAX_DAILY_TRADES
    global buying_committee, CURRENT_BUY_COMMITTEE_MULTIPLIER
    global AI_ENABLED, _detected_model
    global QUANTUM_THRESHOLD, FILTER_QUANTUM_ENABLED
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                                ...
                                if not positions:
                                    ...
                                    total_value = 0.0
                                    total_pnl = 0.0
                                    lines = ["<b>📊 تقرير الصفقات المفتوحة</b>"]
                                    current_prices = {}
                                    ...
                                    with _global_state_lock:
                                        for sym, pos in open_positions.items():
                                            ...
                                            positions_details.append(
                                                f"• {sym} ({pos.side}) {pos.remaining_size:.6f} @ {current_price:.8f} ≈ ${value:.2f} (ربح {profit_pct:+.2f}%)")
                                ...
                                model_display = _detected_model if _detected_model else "غير مكتشف"
                                status = (f"📊 <b>حالة الفلاتر والقيم الحالية</b>\n━━━━━━━━━━━━━━━━━━━━\n"
                                          ...
                                          f"🕓 <b>فلتر شمعة 4 ساعات:</b> {'✅ مفعل' if FILTER_4H_CANDLE_ENABLED else '❌ معطل'}\n"
                                          f"🧠 <b>فلتر الكم (MTF):</b> {'✅ مفعل' if FILTER_QUANTUM_ENABLED else '❌ معطل'} (العتبة: {QUANTUM_THRESHOLD:.2f})\n"
                                          f"⏱️ <b>فترة التبريد:</b> {BASE_COOLDOWN_HOURS:.1f} ساعة\n"
                                          f"━━━━━━━━━━━━━━━━━━━━\n"
                                          ...
                                          f"🎯 <b>عتبة البيع:</b> {SELL_DECISION_THRESHOLD:.2%}\n"
                            ...
                            # ------------------------------------------------------------
                            # 3. أوامر تشغيل/إيقاف الفلاتر الفردية
                            # ------------------------------------------------------------
                                ...
                                save_state()
                            elif text == 'اوقف فلتر الشمعة الصاعدة 4ساعة' or text == 'اوقف فلتر الشمعة الصاعدة 4 ساعة':
                                FILTER_4H_CANDLE_ENABLED = False
                                send_telegram("🔴 تم إيقاف فلتر شمعة 4 ساعات الصاعدة (لن يتم فحص السعر > إغلاق آخر شمعة 4 ساعات)")
                                save_state()
                            elif text == 'شغل فلتر الشمعة الصاعدة 4 ساعة' or text == 'شغل فلتر الشمعة الصاعدة 4ساعة':
                                FILTER_4H_CANDLE_ENABLED = True
                                send_telegram("🟢 تم تشغيل فلتر شمعة 4 ساعات الصاعدة (سيتم فحص السعر > إغلاق آخر شمعة 4 ساعات)")
                                save_state()

                            # ===== تعديل: إضافة أوامر فلتر الكم =====
                            elif text == 'شغل فلتر الكم':
                                FILTER_QUANTUM_ENABLED = True
                                save_state()
                                send_telegram("🟢 تم تشغيل فلتر الكم (MTF) – سيتم رفض العملات ذات الدرجة الأقل من العتبة.")

                            elif text == 'اوقف فلتر الكم':
                                FILTER_QUANTUM_ENABLED = False
                                save_state()
                                send_telegram("🔴 تم إيقاف فلتر الكم (MTF) – لن يتم تطبيقه أثناء التحليل.")

                            elif text.startswith('عتبة الكم'):
                                match = re.search(r'(\d+(?:\.\d+)?)', text)
                                if match:
                                    new_threshold = float(match.group(1))
                                    if 0.0 <= new_threshold <= 1.0:
                                        QUANTUM_THRESHOLD = new_threshold
                                        save_state()
                                        send_telegram(f"✅ تم تعيين عتبة فلتر الكم إلى {new_threshold:.2f}")
                                    else:
                                        send_telegram("⚠️ يجب أن تكون العتبة بين 0 و 1")
                                else:
                                    send_telegram("⚠️ الصيغة الصحيحة: `عتبة الكم 0.60`")

                            # ------------------------------------------------------------
                            # 4. أوامر ضبط عتبات الشراء واللجنة
                            # ------------------------------------------------------------
                                ...
                                if match:
                                    ...
                                    buying_committee.apply_multiplier(new_multiplier)
                                    send_telegram(
                                        f"✅ تم خفض عتبات لجنة الشراء بنسبة {percent}%\n📊 المضاعف الجديد: {new_multiplier:.2f} (أي عتبات ×{new_multiplier:.2f})")
                                else:
                            ...
                            # ------------------------------------------------------------
                            # 5. أوامر ضبط فترة التبريد ومدة المسح
                            # ------------------------------------------------------------
                                ...
                                if match:
                                    ...
                                    BASE_COOLDOWN_HOURS = hours
                                    COOLDOWN_HOURS_GOOD = hours * 0.5
                                    COOLDOWN_HOURS_BAD = hours * 1.5
                                    send_telegram(
                                        f"✅ تم ضبط فترة التبريد إلى {value} {unit}\n📌 تبريد عادي: {BASE_COOLDOWN_HOURS} ساعة\n📌 تبريد سوق جيد: {COOLDOWN_HOURS_GOOD} ساعة\n📌 تبريد سوق سيء: {COOLDOWN_HOURS_BAD} ساعة")
                                    save_state()
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `فلتر التبريد 30 دقيقة` أو `فلتر التبريد 2 ساعة`")
                            ...
                            # ------------------------------------------------------------
                            # 6. أوامر التبديل بين التداول الحقيقي والورقي
                            # ------------------------------------------------------------
                                ...
                                if not ENABLE_TRADING:
                                    ...
                                    real_balance = fetch_real_balance_with_retry(timeout_seconds=120, retry_interval=5,
                                                                                 silent=False)
                                    if real_balance is not None:
                                        with _global_state_lock:
                                            bot_stats.last_balance = real_balance
                                        send_telegram(
                                            f"✅ تم التبديل إلى **التداول الحقيقي**. الرصيد الحقيقي: ${real_balance:.2f} USDT")
                                        save_state()
                                    else:
                                        PAPER_TRADING = True
                                        send_telegram(
                                            "❌ فشل جلب الرصيد الحقيقي من Binance. تأكد من اتصال API والمفاتيح.\n⚠️ لم يتم التبديل إلى التداول الحقيقي. بقيت في **الوضع الورقي**.")
                                else:
                                ...
                                if not ENABLE_TRADING:
                                    ...
                                    TEST_MODE = False
                                    send_telegram(
                                        f"✅ تم التبديل إلى **التداول الورقي** برصيد ابتدائي ${PAPER_INITIAL_BALANCE:.2f} USDT.")
                                    save_state()
                                else:
                                    send_telegram("ℹ️ أنت بالفعل في وضع التداول الورقي أو المحاكاة.")

                            # ------------------------------------------------------------
                            # 7. أوامر رفع/خفض عتبات الاشتباه والبيع
                            # ------------------------------------------------------------
                                ...
                                if match:
                                    ...
                                    if 0.1 <= new_percent <= 0.95:
                                        ...
                                        save_state()
                                        send_telegram(
                                            f"✅ تم خفض عتبة الاشتباه إلى {new_percent:.2%}\n(سيتم رفض العملات التي تتجاوز درجة الاشتباه هذه)")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 10% و 95%")
                                else:
                                ...
                                if match:
                                    ...
                                    if 0.3 <= new_percent <= 0.9:
                                        ...
                                        save_state()
                                        send_telegram(
                                            f"✅ تم خفض عتبة البيع إلى {new_percent:.2%}\n(لجنة البيع ستبيع عندما يتجاوز متوسط الأصوات هذه القيمة)")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 30% و 90%")
                                else:
                            ...
                            # ------------------------------------------------------------
                            # 8. أوامر تعيين عتبات الخسارة الوقائية
                            # ------------------------------------------------------------
                                ...
                                if match:
                                    ...
                                    if 0.001 <= new_percent <= 0.05:
                                        ...
                                        save_state()
                                        send_telegram(
                                            f"✅ تم تعيين عتبة الخسارة (بيع 25%) إلى {new_percent:.2%}\n(سيتم بيع 25% عند خسارة {new_percent:.2%})")
                                    else:
                                ...
                                if match:
                                    ...
                                    if 0.001 <= new_percent <= 0.05:
                                        ...
                                        save_state()
                                        send_telegram(
                                            f"✅ تم تعيين عتبة الخسارة (بيع 33%) إلى {new_percent:.2%}\n(سيتم بيع 33% عند خسارة {new_percent:.2%})")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.1% و 5%")
                                else:
                                ...
                                if match:
                                    ...
                                    if 0.001 <= new_percent <= 0.10:
                                        ...
                                        save_state()
                                        send_telegram(
                                            f"✅ تم تعيين عتبة الخسارة الكاملة إلى {new_percent:.2%}\n(سيتم بيع كامل المركز عند خسارة {new_percent:.2%})")
                                    else:
                            ...
                            # ------------------------------------------------------------
                            # 9. الأوامر الجديدة في v27.8 (عدد رموز التحليل، حجم التعرض، أحجام الصفقة)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 10. الأمر الجديد لتعيين عدد الصفقات المسموحة يومياً
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                                ...
                                elif not test_gemini_api_key():
                                    send_telegram("❌ مفتاح Gemini غير صالح أو النماذج غير متاحة. لم يتم التفعيل.")
                                else:
                                    AI_ENABLED = True
                                    save_state()
                                    model_disp = _detected_model if _detected_model else "غير مكتشف"
                                    send_telegram(
                                        f"🤖 تم **تفعيل** استشارة الذكاء الاصطناعي (Gemini). النموذج الحالي: {model_disp}.")
                                ...
                                if _detected_model:
                                    send_telegram(f"النموذج الحالي هو {_detected_model}. يمكنك التبديل بين النماذج المتاحة عبر الأمر `تحول إلى النموذج الثاني` (إذا كان متاحاً).")
                                else:
                                    send_telegram("❌ لم يتم اكتشاف أي نموذج. تأكد من مفتاح Gemini.")
                            elif text == 'تحول إلى النموذج الثاني':
                                if not GEMINI_API_KEY:
                                    send_telegram("❌ مفتاح Gemini غير موجود.")
                                else:
                                    try:
                                        genai.configure(api_key=GEMINI_API_KEY)
                                        models = genai.list_models()
                                        model_names = [m.name for m in models]
                                        current = _detected_model
                                        alt = None
                                        for name in model_names:
                                            if name.endswith('gemini-2.0-flash') or name.endswith('gemini-1.5-flash') or name.endswith('gemini-pro'):
                                                if name != current:
                                                    alt = name
                                                    break
                                        if alt:
                                            _detected_model = alt
                                            save_state()
                                            send_telegram(f"✅ تم التبديل إلى النموذج: {alt}")
                                        else:
                                            send_telegram("❌ لم يتم العثور على نموذج بديل. النموذج الحالي: " + (current if current else "غير مكتشف"))
                                    except Exception as e:
                                        send_telegram(f"❌ خطأ في التبديل: {e}")

                            # ------------------------------------------------------------
                            # 12. أمر إعادة ضبط الفلاتر
                            # ------------------------------------------------------------
                                ...
                                FILTER_4H_CANDLE_ENABLED = True
                                FILTER_QUANTUM_ENABLED = True
                                QUANTUM_THRESHOLD = 0.55
                                BASE_COOLDOWN_HOURS = 1.0
                                COOLDOWN_HOURS_GOOD = 0.5
                                COOLDOWN_HOURS_BAD = 1.0
                                ...
                                AI_ENABLED = False
                                send_telegram(
                                    "🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (الذكاء الاصطناعي معطل، النموذج الحالي محفوظ)")
                                save_state()

                            # ------------------------------------------------------------
                            # 13. أمر رسم منحنى الرصيد الكلي
                            # ------------------------------------------------------------
                                ...
                                try:
                                    ...
                                    plt.plot(times, values, marker='.', linestyle='-', linewidth=1.5, markersize=3,
                                             color='blue')
                                    plt.title('📈 منحنى الرصيد الكلي (Total Equity) – آخر {} نقطة'.format(len(points)),
                                              fontsize=14)
...
class MarketScanner:
    ...
    def scan(self):
        ...
        try:
                ...
                if new5 != _last_scan_candidates:
                    ...
                    if next_candidates:
                        msg = "<b>🔄 فرص بديلة للتحليل (القائمة الأولى ثابتة)</b>\n" + "\n".join(
                            f"{sym} ({self.scores[sym]:.1f})" for sym in next_candidates)
                        send_telegram(msg)
                    else:
            ...
            all_symbols = set(BASE_SYMBOLS + self.candidates)
            ws_manager.update_subscriptions(all_symbols)
        except Exception as e:
            logger.error(f"خطأ في MarketScanner.scan(): {e}")
            send_telegram(f"⚠️ خطأ في مسح السوق: {str(e)[:100]}")
        finally:
            if self.last_scan == 0:
                self.last_scan = time.time()

    def should_scan(self):
        return (time.time() - self.last_scan) > SCAN_INTERVAL_MINUTES * 60
    ...
    try:
        ...
        for sym, p in open_positions.items():
            ...
            pos_dict[sym] = {
                ...
                'stop_loss': p.stop_loss, 'symbol_type': p.symbol_type,
                'take_profit_levels': tp_levels_serializable,
                ...
                'momentum_calculated': p.momentum_calculated
            }
        data = {
            ...
            'filters': {
                ...
                'detected_model': _detected_model
            },
            'custom_settings': {
        ...
        if os.path.exists(STATE_FILE):
            try:
                os.replace(STATE_FILE, STATE_BAK_FILE)
            except:
                pass
...
def load_state():
    ...
    def _load_from_file(filepath):
        ...
        try:
            ...
            logger.warning(f"فشل تحميل {filepath}: {e}")
            return None
    ...
    if data is None:
        ...
        if data:
            logger.info("✅ تم استعادة الحالة من النسخة الاحتياطية")
        else:
            logger.info("ℹ️ لا توجد حالة سابقة. البدء من الصفر.")
            return
    if not _global_state_lock.acquire(timeout=5):
    ...
    try:
        for sym, pdata in data.get('open_positions', {}).items():
            ...
            pos.entry_momentum_time = datetime.fromisoformat(pdata.get('entry_momentum_time')) if pdata.get(
                'entry_momentum_time') else None
            ...
            pos.crash_monitor_start = datetime.fromisoformat(pdata['crash_monitor_start']) if pdata.get(
                'crash_monitor_start') else None
            ...
            pos.last_target_hit_time = datetime.fromisoformat(pdata['last_target_hit_time']) if pdata.get(
                'last_target_hit_time') else None
            ...
            pos.momentum_calculated = pdata.get('momentum_calculated', False)
        ...
        for sym in list(open_positions.keys()):
            if not validate_restored_position(sym, open_positions[sym]):
                ...
                last_close_time[sym] = datetime.now()
        if removed:
            send_telegram(f"⚠️ تم اكتشاف {len(removed)} مركزاً أُغلقت أثناء التعطل: {', '.join(removed)}")
            save_state()
        ...
        daily_loss_cooldown_until = datetime.fromisoformat(data['daily_loss_cooldown_until']) if data.get(
            'daily_loss_cooldown_until') else None
        ...
        FILTER_QUANTUM_ENABLED = filters.get('quantum_enabled', True)
        QUANTUM_THRESHOLD = filters.get('quantum_threshold', 0.55)
        ...
        _detected_model = filters.get('detected_model', None)
        ...
        if 'buying_committee' in globals():
            buying_committee.apply_multiplier(CURRENT_BUY_COMMITTEE_MULTIPLIER)
...
if __name__ == '__main__':
    ...
    ws_manager.start(list(BASE_SYMBOLS))
    logger.info("✅ تم بدء تشغيل AdvancedWebSocket Manager")
    time.sleep(5)
    ...
    with _global_state_lock:
        for sym, pos in open_positions.items():
            if not pos.momentum_calculated:
                pos.calculate_momentum_later()
    ...
    logger.info(f"🚀 بدء البوت v28.1 (MTF + Full WebSocket - المعدل) - الوضع: {mode_str}")
    send_telegram(
        f"🚀 بوت v28.1 (مع الفلتر الكمي MTF) يعمل – الوضع: {mode_str}\n"
        f"✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n"
        f"✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n"
        f"✅ تقرير يومي يعرض صافي الربح/الخسارة\n"
        f"✅ التحويل بين التداول الورقي والحقيقي\n"
        ...
        f"✅ WebSocket خالص مع إعادة تشغيل تلقائي (شموع 5م، 1س، 4س، دفتر، صفقات، تيكرات)\n"
        f"✅ فلتر كمي (MTF) بثلاثة أطر مع أوامر تشغيل/إيقاف وتعيين العتبة (`شغل فلتر الكم`, `اوقف فلتر الكم`, `عتبة الكم 0.60`)\n"
        f"✅ مراقبة وإعادة تشغيل جميع الخيوط بما فيها WebSocket (مع حماية watchdog)\n"
        f"✅ جلب صبور للرصيد الحقيقي (حتى 120 ثانية)\n"
        f"✅ سرعة مراقبة المراكز 2 ثانية\n"
        f"✅ التحكم بدرجة الاشتباه و عتبة البيع وعتبات الخسارة الوقائية\n"
        ...
        f"✅ **إصلاح مشكلة توقف الفرص بعد عدة دورات**\n"
        f"✅ **إصلاح آلية القفل (العودة إلى منطق v27.8 الآمن)**\n"
        f"✅ **إصلاح التسخين الأولي (عدم الخروج من الدالة)**\n"
        f"✅ **إصلاح last_access_time للرموز**\n"
        f"✅ **زيادة عدد الرموز المشترك فيها إلى 300**")
    ...
    if GEMINI_API_KEY:
        try:
            ...
            if not test_gemini_api_key():
                ...
                model_disp = _detected_model if _detected_model else "غير مكتشف"
                send_telegram(f"🤖 الذكاء الاصطناعي (Gemini) مفعّل، النموذج الحالي: {model_disp} (يمكن إيقافه بأمر `أوقف الذكاء`)")
        except Exception as e:
