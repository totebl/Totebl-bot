# ========================================================================
# بوت التداول المتطور v24.52 - تعديلات الماسح لتوسيع الفرص
# - MIN_24H_CHANGE_PERCENT = 1.0 (1%)
# - MIN_24H_VOLUME_USD = 100000
# - MIN_MARKET_CAP_USD = 2000000 (كما هي)
# - ACTIVE_SYMBOLS_LIMIT = 20
# - الحفاظ على جميع التحديثات السابقة: نظام شك موحد، عتبة 65%، إلخ
# - تعديل آلية البيع عند الخسارة: إزالة شرط الزخم، بيع 25% عند 1.5%، 25% عند 2%، 100% عند 3%
# - إضافة شرط الشراء: السعر الحالي > إغلاق آخر شمعة ساعة
# - تخفيض عتبات الشراء: ميم 0.20، عادي 0.25
# - إعادة محاولة جلب الشموع كل 60 ثانية حتى النجاح (لمدة تصل إلى 5 دقائق)
# ========================================================================
...
# تخفيض عتبات الشراء
STRENGTH_THRESHOLD = float(os.environ.get('STRENGTH_THRESHOLD', 0.25))  # كانت 0.30
SCALP_MIN_PROFIT = float(os.environ.get('SCALP_MIN_PROFIT', 0.20))      # كانت 0.25
...
def fetch_ohlcv_persistent(symbol, timeframe, limit=500, max_duration_seconds=300, retry_interval=60):
    """يحاول جلب الشموع بشكل متكرر كل retry_interval ثانية حتى النجاح أو انتهاء max_duration_seconds"""
    start_time = time.time()
    attempt = 0
    while time.time() - start_time < max_duration_seconds:
        df = fetch_ohlcv_retry_raw(symbol, timeframe, limit, max_retries=1)  # محاولة واحدة سريعة
        if not df.empty:
            return df
        attempt += 1
        logger.info(f"⏳ فشل جلب {symbol} {timeframe}، محاولة {attempt} بعد {retry_interval} ثانية...")
        time.sleep(retry_interval)
    logger.error(f"❌ فشل جلب {symbol} {timeframe} بعد {max_duration_seconds} ثانية من المحاولات.")
    return pd.DataFrame()
...
# --------------------------- لجنة الشراء (تم إزالة فلتر overbought) ---------------------------
...
class Position:
    def __init__(self, symbol, side, size, entry, atr, sl, tp, sym_type, pred, conf, regime, tp_levels=None):
        ...
        # إضافة متغيرات تتبع البيع عند الخسارة
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # ===================== آلية البيع عند الخسارة (بدون شرط الزخم) =====================
                if profit < 0:
                    # عند خسارة 1.5% ولم يتم البيع بعد
                    if profit <= -0.015 and not pos.sold_at_15:
                        # نبيع 25% من الحجم الأصلي
                        sell_percent = 0.25
                        close_partial(sym, sell_percent, cur_price, f"⚠️ بيع وقائي: خسارة {abs(profit):.2%} (بيع 25%)")
                        pos.sold_at_15 = True
                        save_state()
                    
                    # عند خسارة 2% ولم يتم البيع عند 2%
                    elif profit <= -0.02 and not pos.sold_at_20:
                        # نبيع 25% إضافية من الحجم الأصلي (ولكن نأخذ الحجم المتبقي)
                        # نحسب 25% من الحجم الأصلي
                ...
                # فشل تحقيق الهدف الأول بعد ساعة
                if profit > 0.011:
...
def analyze_and_trade():
    ...
    # إصلاح المتغيرات المفقودة - إضافة جميع المتغيرات المستخدمة في التقرير اليومي
    global _daily_total_holding_time_win, _daily_total_holding_time_loss, _daily_holding_count_win, _daily_holding_count_loss
    global _daily_biggest_win, _daily_biggest_loss, _daily_most_traded
    ...
    try:
        ...
        for sym in active:
            ...
            if regime == 'trending_down':
                continue
            # استخدام الدالة الجديدة للمحاولة المستمرة لجلب الشموع (5 دقائق)
            df_primary = fetch_ohlcv_persistent(sym, TIMEFRAMES['primary'], limit=500, max_duration_seconds=300, retry_interval=60)
        ...
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            ...
            if dec != 'buy':
                continue
            if sym_type == 'meme':
                if score < SCALP_MIN_PROFIT:  # الآن 0.20
                    continue
            else:
                if score < STRENGTH_THRESHOLD:  # الآن 0.25
                    continue
            ...
            # ---------- الشرط الجديد: السعر الحالي > إغلاق آخر شمعة ساعة (مع محاولة مستمرة) ----------
            df_1h = fetch_ohlcv_persistent(sym, '1h', limit=2, max_duration_seconds=180, retry_interval=60)
            if df_1h.empty or len(df_1h) < 2:
                logger.debug(f"تخطي {sym}: لا توجد بيانات كافية لشمعة ساعة بعد محاولات متعددة")
                continue
            last_completed_close = df_1h['close'].iloc[-2]  # آخر شمعة مكتملة
            if cur_price <= last_completed_close:
                logger.debug(f"تخطي {sym}: السعر الحالي {cur_price} <= إغلاق آخر ساعة {last_completed_close}")
                continue
            # ----------------------------------------------------------------
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v24.52 – ماسح موسع (حجم 100k, تغير 1%, تحليل 20 رمزاً)", "version":"24.52"})
...
if __name__ == '__main__':
    ...
    send_telegram(f"🚀 بوت v24.52 يعمل – الوضع: {mode_str}\n✅ الماسح: أقل تغير 1% | أقل حجم 100k$ | تحليل 20 رمزاً\n✅ نظام شك موحد (متوسط حسابي) عتبة 65%\n✅ درجة الحيتان تقدمية (0.3 لكل حوت)\n✅ إزالة الرفض المنفرد للحيتان\n✅ فحص السيولة مخفف\n✅ أمر تلغرام 'صفقاتي'\n✅ آلية وقاية الخسارة الواسعة\n✅ بيع عند الخسارة بدون زخم: 25% عند 1.5%، 25% عند 2%، 100% عند 3%\n✅ شرط الشراء: السعر الحالي > إغلاق آخر شمعة ساعة\n✅ تخفيض عتبات الشراء: ميم 0.20، عادي 0.25\n✅ إعادة محاولة جلب الشموع كل 60 ثانية حتى النجاح")
