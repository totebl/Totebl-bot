# ========================================================================
# بوت التداول v27.8 – إصدار مستقر مع إصلاحات شاملة وإضافة جميع الأوامر المفقودة
# + إضافة رسم منحنى الرصيد الكلي (أمر 'منحنى')
# + إضافة أمر تلغرام للتحكم بعدد الصفقات اليومية المسموحة (عدد الصفقات المسموحة)
# + تعديلات أساسية: تحسين تنفيذ الأوامر (Limit → Market Fallback بعد 5 ثوانٍ، هامش 0.3%)
# + تعديل أمر الوقف المسبق إلى stop_market
# + تقليل مدة مراقبة المراكز إلى 2 ثانية
# ========================================================================
...
LIMIT_TO_MARKET_TIMEOUT = int(os.environ.get('LIMIT_TO_MARKET_TIMEOUT', 5))  # ثوانٍ
...
<head><meta charset="UTF-8"><title>بوت v27.8</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v27.8 (محدث ومُصلح)</h1>
<div id="stats"></div>
...
def execute_limit_order(symbol, side, size, price_ref, sym_type='normal'):
    timeout = LIMIT_TO_MARKET_TIMEOUT  # 5 ثوانٍ
    if _should_simulate():
...
# ======================= دالة الوقف المسبق الموحدة (معدلة لاستخدام stop_market) =======================
def place_preemptive_stop_loss(symbol, size, entry_price, sym_type, stop_price_override=None):
    ...
    try:
        ...
        # استخدام stop_market لضمان التنفيذ الفوري عند التفعيل
        order = exchange.create_order(symbol, 'stop_market', 'sell', size, None, {'stopPrice': stop_price})
        oid = order.get('id', 'unknown')
        logger.info(f"✅ أمر وقف مسبق (stop_market) لـ {symbol} | تفعيل عند {stop_price:.8f} ({(1-stop_price/entry_price)*100:.2f}%) | ID: {oid}")
        return oid
    except Exception as e:
        logger.error(f"فشل stop_market لـ {symbol}: {e}")
        # احتياطي: محاولة stop_loss_limit إذا لم يتم دعم stop_market
        try:
            ...
            logger.warning(f"⚠️ تم استخدام stop_loss_limit كبديل لـ {symbol}")
            return order.get('id')
        except Exception as e2:
            logger.error(f"فشل stop_loss_limit البديل لـ {symbol}: {e2}")
            # آخر حل: أمر سوق فوري
            try:
                order = exchange.create_market_sell_order(symbol, size)
                logger.warning(f"🚨 تم استخدام أمر سوق طارئ لـ {symbol} (فشل جميع أوامر الوقف)")
                return order.get('id')
            except Exception as e3:
                logger.error(f"فشل أمر السوق الطارئ لـ {symbol}: {e3}")
                return None
    ...
    try:
        ...
        for sym in symbols_to_check:
            try:
                ...
                stop_orders = [o for o in orders if o.get('type') in ('stop_market', 'stop_loss_limit')]
