...
#   - إزالة BASE_SYMBOLS من دورة التحليل وتحميل الكاش الأولي
#   - رفع TTLs لتقليل استهلاك الوزن: الشموع 60، دفتر الطلبات 5، الرصيد 5 دقائق، المزامنة 30 ثانية
#   - التحديث 3: نظام بيع هرمي معدل (استخدام درجة الثقة مباشرة، شروط ≤، ترتيب البيع الكامل أولاً)
#   - إصلاح خطأ التهيئة: تمرير save=False في apply_multiplier لتجنب استدعاء save_state قبل تعريفها
#   - التحديثات الجديدة (تم تطبيقها):
#     * تعديل القيم الافتراضية لـ SELL_MEMBER_WEIGHTS و SELL_FRAME_WEIGHTS في load_state
#     * إضافة حماية في monitor_positions ضد الأوزان الصفرية
#     * دالة distribute_weights تعيد توزيعاً متساوياً بدلاً من الأصفار (موجود بالفعل)
#   - التحديث 5: إعادة ترتيب أوامر الفلتر في telegram_polling (فحص الفلتر الثاني أولاً)
#   - التحديث 6: تغيير القيم الافتراضية للفلتر الأول إلى rule, 5m, 0.11
#   - التحديث الجديد 7: تحسين اختيار المرشحين – إذا كانت أول 5 رموز ثابتة، انتقل إلى الـ12 التالية.
#   - التحديث الجديد 8: إضافة فلتر النموذج الثالث (مطابق للفلترين) مع أوامر تلغرام وحفظ الحالة.
#   - التحديث 9: إعادة ترتيب أوامر الفلاتر (الثالث، الثاني، الأول) وإضافة كلمة "الأول" لأوامر الفلتر الأول.
#   - التحديث 10: إضافة فلتر رابع (حد أدنى) نموذج VWAP_OBV على 15m عتبة 0.30.
#   - التحديث 11: إضافة فلتر خامس (حد أعلى) نموذج Timing على 1h عتبة 0.70.
# جميع الميزات الأخرى (الوقف المسبق، الذكاء الاصطناعي، الفلاتر، إلخ) محفوظة كما هي.
# ========================================================================

import subprocess, sys, os, logging, time, warnings, threading, atexit, signal, importlib, re, traceback, io
...
import random  # تم الإضافة للتأخير العشوائي
...
# تثبيت مكتبة Google Gemini تلقائياً - مع منع إيقاف البوت عند الفشل
try:
    import google.generativeai as genai
except ImportError:
    if _auto_install("google-generativeai"):
        import google.generativeai as genai
    else:
        AI_ENABLED = False
        logger = None  # سيتم تعريف logger لاحقاً لكننا نمنع رفع الاستثناء
        print("❌ فشل تثبيت google-generativeai، تم تعطيل الذكاء الاصطناعي")
        genai = None
...
MAX_EXPOSED_PERCENT = float(os.environ.get('MAX_EXPOSED_PERCENT', 1.0))  # ✅ 100%
...
STRENGTH_THRESHOLD = float(os.environ.get('STRENGTH_THRESHOLD', 0.55))  # ✅ 0.55
SCALP_MIN_PROFIT = float(os.environ.get('SCALP_MIN_PROFIT', 0.55))      # ✅ 0.55
...
ACTIVE_SYMBOLS_LIMIT = int(os.environ.get('ACTIVE_SYMBOLS_LIMIT', 12))  # ✅ 12
MAX_DAILY_TRADES = int(os.environ.get('MAX_DAILY_TRADES', 45))           # ✅ 45
...
MANIPULATION_REJECT_THRESHOLD = float(os.environ.get('MANIPULATION_REJECT_THRESHOLD', 0.12))  # ✅ 12%

# --------------------------- تبريد الصفقات المتكررة (الجديد - قابل للتعديل عبر تلغرام) ---------------------------
COOLDOWN_WIN_HOURS = float(os.environ.get('COOLDOWN_WIN_HOURS', 8.0))       # ✅ 8 ساعات
COOLDOWN_LOSS_MINUTES = float(os.environ.get('COOLDOWN_LOSS_MINUTES', 120.0))  # ✅ 120 دقيقة
_symbol_cooldown_until = {}  # تخزين وقت انتهاء التبريد لكل رمز: {symbol: datetime}
_cooldown_lock = threading.Lock()  # قفل للوصول الآمن

# --------------------------- إعدادات Google Gemini (ثابتة – بدون بحث ديناميكي) ---------------------------
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY')
# ================================================================
# ✅ تم التحديث إلى الإصدارات الأحدث المتاحة (gemini-2.5)
# ================================================================
DEFAULT_FLASH_MODEL = "gemini-2.5-flash"
DEFAULT_FLASH_LITE_MODEL = "gemini-2.5-flash-lite"
GEMINI_MODEL_FLASH = DEFAULT_FLASH_MODEL
GEMINI_MODEL_FLASH_LITE = DEFAULT_FLASH_LITE_MODEL
AI_ENABLED = os.environ.get('AI_ENABLED', 'false').lower() == 'true'   # تشغيل/إيقاف استشارة AI
CURRENT_AI_MODEL = os.environ.get('CURRENT_AI_MODEL', 'flash')         # 'flash' أو 'flash-lite'
AI_TIMEOUT = 65                                                         # المهلة 65 ثانية
AI_FALLBACK_ACTION = os.environ.get('AI_FALLBACK_ACTION', 'allow')     # allow or reject
AI_NOTIFICATION_COOLDOWN = 60                                           # تبريد إشعارات تلغرام (ثانية)
_last_ai_notification_time = 0                                          # آخر وقت تم إرسال إشعار فيه

# دوال البحث عن أحدث النماذج تم حذفها، نستخدم الثوابت مباشرة
def refresh_gemini_models():
    # هذه الدالة أصبحت فارغة لأن النماذج ثابتة ولا نحتاج لتحديثها
    pass
...
def test_gemini_api_key():
    ...
    try:
        ...
        flash_ok = any(m.endswith(GEMINI_MODEL_FLASH) for m in model_names)
        lite_ok = any(m.endswith(GEMINI_MODEL_FLASH_LITE) for m in model_names)
        if not (flash_ok and lite_ok):
            logger.warning(f"⚠️ النماذج المطلوبة غير متاحة: Flash={flash_ok}, Lite={lite_ok}")
            return False
        return True
    except Exception as e:
        logger.error(f"فشل اختبار مفتاح Gemini: {e}")
        return False
...
def is_model_available(model_name):
    ...
    try:
        genai.configure(api_key=GEMINI_API_KEY)
        models = genai.list_models()
        return any(m.name.endswith(model_name) for m in models)
    except Exception as e:
        logger.error(f"خطأ في التحقق من النموذج {model_name}: {e}")
        return False

# --------------------------- عتبات الخسارة الوقائية (قابلة للتعديل عبر تلغرام) ---------------------------
STOP_LOSS_PARTIAL_1_PERCENT = 0.0122   # ✅ 1.22%
STOP_LOSS_PARTIAL_2_PERCENT = 0.0138   # ✅ 1.38%
STOP_LOSS_FULL_PERCENT = 0.0159        # ✅ 1.59%
...
# --------------------------- المتغيرات العامة ---------------------------
PAUSED = False
PAUSE_NEW_ENTRIES = False   # إضافة: منع دخول الصفقات الجديدة فقط
PAUSE_ANALYSIS = False      # جديد: إيقاف التحليل ومسح السوق (المراكز المفتوحة مستمرة)

# --------------------------- متغير التحكم في المضاعف الجديد ---------------------------
USE_1H_MULTIPLIER = True   # ✅ مفعّل

# --------------------------- فلتر أداء X يوم (عبر ccxt) ---------------------------
FILTER_30D_PROFIT_ENABLED = os.environ.get('FILTER_30D_PROFIT_ENABLED', 'true').lower() == 'true'  # ✅ مفعّل
MIN_30D_PROFIT_PERCENT = float(os.environ.get('MIN_30D_PROFIT_PERCENT', 0.10))   # ✅ 10%
FILTER_30D_DAYS = int(os.environ.get('FILTER_30D_DAYS', 3))                    # ✅ 3 أيام
_30d_performance_cache = {}
_30d_cache_lock = threading.Lock()

# ========== فلتر النموذج الواحد (جديد - يحل محل جمع Rule+EMR) ==========
# ✅ التحديث 6: تغيير القيم الافتراضية للفلتر الأول إلى rule, 5m, 0.11
SINGLE_MODEL_FILTER_MODEL = os.environ.get('SINGLE_MODEL_FILTER_MODEL', 'rule')          # ✅ rule
SINGLE_MODEL_FILTER_THRESHOLD = float(os.environ.get('SINGLE_MODEL_FILTER_THRESHOLD', 0.11))  # ✅ 0.11
SINGLE_MODEL_FILTER_TIMEFRAME = os.environ.get('SINGLE_MODEL_FILTER_TIMEFRAME', '5m')     # ✅ 5m
SINGLE_MODEL_FILTER_ENABLED = os.environ.get('SINGLE_MODEL_FILTER_ENABLED', 'true').lower() == 'true'  # ✅ مفعّل

# ========== فلتر النموذج الثاني (جديد - مشابه للأول) ==========
SECOND_MODEL_FILTER_MODEL = os.environ.get('SECOND_MODEL_FILTER_MODEL', 'cfhm')          # ✅ cfhm
SECOND_MODEL_FILTER_THRESHOLD = float(os.environ.get('SECOND_MODEL_FILTER_THRESHOLD', 0.59))  # ✅ 0.59
SECOND_MODEL_FILTER_TIMEFRAME = os.environ.get('SECOND_MODEL_FILTER_TIMEFRAME', '5m')     # ✅ 5m
SECOND_MODEL_FILTER_ENABLED = os.environ.get('SECOND_MODEL_FILTER_ENABLED', 'true').lower() == 'true'  # ✅ مفعّل
# =====================================================================

# ========== فلتر النموذج الثالث (جديد) ==========
THIRD_MODEL_FILTER_MODEL = os.environ.get('THIRD_MODEL_FILTER_MODEL', 'cfhm')
THIRD_MODEL_FILTER_THRESHOLD = float(os.environ.get('THIRD_MODEL_FILTER_THRESHOLD', 0.50))
...
# ========== فلتر النموذج الرابع (جديد - حد أدنى) ==========
FOURTH_MODEL_FILTER_MODEL = os.environ.get('FOURTH_MODEL_FILTER_MODEL', 'vwap_obv')
FOURTH_MODEL_FILTER_THRESHOLD = float(os.environ.get('FOURTH_MODEL_FILTER_THRESHOLD', 0.30))
...
# ========== فلتر النموذج الخامس (جديد - حد أعلى) ==========
FIFTH_MODEL_FILTER_MODEL = os.environ.get('FIFTH_MODEL_FILTER_MODEL', 'timing')
FIFTH_MODEL_FILTER_THRESHOLD = float(os.environ.get('FIFTH_MODEL_FILTER_THRESHOLD', 0.70))
...
PREEMPTIVE_SL_UPDATE_ENABLED = False   # ✅ معطّل

# عتبات الوقف المسبق الحقيقي (تُرسل إلى Binance)
CUSTOM_PREEMPTIVE_SL_TRIGGER = 0.025   # ✅ 2.5%
CUSTOM_PREEMPTIVE_SL_LIMIT = 0.032     # ✅ 3.2%
# ============================================================

# ========== نظام تتبع الأرباح الجديد (يفعل فوراً) ==========
TRAILING_DISTANCE_PERCENT = float(os.environ.get('TRAILING_DISTANCE_PERCENT', 3.22)) / 100.0  # ✅ 3.22%
# ======================================================================

# =========================== التحديث 3: عتبات البيع الهرمي ===========================
SELL_PARTIAL_THRESHOLD = float(os.environ.get('SELL_PARTIAL_THRESHOLD', 0.40))   # بيع 50%
SELL_FULL_THRESHOLD = float(os.environ.get('SELL_FULL_THRESHOLD', 0.30))         # بيع 100%

# ===== أوزان البيع المستوحاة من التوصية =====
SELL_MEMBER_WEIGHTS = {'rule': 0.05, 'emr': 0.10, 'cfhm': 0.30, 'timing': 0.45, 'vwap_obv': 0.10}
SELL_FRAME_WEIGHTS = {'5m': 0.30, '15m': 0.20, '1h': 0.50}
# ======================================================================

# ======================== تم تحديث SELL_WEIGHTS إلى النسب النهائية ========================
SELL_WEIGHTS = {
    'rule': 0.10,
    'emr': 0.05,
    'cfhm': 0.10,
...
SELL_15M_CHECK_INTERVAL = 540  # كل 540 ثانية (9 دقائق)
_last_15m_sell_check = {}
_15m_check_lock = threading.Lock()
# ========== أوزان الأطر الزمنية لحساب المتوسط النهائي (قابلة للتعديل عبر تلغرام) ==========
# ======================== تم تحديث القيم الافتراضية إلى النسب النهائية ========================
WEIGHT_5M = 50.0   # ✅ 50%
WEIGHT_15M = 1.0   # ✅ 1%
WEIGHT_1H = 49.0   # ✅ 49%
# المجموع يجب أن يساوي 100 (يتم التحقق عند التعديل)
# ============================================================

# =========================== التحديث 2: ملف التوصيات الجديد ===========================
RECOMMENDATION_LOG_FILE = "data/trade_recommendations_log.csv"
RECOMMENDATION_LOG_HEADERS = [
    'trade_id', 'symbol', 'entry_time', 'entry_price', 'total_size',
    'scores_5m_rule', 'scores_5m_emr', 'scores_5m_cfhm', 'scores_5m_timing', 'scores_5m_vwap',
    'scores_15m_rule', 'scores_15m_emr', 'scores_15m_cfhm', 'scores_15m_timing', 'scores_15m_vwap',
    'scores_1h_rule', 'scores_1h_emr', 'scores_1h_cfhm', 'scores_1h_timing', 'scores_1h_vwap',
    'weighted_5m', 'weighted_15m', 'weighted_1h', 'final_score',
    'exit_time', 'exit_price', 'total_pnl', 'status'
]
# ======================================================================

# ===== الحد الأعلى الموحد للثقة (جديد) =====
UPPER_THRESHOLD_GLOBAL = 0.60   # 60% حد أعلى

# ===== متغير التحديث الجديد: تذكر آخر 12 رمزاً تم تحليلها (بدون المراكز المفتوحة) =====
...
open_positions = {}
# last_close_time = {}  # <-- تم إلغاؤه نهائياً واستبداله بـ _symbol_cooldown_until
...
STALE_DATA_MAX_AGE = int(os.environ.get('STALE_DATA_MAX_AGE', 120))  # تم التعديل من 90 إلى 120
...
# ========== التعديل الرئيسي: أصبح لدينا 3 أطر زمنية حقيقية ==========
TIMEFRAMES = {'primary': '15m', 'confirm_1': '5m', 'confirm_2': '1h'}   # أصبح الأساسي 15 دقيقة، والتأكيدي 5 دقائق
SCALP_TIMEFRAME = '15m'  # أصبح إطار عملات الميم 15 دقيقة أيضاً
# ============================================================
...
# FILTER_4H_CANDLE_ENABLED = True  # تم حذفه
...
# ------------------- متغيرات auto_recovery (مع القفل المفقود سابقاً) -------------------
...
CUSTOM_ACTIVE_SYMBOLS_LIMIT = ACTIVE_SYMBOLS_LIMIT
CUSTOM_MAX_EXPOSED_PERCENT = MAX_EXPOSED_PERCENT
CUSTOM_POSITION_PERCENT_GOOD = MAX_POSITION_PERCENT_GOOD
CUSTOM_POSITION_PERCENT_NORMAL = 0.175   # ✅ 17.5%
CUSTOM_POSITION_PERCENT_BAD = 0.15       # ✅ 15%
CUSTOM_MAX_DAILY_TRADES = MAX_DAILY_TRADES   # جديد: عدد الصفقات المسموحة يومياً (قابل للتعديل عبر تلغرام)
CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP = 75   # ✅ 75 دقيقة

# ------------------- متغيرات جديدة للتحكم في التحول من Limit إلى Market -------------------
LIMIT_TO_MARKET_TIMEOUT = int(os.environ.get('LIMIT_TO_MARKET_TIMEOUT', 10))  # تم التعديل إلى 10 ثوانٍ
ORDER_POLL_INTERVAL = float(os.environ.get('ORDER_POLL_INTERVAL', 1.0))      # جديد: فاصل التحقق 1 ثانية
MARKET_ORDER_FALLBACK = os.environ.get('MARKET_ORDER_FALLBACK', 'true').lower() == 'true'
POSITION_MONITOR_INTERVAL = float(os.environ.get('POSITION_MONITOR_INTERVAL', 1.0))  # تم التعديل إلى 1 ثانية
...
def send_telegram_photo(buf, caption=""):
    """إرسال صورة إلى تلغرام باستخدام BytesIO"""
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        return
    try:
...
class BotStats:
    ...
    def add_equity_point(self, total_equity):
        """تخزين الرصيد الكلي (الرصيد الحر + قيمة المراكز) بدلاً من الربح التراكمي"""
...
class Position:
    def __init__(self, symbol, side, size, entry, atr, sl, tp, sym_type, pred, conf, regime, tp_levels=None, ai_approved=False,
        ...
        self.tp_hit_count = 0   # جديد: عدد مرات تحقيق أهداف الربح
        self.ai_approved = ai_approved
        # ===== التحديث 2: تخزين الدرجات =====
    ...
    def _calc_initial_momentum(self):
        try:
            # استخدام الإطار الأساسي (15 دقيقة)
            df = fetch_ohlcv_retry(self.symbol, TIMEFRAMES['primary'], limit=50)
            if len(df)<15: return
            # حساب lookback بناءً على دقائق التحقق (القسمة على 15 لأن الإطار 15 دقيقة)
    ...
    def update(self, cur_price):
        ...
        # ===== التعديل الجديد: تفعيل التتبع فوراً باستخدام النسبة الديناميكية =====
        ...
        try:
            if PAPER_TRADING:
                ...
                for sym, p in pos_list:
                    # الاعتماد على WebSocket أولاً
                    ws_data = ws_manager.get_ticker(sym)
                    if ws_data and ws_data.get('price', 0) > 0:
                        price = ws_data['price']
                    else:
                        # محاولة جلب السعر من API مع إعادة محاولات (محاولة واحدة فقط)
                        ticker = None
                        for retry in range(3):
                            ...
                            price = p.entry_price
                            logger.warning(f"⚠️ فشل جلب السعر الحالي لـ {sym} بعد 3 محاولات (كل 5 ثوانٍ)، استخدام سعر الدخول {price:.8f}")
...
def get_real_balance_usdt(max_retries=5, delay=3.0, silent=True):
    ...
    for attempt in range(max_retries):
        try:
            rest_rate_limiter.wait_if_needed(weight=20)  # وزن عالي لجلب الرصيد
            bal = exchange.fetch_balance()
...
primary_exchange = ccxt.binance({
    ...
    'rateLimit': 3000,
    'timeout': 10000,   # تم خفض المهلة إلى 10 ثوانٍ
    'options': {
        'defaultType': 'spot',
        'recvWindow': 60000,
        'adjustForTimeDifference': True,
        'useServerTime': True,   # تم التفعيل
    },
})
...
def load_markets_with_retry(max_retries=3, initial_delay=30):
    for attempt in range(max_retries):
        try:
            rest_rate_limiter.wait_if_needed(weight=20)  # تم الإضافة لمراقبة الوزن
            if attempt == 0:
                wait = initial_delay
            else:
...
class RateLimiter:
    ...
    def wait_if_needed(self, weight=1):
        with self.lock:
            now = time.time()
            # حذف الطلبات الأقدم من 60 ثانية
            ...
            # إذا كان الوزن المتبقي أقل من 50، انتظر حتى نهاية الدقيقة
            if remaining < 50:
                sleep_time = self.period - (now - self.calls[0][0]) + 0.5
                if sleep_time > 0:
                    time.sleep(sleep_time)
                self.calls.clear()  # نبدأ دقيقة جديدة
...
def fetch_tickers_with_retry(max_retries=2):
    ...
    for attempt in range(max_retries):
        try:
            rest_rate_limiter.wait_if_needed(weight=40)  # ✅ وزن 40 (بدلاً من 20)
        ...
        try:
            ...
            if not bars:
                ...
                if attempt < max_retries:
                    time.sleep(2**attempt + 4)  # 🔥 التغيير الثالث: زيادة التأخير بمقدار 2 ثانية
                continue
...
def add_advanced_features(df):
    ...
    # ===== إضافة VWAP و OBV (لنموذج VWAP_OBV) =====
    typical_price = (df['high'] + df['low'] + df['close']) / 3
    df['vwap'] = (typical_price * df['volume']).cumsum() / df['volume'].cumsum()
    
    # 🔧 تم تغيير 0 إلى 0.0 لمنع خطأ LossySetitemError
    df['obv'] = 0.0
...
# --------------------------- دالة جلب أداء X يوم (عبر ccxt) - معدلة حسب التغييرات 1 و 2 و 3 ---------------------------
def get_30d_performance(symbol, days=None):
    """
    تعيد نسبة التغير الإجمالي لعدد محدد من الأيام باستخدام السعر الحالي مقارنة بالسعر منذ X يوم.
    إذا فشل جلب السعر الحالي، يتم تخطي الفلتر (إرجاع None).
    """
    if days is None:
        days = FILTER_30D_DAYS
    if days < 1:
        return None
    ...
    with _30d_cache_lock:
        if cache_key in _30d_performance_cache:
            entry = _30d_performance_cache[cache_key]
            if now - entry['timestamp'] < cache_ttl:
                return entry['value']
    
    try:
        # 1. جلب الشموع اليومية للحصول على السعر القديم (منذ X يوم) - زيادة المحاولات إلى 3
        df = fetch_ohlcv_retry_raw(symbol, '1d', limit=days + 1, max_retries=3)
        if df.empty or len(df) < 2:
            logger.warning(f"⚠️ بيانات غير كافية لأداء {days} يوم لـ {symbol}")
            return None
        
        price_old = df['close'].iloc[0]   # السعر منذ days يوم
        
        # 2. جلب السعر الحالي الفوري - زيادة المحاولات إلى 3 (التغيير الثالث)
        ticker = fetch_ticker_with_retry(symbol, max_retries=3)
        if ticker is None or ticker.get('last') is None:
            # 🔥 التغيير الثاني: فشل الجلب → نتخطى الفلتر (نعيد None)
            logger.warning(f"⚠️ فشل جلب السعر الحالي لـ {symbol} بعد 3 محاولات، سيتم تخطي فلتر الأداء لهذه العملة")
            # نخزن None في الكاش مؤقتاً لتجنب المحاولات المتكررة الفاشلة (مثلاً 5 دقائق)
            with _30d_cache_lock:
                _30d_performance_cache[cache_key] = {'value': None, 'timestamp': now}
            return None
        
        # 🔥 التغيير الأول: استخدام السعر الحالي الفوري بدلاً من آخر شمعة
        price_now = ticker['last']
        
        if price_old <= 0:
            return None
        
        change_percent = (price_now - price_old) / price_old * 100.0
        
        with _30d_cache_lock:
            _30d_performance_cache[cache_key] = {'value': change_percent, 'timestamp': now}
            if len(_30d_performance_cache) > 500:
                oldest = min(_30d_performance_cache.items(), key=lambda x: x[1]['timestamp'])[0]
                del _30d_performance_cache[oldest]
        
        return change_percent
    except Exception as e:
        logger.warning(f"فشل جلب أداء {days} يوم لـ {symbol}: {e}")
        return None

def check_30d_profit(symbol):
    """تعيد True إذا كان الرمز مقبولاً وفقاً للفلتر، وإلا False."""
    if not FILTER_30D_PROFIT_ENABLED:
        return True
    change = get_30d_performance(symbol, FILTER_30D_DAYS)
    if change is None:
        # في حال تعذر الجلب، نسمح بالدخول (تخطي الفلتر)
        return True
    return change >= MIN_30D_PROFIT_PERCENT
...
def get_market_condition():
    """تحسين حالة السوق باستخدام نسبة الرابحين/الخاسرين (Advance/Decline Ratio)"""
    ...
    if total == 0:
        return 'normal'
    ratio = up / total
    # إذا كانت نسبة الرابحين أكثر من 60% -> جيد، أقل من 40% -> سيئ
    if ratio > 0.60:
        return 'good'
    elif ratio < 0.40:
        return 'bad'
    ...
    with _global_state_lock:
        ...
        with _market_condition_lock:
            if _last_market_condition == 'good':
                pos_percent = CUSTOM_POSITION_PERCENT_GOOD
            elif _last_market_condition == 'bad':
                pos_percent = CUSTOM_POSITION_PERCENT_BAD
            else:
                pos_percent = CUSTOM_POSITION_PERCENT_NORMAL
...
<head><meta charset="UTF-8"><title>بوت v28.0 (Spot + 3 أطر + توصيات ذكية)</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v28.0 (Spot Edition – 3 Timeframes + Smart Recommendations)</h1>
<div id="stats"></div>
...
# =========================== التحديث 2: دوال ملف التوصيات الجديد ===========================
import csv, uuid
from datetime import datetime

def init_recommendation_log():
    """إنشاء ملف التوصيات بالرأس إذا لم يكن موجوداً."""
    if not os.path.exists(RECOMMENDATION_LOG_FILE):
        with open(RECOMMENDATION_LOG_FILE, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(RECOMMENDATION_LOG_HEADERS)
        logger.info(f"✅ تم إنشاء ملف التوصيات الجديد: {RECOMMENDATION_LOG_FILE}")

def add_trade_record(trade_id, symbol, entry_time, entry_price, total_size,
                     scores_5m, scores_15m, scores_1h,
                     weighted_5m, weighted_15m, weighted_1h, final_score):
    """إضافة صف جديد بحالة 'open' عند الدخول."""
    row = [
        trade_id, symbol, entry_time.isoformat(), f"{entry_price:.8f}", f"{total_size:.6f}",
        f"{scores_5m.get('rule', 0):.3f}", f"{scores_5m.get('emr', 0):.3f}",
        f"{scores_5m.get('cfhm', 0):.3f}", f"{scores_5m.get('timing', 0):.3f}",
        f"{scores_5m.get('vwap_obv', 0):.3f}",
        f"{scores_15m.get('rule', 0):.3f}", f"{scores_15m.get('emr', 0):.3f}",
        f"{scores_15m.get('cfhm', 0):.3f}", f"{scores_15m.get('timing', 0):.3f}",
        f"{scores_15m.get('vwap_obv', 0):.3f}",
        f"{scores_1h.get('rule', 0):.3f}", f"{scores_1h.get('emr', 0):.3f}",
        f"{scores_1h.get('cfhm', 0):.3f}", f"{scores_1h.get('timing', 0):.3f}",
        f"{scores_1h.get('vwap_obv', 0):.3f}",
        f"{weighted_5m:.3f}", f"{weighted_15m:.3f}", f"{weighted_1h:.3f}", f"{final_score:.3f}",
        "", "", "", "open"
    ]
    with open(RECOMMENDATION_LOG_FILE, 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(row)

def update_trade_record(trade_id, exit_time, exit_price, total_pnl):
    """تحديث الصف عند الإغلاق الكامل (حالة 'closed')."""
    rows = []
    updated = False
    with open(RECOMMENDATION_LOG_FILE, 'r', newline='', encoding='utf-8') as f:
        reader = csv.reader(f)
        headers = next(reader)
        rows.append(headers)
        for row in reader:
            if row[0] == trade_id and row[-1] == 'open':
                row[-4] = exit_time.isoformat()      # exit_time
                row[-3] = f"{exit_price:.8f}"        # exit_price
                row[-2] = f"{total_pnl:.4f}"          # total_pnl
                row[-1] = "closed"                    # status
                updated = True
            rows.append(row)
    ...
    if updated:
        ...
        logger.info(f"✅ تم تحديث صفقة {trade_id} في سجل التوصيات (PNL={total_pnl:.2f})")
    else:
        logger.warning(f"⚠️ لم يتم العثور على صفقة مفتوحة بالمعرف {trade_id} لتحديثها.")
# ======================================================================

# --------------------------- دالة استشارة Google Gemini (محسنة) ---------------------------
def ask_ai_for_approval(symbol, df, current_price, sym_type, market_condition, ticker_data=None):
    if not AI_ENABLED or not GEMINI_API_KEY or genai is None:
        return True

    # تحديد العتبة حسب نوع العملة
    if sym_type == 'meme':
        threshold = SCALP_MIN_PROFIT
        desc = f"{threshold:.0%} (عملات ميم – تتطلب زخماً قوياً)"
    else:
        threshold = STRENGTH_THRESHOLD
        desc = f"{threshold:.0%} (عملات عادية)"
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(
            _ask_ai_internal,
            symbol, df, current_price, sym_type, market_condition,
            ticker_data, threshold, desc
        )
        try:
            result = future.result(timeout=AI_TIMEOUT)
            return result
        except FutureTimeoutError:
            logger.error(f"⏰ مهلة Gemini ({AI_TIMEOUT} ثانية) لـ {symbol} – استغرقت العملية أكثر من {AI_TIMEOUT} ثانية.")
            global _last_ai_notification_time
            now = time.time()
            if now - _last_ai_notification_time > AI_NOTIFICATION_COOLDOWN:
                send_telegram(f"⏰ مهلة استشارة Gemini لـ {symbol} بعد {AI_TIMEOUT} ثانية – تم السماح بالصفقة تلقائياً (AI_FALLBACK_ACTION={AI_FALLBACK_ACTION}).")
                _last_ai_notification_time = now
            return AI_FALLBACK_ACTION == 'allow'
        except Exception as e:
            logger.error(f"خطأ غير متوقع في استدعاء Gemini لـ {symbol}: {e}")
            return AI_FALLBACK_ACTION == 'allow'

def _ask_ai_internal(symbol, df, current_price, sym_type, market_condition, ticker_data, threshold, desc):
    if df.empty or len(df) < 20:
        logger.warning(f"⚠️ بيانات غير كافية لـ {symbol} لاستشارة AI")
        return AI_FALLBACK_ACTION == 'allow'
    ...
    last_macd_diff = df['macd_diff'].iloc[-1] if 'macd_diff' in df and not pd.isna(df['macd_diff'].iloc[-1]) else None
    last_macd_signal = df['macd_signal'].iloc[-1] if 'macd_signal' in df and not pd.isna(df['macd_signal'].iloc[-1]) else None
    last_volume_ratio = df['volume_ratio'].iloc[-1] if 'volume_ratio' in df and not pd.isna(df['volume_ratio'].iloc[-1]) else None
    last_ema_9 = df['ema_9'].iloc[-1] if 'ema_9' in df and not pd.isna(df['ema_9'].iloc[-1]) else None
    last_ema_21 = df['ema_21'].iloc[-1] if 'ema_21' in df and not pd.isna(df['ema_21'].iloc[-1]) else None
    last_bb_upper = df['bb_upper'].iloc[-1] if 'bb_upper' in df and not pd.isna(df['bb_upper'].iloc[-1]) else None
    last_bb_lower = df['bb_lower'].iloc[-1] if 'bb_lower' in df and not pd.isna(df['bb_lower'].iloc[-1]) else None
    last_bb_width = df['bb_width'].iloc[-1] if 'bb_width' in df and not pd.isna(df['bb_width'].iloc[-1]) else None
    last_atr_percent = df['atr_percent'].iloc[-1] if 'atr_percent' in df and not pd.isna(df['atr_percent'].iloc[-1]) else None
    last_adx = df['adx'].iloc[-1] if 'adx' in df and not pd.isna(df['adx'].iloc[-1]) else None
    momentum_5 = (df['close'].iloc[-1] - df['close'].iloc[-6]) / df['close'].iloc[-6] if len(df) >= 6 else None
    if isinstance(momentum_5, float) and (np.isnan(momentum_5) or np.isinf(momentum_5)):
        momentum_5 = None

    change_24h = ticker_data.get('percentage', 0) if ticker_data else None
    volume_24h_usd = ticker_data.get('quoteVolume', 0) if ticker_data else None
    ema_cross_bullish = (last_ema_9 > last_ema_21) if last_ema_9 is not None and last_ema_21 is not None else None
    ema_cross_text = "نعم" if ema_cross_bullish else ("لا" if ema_cross_bullish is not None else "غير متاحة")

    prompt = f"""أنت خبير تداول. قم بتحليل الرمز {symbol} (نوع: {sym_type}) بناءً على المعطيات الفنية التالية فقط.
السعر الحالي: {current_price:.8f}
آخر سعر إغلاق: {last_close:.8f}
التغير خلال 24 ساعة: {fmt(change_24h, 2)}%
حجم التداول 24 ساعة (USDT): {fmt(volume_24h_usd, 0)} إذا كان متوفراً

المؤشرات الفنية (آخر شمعة):
- RSI (14): {fmt(last_rsi, 2)}
- MACD: {fmt(last_macd_diff, 6)} (فرق) والإشارة: {fmt(last_macd_signal, 6)}
- نسبة الحجم إلى المتوسط: {fmt(last_volume_ratio, 2)}
- EMA 9: {fmt(last_ema_9, 8)} | EMA 21: {fmt(last_ema_21, 8)} | تقاطع صاعد: {ema_cross_text}
- البولينجر باند: علوي {fmt(last_bb_upper, 8)} | سفلي {fmt(last_bb_lower, 8)} | العرض: {fmt(last_bb_width, 3)}
- ATR%: {fmt(last_atr_percent, 4)}
- ADX: {fmt(last_adx, 1)}
- الزخم لآخر 5 شموع: {fmt(momentum_5, 4)}

عتبة الشراء الموصى بها لهذا النوع: {desc}
السؤال: بناءً على هذه البيانات، وبناءً على تحليلك العميق جدٱ ومعلوماتك عن هذا الرمز وتحليل مشاعر الخوف والطمع للمتداولين الأخرين هل توافقني على شراء هذا الرمز الآن أنا افكر بتداوله لمدة ساعتين فقط بأهداف ربح من 2 إلى 4 بالمئة مع وقف خسارة عند 1.69 بحجم صفقة فقط من 100 دولار إلى 300 دولار لا اريدك ان تكون متشددٱ ولا متساهلٱ اعمل بكامل طاقتك وأجب هل يحقق درجة ثقة 47 بالمئة حسب هذه المعلومات ؟ أجب فقط بـ "نعم" أو "لا" ثم الدرجة المئوية للثقة مع سبب المختصر جداً."""

    if CURRENT_AI_MODEL == 'flash':
        model_name = GEMINI_MODEL_FLASH
    else:
        model_name = GEMINI_MODEL_FLASH_LITE
    if not model_name:
        model_name = DEFAULT_FLASH_MODEL

    # استخدام النموذج المحدد مباشرة (بدون احتياطي)
    try:
        model = genai.GenerativeModel(model_name)
    except Exception as e:
        logger.error(f"فشل إنشاء النموذج {model_name} لـ {symbol}: {e}")
        return AI_FALLBACK_ACTION == 'allow'
    ...
    try:
        ...
        if any(keyword in error_str for keyword in ['resourceexhausted', 'rate limit', 'connection', 'timeout', 'network', 'unavailable']):
            logger.warning(f"⚠️ خطأ مؤقت في استدعاء Gemini لـ {symbol}: {e}. إعادة المحاولة بعد 30 ثانية...")
            time.sleep(30)
            try:
                answer = call_gemini()
                logger.info(f"✅ نجحت إعادة محاولة Gemini لـ {symbol}")
            except Exception as e2:
                logger.error(f"فشلت إعادة المحاولة لـ {symbol}: {e2}")
                raise
        else:
            raise

    approved = "نعم" in answer or "yes" in answer
    reason = answer.strip()
    ...
    if now - _last_ai_notification_time > AI_NOTIFICATION_COOLDOWN:
        if approved:
            send_telegram(f"🤖 Gemini ({model_name}) ✅ **وافق** على شراء {symbol}\nالسبب: {reason[:200]}")
        else:
            send_telegram(f"🤖 Gemini ({model_name}) ❌ **رفض** شراء {symbol}\nالسبب: {reason[:200]}")
        _last_ai_notification_time = now
...
def execute_limit_order(symbol, side, size, price_ref, sym_type='normal'):
    timeout = LIMIT_TO_MARKET_TIMEOUT  # 10 ثوانٍ (تم التعديل)
    if _should_simulate():
    ...
    # تعديل السعر لضمان التنفيذ السريع: للشراء أعلى، للبيع أقل
    if side == 'buy':
        adjusted_price = price_ref * (1 + LIMIT_ORDER_SLIPPAGE)  # أعلى من السوق بـ 0.3%
    else:
        adjusted_price = price_ref * (1 - LIMIT_ORDER_SLIPPAGE)  # أقل من السوق بـ 0.3%
        ...
        try:
            ...
            while time.time() - start < timeout:
                try:
                    ...
                    pass
                time.sleep(ORDER_POLL_INTERVAL)  # تم التعديل إلى 1 ثانية
            # بعد انتهاء المهلة
            if filled > 0:
                try:
                    rest_rate_limiter.wait_if_needed(weight=2)
...
def execute_limit_close(symbol, side, size, price_ref, sym_type='normal', extra_attempt=False):
    ...
    # تعديل السعر لضمان التنفيذ السريع: للبيع أقل من السوق
    if side == 'sell':
...
# ======================= دالة الوقف المسبق الموحدة (معدلة لاستخدام stop_loss_limit فقط لضمان التوافق مع Spot) =======================
def place_preemptive_stop_loss(symbol, size, entry_price, sym_type, stop_price_override=None):
    """
    إنشاء أمر وقف مسبق (stop_loss_limit) لصفقة شراء في السوق الفوري.
    يتم تفعيل الأمر عند وصول السعر إلى stop_price، ثم يتم إنشاء أمر بيع محدد بسعر limit_price.
    هذه الطريقة مدعومة لجميع الأزواج في Binance Spot، بما فيها BTC و ETH.
    """
    if _should_simulate():
        return None
    exchange = get_active_exchange()
    # تحديد سعر التفعيل (stopPrice) باستخدام القيم المخصصة
    if stop_price_override is not None:
        stop_price = stop_price_override
    else:
        stop_price = entry_price * (1 - CUSTOM_PREEMPTIVE_SL_TRIGGER)
    if stop_price <= 0:
        logger.error(f"قيم غير صالحة للوقف: stop={stop_price}")
        return None
    try:
        rest_rate_limiter.wait_if_needed(weight=5)
        market = exchange.market(symbol)
        ...
        # حساب سعر الحد (limitPrice) الذي سيتم به البيع بعد التفعيل باستخدام القيم المخصصة
        limit_price = entry_price * (1 - CUSTOM_PREEMPTIVE_SL_LIMIT)
        limit_price = float(exchange.price_to_precision(symbol, limit_price))
        # التحقق من صحة الأسعار: في أوامر البيع، يجب أن يكون limit_price أقل من stop_price
        if limit_price >= stop_price:
            logger.warning(f"⚠️ limit_price ({limit_price}) >= stop_price ({stop_price}) لـ {symbol}. قد يرفض الأمر.")
            # تصحيح بسيط: جعل limit_price أقل قليلاً من stop_price
            limit_price = stop_price * 0.999
            limit_price = float(exchange.price_to_precision(symbol, limit_price))
        # إنشاء أمر stop_loss_limit (مدعوم في جميع أزواج السوق الفوري)
        params = {
        ...
        order = exchange.create_order(
            ...
            side='sell',              # بيع لإغلاق المركز
            amount=size,
            price=limit_price,
            params=params
        )
        oid = order.get('id', 'unknown')
        logger.info(f"✅ أمر وقف مسبق (stop_loss_limit) لـ {symbol} | تفعيل عند {stop_price:.8f} | حد السعر {limit_price:.8f} ({(1-stop_price/entry_price)*100:.2f}% / {(1-limit_price/entry_price)*100:.2f}%) | ID: {oid}")
        return oid
    except Exception as e:
        logger.error(f"فشل إنشاء الوقف المسبق (stop_loss_limit) لـ {symbol}: {e}")
        # لا نحاول استخدام stop_loss كبديل لأنه غير مدعوم للعديد من الأزواج.
        # نرسل تنبيهاً ونعيد None (سيتم إلغاء الصفقة في الدالة العليا)
        send_telegram(f"🚨 فشل إنشاء أمر الوقف لـ {symbol}: {str(e)[:100]}")
        generate_error_report("فشل_أمر", "وقف_مسبق", f"{symbol}: {e}")
        return None
...
def update_preemptive_stop_loss(symbol, pos, new_stop_price):
    if _should_simulate():
        return
    # ===== التحقق: هل تحديث الوقف المسبق الحقيقي مفعل؟ =====
    if not PREEMPTIVE_SL_UPDATE_ENABLED:
        logger.debug(f"⏸️ تحديث الوقف المسبق الحقيقي لـ {symbol} معطل (PREEMPTIVE_SL_UPDATE_ENABLED=False)")
        return
    ...
    try:
        ...
        for sym in symbols_to_check:
            try:
                ...
                stop_orders = [o for o in orders if o.get('type') in ('stop_loss', 'stop_loss_limit')]
...
class AdvancedRuleModel:
    """
    نموذج متقدم لاكتشاف أنماط الشموع والاختراقات وفق أحدث الدراسات.
    يتعرف على:
    1. نمط التراكم القصير (3-4 شموع خضراء قصيرة قبل الاختراق)
    2. نمط الاختراق المفاجئ (شمعة خضراء طويلة تخترق المقاومة)
    3. نمط الانعكاس الصاعد (المطرقة، الابتلاع، الثلاثة جنود البيض)
    4. نمط الزخم التصاعدي مع حجم قوي
    """
    def __init__(self):
        pass
    ...
    def get_signal_percent(self, df):
        ...
        try:
            ...
            # الحصول على آخر 10 شموع للتحليل
            n = min(10, len(df))
            last_close = close[-1]
            last_open = open_p[-1]
            last_high = high[-1]
            ...
            # ============================================================
            # 1. كشف نمط التراكم القصير (3-4 شموع خضراء قصيرة مع حجم منخفض)
            # ============================================================
            ...
            if green_count >= 3:
                ...
                # الشموع الخضراء تكون صغيرة (نسبة الجسم < 0.5) وحجم منخفض
                if avg_body_ratio < 0.5:
                    accumulation_score = 0.6 + (green_count - 3) * 0.1  # 0.6, 0.7
                    # التحقق من أن الشمعة الأخيرة (الحالية) صاعدة وبدأت تخترق
                    if close[-1] > open_p[-1] and close[-1] > high[-2]:
                        accumulation_score += 0.2  # بداية اختراق
                # إذا كانت الشموع متوسطة (نسبة الجسم 0.5-0.7)
                elif avg_body_ratio < 0.7:
                    accumulation_score = 0.4 + (green_count - 3) * 0.1
            accumulation_score = min(1.0, accumulation_score)

            # ============================================================
            # 2. كشف الاختراق المفاجئ (شمعة خضراء طويلة تخترق المقاومة)
            # ============================================================
            breakout_score = 0.0
            # تعريف المقاومة: أعلى سعر في آخر 10 شموع (باستثناء الشمعة الحالية)
            ...
            if last_close > resistance:
                ...
                body_ratio = body / range_p
                if body_ratio > 0.6 and last_close > last_open:  # شمعة صاعدة قوية
                    breakout_score = 0.9
                    # مكافأة إضافية إذا كان الحجم أعلى من المتوسط
                    ...
                    breakout_score = 0.4
            # إذا كان الاختراق ضعيفاً (اقترب من المقاومة)
            elif last_close > resistance * 0.995:
                breakout_score = 0.3

            # ============================================================
            # 3. كشف أنماط الانعكاس الصاعد (المطرقة، الابتلاع، الثلاثة جنود البيض)
            # ============================================================
            reversal_score = 0.0

            # 3.1 نمط المطرقة (Hammer): شمعة سفلية طويلة، جسم صغير، في نهاية اتجاه هابط
            if len(high) >= 3:
            ...
            if len(close) >= 6:
                ...
                if mom_5 > 0.02:
                    ...
                    momentum_score = 0.2
                # مكافأة الحجم إذا كان أكبر من المتوسط
                if volume_sma is not None and last_volume > volume_sma[-1] * 1.5:
                    momentum_score = min(1.0, momentum_score + 0.1)

            # ============================================================
            # 5. الدمج النهائي مع أوزان ديناميكية
            # ============================================================
            # نعطي وزن أكبر للاختراق (0.35) والتراكم (0.25) والانعكاس (0.20) والزخم (0.20)
            ...
            # درجة البيع هي عكس درجة الشراء مع هامش صغير
            sell_score = max(0.0, min(1.0, 1.0 - buy_score + 0.05))

            return buy_score, sell_score

        except Exception as e:
            # في حالة أي خطأ، نعود إلى القيم المحايدة
            return 0.5, 0.5
...
class AdvancedMomentumFlowModel:
    ...
    يعتمد على منطق الخوف من التصحيح مع علاقات غير خطية:
    1. اكتشاف ذروة الشراء/البيع باستخدام RSI وعرض البولينجر باند.
    2. اكتشاف التباعد السلبي بين السعر والزخم.
    3. عقوبة ضعف الحجم عند الارتفاع.
    4. عقوبات قاسية عند توفر عدة إشارات سلبية، ومكافآت للإشارات الإيجابية.
    """
    def __init__(self):
        pass
    ...
    def update(self, symbol, df):
        ...
        try:
            ...
            # المؤشرات المحسوبة من آخر 3 شموع
            last_rsi = rsi[-1]
            ...
            # 1. اكتشاف ذروة الشراء باستخدام RSI وعرض الباند
            ...
            # إضافة عرض الباند: إذا كان السعر قريباً من الحد العلوي مع RSI مرتفع => خوف أكبر
            ...
            if last_vol_ratio < 0.8 and last_close > prev_close:
                ...
                volume_weakness = -0.3  # مكافأة (تقليل الخوف)

            # 4. حساب درجة الخوف النهائية (كلما زادت، قلت درجة الشراء)
            fear_score = (overbought_score * 0.40 +
                         divergence_penalty * 0.30 +
                         max(0, volume_weakness) * 0.20 +
                         (1 - oversold_score) * 0.10)  # إذا لم تكن منطقة تشبع بيعي، يزيد الخوف

            # تطبيق حد أقصى 1.0
            fear_score = max(0.0, min(1.0, fear_score))

            # درجة الشراء = 1 - الخوف (مع مكافأة إضافية في حالة التشبع البيعي)
            buy_score = 1.0 - fear_score
            # مكافأة للتشبع البيعي القوي
            if oversold_score > 0.7:
                buy_score = min(1.0, buy_score + 0.2)
            # عقوبة للتباعد السلبي الشديد
...
class VWAP_OBV_Model:
    ...
    يعتمد على منطق الجشع مع علاقات غير خطية:
    1. تحليل الانحراف عن VWAP (عقوبات للمبالغة، مكافآت للتماسك).
    2. زخم OBV مع اتجاهه (ميل خلال 5 شموع).
    3. اختراق الحجم عند مستويات VWAP.
    4. دمج غير خطي يعطي درجات متباينة.
    """
    def get_score(self, df):
        ...
        try:
            ...
            # 1. تحليل الانحراف عن VWAP
            if last_vwap > 0:
                vwap_dev = (last_close - last_vwap) / last_vwap
            else:
                vwap_dev = 0.0

            # شرائح غير خطية للانحراف
            if vwap_dev > 0.05:          # أكثر من 5% فوق VWAP (جشع مفرط)
                vwap_score = 0.1
            elif vwap_dev > 0.03:        # 3-5% فوق
                vwap_score = 0.3
            elif vwap_dev > 0.01:        # 1-3% فوق
                vwap_score = 0.6
            elif vwap_dev > -0.01:       # ضمن ±1% (تماسك مثالي)
                vwap_score = 0.9
            elif vwap_dev > -0.03:       # 1-3% تحت (فرصة شراء)
                vwap_score = 0.7
            else:                         # أكثر من 3% تحت (ضعف)
                vwap_score = 0.4

            # 2. زخم OBV: حساب الميل خلال آخر 5 شموع
            if len(obv) >= 6:
                obv_slope = (obv[-1] - obv[-6]) / (abs(obv[-6]) + 1e-6)
                # تطبيع الميل لتقييم بين 0 و 1
                if obv_slope > 0.05:
                    obv_score = 0.9
                elif obv_slope > 0.02:
                    obv_score = 0.7
            ...
            # 3. اختراق الحجم عند VWAP
            volume_breakout = 0.0
            if last_vol_ratio > 1.5 and vwap_dev > 0.01:
                volume_breakout = 0.8  # اختراق حجمي قوي مع سعر فوق VWAP
            elif last_vol_ratio > 1.2 and vwap_dev > 0.005:
                volume_breakout = 0.6
            elif last_vol_ratio > 1.5 and vwap_dev < -0.01:
                volume_breakout = 0.2  # حجم كبير مع سعر تحت VWAP (ضعف)
            else:
                volume_breakout = 0.3

            # 4. الدمج غير الخطي مع أوزان ديناميكية
            # نعطي وزن أكبر لـ VWAP (0.45) و OBV (0.35) والحجم (0.20)
            buy_score = (vwap_score * 0.45 +
                         obv_score * 0.35 +
                         volume_breakout * 0.20)

            # تطبيق عقوبات إضافية:
            # - إذا كان الانحراف > 5% و OBV سالب -> خفض شديد
            if vwap_dev > 0.05 and obv_score < 0.4:
                buy_score = max(0.0, buy_score - 0.3)
            # - إذا كان الانحراف ضمن ±1% و OBV إيجابي -> مكافأة
...
class MomentumDecayModel:
    def get_sell_vote(self, df, pos, cur_price):
        ...
        # استخدام الإطار الأساسي (15 دقيقة) - القسمة على 15
        lookback = max(1, int(pos.momentum_check_minutes / 15))
        if len(df) > lookback:
...
class EntryTimingModel:
    ...
    يعطي درجة عالية (قريبة من 1) إذا كنا في بداية موجة صاعدة،
    ودرجة منخفضة (قريبة من 0) إذا كنا في قمة أو ذروة شراء.
    """
    def get_entry_score(self, df):
        """
        تعيد درجة من 0 إلى 1 تقيم جودة التوقيت الحالي للدخول.
        """
        if len(df) < 20:
            return 0.5  # درجة محايدة في حال عدم كفاية البيانات

        # 1. تحليل القمم والقيعان خلال آخر 6 شموع (90 دقيقة)
        ...
        # نسبة السعر الحالي من القمة (0 = في القاع، 1 = في القمة)
        position_in_range = (current_price - recent_low) / price_range
        
        # 2. حساب الزخم اللحظي (آخر شمعة)
        last_candle_change = (df['close'].iloc[-1] - df['close'].iloc[-2]) / df['close'].iloc[-2] if len(df) >= 2 else 0
        
        # 3. حساب الزخم العام (آخر 5 شموع)
        momentum_5 = (df['close'].iloc[-1] - df['close'].iloc[-6]) / df['close'].iloc[-6] if len(df) >= 6 else 0
        
        # ----- تقييم التوقيت -----
        score = 0.5  # افتراضي محايد
        
        # الحالة المثالية: السعر في منتصف الموجة (وليس في القمة)، مع زخم إيجابي معتدل
        if 0.3 < position_in_range < 0.7 and momentum_5 > 0.005 and momentum_5 < 0.03:
            score = 0.9  # ✅ بداية موجة صاعدة مثالية
        # الحالة الجيدة: السعر قريب من القاع، بدأ يصعد قليلاً
        elif position_in_range < 0.4 and last_candle_change > 0.002:
            score = 0.8  # ✅ ارتداد من القاع
        # الحالة المتوسطة: السعر في منتصف الموجة، زخم ضعيف
        elif 0.2 < position_in_range < 0.8:
            score = 0.6
        # الحالة السيئة: السعر قريب جداً من القمة (أكثر من 95% من أعلى سعر)
        elif position_in_range > 0.95:
            score = 0.2  # ❌ قمة مؤقتة (خطر الانعكاس)
        # الحالة السيئة جداً: زخم مفرط (ارتفاع حاد) قد ينعكس بسرعة
        elif momentum_5 > 0.04:
            score = 0.3  # ❌ ارتفاع مفرط (ذروة شراء)
        # الحالة المتطرفة: السعر في القمة والزخم ضعيف (انعكاس وشيك)
        elif position_in_range > 0.85 and momentum_5 < 0.01:
            score = 0.1  # ❌❌ قمة مع ضعف زخم (انعكاس مؤكد تقريباً)
        
        # ========== التعديل الجديد: تحويل أعلى درجتين فقط ==========
        # 1.0 → 0.1  و  0.9 → 0.2
        if score >= 0.99:      # 1.0
            return 0.1
        elif score >= 0.85:    # 0.9
            ...
            return score       # باقي الدرجات تبقى كما هي

# --------------------------- دالة مساعدة لحساب المضاعف من درجة 5 دقائق (معدل) ---------------------------
def get_multiplier_from_score(score_5m, score_15m=None):
    """
    حساب المضاعف بناءً على نسبة درجة 5 دقائق إلى 15 دقيقة.
    - إذا كانت 5 دقائق أعلى بكثير من 15 دقيقة (زخم لحظي مفرط) → مضاعف منخفض (عقاب).
    - إذا كانت 5 دقائق أقل بكثير (تصحيح مؤقت) → مضاعف مرتفع (مكافأة).
    - إذا كانت متقاربة → مضاعف محايد (1.0).
    الحدود القصوى: 0.70 إلى 1.30.
    """
    if score_15m is None:
        # احتياط: المنطق القديم
...
# --------------------------- لجنة الشراء (المحسّنة لـ 3 أطر حقيقية) ---------------------------
class BuyingCommittee:
    def __init__(self):
        ...
        # ======================== تم تحديث العتبات الافتراضية إلى 0.52 ========================
        ...
        self.apply_multiplier(1.0, save=False)  # ✅ لا نحفظ الحالة أثناء التهيئة لتجنب NameError

    def apply_multiplier(self, percent_multiplier, save=True):  # ✅ معامل save اختياري
        ...
        if save:   # ✅ نحفظ فقط إذا كان save=True
            save_state()
    ...
    # ===== الدالة المساعدة الجديدة لحساب درجات نموذج واحد على إطار معين =====
    def _get_scores(self, df):
        """حساب درجات النماذج الخمسة لإطار زمني واحد"""
    ...
    # ===== دالة حساب المتوسط المرجح الجديدة (تستقبل 3 أطر) =====
    def calculate_weighted_average(self, df_5m, df_15m, df_1h, symbol, sym_type='normal', market_condition='normal'):
        """تحسب المتوسط المرجح للدرجات عبر 3 أطر زمنية حقيقية (5م، 15د، 1س) مع أوزان ديناميكية"""
    ...
    def decide(self, df_5m, df_15m, df_1h, symbol, sym_type='normal', market_condition='normal'):
        ...
        if SINGLE_MODEL_FILTER_ENABLED:
            ...
            if selected_score < SINGLE_MODEL_FILTER_THRESHOLD:
                reason = f"❌ رفض (فلتر1): {SINGLE_MODEL_FILTER_MODEL} ({SINGLE_MODEL_FILTER_TIMEFRAME}) = {selected_score:.3f} < {SINGLE_MODEL_FILTER_THRESHOLD:.2f}"
                return 'neutral', 0, 0, reason, avg_scores, scores_5m, scores_15m, scores_1h

        # ===== فلتر النموذج الثاني (جديد) =====
        if SECOND_MODEL_FILTER_ENABLED:
            ...
            if selected_score < SECOND_MODEL_FILTER_THRESHOLD:
                reason = f"❌ رفض (فلتر2): {SECOND_MODEL_FILTER_MODEL} ({SECOND_MODEL_FILTER_TIMEFRAME}) = {selected_score:.3f} < {SECOND_MODEL_FILTER_THRESHOLD:.2f}"
                return 'neutral', 0, 0, reason, avg_scores, scores_5m, scores_15m, scores_1h
        ...
        if THIRD_MODEL_FILTER_ENABLED:
            ...
            if selected_score < THIRD_MODEL_FILTER_THRESHOLD:
                reason = f"❌ رفض (فلتر3): {THIRD_MODEL_FILTER_MODEL} ({THIRD_MODEL_FILTER_TIMEFRAME}) = {selected_score:.3f} < {THIRD_MODEL_FILTER_THRESHOLD:.2f}"
                return 'neutral', 0, 0, reason, avg_scores, scores_5m, scores_15m, scores_1h
...
def fetch_orderbook_with_cache(symbol, limit=LIQUIDITY_CHECK_DEPTH, ttl=5):
    ...
    try:
        rest_rate_limiter.wait_if_needed(weight=5)  # ✅ وزن 5 (بدلاً من 2)
...
# ======================= محرك التوصيات الذكي (جديد - يستخدم الملف الجديد) =======================
def calculate_suggested_weights():
    """
    تحليل آخر الصفقات لتقديم توصية بأوزان النماذج والأطر.
    تعتمد على trade_recommendations_log.csv.
    """
    if not os.path.exists(RECOMMENDATION_LOG_FILE) or os.path.getsize(RECOMMENDATION_LOG_FILE) < 100:
        return None, "⚠️ سجل التوصيات فارغ أو غير موجود. شغّل البوت أولاً."

    try:
        df = pd.read_csv(RECOMMENDATION_LOG_FILE, encoding='utf-8')
    except Exception as e:
        return None, f"❌ فشل قراءة سجل التوصيات: {e}"

    df_closed = df[df['status'] == 'closed'].copy()
    if df_closed.empty:
        return None, "⚠️ لا توجد صفقات مغلقة في سجل التوصيات حتى الآن."

    if len(df_closed) > 1000:
        df_closed = df_closed.tail(1000)

    pnl = df_closed['total_pnl'].apply(pd.to_numeric, errors='coerce')

    weighted_5m = df_closed['weighted_5m'].apply(pd.to_numeric, errors='coerce')
    weighted_15m = df_closed['weighted_15m'].apply(pd.to_numeric, errors='coerce')
    weighted_1h = df_closed['weighted_1h'].apply(pd.to_numeric, errors='coerce')

    members = ['rule', 'emr', 'cfhm', 'timing', 'vwap']
    frames = ['5m', '15m', '1h']
    member_scores = {}
    for member in members:
        member_scores[member] = {}
        for frame in frames:
            col_name = f'scores_{frame}_{member}'
            if col_name in df_closed.columns:
                member_scores[member][frame] = df_closed[col_name].apply(pd.to_numeric, errors='coerce')
            else:
                member_scores[member][frame] = pd.Series([np.nan] * len(df_closed))

    valid_mask = pnl.notna() & weighted_5m.notna() & weighted_15m.notna() & weighted_1h.notna()
    for member in members:
        for frame in frames:
            valid_mask &= member_scores[member][frame].notna()

    pnl_valid = pnl[valid_mask].values
    weighted_5m_valid = weighted_5m[valid_mask].values
    weighted_15m_valid = weighted_15m[valid_mask].values
    weighted_1h_valid = weighted_1h[valid_mask].values

    member_scores_valid = {}
    for member in members:
        member_scores_valid[member] = {}
        for frame in frames:
            member_scores_valid[member][frame] = member_scores[member][frame][valid_mask].values

    n = len(pnl_valid)
    if n < 5:
        return None, f"⚠️ عدد الصفقات الصالحة = {n} (أقل من 5). شغّل البوت لفترة أطول."

    warning_msg = ""
    if n < 20:
        warning_msg = f"\n⚠️ تحذير: عدد الصفقات الصالحة = {n} فقط (أقل من 20). قد لا تكون التوصية دقيقة."

    # وزناً زمنياً: الصفقات الأحدث لها وزن أكبر
    ages = np.arange(n-1, -1, -1) / (24 * 7)
    decay_days = 7
    ...
    def calculate_performance(series):
        ...
        for i in range(len(bins)-1):
            ...
            if mask.sum() > 0:
                avg_pnl = np.average(pnl_valid[mask], weights=weights[mask])
                segment_profits.append(avg_pnl)
            else:
                segment_profits.append(0.0)
        best = max(segment_profits)
        worst = min(segment_profits)
        return best - worst

    frame_perf = {
        '5m': calculate_performance(weighted_5m_valid),
        '15m': calculate_performance(weighted_15m_valid),
        '1h': calculate_performance(weighted_1h_valid)
    }

    member_perf_by_frame = {}
    for member in members:
        member_perf_by_frame[member] = {}
        for frame in frames:
            series = member_scores_valid[member][frame]
            perf = calculate_performance(series)
            member_perf_by_frame[member][frame] = max(0, perf)

    def distribute_weights(performance_dict):
        positive_perf = {k: v for k, v in performance_dict.items() if v > 0}
        if not positive_perf:
            n_items = len(performance_dict)
            return {k: 1.0/n_items for k in performance_dict}
        total = sum(positive_perf.values())
        weights = {k: v / total for k, v in positive_perf.items()}
        for k in performance_dict:
            if k not in weights:
                weights[k] = 0.0
        return weights

    frame_weights_raw = distribute_weights(frame_perf)
    member_final_perf = {}
    for member in members:
        weighted_perf = 0.0
        total_weight = 0.0
        for frame in frames:
            weight = frame_weights_raw.get(frame, 0)
            perf = member_perf_by_frame[member].get(frame, 0)
            weighted_perf += weight * perf
            total_weight += weight
        if total_weight > 0:
            member_final_perf[member] = weighted_perf / total_weight
        else:
            member_final_perf[member] = 0.0

    frame_weights = distribute_weights(frame_perf)
    member_weights = distribute_weights(member_final_perf)

    # ===== تحديث أوزان البيع المستوحاة من التوصية =====
    global SELL_MEMBER_WEIGHTS, SELL_FRAME_WEIGHTS
    SELL_FRAME_WEIGHTS = {
        '5m': frame_weights.get('5m', 0),
        '15m': frame_weights.get('15m', 0),
        '1h': frame_weights.get('1h', 0)
    }
    member_weights_sell = {}
    for k, v in member_weights.items():
        key = 'vwap_obv' if k == 'vwap' else k
        member_weights_sell[key] = v
    SELL_MEMBER_WEIGHTS = member_weights_sell
    save_state()

    result_msg = f"<b>📊 توصية الأوزان المثلى (تحليل {n} صفقة)</b>{warning_msg}\n\n"
    
    result_msg += f"<b>⏱️ أوزان الأطر (5م، 15د، 1س):</b>\n"
    for f in ['5m', '15m', '1h']:
        weight = frame_weights.get(f, 0) * 100
        result_msg += f"   {f}: {weight:.1f}%\n"
    
    tg_frames = f"تعيين أوزان الأطر {int(frame_weights.get('5m',0)*100)} {int(frame_weights.get('15m',0)*100)} {int(frame_weights.get('1h',0)*100)}"
    result_msg += f"\n📨 <code>{tg_frames}</code>\n\n"
    
    result_msg += f"<b>🤖 أوزان النماذج (محسوبة كمتوسط مرجح لأدائها على جميع الإطارات):</b>\n"
    name_map = {'rule':'Rule','emr':'EMR','cfhm':'CFHM','timing':'Timing','vwap':'VWAP_OBV'}
    for m in members:
        weight = member_weights.get(m, 0) * 100
        result_msg += f"   {name_map[m]}: {weight:.1f}%\n"
    
    tg_models = f"تعيين أوزان النماذج Rule={int(member_weights.get('rule',0)*100)} EMR={int(member_weights.get('emr',0)*100)} CFHM={int(member_weights.get('cfhm',0)*100)} Timing={int(member_weights.get('timing',0)*100)} VWAP={int(member_weights.get('vwap',0)*100)}"
    result_msg += f"\n📨 <code>{tg_models}</code>"
...
def log_trade(symbol, side, entry, exit, size, pnl, reason, pred, conf, regime, sym_type, ai_approved=False):
    ...
    # تخزين الرصيد الكلي بعد إغلاق الصفقة
...
def close_partial(symbol, percent, price, reason, pnl_usdt=None):
    ...
    with _global_state_lock:
        ...
        if not validate_restored_position(symbol, pos):
            ...
            # إضافة تبريد افتراضي عند الحذف اليدوي
            with _cooldown_lock:
        ...
        pos._closing = True
    try:
        # التحقق من min_cost لتجنب البيع الجزئي الصغير جداً (فقط إذا كان البيع جزئياً)
        if percent < 1.0:
            try:
        ...
        try:
            ...
            with _global_state_lock:
                ...
                if pos.remaining_size <= 1e-8:
                    ...
                    # ===== التحديث 2: تحديث سجل التوصيات عند الإغلاق الكامل =====
                    if hasattr(pos, 'trade_id') and pos.trade_id:
                        update_trade_record(pos.trade_id, datetime.now(), fill_price, pos.closed_pnl)
                    
                    del open_positions[symbol]
                    # ========== نظام التبريد الذكي الجديد ==========
                    with _cooldown_lock:
                        if pnl_net > 0:
                            # صفقة رابحة → تبريد طويل
                    ...
                    # تخزين الرصيد الكلي بعد إغلاق كامل
                    bot_stats.add_equity_point(get_total_equity())
                else:
                    if not is_sim:
                        new_stop = None
...
def get_current_momentum(symbol, lookback_minutes):
    try:
        # استخدام الإطار الأساسي (15 دقيقة)
        df = fetch_ohlcv_retry(symbol, TIMEFRAMES['primary'], limit=50)
        if len(df) < 4: return None
        lookback = max(1, int(lookback_minutes / 15))   # القسمة على 15 بدلاً من 5
...
def monitor_positions():
    global _last_market_condition_time, _last_market_condition, CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP
    global TRAILING_DISTANCE_PERCENT, SELL_15M_CHECK_INTERVAL, _last_15m_sell_check
    # SELL_15M_THRESHOLD تم حذفه من global لأنه غير مستخدم ويمكن أن يسبب خطأ
    while True:
        try:
            with _global_state_lock: snapshot = list(open_positions.items())
            # نستعمل WebSocket مباشرة للحصول على أسعار كل رمز بدلاً من دفعة واحدة
            for sym, pos in snapshot:
                # 1️⃣ الحصول على أحدث سعر من WebSocket مباشرة
                ws_data = ws_manager.get_ticker(sym)
                if ws_data and ws_data.get('price', 0) > 0:
                    cur_price = ws_data['price']
                else:
                    # محاولة جلب السعر من API (محاولة سريعة واحدة)
                    ticker = fetch_ticker_with_retry(sym, max_retries=1)
                    if ticker and ticker.get('last', 0) > 0:
                        cur_price = ticker['last']
                    else:
                        continue  # تخطي هذه الدورة إذا لم نتمكن من الحصول على السعر
                
                # 2️⃣ تحديث أعلى/أدنى سعر للمركز (والوقف المتحرك فوراً)
                profit = pos.update(cur_price)
                
                # 3️⃣ البيع الفوري عند كسر الوقف المتحرك (نسبة التتبع الديناميكية)
                if pos.trailing_activated and pos.trailing_stop is not None:
                    # تحديث أمر الوقف المسبق (إذا ارتفع السعر بشكل كافٍ)
                    ...
                    # التحقق من كسر التريلينغ (بيع كامل فوري)
                    if (pos.side == 'buy' and cur_price <= pos.trailing_stop) or \
                       (pos.side == 'sell' and cur_price >= pos.trailing_stop):
                        close_partial(sym, 1.0, cur_price, 
                                      f"🏃 وقف متحرك فوري (نسبة {TRAILING_DISTANCE_PERCENT:.2%})")
                        continue
                
                # =========================== التحديث 3: نظام البيع الهرمي الجديد (معدل) ===========================
                now = time.time()
                last_check = _last_15m_sell_check.get(sym, 0)
                if now - last_check >= SELL_15M_CHECK_INTERVAL:
                    try:
                        df_5m = get_cached_features(sym, '5m', limit=200, ttl=60)
                        df_15m = get_cached_features(sym, '15m', limit=500, ttl=60)
                        df_1h = get_cached_features(sym, '1h', limit=500, ttl=60)
                        if df_5m.empty or df_15m.empty or df_1h.empty:
                            _last_15m_sell_check[sym] = now
                            continue
                        scores_5m = buying_committee._get_scores(df_5m)
                        scores_15m = buying_committee._get_scores(df_15m)
                        scores_1h = buying_committee._get_scores(df_1h)
                        w_sell = SELL_MEMBER_WEIGHTS
                        # 🔒 حماية: إذا كانت جميع الأوزان صفراً، استخدم الأوزان الافتراضية
                        if sum(w_sell.values()) == 0:
                            w_sell = {'rule': 0.05, 'emr': 0.10, 'cfhm': 0.30, 'timing': 0.45, 'vwap_obv': 0.10}
                        valid_keys = [k for k in w_sell if k in scores_5m]
                        if not valid_keys:
                            w_sell = {k: 1.0/len(scores_5m) for k in scores_5m}
                        weighted_5m = sum(scores_5m[k] * w_sell.get(k, 0) for k in w_sell if k in scores_5m)
                        weighted_15m = sum(scores_15m[k] * w_sell.get(k, 0) for k in w_sell if k in scores_15m)
                        weighted_1h = sum(scores_1h[k] * w_sell.get(k, 0) for k in w_sell if k in scores_1h)
                        f_sell = SELL_FRAME_WEIGHTS
                        # 🔒 حماية: إذا كانت جميع الأوزان صفراً، استخدم الأوزان الافتراضية
                        if sum(f_sell.values()) == 0:
                            f_sell = {'5m': 0.30, '15m': 0.10, '1h': 0.60}
                        total_frame_weight = sum(f_sell.values())
                        if total_frame_weight == 0:
                            f_sell = {'5m': 1/3, '15m': 1/3, '1h': 1/3}
                            total_frame_weight = 1.0
                        # ===== التعديل: استخدام درجة الثقة مباشرة (بدون عكس) =====
                        sell_score = (
                            weighted_5m * f_sell.get('5m', 0) +
                            weighted_15m * f_sell.get('15m', 0) +
                            weighted_1h * f_sell.get('1h', 0)
                        ) / total_frame_weight
                        # ===== شروط البيع عند انخفاض الثقة (≤ العتبة) =====
                        if sell_score <= SELL_FULL_THRESHOLD:
                            reason = f"📉 بيع 100% (كامل): درجة الثقة {sell_score:.3f} ≤ {SELL_FULL_THRESHOLD:.2f}"
                            close_partial(sym, 1.0, cur_price, reason)
                        elif sell_score <= SELL_PARTIAL_THRESHOLD:
                            reason = f"📉 بيع 50% (جزئي): درجة الثقة {sell_score:.3f} ≤ {SELL_PARTIAL_THRESHOLD:.2f}"
                            close_partial(sym, 0.5, cur_price, reason)
                        _last_15m_sell_check[sym] = now
                    except Exception as e:
                        logger.warning(f"⚠️ فشل فحص البيع الهرمي لـ {sym}: {e}")
                        _last_15m_sell_check[sym] = now
                # =====================================================================================================
                
                # 5️⃣ شروط الحماية الاحتياطية (وقف الخسارة، الانهيار، جني الأرباح)
                if profit < 0:
                ...
                # 6️⃣ الانهيار السريع (>4% خلال دقيقتين)
                ...
                # 7️⃣ جني الأرباح التدريجي (أهداف محددة)
                if profit > 0.011:
                ...
                # 8️⃣ وقف الخسارة الثابت (من الدخول)
                if pos.stop_loss:
                ...
                # 9️⃣ أقصى خسارة مطلقة (6%)
                if profit <= -0.06:
                    close_partial(sym, 1.0, cur_price, "💥 أقصى خسارة 6%")
                    continue
            
            time.sleep(POSITION_MONITOR_INTERVAL)  # 1 ثانية
...
# =========================== التحديث 2: تعديل execute_full_trade لتسجيل التوصيات ===========================
def execute_full_trade(symbol, side, size, price, atr, sl, tp, sym_type, pred, conf, regime, ai_approved=False, approval_reason="",
    ...
    try:
        ...
        if _should_simulate():
            ...
            # ===== تسجيل في ملف التوصيات الجديد =====
            trade_id = f"{int(time.time()*1000)}_{symbol.replace('/', '')}"
            pos.trade_id = trade_id
            add_trade_record(
                trade_id, symbol, pos.open_time, price, pos.total_size,
                scores_5m or {}, scores_15m or {}, scores_1h or {},
                weighted_score_5m or 0.0, weighted_score_15m or 0.0, weighted_score_1h or 0.0, final_score or 0.0
            )
        ...
        if success and fill_price and filled_size>0:
            ...
            pos.trade_id = trade_id
            add_trade_record(
                trade_id, symbol, pos.open_time, fill_price, pos.total_size,
                scores_5m or {}, scores_15m or {}, scores_1h or {},
                weighted_score_5m or 0.0, weighted_score_15m or 0.0, weighted_score_1h or 0.0, final_score or 0.0
            )
            new_balance = _safe_fetch_balance_after_trade(attempts=10, delay=3.0, silent=True)
...
# ======================= خيط auto_recovery المعدل النهائي =======================
def auto_recovery_monitor():
    """خيط مستقل لمراقبة الاتصال بـ Binance.
    عند فشل الاتصال، ينتظر 12 دقيقة ثم يحاول 3 مرات (محاولات نظيفة) قبل إجراء الاسترداد التلقائي.
    لا يرسل أي طلبات إضافية خلال فترة الانتظار.
    """
    ...
    while True:
        # محاولة جلب تيكر بسيط
        ticker = None
        try:
            ticker = exchange.fetch_ticker(test_symbol)
            if ticker and ticker.get('last') is not None:
                # نجاح: إعادة تعيين كل شيء
        ...
        # إذا وصلنا إلى هنا، فهذا يعني فشل المحاولة الحالية
        ...
        # إذا لم تمضِ 12 دقيقة بعد، فقط نم ولا تفعل شيئاً (لا ترسل طلبات إضافية)
        if elapsed < 720:
        ...
        # --- بعد 12 دقيقة ---
        # إذا لم نبدأ المحاولات بعد، ابدأها بـ 3 محاولات جديدة
        ...
        # إذا كان لدينا محاولات متبقية، قم بمحاولة واحدة الآن
        if recovery_attempts_remaining > 0:
            time.sleep(10)
            ...
            if recovery_attempts_remaining == 0:
                ...
                try:
                    # 1. إعادة تعيين RateLimiter بنفس القيم الأصلية (1200)
            ...
            # الوضع العادي: ننام 60 ثانية
            time.sleep(60)

# --------------------------- دالة التحليل الرئيسية (معدلة لـ 3 أطر + التحديث 2) ---------------------------
def analyze_and_trade():
    ...
    global _last_analyzed_candidates   # 🔥 أضفنا هذا السطر
    ...
    try:
        ...
        # تحميل الكاش الأولي للرموز الأساسية تم إزالتها بالكامل (BASE_SYMBOLS لم تعد تُحلل تلقائياً)
        ...
        # ✅ استخدام الرصيد المخزن بدلاً من طلب جديد
        with _global_state_lock:
        ...
        # استخدام المتغيرات الديناميكية لحجم الصفقة الواحدة
        if market_cond == 'good':
            position_percent = CUSTOM_POSITION_PERCENT_GOOD
        elif market_cond == 'bad':
            position_percent = CUSTOM_POSITION_PERCENT_BAD
        else:
            position_percent = CUSTOM_POSITION_PERCENT_NORMAL

        max_pos_usdt = total_eq * position_percent
        # تم إلغاء التبريد القديم (cooldown_hours)
        ...
        # BASE_SYMBOLS تم إزالتها من قائمة التحليل

        # ---- منطق اختيار المرشحين الذكي (جديد) ----
        ...
        if not available:
            ...
            if len(available) <= desired_count:
                ...
                # أول 5 رموز للمقارنة
                first_5 = available[:5]
                # آخر 5 رموز تم تحليلها (مخزنة في _last_analyzed_candidates)
                last_5 = _last_analyzed_candidates[:5] if _last_analyzed_candidates else []
                
                # إذا كانت الخمسة الأولى ثابتة ولدينا ذاكرة سابقة
                if set(first_5) == set(last_5) and len(last_5) == 5:
                    # الخمسة الأولى ثابتة → نأخذ الـ12 التالية (من 12 إلى 24)
                    next_12 = available[12:24]  # indices 12 to 23 (0-based)
                    if len(next_12) >= desired_count:
                        selected_candidates = next_12[:desired_count]
                    elif next_12:
                        # إذا كان عددها أقل من 12، نأخذها كلها ونكمل من الأولى إن احتجنا
                        selected_candidates = next_12 + available[:desired_count - len(next_12)]
                    else:
                        # لا يوجد رموز تالية، نعود للأولى
                        selected_candidates = available[:desired_count]
                else:
                    # الخمسة الأولى تغيرت → نأخذ أول 12 ونحدث الذاكرة
                    selected_candidates = available[:desired_count]
                    _last_analyzed_candidates = selected_candidates

        # بناء القائمة النشطة: المراكز المفتوحة أولاً، ثم المرشحين المختارين
        ...
        if remaining > 0:
            for sym in selected_candidates:
                if sym not in active:
                    ...
                    if remaining == 0:
                        break
        # ---- نهاية منطق اختيار المرشحين الذكي ----
        ...
        for sym in active:
            ...
            with _cooldown_lock:
                if sym in _symbol_cooldown_until:
                    ...
                    if datetime.now() < cooldown_end:
                        ...
                        # انتهت فترة التبريد، نحذف المدخل
                        del _symbol_cooldown_until[sym]
            # =====================================
            
            regime = detect_market_regime(sym)
            if regime == 'trending_down': continue
            # TTL متغير حسب وجود المركز
            if sym in open_positions:
                ttl_used = 30
            else:
                ttl_used = 120
            
            # ===== جلب الأطر الثلاثة (مع إعادة محاولة ضمن get_cached_features) =====
            ...
            # آلية الاحتياط: إذا فشل إطار، نعوضه بإطار 15 دقيقة
            if df_5m.empty:
                df_5m = df_15m
            if df_1h.empty:
                df_1h = df_15m
            if df_15m.empty or len(df_15m) < 50:
                continue
            time.sleep(random.uniform(0.2, 0.6))  # تأخير عشوائي
            
            # === فلتر أداء X يوم ===
            if not check_30d_profit(sym):
                logger.info(f"🚫 تم رفض {sym}: أداء {FILTER_30D_DAYS} يوم = {get_30d_performance(sym):.2f}% < {MIN_30D_PROFIT_PERCENT:.2f}%")
                continue
            
            sym_type = classify_symbol(sym, df=df_15m, ticker_data=all_tickers.get(sym))
            candidates_for_manipulation.append((sym, sym_type, df_5m, df_15m, df_1h, regime))
        
        manipulation_results = {}
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(check_manipulation, sym, sym_type): sym for sym, sym_type, _, _, _, _ in candidates_for_manipulation}
        ...
        pending = []
        for sym, sym_type, df_5m, df_15m, df_1h, regime in candidates_for_manipulation:
            ...
            # ===== استدعاء لجنة الشراء الجديدة بـ 3 أطر =====
            dec, avg_score, conf, reason, avg_scores, scores_5m, scores_15m, scores_1h = buy_comm.decide(df_5m, df_15m, df_1h, sym, sym_type, market_cond)
            if dec != 'buy':
                continue

            # ===== حساب المضاعف من إطار 5 دقائق و 15 دقيقة (منطق ذكي) =====
            if not df_5m.empty and len(df_5m) >= 20 and not df_15m.empty and len(df_15m) >= 20:
                res_5m = buy_comm.calculate_weighted_average(df_5m, None, None, sym, sym_type, market_cond)
                score_5m = res_5m['weighted_avg']
                res_15m = buy_comm.calculate_weighted_average(df_15m, None, None, sym, sym_type, market_cond)
            ...
            # استخراج العتبة thresh من cfg
            cfg = buy_comm.thresholds.get(sym_type, {}).get(market_cond, buy_comm.thresholds['normal']['normal'])
            thresh = cfg.get('thresh1', 0.45)
            
            # ======== بناء السبب المفصل (نتائج الأعضاء على كل إطار) ========
            ...
            # ===== حساب المتوسطات المرجحة لكل إطار (للتسجيل في التوصيات) =====
            ...
            pending.append({
                ...
                'conf': avg_score,
                'regime': regime,
                'df_primary': df_15m,  # استخدمنا df_15m كمرجع
                'reason': reason,
                'scores_5m': scores_5m,
                'scores_15m': scores_15m,
                'scores_1h': scores_1h,
        ...
        if pending:
            ...
            if market_cond == 'good':
                pos_usdt = total_eq2 * CUSTOM_POSITION_PERCENT_GOOD
            elif market_cond == 'bad':
                pos_usdt = total_eq2 * CUSTOM_POSITION_PERCENT_BAD
            else:
                pos_usdt = total_eq2 * CUSTOM_POSITION_PERCENT_NORMAL
            
            # ===== التحقق من إيقاف دخول الصفقات الجديدة =====
            ...
            for opp in pending:
                ...
                approved = True
                if AI_ENABLED and GEMINI_API_KEY and genai is not None:
                    ticker_data = all_tickers.get(opp['symbol'])
                    approved = ask_ai_for_approval(opp['symbol'], opp['df_primary'], opp['price'], opp['type'], market_cond, ticker_data)
                    if not approved:
                        logger.info(f"🚫 تم رفض شراء {opp['symbol']} بواسطة AI")
                        continue
...
def background_scanner():
    ...
    while True:
        if not PAUSE_ANALYSIS and scanner and scanner.should_scan():
            scanner.scan()
        time.sleep(10)   # ✅ تحقق كل 10 ثوانٍ بدلاً من النوم الطويل

# =========================== التحديث 4: تغيير فاصل التحليل من 100 إلى 180 ثانية ===========================
def background_analyzer():
    ...
    while True:
        if PAUSE_ANALYSIS:
            ...
            continue
        analyze_and_trade()
        time.sleep(100)   # تم التعديل من 180 إلى 100 ثانية

def background_balance_updater():
    time.sleep(120)  # انتظار دقيقتين عند البدء
    while True:
        ...
        # ✅ تأخير عشوائي بين 9 و 10 دقائق (540-600 ثانية)
...
def cooldown_cleanup():
    """خيط لتنظيف تبريد الصفقات منتهية الصلاحية تلقائياً"""
    while True:
        time.sleep(60)
        now = datetime.now()
        with _cooldown_lock:
...
# =========================== التحديث 1: تغيير تردد خيط المزامنة إلى 600 ثانية ===========================
def sync_preemptive_stop_orders():
    while True:
            ...
            for sym in symbols_to_check:
                try:
                    ...
                    with _global_state_lock:
                        ...
                        # ---------- الحالة: أمر الوقف المسبق مفقود ----------
                        if current_oid and current_oid not in order_map.values():
                            logger.warning(f"⚠️ أمر الوقف المسبق {current_oid} لـ {sym} غير موجود على المنصة. نتحقق من الرصيد...")
                            
                            # جلب رصيد العملة الحالي
                            rest_rate_limiter.wait_if_needed(weight=20)
                            balance = exchange.fetch_balance()
                            coin = sym.split('/')[0]
                            current_coin_balance = balance.get(coin, {}).get('total', 0)
                            
                            # إذا كان الرصيد أقل من 1% من الكمية المسجلة -> تم البيع بالكامل تقريباً
                            if current_coin_balance < pos.remaining_size * 0.01:
                                logger.info(f"✅ تم اكتشاف إغلاق تلقائي لـ {sym} (الرصيد {current_coin_balance} < {pos.remaining_size*0.01}). جاري حساب الربح/الخسارة...")
                                
                                # جلب صفقات العميل لهذا الرمز منذ فتح المركز
                                open_time_ms = int(pos.open_time.timestamp() * 1000)
                                rest_rate_limiter.wait_if_needed(weight=10)  # ✅ وزن 10 (بدلاً من 5)
                                all_trades = exchange.fetch_my_trades(symbol=sym, since=open_time_ms - 5000, limit=100)
                                
                                # حساب إجمالي الكمية المباعة ومتوسط السعر
                                total_sold_qty = 0.0
                                total_sold_value = 0.0
                                for t in all_trades:
                                    if t['side'] == 'sell' and t['timestamp'] > open_time_ms - 5000:
                                        total_sold_qty += t['amount']
                                        total_sold_value += t['amount'] * t['price']
                                
                                if total_sold_qty > 0:
                                    avg_price = total_sold_value / total_sold_qty
                                    total_pnl = calculate_net_pnl(sym, pos.entry_price, avg_price, total_sold_qty, pos.side, pos.symbol_type)
                                    recorded_pnl = pos.closed_pnl
                                    new_pnl = total_pnl - recorded_pnl
                                    
                                    pos.remaining_size = 0
                                    pos.closed_pnl = total_pnl
                                    bot_stats.total_pnl_usdt += new_pnl
                                    bot_stats.daily_pnl += new_pnl
                                    if total_pnl > 0:
                                        bot_stats.winning_trades += 1
                                        _daily_winning_trades += 1
                                    else:
                                    ...
                                    trade_logger.info(f"{sym},{pos.side},{pos.entry_price},{avg_price},{total_sold_qty},{total_pnl},auto_stop_loss,{pos.pred},{pos.confidence},{pos.regime},{pos.symbol_type}")
                                    
                                    direction = "🔴" if total_pnl < 0 else "🟢"
                                    send_telegram(f"{direction} <b>وقف مسبق تم تفعيله تلقائياً</b> {sym}\nالحجم المباع: {total_sold_qty:.6f}\nالسعر المتوسط: {avg_price:.8f}\nالربح الصافي الكلي: {total_pnl:+.4f} USDT")
                                    
                                    if not PAPER_TRADING:
                                        new_bal = _safe_fetch_balance_after_trade(attempts=5, delay=2, silent=True)
                                        if new_bal:
                                            with _global_state_lock:
                                                bot_stats.last_balance = new_bal
                                    
                                    cancel_preemptive_stop_loss(sym, current_oid)
                                    # ===== التحديث 2: تحديث سجل التوصيات =====
                                    if hasattr(pos, 'trade_id') and pos.trade_id:
                                        update_trade_record(pos.trade_id, datetime.now(), avg_price, pos.closed_pnl)
                                    del open_positions[sym]
                                    # تبريد افتراضي عند الإغلاق التلقائي (نعتبره خسارة إن لم نعرف)
                                    with _cooldown_lock:
                                        _symbol_cooldown_until[sym] = datetime.now() + timedelta(minutes=COOLDOWN_LOSS_MINUTES)
                                    _local_pending_symbols.discard(sym)
                                    _exchange_pending_symbols.discard(sym)
                                    save_state()
                                    continue
                                else:
                                    logger.warning(f"لم يتم العثور على صفقات بيع لـ {sym} رغم نقص الرصيد. حذف المركز دون تحديث الربح.")
                                    del open_positions[sym]
                                    continue
                            
                            # إذا كان الرصيد لا يزال كبيراً -> إعادة إنشاء الوقف
                            else:
                                logger.warning(f"⚠️ أمر الوقف {current_oid} مفقود لكن الرصيد لا يزال {current_coin_balance:.6f} مقابل {pos.remaining_size:.6f}. إعادة إنشاء الوقف.")
                                if pos.trailing_activated and pos.trailing_stop is not None:
                                    new_stop = pos.trailing_stop
                                else:
                                    new_stop = pos.entry_price * (1 - PREPLACED_SL_TRIGGER_PERCENT)
                        ...
                        # ---------- الحالة: لا يوجد أمر وقف مسبق مطلقاً ----------
                        elif not current_oid and sym not in order_map:
                            logger.info(f"⚠️ المركز {sym} بدون أمر وقف مسبق. جاري إنشاء واحد.")
                            if pos.trailing_activated and pos.trailing_stop is not None:
                                new_stop = pos.trailing_stop
                            else:
                                new_stop = pos.entry_price * (1 - PREPLACED_SL_TRIGGER_PERCENT)
                        ...
                        # ---------- الحالة: معرف الأمر مختلف ----------
                        elif sym in order_map and current_oid != order_map.get(sym):
                            logger.info(f"🔄 تحديث معرف الوقف لـ {sym} من {current_oid} إلى {order_map[sym]}")
                            pos.preemptive_sl_order_id = order_map[sym]
        ...
        time.sleep(600)   # تم التعديل من 60 إلى 600 ثانية (10 دقائق)
...
def telegram_polling():
    ...
    global _daily_biggest_win, _daily_biggest_loss, _daily_most_traded
    global _daily_total_holding_time_win, _daily_total_holding_time_loss, _daily_holding_count_win, _daily_holding_count_loss
    global MANIPULATION_REJECT_THRESHOLD
    ...
    global CUSTOM_POSITION_PERCENT_GOOD, CUSTOM_POSITION_PERCENT_NORMAL, CUSTOM_POSITION_PERCENT_BAD
    global CUSTOM_MAX_DAILY_TRADES, CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP
    global buying_committee, CURRENT_BUY_COMMITTEE_MULTIPLIER
    global AI_ENABLED, CURRENT_AI_MODEL
    global PAUSE_NEW_ENTRIES
    ...
    global FOURTH_MODEL_FILTER_MODEL, FOURTH_MODEL_FILTER_THRESHOLD, FOURTH_MODEL_FILTER_TIMEFRAME, FOURTH_MODEL_FILTER_ENABLED
    global FIFTH_MODEL_FILTER_MODEL, FIFTH_MODEL_FILTER_THRESHOLD, FIFTH_MODEL_FILTER_TIMEFRAME, FIFTH_MODEL_FILTER_ENABLED
    # ===== التحديث 3: متغيرات البيع =====
    global SELL_PARTIAL_THRESHOLD, SELL_FULL_THRESHOLD, SELL_MEMBER_WEIGHTS, SELL_FRAME_WEIGHTS
    # ===== الحد الأعلى الموحد =====
    global UPPER_THRESHOLD_GLOBAL
                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                            # 1. أوامر التوقف والاستئناف وإغلاق الصفقات
                            # ------------------------------------------------------------
                            ...
                            # ======================== تم تحديث أمر حالة الفلاتر لعرض أوزان النماذج وعتبات البيع ========================
                            elif text == 'حالة الفلاتر':
                                model_display = "Flash" if CURRENT_AI_MODEL == 'flash' else "Flash-Lite"
                                w = buying_committee.weights
                                status = (f"📊 <b>حالة الفلاتر والقيم الحالية</b>\n━━━━━━━━━━━━━━━━━━━━\n"
                                          f"🛡️ <b>فلتر التلاعب:</b> {'✅ مفعل' if FILTER_MANIPULATION_ENABLED else '❌ معطل'}\n"
                                          ...
                                          f"📊 <b>فلتر أداء {FILTER_30D_DAYS} يوم:</b> {'✅ مفعل' if FILTER_30D_PROFIT_ENABLED else '❌ معطل'} (العتبة: {MIN_30D_PROFIT_PERCENT:.2f}%)\n"
                                          ...
                                          f"📉 <b>عتبة بيع 15 دقيقة (المرادفة لعتبة البيع الكامل):</b> {SELL_FULL_THRESHOLD:.0%}\n"
                                          f"━━━━━━━━━━━━━━━━━━━━\n"
                                          ...
                                          f"📈 <b>حجم الصفقة في السوق الجيد:</b> {CUSTOM_POSITION_PERCENT_GOOD:.1%}\n"
                                          f"📊 <b>حجم الصفقة في السوق العادي:</b> {CUSTOM_POSITION_PERCENT_NORMAL:.1%}\n"
                                          f"📉 <b>حجم الصفقة في السوق السيئ:</b> {CUSTOM_POSITION_PERCENT_BAD:.1%}\n"
                                          ...
                                          f"🎯 <b>عتبة الاشتباه (التلاعب):</b> {MANIPULATION_REJECT_THRESHOLD:.2%}\n"
                                          ...
                                          f"📉 <b>عتبات البيع الهرمي:</b>\n"
                                          f"   - بيع 50% عند درجة ≥ {SELL_PARTIAL_THRESHOLD:.0%}\n"
                                          f"   - بيع 100% عند درجة ≥ {SELL_FULL_THRESHOLD:.0%}\n"
                                          f"🔄 <b>مدة المسح:</b> {SCAN_INTERVAL_MINUTES} دقيقة\n"
                                          f"🤖 <b>الذكاء الاصطناعي (Gemini):</b> {'✅ مفعل' if AI_ENABLED else '❌ معطل'} | النموذج الحالي: {model_display}\n"
                                          f"━━━━━━━━━━━━━━━━━━━━\n"
                            ...
                            # ------------------------------------------------------------
                            # 3. أوامر تشغيل/إيقاف الفلاتر الفردية (محفوظة)
                            # ------------------------------------------------------------
                            elif text == 'اوقف فلتر التلاعب':
                                FILTER_MANIPULATION_ENABLED = False
                                send_telegram("🔴 تم إيقاف فلتر التلاعب (لن يتم رفض الصفقات بسبب التلاعب)")
                                save_state()
                            elif text == 'شغل فلتر التلاعب':
                                FILTER_MANIPULATION_ENABLED = True
                                send_telegram("🟢 تم تشغيل فلتر التلاعب (سيتم رفض الصفقات المشبوهة)")
                                save_state()
                                ...
                                save_state()
                            # تم حذف أوامر فلتر 4 ساعات بالكامل
                            # ------------------------------------------------------------
                            # 4. أوامر شغل/إيقاف فلتر الأداء
                            # ------------------------------------------------------------
                            elif text == 'شغل فلتر الأداء':
                                FILTER_30D_PROFIT_ENABLED = True
                                save_state()
                                send_telegram(f"🟢 تم تشغيل فلتر أداء {FILTER_30D_DAYS} يوم. العتبة: {MIN_30D_PROFIT_PERCENT:.2f}%")
                            elif text == 'اوقف فلتر الأداء':
                                FILTER_30D_PROFIT_ENABLED = False
                                save_state()
                                send_telegram(f"🔴 تم إيقاف فلتر أداء {FILTER_30D_DAYS} يوم.")
                            elif text.startswith('عتبة فلتر الأداء'):
                                match = re.search(r'(\d+(?:\.\d+)?)', text)
                                if match:
                                    new_threshold = float(match.group(1))
                                    MIN_30D_PROFIT_PERCENT = new_threshold
                                    save_state()
                                    send_telegram(f"✅ تم تعيين عتبة فلتر الأداء إلى {new_threshold:.2f}%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `عتبة فلتر الأداء 2.5`")
                            elif text.startswith('تعيين ايام فلتر الأداء'):
                                match = re.search(r'(\d+)', text)
                                if match:
                                    new_days = int(match.group(1))
                                    if 1 <= new_days <= 365:
                                        FILTER_30D_DAYS = new_days
                                        with _30d_cache_lock:
                                            _30d_performance_cache.clear()
                                        save_state()
                                        send_telegram(f"✅ تم تعيين عدد أيام فلتر الأداء إلى {new_days} يوم.")
                                    else:
                                        send_telegram("⚠️ يرجى إدخال عدد أيام بين 1 و 365.")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين ايام فلتر الأداء 45`")
                            # ------------------------------------------------------------
                            # 5. أوامر ضبط عتبات الشراء واللجنة
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 6. أوامر ضبط مدة المسح ووقت الانتظار
                            # ------------------------------------------------------------
                                ...
                                if match:
                                    ...
                                    if 1 <= new_interval <= 60:
                                        ...
                                        send_telegram("⚠️ يرجى إدخال قيمة بين 1 و 60 دقيقة")
                                else:
                            ...
                            # ------------------------------------------------------------
                            # 7. أوامر التبديل بين التداول الحقيقي والورقي
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 8. أوامر رفع/خفض عتبات الاشتباه (محفوظة)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 9. أوامر تعيين عتبات الخسارة الوقائية (محفوظة)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 10. الأوامر الديناميكية (عدد رموز التحليل، حجم التعرض، أحجام الصفقة، عدد الصفقات اليومية)
                            # ------------------------------------------------------------
                            ...
                            elif text.startswith('حجم صفقة السوق الجيد'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_percent = float(match.group(1)) / 100.0
                                    if 0.05 <= new_percent <= 0.5:
                                        CUSTOM_POSITION_PERCENT_GOOD = new_percent
                                        save_state()
                                        send_telegram(f"✅ تم تعيين حجم الصفقة في السوق الجيد إلى {new_percent:.1%}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 5% و 50%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `حجم صفقة السوق الجيد 30%`")
                            elif text.startswith('حجم صفقة السوق العادي'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_percent = float(match.group(1)) / 100.0
                                    if 0.05 <= new_percent <= 0.5:
                                        CUSTOM_POSITION_PERCENT_NORMAL = new_percent
                                        save_state()
                                        send_telegram(f"✅ تم تعيين حجم الصفقة في السوق العادي إلى {new_percent:.1%}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 5% و 50%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `حجم صفقة السوق العادي 25%`")
                            elif text.startswith('حجم صفقة السوق السيئ'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_percent = float(match.group(1)) / 100.0
                                    if 0.05 <= new_percent <= 0.5:
                                        CUSTOM_POSITION_PERCENT_BAD = new_percent
                                        save_state()
                                        send_telegram(f"✅ تم تعيين حجم الصفقة في السوق السيئ إلى {new_percent:.1%}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 5% و 50%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `حجم صفقة السوق السيئ 20%`")
                            ...
                            # ------------------------------------------------------------
                            # 11. أوامر الذكاء الاصطناعي (تشغيل/إيقاف والتبديل بين النموذجين)
                            # ------------------------------------------------------------
                            elif text == 'شغل الذكاء' or text == 'تفعيل الذكاء':
                                if not GEMINI_API_KEY:
                                    send_telegram("❌ مفتاح Gemini غير موجود في البيئة (GEMINI_API_KEY). لم يتم التفعيل.")
                                else:
                                    if not test_gemini_api_key():
                                        send_telegram("❌ مفتاح Gemini غير صالح أو النماذج غير متاحة. لم يتم التفعيل.")
                                    else:
                                        AI_ENABLED = True
                                        save_state()
                                        model_disp = "Flash" if CURRENT_AI_MODEL == 'flash' else "Flash-Lite"
                                        send_telegram(f"🤖 تم **تفعيل** استشارة الذكاء الاصطناعي (Gemini). النموذج الحالي: {model_disp}.")
                            elif text == 'أوقف الذكاء' or text == 'تعطيل الذكاء':
                                AI_ENABLED = False
                                save_state()
                                send_telegram("🤖 تم **إيقاف** استشارة الذكاء الاصطناعي. سأعمل بشكل طبيعي بدون استشارة.")
                            elif text == 'تحول إلى النموذج الأول':
                                flash_model = GEMINI_MODEL_FLASH
                                if flash_model and is_model_available(flash_model):
                                    CURRENT_AI_MODEL = 'flash'
                                    save_state()
                                    send_telegram(f"✅ تم التبديل إلى نموذج **Gemini Flash** ({flash_model}).")
                                else:
                                    send_telegram(f"❌ النموذج {flash_model or 'غير معروف'} غير متاح. لم يتم التبديل.")
                            elif text == 'تحول إلى النموذج الثاني':
                                lite_model = GEMINI_MODEL_FLASH_LITE
                                if lite_model and is_model_available(lite_model):
                                    CURRENT_AI_MODEL = 'flash-lite'
                                    save_state()
                                    send_telegram(f"✅ تم التبديل إلى نموذج **Gemini Flash-Lite** ({lite_model}).")
                                else:
                                    send_telegram(f"❌ النموذج {lite_model or 'غير معروف'} غير متاح. لم يتم التبديل.")
                            
                            # ------------------------------------------------------------
                            # 12. أوامر نظام التبريد الذكي الجديد
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # أوامر إيقاف/تشغيل التحليل (جديد)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 13. أوامر فلتر النموذج الأول والثاني والثالث والرابع والخامس (مرتبة)
                            # ------------------------------------------------------------
                            ...
                            # === فلتر النموذج الأول (حد أدنى) - مع كلمة "الأول" ===
                            ...
                            # === فلتر النموذج الرابع (حد أدنى) - جديد ===
                            ...
                            # === فلتر النموذج الخامس (حد أعلى) - جديد ===
                            ...
                            # ------------------------------------------------------------
                            elif text == 'شغل تحديث الوقف':
                                PREEMPTIVE_SL_UPDATE_ENABLED = True
                                save_state()
                                send_telegram("🟢 تم تفعيل تحديث الوقف المسبق الحقيقي (سيتم تحديث أوامر الوقف على Binance مع ارتفاع السعر)")

                            elif text == 'أوقف تحديث الوقف':
                                PREEMPTIVE_SL_UPDATE_ENABLED = False
                                save_state()
                                send_telegram("🔴 تم إيقاف تحديث الوقف المسبق الحقيقي (سيبقى أمر الوقف الأولي فقط دون تحديث)")

                            elif text == 'عرض إعدادات الوقف':
                                send_telegram(f"⚙️ إعدادات الوقف المسبق الحقيقي (يُرسل إلى Binance):\n"
                                              f"   حالة التحديث: {'🟢 مفعل' if PREEMPTIVE_SL_UPDATE_ENABLED else '🔴 معطل'}\n"
                                              f"   نسبة التفعيل (Stop Trigger): {CUSTOM_PREEMPTIVE_SL_TRIGGER:.2%}\n"
                                              f"   نسبة التنفيذ (Stop Limit): {CUSTOM_PREEMPTIVE_SL_LIMIT:.2%}")

                            elif text.startswith('تعيين تفعيل الوقف'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_val = float(match.group(1)) / 100.0
                                    if 0.005 <= new_val <= 0.10:
                                        CUSTOM_PREEMPTIVE_SL_TRIGGER = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تعيين نسبة تفعيل الوقف المسبق إلى {new_val:.2%}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.5% و 10%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين تفعيل الوقف 2.5%`")

                            elif text.startswith('تعيين حد تنفيذ الوقف'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_val = float(match.group(1)) / 100.0
                                    if 0.005 <= new_val <= 0.15:
                                        CUSTOM_PREEMPTIVE_SL_LIMIT = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تعيين حد تنفيذ الوقف المسبق إلى {new_val:.2%}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.5% و 15%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين حد تنفيذ الوقف 3%`")

                            # ------------------------------------------------------------
                            # 15. أوامر تتبع الأرباح (الوقف المتحرك الديناميكي)
                            # ------------------------------------------------------------
                            elif text.startswith('تتبع الأرباح عند نسبة'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_percent = float(match.group(1)) / 100.0
                                    if 0.001 <= new_percent <= 0.10:  # بين 0.1% و 10%
                            ...
                            # ------------------------------------------------------------
                            # 16. أوامر عتبات البيع الهرمية (جديدة)
                            # ------------------------------------------------------------
                            elif text.startswith('عتبة بيع جزئي'):
                                match = re.search(r'(\d+(?:\.\d+)?)', text)
                                if match:
                                    new_val = float(match.group(1)) / 100.0
                                    if 0.10 <= new_val <= 0.90:
                                        SELL_PARTIAL_THRESHOLD = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تعيين عتبة البيع الجزئي (بيع 50%) إلى {new_val:.0%}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 10% و 90%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `عتبة بيع جزئي 40`")
                            
                            elif text.startswith('عتبة بيع كامل'):
                                match = re.search(r'(\d+(?:\.\d+)?)', text)
                                if match:
                                    new_val = float(match.group(1)) / 100.0
                                    if 0.10 <= new_val <= 0.90:
                                        SELL_FULL_THRESHOLD = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تعيين عتبة البيع الكامل (بيع 100%) إلى {new_val:.0%}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 10% و 90%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `عتبة بيع كامل 30`")
                            
                            # الأمر القديم أصبح مرادفاً لعتبة البيع الكامل
                            elif text.startswith('تعيين عتبة بيع 15 دقيقة'):
                                numbers = re.findall(r'(\d+(?:\.\d+)?)', text)
                                if numbers:
                                    new_val = float(numbers[-1]) / 100.0
                                    if 0.10 <= new_val <= 0.90:
                                        SELL_FULL_THRESHOLD = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تحديث عتبة البيع الكامل (المرادفة لعتبة 15 دقيقة) إلى {new_val:.0%}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 10% و 90%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين عتبة بيع 15 دقيقة 30`")

                            # ------------------------------------------------------------
                            # 17. أمر تعيين أوزان الأطر (جديد)
                            # ------------------------------------------------------------
                            ...
                            # ------------------- أمر ضبط أوزان النماذج الخمسة دفعة واحدة (جديد) -------------------
                            elif text.startswith('تعيين أوزان النماذج') or text.startswith('ضبط أوزان اللجنة'):
                                # استخراج الأسماء والقيم باستخدام regex قوي
                                ...
                                # تحديث الأوزان في الكائنات
                                global SELL_WEIGHTS
                                buying_committee.weights = new_weights
                                SELL_WEIGHTS = new_weights.copy()
                                save_state()
                            ...
                            # ------------------- أمر عرض أوزان النماذج الحالية (جديد) -------------------
                            ...
                            # ------------------- الأمر الجديد: توصية بالأوزان -------------------
                            elif text == 'توصية بالأوزان' or text == 'الأوزان المثلى':
                                result, err = calculate_suggested_weights()
                                if err:
                                    send_telegram(err)
                                else:
                            ...
                            # ------------------------------------------------------------
                                ...
                                FILTER_CHANGE_24H_ENABLED = True
                                FILTER_HOUR_CANDLE_ENABLED = True
                                # تم حذف FILTER_4H_CANDLE_ENABLED
                                STRENGTH_THRESHOLD = 0.25
                                SCALP_MIN_PROFIT = 0.20
                                ...
                                CUSTOM_MAX_EXPOSED_PERCENT = MAX_EXPOSED_PERCENT
                                CUSTOM_POSITION_PERCENT_GOOD = MAX_POSITION_PERCENT_GOOD
                                CUSTOM_POSITION_PERCENT_NORMAL = 0.25
                                CUSTOM_POSITION_PERCENT_BAD = 0.20
                                CUSTOM_MAX_DAILY_TRADES = MAX_DAILY_TRADES
                                CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP = 60
                                AI_ENABLED = False
                                CURRENT_AI_MODEL = 'flash'
                                FILTER_30D_PROFIT_ENABLED = False
                                MIN_30D_PROFIT_PERCENT = 0.0
                                FILTER_30D_DAYS = 30
                                PAUSE_NEW_ENTRIES = False
                                # إعادة ضبط التبريد الجديد للقيم الافتراضية
                                COOLDOWN_WIN_HOURS = 12.0
                                COOLDOWN_LOSS_MINUTES = 5.0
                                with _cooldown_lock:
                                    _symbol_cooldown_until.clear()
                                with _30d_cache_lock:
                                    _30d_performance_cache.clear()
                                # ✅ التحديث 6: إعادة ضبط فلتر النموذج الأول إلى القيم الجديدة
                                SINGLE_MODEL_FILTER_MODEL = 'rule'
                                SINGLE_MODEL_FILTER_THRESHOLD = 0.11
                                SINGLE_MODEL_FILTER_TIMEFRAME = '5m'
                                SINGLE_MODEL_FILTER_ENABLED = True
                                # إعادة ضبط فلتر النموذج الثاني
                                SECOND_MODEL_FILTER_MODEL = 'cfhm'
                                SECOND_MODEL_FILTER_THRESHOLD = 0.59
                                SECOND_MODEL_FILTER_TIMEFRAME = '5m'
                                SECOND_MODEL_FILTER_ENABLED = True
                                # إعادة ضبط فلتر النموذج الثالث
                                THIRD_MODEL_FILTER_MODEL = 'cfhm'
                                THIRD_MODEL_FILTER_THRESHOLD = 0.50
                                THIRD_MODEL_FILTER_TIMEFRAME = '1h'
                                THIRD_MODEL_FILTER_ENABLED = True
                                # إعادة ضبط الفلترين الجديدين
                                FOURTH_MODEL_FILTER_MODEL = 'vwap_obv'
                                FOURTH_MODEL_FILTER_THRESHOLD = 0.30
                                FOURTH_MODEL_FILTER_TIMEFRAME = '15m'
                                FOURTH_MODEL_FILTER_ENABLED = True
                                FIFTH_MODEL_FILTER_MODEL = 'timing'
                                FIFTH_MODEL_FILTER_THRESHOLD = 0.70
                                FIFTH_MODEL_FILTER_TIMEFRAME = '1h'
                                FIFTH_MODEL_FILTER_ENABLED = True
                                # إعادة ضبط إعدادات الوقف
                                PREEMPTIVE_SL_UPDATE_ENABLED = True
                                CUSTOM_PREEMPTIVE_SL_TRIGGER = PREPLACED_SL_TRIGGER_PERCENT
                                CUSTOM_PREEMPTIVE_SL_LIMIT = PREPLACED_SL_LIMIT_PERCENT
                                # إعادة ضبط المضاعف
                                USE_1H_MULTIPLIER = True
                                # إعادة ضبط التتبع وبيع 15 دقيقة
                                TRAILING_DISTANCE_PERCENT = 0.01
                                # SELL_15M_THRESHOLD تم استبداله بـ SELL_FULL_THRESHOLD
                                SELL_FULL_THRESHOLD = 0.30
                                # إعادة ضبط أوزان الأطر
                                WEIGHT_5M = 10.0
                                WEIGHT_15M = 30.0
                                WEIGHT_1H = 60.0
                                # إعادة ضبط PAUSE_ANALYSIS إلى False (تشغيل التحليل)
                                PAUSE_ANALYSIS = False
                                ...
                                # إعادة ضبط عتبات البيع
                                SELL_PARTIAL_THRESHOLD = 0.40
                                SELL_FULL_THRESHOLD = 0.30
                                SELL_MEMBER_WEIGHTS = {'rule': 0.0, 'emr': 0.0, 'cfhm': 0.0, 'timing': 0.0, 'vwap_obv': 0.0}
                                SELL_FRAME_WEIGHTS = {'5m': 0.0, '15m': 0.0, '1h': 0.0}
                                # إعادة ضبط الحد الأعلى للثقة
                                UPPER_THRESHOLD_GLOBAL = 0.60
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية (مع فلاتر النماذج الأول والثاني والثالث والرابع والخامس).")
                                save_state()
                            
                            # ------------------------------------------------------------
                            # 19. أمر رسم منحنى الرصيد الكلي
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 20. أوامر إيقاف دخول الصفقات الجديدة
                            # ------------------------------------------------------------
                            ...
                            # ============================================================
                            # 21. أوامر تشغيل/إيقاف المضاعف (يبقى كمفتاح، لكن مصدره أصبح 5 دقائق)
                            # ============================================================
                            ...
                            # ------------------------------------------------------------
                            # 22. أمر الحد الأعلى الموحد للثقة (جديد)
                            # ------------------------------------------------------------
...
class MarketScanner:
    ...
    def scan(self):
        ...
        # ✅ حساب الموعد التالي مع تأخير عشوائي
...
def thread_watchdog():
    essential_threads = {
        ...
        'cooldown_cleanup': cooldown_cleanup   # الخيط الجديد لتنظيف التبريد
    }
...
def save_state():
    ...
    try:
        ...
        # إضافة تحقق من وجود buying_committee قبل حفظ الأوزان
        model_weights = buying_committee.weights if 'buying_committee' in globals() else {'rule':0.10, 'emr':0.05, 'cfhm':0.10, 'timing':0.55, 'vwap_obv':0.20}
        data = {
            ...
            'filters': {
                ...
                # ===== تم إزالة sell_15m_threshold واستبداله بما يلي: =====
                'sell_partial_threshold': SELL_PARTIAL_THRESHOLD,
                'sell_full_threshold': SELL_FULL_THRESHOLD,
                'sell_member_weights': SELL_MEMBER_WEIGHTS,
                'sell_frame_weights': SELL_FRAME_WEIGHTS,
                # ===== الحد الأعلى الموحد =====
            ...
            'custom_settings': {
                ...
                'position_percent_good': CUSTOM_POSITION_PERCENT_GOOD,
                'position_percent_normal': CUSTOM_POSITION_PERCENT_NORMAL,
                'position_percent_bad': CUSTOM_POSITION_PERCENT_BAD,
            ...
            'weights': {
                ...
                '15m': WEIGHT_15M,
                '1h': WEIGHT_1H,
                'models': model_weights  # حفظ أوزان النماذج مع التحقق
            }
        }
...
def load_state():
    ...
    global SELL_PARTIAL_THRESHOLD, SELL_FULL_THRESHOLD, SELL_MEMBER_WEIGHTS, SELL_FRAME_WEIGHTS
    # === الحد الأعلى الموحد ===
    global UPPER_THRESHOLD_GLOBAL   # <-- تم إضافة هذا السطر
    ...
    try:
        ...
        AI_ENABLED = filters.get('ai_enabled', False)
        CURRENT_AI_MODEL = filters.get('current_ai_model', 'flash')
        ...
        # ✅ التحديث 6: قراءة فلتر النموذج الأول مع القيم الافتراضية الجديدة
        ...
        # === قراءة فلتر النموذج الثاني ===
        SECOND_MODEL_FILTER_MODEL = filters.get('second_model_filter_model', 'cfhm')
        SECOND_MODEL_FILTER_THRESHOLD = filters.get('second_model_filter_threshold', 0.59)
        ...
        # === قراءة فلتر النموذج الثالث ===
        THIRD_MODEL_FILTER_MODEL = filters.get('third_model_filter_model', 'cfhm')
        THIRD_MODEL_FILTER_THRESHOLD = filters.get('third_model_filter_threshold', 0.50)
        ...
        # === قراءة فلتر النموذج الرابع ===
        FOURTH_MODEL_FILTER_MODEL = filters.get('fourth_model_filter_model', 'vwap_obv')
        FOURTH_MODEL_FILTER_THRESHOLD = filters.get('fourth_model_filter_threshold', 0.30)
        ...
        # === قراءة فلتر النموذج الخامس ===
        FIFTH_MODEL_FILTER_MODEL = filters.get('fifth_model_filter_model', 'timing')
        FIFTH_MODEL_FILTER_THRESHOLD = filters.get('fifth_model_filter_threshold', 0.70)
        ...
        TRAILING_DISTANCE_PERCENT = filters.get('trailing_distance_percent', 0.01)
        # === قراءة الحد الأعلى الموحد ===
        UPPER_THRESHOLD_GLOBAL = filters.get('upper_threshold_global', 0.60)
        # === قراءة عتبات البيع (تم إزالة sell_15m_threshold) ===
        SELL_PARTIAL_THRESHOLD = filters.get('sell_partial_threshold', 0.40)
        SELL_FULL_THRESHOLD = filters.get('sell_full_threshold', 0.30)
        # 🔥 التعديل الأول: القيم الافتراضية لأوزان البيع أصبحت صحيحة بدلاً من الأصفار
        SELL_MEMBER_WEIGHTS = filters.get('sell_member_weights', {'rule':0.05, 'emr':0.10, 'cfhm':0.30, 'timing':0.45, 'vwap_obv':0.10})
        SELL_FRAME_WEIGHTS = filters.get('sell_frame_weights', {'5m':0.30, '15m':0.10, '1h':0.60})
        PAUSE_NEW_ENTRIES = data.get('pause_new_entries', False)
        cust = data.get('custom_settings', {})
        CUSTOM_ACTIVE_SYMBOLS_LIMIT = cust.get('active_symbols_limit', ACTIVE_SYMBOLS_LIMIT)
        CUSTOM_MAX_EXPOSED_PERCENT = cust.get('max_exposed_percent', MAX_EXPOSED_PERCENT)
        CUSTOM_POSITION_PERCENT_GOOD = cust.get('position_percent_good', MAX_POSITION_PERCENT_GOOD)
        CUSTOM_POSITION_PERCENT_NORMAL = cust.get('position_percent_normal', 0.25)
        CUSTOM_POSITION_PERCENT_BAD = cust.get('position_percent_bad', 0.20)
        CUSTOM_MAX_DAILY_TRADES = cust.get('max_daily_trades', MAX_DAILY_TRADES)
        CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP = cust.get('max_hold_minutes_before_tp', 60)
        cooldown = data.get('cooldown_settings', {})
        COOLDOWN_WIN_HOURS = cooldown.get('win_hours', 12.0)
        COOLDOWN_LOSS_MINUTES = cooldown.get('loss_minutes', 5.0)
        _symbol_cooldown_until.clear()
        ...
        WEIGHT_5M = weights.get('5m', 10.0)
        WEIGHT_15M = weights.get('15m', 30.0)
        WEIGHT_1H = weights.get('1h', 60.0)
        # استعادة أوزان النماذج
        model_weights = weights.get('models', {'rule':0.10, 'emr':0.05, 'cfhm':0.10, 'timing':0.55, 'vwap_obv':0.20})
        buying_committee.weights = model_weights
        SELL_WEIGHTS = model_weights.copy()
...
if __name__ == '__main__':
    ...
    # انتظار عشوائي بين 30 و 60 ثانية بدلاً من 120 ثانية ثابتة
    ...
    # ========== تعيين أوزان النماذج الافتراضية (بدون إعادة تعريف buying_committee) ==========
    buying_committee.weights = {'rule': 0.0, 'emr': 0.0, 'cfhm': 0.4, 'timing': 0.6, 'vwap_obv': 0.0}
    SELL_WEIGHTS = {'rule': 0.0, 'emr': 0.0, 'cfhm': 0.4, 'timing': 0.6, 'vwap_obv': 0.0}
    logger.info("✅ تم تعيين أوزان النماذج الافتراضية: CFHM=40%, Timing=60%")
    # =========================== التحديث 2: تهيئة ملف التوصيات ===========================
    init_recommendation_log()
    load_state()
    
    # تحميل الأسواق مع مراقبة الوزن (تم تقليل عدد المحاولات من 8 إلى 3)
    ...
    logger.info(f"🚀 بدء البوت v28.0 (Spot + Gemini AI + تبريد ذكي + 3 أطر حقيقية + أوزان ديناميكية + توصيات ذكية) - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v28.0 (الإصدار المحسن بـ 3 أطر زمنية 5م، 15د، 1س مع أوزان ديناميكية وتوصيات ذكية) يعمل – الوضع: {mode_str}\n"
                  f"✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n"
                  f"✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n"
                  f"✅ تقرير يومي يعرض صافي الربح/الخسارة\n"
                  f"✅ التحويل بين التداول الورقي والحقيقي: `تداول حقيقي` / `تداول ورقي`\n"
                  f"✅ أمر `منحنى` أو `رسم` لإرسال صورة بيانية لمنحنى الرصيد الكلي\n"
                  f"✅ أمر `عدد الصفقات المسموحة` للتحكم بالحد اليومي للصفقات\n"
                  f"✅ تحسينات كبيرة في الذاكرة ومنع التسريب\n"
                  f"✅ WebSocket خالص مع إعادة تشغيل تلقائي\n"
                  f"✅ مراقبة وإعادة تشغيل جميع الخيوط بما فيها WebSocket (مع حماية watchdog)\n"
                  f"✅ جلب صبور للرصيد الحقيقي (حتى 120 ثانية)\n"
                  f"✅ سرعة مراقبة المراكز 1 ثانية\n"
                  f"✅ التحكم بدرجة الاشتباه: `رفع عتبة الاشتباه 70%` أو `خفض عتبة الاشتباه 50%`\n"
                  f"✅ التحكم بعتبات الخسارة الوقائية:\n"
                  f"   - `تعيين عتبة خسارة 25% 0.9`\n"
                  f"   - `تعيين عتبة خسارة 33% 1.5`\n"
                  f"   - `تعيين عتبة خسارة 100% 2.0`\n"
                  f"   - `عتبات الخسارة` لعرض القيم الحالية\n"
                  f"✅ البيع التدريجي الصارم (بدون إغلاق مبكر بسبب المبلغ الصغير)\n"
                  f"✅ نسب البيع: 15%, 15%, 15%, 15%, 20%, 50%, 100%\n"
                  f"✅ دعم أفضل لمنصة Render (منفذ 8080)\n"
                  f"✅ **إصلاح ذاتي كامل - لا حاجة لإعادة تشغيل خارجية**\n"
                  f"✅ **الوقف المسبق الموحد للعادي والميم: stop_loss_limit (مدعوم لجميع أزواج Spot)**\n"
                  f"✅ **خيط auto_recovery خفيف (بدون إعادة تحميل الأسواق) - مع منطق الانتظار 12 دقيقة ثم 3 محاولات قبل الاسترداد**\n"
                  f"✅ **تحسين حساب قيمة المراكز المفتوحة في التداول الورقي والحقيقي مع فاصل زمني 5 ثوانٍ وإعادة محاولات واعتماد على WebSocket**\n"
                  f"✅ **إصلاح تقلص حجم الصفقات المتزامنة (تم تعديل حساب total_eq2 مرة واحدة قبل حلقة الفرص)**\n"
                  f"✅ **التحكم الديناميكي بعدد الرموز وحجم التعرض وحجم الصفقة حسب حالة السوق عبر أوامر تلغرام**\n"
                  f"✅ **منع البيع الجزئي الصغير جداً (أقل من min_cost) تلقائياً**\n"
                  f"✅ **ترقيم أهداف الربح بشكل متسلسل (الهدف الأول، الثاني، ...)**\n"
                  f"✅ **إضافة جميع أوامر v27.7 المفقودة (تشغيل/إيقاف الفلاتر الفردية، تعديل عتبات الشراء، فترة التبريد، مدة المسح)**\n"
                  f"✅ **إضافة أمر تلغرام للتحكم بعدد الصفقات اليومية المسموحة**\n"
                  f"✅ **تحسين تنفيذ الأوامر: هامش ±0.3%، مهلة 10 ثوانٍ، فاصل تحقق 1 ثانية، تحول إلى أمر سوق تلقائي مع إعادة جلب الحالة قبل الإلغاء**\n"
                  f"✅ **إصلاح خطير: استخدام stop_loss_limit بدلاً من stop_loss لضمان التوافق مع سوق Spot**\n"
                  f"✅ **إضافة فلتر Google Gemini مع نموذجين (Flash و Flash-Lite) والتحكم بهما عبر تلغرام**\n"
                  f"✅ **تحسينات شاملة للذكاء الاصطناعي: مهلة 65 ثانية، إعادة محاولة واحدة للأخطاء المؤقتة، إشعارات القبول والرفض مع السبب، اختبار المفتاح عند بدء التشغيل، استخدام بيانات كاملة، تسجيل الموافقة في سجل التداول**\n"
                  f"✅ **فلتر أداء X يوم (جديد) مع تحكم كامل بعدد الأيام والعتبة عبر تلغرام**\n"
                  f"✅ **إيقاف دخول الصفقات الجديدة (بدون إغلاق المراكز المفتوحة) عبر أمر `توقف عن دخول الصفقات`**\n"
                  f"✅ **تحسين حالة السوق باستخدام نسبة الرابحين/الخاسرين (Advance/Decline Ratio)**\n"
                  f"✅ **تغيير نموذج التأكيد في لجنة الشراء من rule إلى emr**\n"
                  f"✅ **زيادة فترة الزخم في النموذج القاعدي من 6 إلى 12 شمعة**\n"
                  f"✅ **إضافة أوزان مرجحة للنماذج في لجنة الشراء (أوزان مختلفة للـ meme والعادي)**\n"
                  f"✅ **حل مشكلة البداية الباردة لنموذج EMR بإرجاع 0.5 بدلاً من None**\n"
                  f"✅ **إضافة سبب القبول إلى رسالة الشراء على تلغرام**\n"
                  f"✅ **التحكم الديناميكي بفترة الانتظار قبل البيع الكامل عبر أمر `فترة الانتظار X دقيقة`**\n"
                  f"✅ **إضافة نموذج توقيت الدخول (EntryTimingModel) كعضو مصوت رابع في لجنة الشراء**\n"
                  f"✅ **استبدال نموذج EMR القديم بالنموذج الهجين HybridMomentumRSIModel**\n"
                  f"✅ **تعديل علاقة RSI في النموذج الهجين إلى علاقة غير خطية لتجنب التفاؤل المفرط في القمم**\n"
                  f"✅ **تعديل أوزان لجنة الشراء إلى: CFHM 30%، Rule 10%، Hybrid 15%، Timing 30%، VWAP_OBV 15%**\n"
                  f"✅ **نظام تبريد ذكي جديد (قابل للتعديل عبر تلغرام): 12 ساعة للصفقة الرابحة، 5 دقائق للصفقة الخاسرة**\n"
                  f"✅ **أوامر تلغرام للتحكم بالتبريد: `تعيين تبريد الرابح X ساعة` و `تعيين تبريد الخاسر X دقيقة` و `عرض التبريد`**\n"
                  f"✅ **إضافة نموذج VWAP+OBV كعضو مصوت خامس في لجنة الشراء بوزن 15%**\n"
                  ...
                  f"✅ **نظام بيع 15 دقيقة جديد (متوسط 5م+15د) مع إعادة محاولة جلب الشموع وعرض الدرجات في رسالة البيع**\n"
                  f"✅ **أمر `تصفير التبريد` لمسح كل التبريدات**\n"
                  f"✅ **أمر `إيقاف التحليل` و `تشغيل التحليل` للتحكم في التحليل والمسح دون التأثير على المراكز المفتوحة**\n"
                  f"✅ **تم حذف فلتر شمعة 4 ساعات بالكامل**\n"
                  f"✅ **تم إضافة أوزان الأطر (5م، 15د، 1س) قابلة للتعديل عبر أمر `تعيين أوزان الأطر`**\n"
                  f"✅ **عرض الأوزان في تقرير الحالة والتقرير الدوري**\n"
                  f"✅ **أمر `توصية بالأوزان` لتحليل آخر الصفقات واقتراح الأوزان المثلى للنماذج والأطر باستخدام متوسط الربح الشرطي ووزن زمني**\n"
                  f"✅ **تم إصلاح النماذج الثلاثة (Rule, EMR, VWAP_OBV) بعلاقات غير خطية وعقوبات قاسية لتحسين التمييز بين الرابح والخاسر**\n"
                  f"✅ **تحسين RateLimiter: حد أقصى 1000 نقطة، هامش أمان 50 نقطة**\n"
                  f"✅ **إضافة الأوزان الصحيحة للطلبات مع wait_if_needed قبل load_markets**\n"
                  f"✅ **بدء تشغيل بانتظار عشوائي 30-60 ثانية بدلاً من 120 ثانية**\n"
                  f"✅ **إزالة BASE_SYMBOLS من دورة التحليل وتحميل الكاش الأولي**\n"
                  f"✅ **رفع TTLs لتقليل استهلاك الوزن: الشموع 60، دفتر الطلبات 5، الرصيد 5 دقائق، المزامنة 30 ثانية**\n"
                  f"✅ **التحديث 1: تردد خيط مزامنة أوامر الوقف المسبق من 60 ثانية إلى 600 ثانية (10 دقائق)**\n"
                  f"✅ **التحديث 2: نظام توصيات جديد يعتمد على ملف `trade_recommendations_log.csv` مع تسجيل جميع الدرجات والربح النهائي، واستبدال دالة التوصية بتصنيف هرمي يعتمد على متوسط الربح الشرطي ووزن زمني**\n"
                  f"✅ **التحديث 3: لجنة بيع جديدة بعتبات هرمية (40% بيع 50%، 30% بيع 100%) وأوزان بيع مستوحاة من التوصية، مع أوامر تلغرام للتحكم**\n"
                  f"✅ **التحديث 4: فاصل التحليل من 100 ثانية إلى 180 ثانية (3 دقائق) لتقليل استهلاك الوزن**\n"
                  f"✅ **التحديث 5: إعادة ترتيب أوامر الفلتر بحيث يُفحص الفلتر الثاني أولاً**\n"
                  f"✅ **التحديث 6: تغيير القيم الافتراضية للفلتر الأول إلى rule, 5m, 0.11**\n"
                  f"✅ **التحديث 7: تحسين اختيار المرشحين (الانتقال إلى الـ12 التالية عند ثبات أول 5)**\n"
                  f"✅ **التحديث 8: إضافة فلتر النموذج الثالث مع أوامر تلغرام وحفظ الحالة**\n"
                  f"✅ **التحديث 9: إعادة ترتيب أوامر الفلاتر (الثالث، الثاني، الأول) وإضافة كلمة 'الأول' لأوامر الفلتر الأول**\n"
                  f"✅ **التحديث 10: إضافة فلتر رابع (حد أدنى) نموذج VWAP_OBV على 15m عتبة 0.30**\n"
                  f"✅ **التحديث 11: إضافة فلتر خامس (حد أعلى) نموذج Timing على 1h عتبة 0.70**")
    ...
    if GEMINI_API_KEY:
        try:
            ...
            if not test_gemini_api_key():
                AI_ENABLED = False
                send_telegram("⚠️ مفتاح Gemini غير صالح أو النماذج غير متاحة – تم تعطيل الذكاء الاصطناعي")
            else:
                model_display = "Flash" if CURRENT_AI_MODEL == 'flash' else "Flash-Lite"
                send_telegram(f"🤖 الذكاء الاصطناعي (Gemini) مفعّل، النموذج الحالي: {model_display} (يمكن إيقافه بأمر `أوقف الذكاء`)")
        except Exception as e:
            logger.error(f"فشل تكوين Gemini: {e}")
            AI_ENABLED = False
            send_telegram("⚠️ فشل تكوين Gemini – تم تعطيل الذكاء الاصطناعي")
    else:
        AI_ENABLED = False
        send_telegram("🤖 الذكاء الاصطناعي (Gemini) معطل (يمكن تفعيله بأمر `شغل الذكاء`، مع ضرورة تعيين GEMINI_API_KEY في البيئة)")
