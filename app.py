...
# + التحديثات الجديدة (v29.1): نظام المسارين (Fast/Slow Path) + إحصائيات منفصلة
# + التحديثات الجديدة (v29.2):
#     • Trailing يُفعَّل عند +2.2% (بدل +3%) والتتبع عند 1.2% (بدل 1%)
#     • إجبار الفلاتر الأول والثاني والثالث (Rule + CFHM) في المسار السريع
# ========================================================================
...
MIN_24H_VOLUME_USD = float(os.environ.get('MIN_24H_VOLUME_USD', 1_000_000))
...
class BotStats:
    def __init__(self):
        ...
        self.total_trades = 0
        # ========== عدّادات المسارين (جديد) ==========
        self.fast_path_trades = 0
        self.slow_path_trades = 0
        self.fast_path_wins = 0
        self.slow_path_wins = 0
        self.fast_path_pnl = 0.0
        self.slow_path_pnl = 0.0
        # ============================================
        self.daily_pnl = 0.0
        self.weekly_pnl = 0.0
        self.weekly_wins = 0
...
class Position:
    def __init__(self, symbol, side, size, entry, atr, sl, tp, sym_type, pred, conf, regime, tp_levels=None, ai_approved=False,
                 ...
                 final_score=None, entry_path=None):
        self.symbol = symbol
        self.side = side
        self.entry_path = entry_path or 'slow'
        ...
        self.trailing_drop_threshold = 0.012      # ✅ نسبة الانخفاض (1.2%)
        self.trailing_check_interval = 60         # ثوانٍ بين كل فحص
        # ============================================
...
def execute_limit_order(symbol, side, size, price_ref, sym_type='normal'):
    timeout = LIMIT_TO_MARKET_TIMEOUT  # الآن 45 ثانية من المتغيرات
    if _should_simulate():
    ...
    for attempt in range(2):
        try:
            ...
            # --- محاولة أخيرة لجلب الحالة بعد المهلة ---
            time.sleep(2)
            try:
                status = exchange.fetch_order(oid, symbol)
                if status['status'] == 'closed':
...
class BuyingCommittee:
    ...
    # ✅ التحديث الجديد: تحديد الفلاتر الإجبارية في المسار السريع
    def _should_check_filter(self, filter_num, skip_individual_filters):
        """
        يحدد ما إذا كان يجب فحص الفلتر:
        - المسار العادي (skip=False): يفحص كل الفلاتر السبعة
        - المسار السريع (skip=True): يفحص الفلاتر 1 و 2 و 3 (مفروضة إجبارياً)
        """
        if not skip_individual_filters:
            return True   # المسار العادي: فحص كل الفلاتر
        return filter_num in (1, 2, 3)   # ⚡ المسار السريع: الفلاتر 1 و 2 و 3 مفروضة

    def decide(self, df_5m, df_15m, df_1h, symbol, sym_type='normal', market_condition='normal', skip_individual_filters=False):
        ...
        # ⚡⚡ الفلتر 1 — مفروض إجبارياً في المسارين (سريع + عادي)
        if self._should_check_filter(1, skip_individual_filters) and SINGLE_MODEL_FILTER_ENABLED:
        ...
        # ⚡⚡ الفلتر 2 — مفروض إجبارياً في المسارين (سريع + عادي)
        if self._should_check_filter(2, skip_individual_filters) and SECOND_MODEL_FILTER_ENABLED:
        ...
        # ⚡⚡ الفلتر 3 — مفروض إجبارياً في المسارين (سريع + عادي)
        if self._should_check_filter(3, skip_individual_filters) and THIRD_MODEL_FILTER_ENABLED:
        ...
        # الفلتر 4 — يُفحص فقط في المسار العادي
        if self._should_check_filter(4, skip_individual_filters) and FOURTH_MODEL_FILTER_ENABLED:
        ...
        # الفلتر 5 — يُفحص فقط في المسار العادي
        if self._should_check_filter(5, skip_individual_filters) and FIFTH_MODEL_FILTER_ENABLED:
        ...
        # الفلتر 6 — يُفحص فقط في المسار العادي
        if self._should_check_filter(6, skip_individual_filters) and SIXTH_MODEL_FILTER_ENABLED:
        ...
        # الفلتر 7 — يُفحص فقط في المسار العادي
        if self._should_check_filter(7, skip_individual_filters) and SEVENTH_MODEL_FILTER_ENABLED:
        ...
        try:
            ...
            with _global_state_lock:
                ...
                if pos.remaining_size <= 1e-8:
                    ...
                    # =====================================================
                    # ========== عدّادات المسار (جديد) ==========
                    _path = getattr(pos, 'entry_path', 'slow')
                    if _path == 'fast':
                        bot_stats.fast_path_pnl += pnl_net
                        if pnl_net > 0:
                            bot_stats.fast_path_wins += 1
                        bot_stats.fast_path_trades += 1
                    else:
                        bot_stats.slow_path_pnl += pnl_net
                        if pnl_net > 0:
                            bot_stats.slow_path_wins += 1
                        bot_stats.slow_path_trades += 1
                    # ==========================================
            ...
            for sym, pos in snapshot:
                ...
                if not pos.trailing_active and profit >= pos.trailing_activation_profit:
                    ...
                    send_telegram(
                        ...
                        f"ℹ️ سيتم البيع الكامل إذا انخفض السعر 1.2% عن الذروة "
                ...
                if pos.trailing_active:
                    ...
                    if drop >= pos.trailing_drop_threshold:
                        ...
                        if elapsed >= pos.trailing_check_interval:
                            pos.trailing_last_check_time = datetime.now()
                            ...
                            if not df_15m_trail.empty and len(df_15m_trail) >= 2:
                                ...
                                if cur_price < last_closed_close:
                                    ...
                                    logger.info(f"⏸️ {sym}: Trailing - السعر ≥ إغلاق 15د، الانتظار")
                            else:
...
def execute_full_trade(symbol, side, size, price, atr, sl, tp, sym_type, pred, conf, regime, ai_approved=False, approval_reason="",
                        scores_5m=None, scores_15m=None, scores_1h=None,
                        weighted_score_5m=None, weighted_score_15m=None, weighted_score_1h=None,
                        final_score=None, entry_path='slow'):
    ...
    try:
        ...
        if _should_simulate():
            ...
            pos = Position(symbol, side, size, price, atr, sl, tp, sym_type, pred, conf, regime, ai_approved=ai_approved,
                           ...
                           weighted_score_1h=weighted_score_1h, final_score=final_score, entry_path=entry_path)
            with _global_state_lock: open_positions[symbol] = pos
            path_icon = "⚡" if entry_path == 'fast' else "🐢"
            send_telegram(f"🧪 [{'محاكاة' if TEST_MODE else 'ورقي'}] شراء {symbol} ({sym_type}) {path_icon} | الحجم: {size:.6f} | السعر: {price:.8f}\n📊 سبب الموافقة: {approval_reason}")
            save_state()
            return True, size
        ...
        if success and fill_price and filled_size>0:
            ...
            if new_balance is not None:
                with _global_state_lock: bot_stats.last_balance = new_balance
            path_icon = "⚡" if entry_path == 'fast' else "🐢"
            send_telegram(f"✅ <b>شراء</b> {symbol} ({sym_type}) {path_icon} | الحجم: {filled_size:.6f} | السعر: {fill_price:.8f}\n📊 سبب الموافقة: {approval_reason}")
            save_state()
...
def analyze_and_trade():
    ...
    try:
        ...
        with _global_state_lock:
            if _daily_trades_date != today:
                if _daily_trades_count > 0:
                    ...
                    if bot_stats.fast_path_trades > 0 or bot_stats.slow_path_trades > 0:
                        _fast_wr = (bot_stats.fast_path_wins / bot_stats.fast_path_trades * 100) if bot_stats.fast_path_trades > 0 else 0
                        _slow_wr = (bot_stats.slow_path_wins / bot_stats.slow_path_trades * 100) if bot_stats.slow_path_trades > 0 else 0
                        report += (
                            f"\n\n<b>⚡ مقارنة المسارين (تراكمي):</b>\n"
                            f"🚀 سريع: {bot_stats.fast_path_trades} صفقة | نجاح {_fast_wr:.1f}% | PnL ${bot_stats.fast_path_pnl:+.2f}\n"
                            f"🐢 عادي: {bot_stats.slow_path_trades} صفقة | نجاح {_slow_wr:.1f}% | PnL ${bot_stats.slow_path_pnl:+.2f}"
                        )
                    send_telegram(report)
        ...
        for sym in active:
            ...
            # ========== حساب المضاعف أولاً (قبل decide) ==========
            if not df_5m.empty and len(df_5m) >= 20 and not df_15m.empty and len(df_15m) >= 20:
                res_5m_pre = buy_comm.calculate_weighted_average(df_5m, None, None, sym, sym_type, 'normal')
                score_5m = res_5m_pre['weighted_avg']
                res_15m_pre = buy_comm.calculate_weighted_average(df_15m, None, None, sym, sym_type, 'normal')
                score_15m = res_15m_pre['weighted_avg']
            ...
            # ========== شرط تخطي الفلاتر ==========
            skip_individual = USE_1H_MULTIPLIER and (multiplier > 1.0)
            # =====================================

            dec, avg_score, conf, reason, avg_scores, scores_5m, scores_15m, scores_1h = buy_comm.decide(
                df_5m, df_15m, df_1h, sym, sym_type, 'normal',
                skip_individual_filters=skip_individual
            )
            if dec != 'buy':
                continue
            ...
            path_icon = "⚡" if skip_individual else "🐢"
            path_name = "سريع (تخطي الفلاتر)" if skip_individual else "عادي (الفلاتر مفعلة)"
            
            reason = (
                f"✅ موافقة: المتوسط النهائي={avg_score:.3f} | العتبة={thresh:.3f} | المضاعف={multiplier:.2f}x {path_icon}\n"
                f"🛤️ المسار: {path_name}\n"
                f"⚖️ {weights_str}\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"📊 إطار 5 دقائق:\n"
            ...
            pending.append({
                ...
                'path': 'fast' if skip_individual else 'slow'
            })
        ...
        try:
            ...
            if positions_count > 0:
                ...
                for sym, pos in open_positions.items():
                    ...
                    path_icon = "⚡" if getattr(pos, 'entry_path', 'slow') == 'fast' else "🐢"
                    positions_detail += f"• {sym} ({pos.side}) {path_icon} ربح: {profit:.2%}\n"
...
# ======================================================================
# =========== telegram_polling (يحتوي التحديث 14: أمر مزامنة) ==========
# ======================================================================
                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ========== إحصائيات المسارين (جديد) ==========
                            elif text == 'احصائيات المسارين' or text == 'المسارين' or text == 'إحصائيات المسارين':
                                total_path = bot_stats.fast_path_trades + bot_stats.slow_path_trades
                                fast_wr = (bot_stats.fast_path_wins / bot_stats.fast_path_trades * 100) if bot_stats.fast_path_trades > 0 else 0
                                slow_wr = (bot_stats.slow_path_wins / bot_stats.slow_path_trades * 100) if bot_stats.slow_path_trades > 0 else 0
                                fast_avg = (bot_stats.fast_path_pnl / bot_stats.fast_path_trades) if bot_stats.fast_path_trades > 0 else 0
                                slow_avg = (bot_stats.slow_path_pnl / bot_stats.slow_path_trades) if bot_stats.slow_path_trades > 0 else 0
                                send_telegram(
                                    f"<b>📊 إحصائيات المسارين</b>\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                    f"⚡ <b>المسار السريع</b> (مضاعف > 1.0 - تخطي الفلاتر):\n"
                                    f"   • عدد الصفقات: <b>{bot_stats.fast_path_trades}</b>\n"
                                    f"   • صفقات رابحة: {bot_stats.fast_path_wins} ({fast_wr:.1f}%)\n"
                                    f"   • صافي PnL: <b>${bot_stats.fast_path_pnl:+.2f}</b>\n"
                                    f"   • متوسط الصفقة: ${fast_avg:+.2f}\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                    f"🐢 <b>المسار العادي</b> (مضاعف ≤ 1.0 - الفلاتر مفعّلة):\n"
                                    f"   • عدد الصفقات: <b>{bot_stats.slow_path_trades}</b>\n"
                                    f"   • صفقات رابحة: {bot_stats.slow_path_wins} ({slow_wr:.1f}%)\n"
                                    f"   • صافي PnL: <b>${bot_stats.slow_path_pnl:+.2f}</b>\n"
                                    f"   • متوسط الصفقة: ${slow_avg:+.2f}\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                    f"📈 إجمالي: {total_path} صفقة\n"
                                    f"💡 إذا كان السريع خاسراً، أوقف المضاعف بـ: <code>أوقف المضاعف</code>"
                                )
    ...
    try:
        ...
        for sym, p in open_positions.items():
            ...
            pos_dict[sym] = {
                ...
                'trade_id': p.trade_id,
                'entry_path': getattr(p, 'entry_path', 'slow'),
                ...
                'trailing_drop_threshold': getattr(p, 'trailing_drop_threshold', 0.012),         # ✅
        ...
        data = {
            ...
            'bot_stats':{
                ...
                'fast_path_trades': bot_stats.fast_path_trades,
                'slow_path_trades': bot_stats.slow_path_trades,
                'fast_path_wins': bot_stats.fast_path_wins,
                'slow_path_wins': bot_stats.slow_path_wins,
                'fast_path_pnl': bot_stats.fast_path_pnl,
                'slow_path_pnl': bot_stats.slow_path_pnl,
...
def load_state():
    ...
    try:
        for sym, pdata in data.get('open_positions', {}).items():
            ...
            pos.entry_path = pdata.get('entry_path', 'slow')
            ...
            pos.trailing_activation_profit = pdata.get('trailing_activation_profit', 0.022)   # ✅
            pos.trailing_drop_threshold = pdata.get('trailing_drop_threshold', 0.012)         # ✅
            pos.trailing_check_interval = pdata.get('trailing_check_interval', 60)
            # ============================================================
        ...
        bot_stats.fast_path_trades = stats.get('fast_path_trades', 0)
        bot_stats.slow_path_trades = stats.get('slow_path_trades', 0)
        bot_stats.fast_path_wins = stats.get('fast_path_wins', 0)
        bot_stats.slow_path_wins = stats.get('slow_path_wins', 0)
        bot_stats.fast_path_pnl = stats.get('fast_path_pnl', 0.0)
        bot_stats.slow_path_pnl = stats.get('slow_path_pnl', 0.0)
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v29.2 (نسخة مبسطة + التحديثات) - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v29.2 (النسخة المبسطة بعد التنظيف + التحديثات) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"🆕 تحديثات v29.2:\n"
                  f"   • Trailing يُفعَّل عند +2.2% (بدل +3%)\n"
                  f"   • نسبة التتبع 1.2% (بدل 1%)\n"
                  f"   • الفلاتر 1 و 2 و 3 (Rule + CFHM) مفروضة في المسار السريع\n"
                  f"   • نظام المسارين (⚡ سريع / 🐢 عادي)\n"
                  f"   • إحصائيات منفصلة لكل مسار: `احصائيات المسارين`")
