...
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        ...
        # تطبيق الأوزان المرجحة بدلاً من المتوسط العادي
        if sym_type == 'meme':
            weights = {'rule': 0.2, 'emr': 0.4, 'cfhm': 0.4}
        else:
            weights = {'rule': 0.4, 'emr': 0.3, 'cfhm': 0.3}
        
        # ترتيب القيم حسب ترتيب النماذج: rule, emr, cfhm
        weighted_avg = 0.0
        weight_sum = 0.0
        for i, model_key in enumerate(['rule', 'emr', 'cfhm']):
            if i < len(adjusted):
