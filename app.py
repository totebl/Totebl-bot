...
# التعديلات الجديدة: منع البيع الجزئي للأجزاء الصغيرة (< min_cost) وإضافة عداد تراكمي للأهداف.
# التعديلات الإضافية (2026-06-09):
#   - رفع الحد الأقصى للتعرض الكلي (MAX_EXPOSED_PERCENT) من 65% إلى 70% ثم إلى 80%.
#   - إضافة تفصيل حجم الصفقة حسب حالة السوق: جيد (25% -> 20%)، عادي (20%)، سيء (15%).
#   - إصلاح دالة get_total_equity للوضع الورقي لحساب إجمالي رأس المال (حر + قيمة المراكز).
# ========================================================================
...
MAX_POSITION_PERCENT = float(os.environ.get('MAX_POSITION_PERCENT', 0.15))  # سيتم استخدامه للسوق العادي (سنغير اسمه قليلاً)
MAX_POSITION_PERCENT_GOOD = float(os.environ.get('MAX_POSITION_PERCENT_GOOD', 0.20))   # 20% (تم التعديل من 0.25)
MAX_POSITION_PERCENT_NORMAL = float(os.environ.get('MAX_POSITION_PERCENT_NORMAL', 0.20))  # 20%
MAX_POSITION_PERCENT_BAD = float(os.environ.get('MAX_POSITION_PERCENT_BAD', 0.15))       # 15%
...
MAX_EXPOSED_PERCENT = float(os.environ.get('MAX_EXPOSED_PERCENT', 0.80))   # تم الرفع من 0.70 إلى 0.80
...
class Position:
    def __init__(self, symbol, side, size, entry, atr, sl, tp, sym_type, pred, conf, regime, tp_levels=None):
        ...
        # ===== التعديل الجديد: عداد الأهداف المحققة =====
        self.tp_hit_count = 0
        # ================================================
...
def get_total_equity():
    ...
    for attempt in range(3):
        try:
            if PAPER_TRADING:
                # الحصول على الرصيد الحر وقائمة المراكز تحت قفل قصير
                with _global_state_lock:
                    free = bot_stats.last_balance
                    pos_list = list(open_positions.items())
                # حساب قيمة المراكز خارج القفل (لتجنب تأخير الشبكة)
                pos_val = 0.0
                ...
                # الوضع الحقيقي (بدون تغيير جوهري)
    ...
    with _global_state_lock:
        ...
        with _market_condition_lock:
            if _last_market_condition == 'good':
                ...
                pos_percent = MAX_POSITION_PERCENT_BAD
            else:  # normal
                pos_percent = MAX_POSITION_PERCENT_NORMAL
...
def close_partial(symbol, percent, price, reason, pnl_usdt=None):
    ...
    with _global_state_lock:
        ...
        pos._closing = True
    try:
        # ===== التعديل الجديد: منع البيع الجزئي الصغير =====
        _, _, min_cost = get_amount_limits(symbol)
        if min_cost is None:
            min_cost = 5.0  # قيمة افتراضية آمنة
            ...
            for sym, pos in snapshot:
                ...
                for idx, (target, pct) in enumerate(tp_levels_copy):
                    if (pos.side == 'buy' and cur_price >= target * (1 - tp_tolerance)) or (pos.side == 'sell' and cur_price <= target * (1 + tp_tolerance)):
                        ...
                        # ===== التعديل الجديد: استخدام عداد الأهداف التراكمي =====
                        pos.tp_hit_count += 1
                        target_number = pos.tp_hit_count
                        # ======================================================
                        if idx >= 5:  # الأهداف الكاملة (السادس والسابع)
...
def analyze_and_trade():
    ...
    try:
        ...
        # ===== التعديل الجديد: تحديد نسبة حجم الصفقة حسب حالة السوق (3 فئات) =====
        if market_cond == 'good':
            position_percent = MAX_POSITION_PERCENT_GOOD
        elif market_cond == 'bad':
            position_percent = MAX_POSITION_PERCENT_BAD
        else:  # normal
            position_percent = MAX_POSITION_PERCENT_NORMAL
        # ========================================================================
        ...
        max_allowed_exposure = total_eq * 0.80 if market_cond == 'good' else total_eq * MAX_EXPOSED_PERCENT
        ...
        if pending:
            ...
            for opp in pending:
                ...
                max_exp2 = total_eq2 * 0.80 if market_cond == 'good' else total_eq2 * MAX_EXPOSED_PERCENT
                # ===== نفس التعديل هنا أيضاً لتحديد حجم الصفقة حسب حالة السوق =====
                if market_cond == 'good':
                    pos_usdt = total_eq2 * MAX_POSITION_PERCENT_GOOD
                elif market_cond == 'bad':
                    pos_usdt = total_eq2 * MAX_POSITION_PERCENT_BAD
                else:
                    pos_usdt = total_eq2 * MAX_POSITION_PERCENT_NORMAL
                # =================================================================
    ...
    try:
        ...
        for sym, p in open_positions.items():
            ...
            pos_dict[sym] = {
                ...
                # ===== التعديل الجديد: حفظ عداد الأهداف =====
...
def load_state():
    ...
    try:
        for sym, pdata in data.get('open_positions', {}).items():
            ...
            # ===== التعديل الجديد: استعادة عداد الأهداف =====
