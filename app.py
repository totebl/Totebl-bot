...
# --------------------------- دالة جلب أداء X يوم (عبر ccxt) ---------------------------
def get_30d_performance(symbol, days=None):
    """
    تعيد نسبة التغير الإجمالي لعدد محدد من الأيام باستخدام شموع يومية من ccxt.
    إذا لم يُحدد days، يستخدم القيمة العامة FILTER_30D_DAYS.
    """
    if days is None:
    ...
    try:
        df = fetch_ohlcv_retry_raw(symbol, '1d', limit=days + 1, max_retries=2)
        if df.empty or len(df) < 2:
            logger.warning(f"⚠️ بيانات غير كافية لأداء {days} يوم لـ {symbol}")
            return None
        
        price_old = df['close'].iloc[0]   # السعر منذ days يوم
        price_now = df['close'].iloc[-1]   # السعر الحالي (آخر شمعة)
...
def check_30d_profit(symbol):
    ...
    if change is None:
        # في حال تعذر الجلب، نسمح بالدخول (يمكن تغييرها إلى False للرفض)
        return True
...
# --------------------------- لجنة الشراء (محسّنة مع العضو الرابع والخامس) ---------------------------
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        # ===== إعلان المتغير العام في بداية الدالة =====
        ...
        # ============================================================
        # 7. الأوزان الجديدة (Rule 20%, Hybrid 20%, CFHM 35%, Timing 10%, VWAP_OBV 15%)
        # ============================================================
        if sym_type == 'meme':
            weights = {'rule': 0.20, 'emr': 0.20, 'cfhm': 0.35, 'timing': 0.10, 'vwap_obv': 0.15}
        else:
            weights = {'rule': 0.20, 'emr': 0.20, 'cfhm': 0.35, 'timing': 0.10, 'vwap_obv': 0.15}
...
def telegram_polling():
    ...
    # ===== إضافة المتغيرات الجديدة للتبريد الذكي =====
    global COOLDOWN_WIN_HOURS, COOLDOWN_LOSS_MINUTES, _symbol_cooldown_until
    # ===== إضافة المتغير الجديد لعتبة مجموع النموذجين (نقل إلى الأعلى) =====
    global RULE_EMR_SUM_THRESHOLD
