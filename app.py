...
# تم تطبيق التحديثات الستة:
# 1- تحسين مزامنة الأوامر المعلقة (periodic_sync_pending)
# 2- إصلاح monitor_positions (فاصل 3 ثوانٍ، ThreadPoolExecutor، تأخير 0.5 ث)
# 3- رفع TTL كاش الشموع إلى 200 ثانية
# 4- الاعتماد على آخر سعر من WebSocket لمدة 60 ثانية
# 5- نظام التشغيل المتقطع (الدورية) مع أوامر تلغرام وحفظ الحالة
# 6- خيط إعادة تعيين الاتصال المستقل (قطع وإعادة الاتصال كل فترة زمنية محددة، دون إيقاف التحليل)
# ========================================================================
...
STRENGTH_THRESHOLD = float(os.environ.get('STRENGTH_THRESHOLD', 0.55))
SCALP_MIN_PROFIT = float(os.environ.get('SCALP_MIN_PROFIT', 0.55))
...
COOLDOWN_WIN_HOURS = float(os.environ.get('COOLDOWN_WIN_HOURS', 8.0))
COOLDOWN_LOSS_MINUTES = float(os.environ.get('COOLDOWN_LOSS_MINUTES', 120.0))
_symbol_cooldown_until = {}
_cooldown_lock = threading.Lock()

# --------------------------- عتبات الخسارة الوقائية (قابلة للتعديل عبر تلغرام) ---------------------------
STOP_LOSS_PARTIAL_1_PERCENT = 0.0122
STOP_LOSS_PARTIAL_2_PERCENT = 0.0138
STOP_LOSS_FULL_PERCENT = 0.0159
...
# ========== التحكم في تحديث الوقف المسبق الحقيقي ==========
PREEMPTIVE_SL_UPDATE_ENABLED = False
CUSTOM_PREEMPTIVE_SL_TRIGGER = 0.025
CUSTOM_PREEMPTIVE_SL_LIMIT = 0.032

# ========== نظام تتبع الأرباح الجديد (يفعل فوراً) ==========
TRAILING_DISTANCE_PERCENT = float(os.environ.get('TRAILING_DISTANCE_PERCENT', 3.22)) / 100.0
...
POSITION_SIZE_PERCENT = 0.20   # 20% افتراضياً
...
CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP = 75
...
# تحديث 1: رفع فاصل المراقبة من 1.0 إلى 3.0 ثانية
POSITION_MONITOR_INTERVAL = float(os.environ.get('POSITION_MONITOR_INTERVAL', 3.0))

# ===== تحديث 5: نظام التشغيل المتقطع (الدورية) =====
ENABLE_CYCLE_MODE = os.environ.get('ENABLE_CYCLE_MODE', 'false').lower() == 'true'
CYCLE_ACTIVE_MINUTES = float(os.environ.get('CYCLE_ACTIVE_MINUTES', 60.0))
CYCLE_PAUSE_MINUTES = float(os.environ.get('CYCLE_PAUSE_MINUTES', 15.0))
_cycle_state = 'active'  # 'active' أو 'paused'
_cycle_timer_start = time.time()
_cycle_lock = threading.Lock()

# ===== تحديث 6: خيط إعادة تعيين الاتصال المستقل =====
ENABLE_CONNECTION_RESET = os.environ.get('ENABLE_CONNECTION_RESET', 'false').lower() == 'true'
CONNECTION_RESET_INTERVAL_HOURS = float(os.environ.get('CONNECTION_RESET_INTERVAL_HOURS', 1.0))
_connection_reset_lock = threading.Lock()
_last_connection_reset_time = 0
...
# ===== تحديث 6: دالة إعادة تعيين الاتصال (مستقلة) =====
def reset_connection():
    """قطع وإعادة الاتصال بالمنصة بالكامل (REST + WebSocket) - مستقل عن PAUSE_ANALYSIS"""
    global primary_exchange, ws_manager, _last_connection_reset_time
    if _should_simulate():
        logger.debug("⏭️ تخطي إعادة تعيين الاتصال في وضع المحاكاة")
        return

    # محاولة الحصول على قفل إعادة التعيين (مهلة 5 ثوانٍ)
    if not _connection_reset_lock.acquire(timeout=5):
        logger.warning("⚠️ لم نتمكن من الحصول على قفل إعادة التعيين، سيتم التخطي")
        return

    try:
        logger.info("🔄 [مستقل] قطع وإعادة الاتصال بالمنصة (REST + WebSocket)...")
        
        # إعادة إنشاء كائن exchange
        primary_exchange = ccxt.binance({
            'apiKey': BINANCE_API_KEY,
            'secret': BINANCE_SECRET_KEY,
            'enableRateLimit': True,
        ...
        if BINANCE_SANDBOX:
            primary_exchange.set_sandbox_mode(True)
        logger.info("✅ [مستقل] تم إعادة تهيئة كائن exchange")
        
        # إعادة تشغيل WebSocket Manager
        ws_manager.stop()
        time.sleep(2)
        ws_manager.start()
        logger.info("✅ [مستقل] تم إعادة تشغيل WebSocket Manager")
        
        # إعادة تحميل الأسواق (اختياري)
        try:
            primary_exchange.load_markets()
            logger.info("✅ [مستقل] تم إعادة تحميل الأسواق")
        except Exception as e:
            logger.warning(f"فشل إعادة تحميل الأسواق: {e}")
            
        _last_connection_reset_time = time.time()
        logger.info("✅ [مستقل] تم إعادة الاتصال بالمنصة بنجاح")
    except Exception as e:
        logger.error(f"خطأ في إعادة التعيين المستقل: {e}")
    finally:
        if _connection_reset_lock.locked():
            _connection_reset_lock.release()
...
class SimpleWebSocketManager:
    ...
    # تحديث 4: الاعتماد على آخر سعر من WebSocket لمدة 60 ثانية
    def get_ticker(self, symbol, max_age=60):
        """تعيد السعر من WebSocket، وإذا كانت البيانات قديمة تعيد آخر سعر معروف إذا كان عمره أقل من max_age ثانية"""
        with self.lock:
            ...
            if data:
                ...
                if age < max_age:
                    ...
                    # البيانات قديمة (أكثر من دقيقة)، نعيد None لتشغيل REST
                    logger.debug(f"⚠️ بيانات WebSocket لـ {symbol} منتهية الصلاحية ({age:.1f}ث > {max_age}ث)")
                    return None
            return None
...
# تحديث 2: إصلاح monitor_positions (النسخة المدمجة)
def monitor_positions():
    global CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP, TRAILING_DISTANCE_PERCENT
    executor = ThreadPoolExecutor(max_workers=2)  # حد أقصى 2 طلبات متزامنة
    while True:
        try:
            with _global_state_lock:
            ...
            # التحقق من صحة WebSocket وإعادة التشغيل إذا لزم الأمر
            if not ws_manager.is_alive():
                logger.warning("⚠️ WebSocket غير نشط، محاولة إعادة التشغيل...")
                ws_manager.restart()
                time.sleep(2)  # انتظار قليلاً لإعادة الاتصال

            # قائمة المهام للرموز التي فشل فيها WebSocket
            rest_tasks = []
            ...
            for sym, pos in snapshot:
                ...
                if ws_data and ws_data.get('price', 0) > 0:
                    cur_price = ws_data['price']
                    # ===== المعالجة المباشرة (WebSocket يعمل) =====
                    ...
                    # منطق البيع بعد فترة زمنية دون تحقيق الهدف
                    if profit > 0.011:
                    ...
                    # WebSocket فشل، نضيف المهمة للمعالجة عبر REST
                    rest_tasks.append((sym, pos))

            # ===== معالجة طلبات REST مع حد أقصى 2 بالتوازي =====
            if rest_tasks:
                futures = []
                for sym, pos in rest_tasks:
                    def fetch_and_process(s, p):
                        # تأخير إلزامي 0.5 ثانية بين الطلبات
                        time.sleep(0.5)
                        ticker = fetch_ticker_with_retry(s, max_retries=1)
                        if ticker and ticker.get('last', 0) > 0:
                            cur_price = ticker['last']
                            # ===== تكرار منطق المعالجة نفسه =====
                            profit = p.update(cur_price)
                            
                            # منطق الوقف المتحرك
                            if p.trailing_activated and p.trailing_stop is not None:
                                if p.last_trailing_stop_sent is None:
                                    should_update = True
                                else:
                                    increase_ratio = (p.trailing_stop - p.last_trailing_stop_sent) / p.last_trailing_stop_sent
                                    should_update = increase_ratio >= 0.01
                                if should_update:
                                    update_preemptive_stop_loss(s, p, p.trailing_stop)
                                if (p.side == 'buy' and cur_price <= p.trailing_stop) or \
                                   (p.side == 'sell' and cur_price >= p.trailing_stop):
                                    close_partial(s, 1.0, cur_price, 
                                                  f"🏃 وقف متحرك فوري (نسبة {TRAILING_DISTANCE_PERCENT:.2%})")
                                    return
                            
                            # منطق الخسارة الوقائية الجزئية
                            if profit < 0:
                                if profit <= -STOP_LOSS_PARTIAL_1_PERCENT and not p.sold_at_15:
                                    close_partial(s, 0.25, cur_price, 
                                                  f"⚠️ بيع وقائي: خسارة {abs(profit):.2%} (بيع 25%) [عتبة {STOP_LOSS_PARTIAL_1_PERCENT:.2%}]")
                                    p.sold_at_15 = True
                                    save_state()
                                elif profit <= -STOP_LOSS_PARTIAL_2_PERCENT and not p.sold_at_20:
                                    close_partial(s, 0.33, cur_price, 
                                                  f"⚠️ بيع وقائي: خسارة {abs(profit):.2%} (بيع 33%) [عتبة {STOP_LOSS_PARTIAL_2_PERCENT:.2%}]")
                                    p.sold_at_20 = True
                                    save_state()
                                elif profit <= -STOP_LOSS_FULL_PERCENT:
                            ...
                            # منطق الانهيار المفاجئ
                            if p.crash_monitor_start is None:
                                p.crash_monitor_start = datetime.now()
                                p.lowest_drop = 0.0
                            else:
                                drop = (p.entry_price - cur_price) / p.entry_price if p.side == 'buy' else (cur_price - p.entry_price) / p.entry_price
                                if drop > p.lowest_drop:
                                    p.lowest_drop = drop
                                elapsed_seconds = (datetime.now() - p.crash_monitor_start).total_seconds()
                                if elapsed_seconds <= 120 and drop >= 0.04:
                                    close_partial(s, 1.0, cur_price, "💥 انهيار مفاجئ >4% خلال دقيقتين (إغلاق كامل)")
                                    return
                            
                            # منطق البيع بعد فترة زمنية دون تحقيق الهدف
                            if profit > 0.011:
                                now_time = datetime.now()
                                if p.last_target_hit_index == -1:
                                    if (now_time - p.open_time).total_seconds() > (CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP * 60):
                                        first_target = p.take_profit_levels[0][0] if p.take_profit_levels else None
                                        if first_target and cur_price < first_target:
                                            close_partial(s, 1.0, cur_price, 
                                                          f"🕒 انقضت {CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP} دقيقة دون تحقيق الهدف الأول – جني أرباح")
                                            return
                                else:
                                    next_target_idx = p.last_target_hit_index + 1
                                    if next_target_idx < len(p.take_profit_levels):
                                        next_target = p.take_profit_levels[next_target_idx][0]
                                        if (now_time - p.last_target_hit_time).total_seconds() > (CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP * 60) and cur_price < next_target:
                                            close_partial(s, 1.0, cur_price, 
                                                          f"🕒 انقضت {CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP} دقيقة بعد تحقيق الهدف {p.last_target_hit_index+1} دون الوصول للهدف التالي – جني أرباح")
                                            return
                            
                            # منطق جني الأرباح المستهدف
                            tp_tolerance = 0.001
                            tp_levels_copy = list(p.take_profit_levels)
                            executed = False
                            for idx, (target, pct) in enumerate(tp_levels_copy):
                                if (p.side == 'buy' and cur_price >= target * (1 - tp_tolerance)) or \
                                   (p.side == 'sell' and cur_price <= target * (1 + tp_tolerance)):
                                    p.last_target_hit_time = datetime.now()
                                    p.last_target_hit_index = idx
                                    p.tp_hit_count += 1
                                    target_number = p.tp_hit_count
                                    if idx >= 5:
                                        close_partial(s, 1.0, cur_price, f"🎯 جني أرباح كامل (الهدف {target_number})")
                                    else:
                                        close_partial(s, pct, cur_price, f"🎯 جني أرباح جزئي (الهدف {target_number})")
                                    if [target, pct] in p.take_profit_levels:
                                        p.take_profit_levels.remove([target, pct])
                                    executed = True
                                    break
                            if executed:
                                return
                            ...
                            # منطق وقف الخسارة الثابت
                            if p.stop_loss:
                                sl = p.stop_loss * (0.997 if p.side=='buy' else 1.003)
                                if (p.side=='buy' and cur_price <= sl) or (p.side=='sell' and cur_price >= sl):
                                    close_partial(s, 1.0, cur_price, "🛑 وقف خسارة")
                                    return
                            
                            # أقصى خسارة مطلقة
                            if profit <= -0.06:
                                close_partial(s, 1.0, cur_price, "💥 أقصى خسارة 6%")
                                return
                        else:
                            logger.warning(f"⚠️ فشل جلب السعر لـ {s} (REST)")
                    future = executor.submit(fetch_and_process, sym, pos)
                    futures.append(future)
                # انتظار اكتمال جميع المهام (مع مهلة)
                for future in futures:
                    try:
                        future.result(timeout=5)
                    except Exception as e:
                        logger.error(f"خطأ في معالجة REST: {e}")
...
def analyze_and_trade():
    ...
    global _last_analyzed_candidates
    ...
    try:
        ...
        for sym in active:
            ...
            if sym in open_positions:
                ...
                ttl_used = 200  # تحديث 3: تم الرفع من 120 إلى 200 ثانية
...
# تحديث 1: تحسين sync_pending_orders و periodic_sync_pending
def sync_pending_orders():
    global _exchange_pending_symbols
    if TEST_MODE or PAPER_TRADING or not ENABLE_TRADING: return
    exchange = get_active_exchange()
    try:
        rest_rate_limiter.wait_if_needed(weight=10)  # الوزن الصحيح لـ /openOrders بدون رمز
...
# ===== تحديث 5: نظام التشغيل المتقطع (الدورية) =====
def background_cycle_manager():
    """تدير نظام التشغيل المتقطع (تبديل بين العمل والتجميد وإعادة تشغيل WebSocket)"""
    global PAUSE_ANALYSIS, _cycle_state, _cycle_timer_start
    global ENABLE_CYCLE_MODE, CYCLE_ACTIVE_MINUTES, CYCLE_PAUSE_MINUTES
    
    time.sleep(60)  # انتظار بدء التشغيل
    while True:
        try:
            if not ENABLE_CYCLE_MODE:
                time.sleep(30)
                continue

            with _cycle_lock:
                elapsed = (time.time() - _cycle_timer_start) / 60.0  # بالدقائق
                
                if _cycle_state == 'active' and elapsed >= CYCLE_ACTIVE_MINUTES:
                    # ------> التبديل إلى التجميد <------
                    logger.info(f"⏸️ [الدورية] انتهت مدة العمل ({CYCLE_ACTIVE_MINUTES} دقيقة). التبديل إلى التجميد لمدة {CYCLE_PAUSE_MINUTES} دقيقة.")
                    
                    # إيقاف التحليل
                    PAUSE_ANALYSIS = True
                    
                    # إعادة تشغيل WebSocket لكسر الاستمرارية
                    logger.info("🔄 [الدورية] قطع وإعادة تشغيل WebSocket (لتجنب الاكتشاف)...")
                    ws_manager.restart()
                    time.sleep(5)  # انتظار 5 ثوانٍ لإعادة الاتصال
                    
                    _cycle_state = 'paused'
                    _cycle_timer_start = time.time()
                    send_telegram(f"⏸️ [الدورية] دخل البوت في وضع التجميد لمدة {CYCLE_PAUSE_MINUTES} دقيقة. (التحليل متوقف، المراكز تحت المراقبة).")

                elif _cycle_state == 'paused' and elapsed >= CYCLE_PAUSE_MINUTES:
                    # ------> التبديل إلى العمل <------
                    logger.info(f"▶️ [الدورية] انتهت مدة التجميد ({CYCLE_PAUSE_MINUTES} دقيقة). العودة إلى العمل لمدة {CYCLE_ACTIVE_MINUTES} دقيقة.")
                    ...
                    _cycle_state = 'active'
                    _cycle_timer_start = time.time()
                    send_telegram(f"▶️ [الدورية] عاد البوت إلى وضع العمل لمدة {CYCLE_ACTIVE_MINUTES} دقيقة. (التحليل مفعل).")
                    
            time.sleep(30)  # التحقق كل 30 ثانية
            
        except Exception as e:
            logger.error(f"خطأ في background_cycle_manager: {e}")
            time.sleep(60)

# ===== تحديث 6: خيط إعادة تعيين الاتصال المستقل =====
def background_connection_reset():
    """خيط مستقل يعيد تعيين الاتصال بالمنصة كل فترة زمنية محددة دون التأثير على PAUSE_ANALYSIS"""
    global ENABLE_CONNECTION_RESET, CONNECTION_RESET_INTERVAL_HOURS, _last_connection_reset_time
    
    time.sleep(120)  # انتظار بدء التشغيل
    while True:
        try:
            if not ENABLE_CONNECTION_RESET:
                time.sleep(30)
                continue

            elapsed_hours = (time.time() - _last_connection_reset_time) / 3600.0
            
            if elapsed_hours >= CONNECTION_RESET_INTERVAL_HOURS:
                logger.info(f"🔄 [مستقل] مضى {elapsed_hours:.2f} ساعة ({CONNECTION_RESET_INTERVAL_HOURS} ساعة هي الفترة). جاري إعادة تعيين الاتصال...")
                reset_connection()
                # الانتظار 5 ثوانٍ بعد إعادة التعيين (مضمن في reset_connection)
                time.sleep(2)
            else:
                time.sleep(60)  # التحقق كل دقيقة
            
        except Exception as e:
            logger.error(f"خطأ في background_connection_reset: {e}")
            time.sleep(60)
...
def telegram_polling():
    ...
    global ENABLE_CYCLE_MODE, _cycle_state, _cycle_timer_start, CYCLE_ACTIVE_MINUTES, CYCLE_PAUSE_MINUTES
    global ENABLE_CONNECTION_RESET, CONNECTION_RESET_INTERVAL_HOURS
                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ========== الأوامر الأساسية (موجودة مسبقاً) ==========
                            ...
                            # ========== حالة الفلاتر (محدّثة) ==========
                                ...
                                cycle_status = ""
                                if ENABLE_CYCLE_MODE:
                                    with _cycle_lock:
                                        elapsed = (time.time() - _cycle_timer_start) / 60.0
                                        remaining = (CYCLE_ACTIVE_MINUTES - elapsed) if _cycle_state == 'active' else (CYCLE_PAUSE_MINUTES - elapsed)
                                        cycle_status = (
                                            f"🔄 <b>الدورية (متقطع):</b> 🟢 مفعل\n"
                                            f"   - الحالة الحالية: {'🟢 عمل' if _cycle_state == 'active' else '⏸️ تجميد'}\n"
                                            f"   - مدة العمل: {CYCLE_ACTIVE_MINUTES:.0f} دقيقة\n"
                                            f"   - مدة التجميد: {CYCLE_PAUSE_MINUTES:.0f} دقيقة\n"
                                            f"   - الوقت المتبقي: {max(0, remaining):.1f} دقيقة\n"
                                        )
                                else:
                                    cycle_status = f"🔄 <b>الدورية (متقطع):</b> 🔴 معطل\n"
                                
                                reset_status = ""
                                if ENABLE_CONNECTION_RESET:
                                    elapsed_hours = (time.time() - _last_connection_reset_time) / 3600.0
                                    remaining_hours = max(0, CONNECTION_RESET_INTERVAL_HOURS - elapsed_hours)
                                    reset_status = (
                                        f"🔄 <b>إعادة تعيين الاتصال (مستقل):</b> 🟢 مفعل\n"
                                        f"   - الفترة: {CONNECTION_RESET_INTERVAL_HOURS:.1f} ساعة\n"
                                        f"   - المتبقي: {remaining_hours:.2f} ساعة\n"
                                    )
                                else:
                                    reset_status = f"🔄 <b>إعادة تعيين الاتصال (مستقل):</b> 🔴 معطل\n"
                            ...
                            # ========== الأوامر الأساسية الأخرى (موجودة مسبقاً) ==========
                            # تم حذفها للاختصار - موجودة في الكود الأصلي

                            # ========== أوامر إعادة تعيين الاتصال (تحديث 6) ==========
                            elif text == 'تفعيل إعادة التعيين':
                                global ENABLE_CONNECTION_RESET, _last_connection_reset_time
                                ENABLE_CONNECTION_RESET = True
                                _last_connection_reset_time = time.time()
                                send_telegram(f"🔄 تم تفعيل إعادة تعيين الاتصال المستقل.\n⏱️ الفترة: {CONNECTION_RESET_INTERVAL_HOURS} ساعة.")
                                save_state()

                            elif text == 'إيقاف إعادة التعيين':
                                global ENABLE_CONNECTION_RESET
                                ENABLE_CONNECTION_RESET = False
                                send_telegram("⏹️ تم إيقاف إعادة تعيين الاتصال المستقل.")
                                save_state()

                            elif text == 'إعادة تعيين الآن':
                                send_telegram("🔄 جاري إعادة تعيين الاتصال بالمنصة فوراً...")
                                reset_connection()
                                send_telegram("✅ تم إعادة تعيين الاتصال بالمنصة بنجاح.")

                            elif text.startswith('ضبط فترة إعادة التعيين'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*ساعة', text)
                                if match:
                                    new_val = float(match.group(1))
                                    if 0.5 <= new_val <= 24:
                                        CONNECTION_RESET_INTERVAL_HOURS = new_val
                                        _last_connection_reset_time = time.time()  # إعادة ضبط العداد
                                        send_telegram(f"✅ تم ضبط فترة إعادة التعيين إلى {new_val:.1f} ساعة.")
                                        save_state()
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.5 و 24 ساعة.")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `ضبط فترة إعادة التعيين 2 ساعة`")

                            elif text == 'حالة إعادة التعيين':
                                if ENABLE_CONNECTION_RESET:
                                    elapsed = (time.time() - _last_connection_reset_time) / 3600.0
                                    remaining = max(0, CONNECTION_RESET_INTERVAL_HOURS - elapsed)
                                    send_telegram(f"🔄 <b>حالة إعادة تعيين الاتصال</b>\n"
                                                  f"📌 الحالة: 🟢 مفعل\n"
                                                  f"⏱️ الفترة: {CONNECTION_RESET_INTERVAL_HOURS:.1f} ساعة\n"
                                                  f"⏳ المتبقي: {remaining:.2f} ساعة\n"
                                                  f"📅 آخر إعادة تعيين: {datetime.fromtimestamp(_last_connection_reset_time).strftime('%H:%M:%S')}")
                                else:
                                    send_telegram("🔄 <b>حالة إعادة تعيين الاتصال</b>\n📌 الحالة: 🔴 معطل")
                            
                            # ========== أوامر الدورية (تحديث 5) ==========
                            elif text == 'تفعيل الدورية':
                                global ENABLE_CYCLE_MODE, _cycle_state, _cycle_timer_start
                                ENABLE_CYCLE_MODE = True
                                _cycle_state = 'active'
                                _cycle_timer_start = time.time()
                                PAUSE_ANALYSIS = False
                                send_telegram(f"🔄 تم تفعيل التشغيل المتقطع (الدورية).\n⏱️ مدة العمل: {CYCLE_ACTIVE_MINUTES} دقيقة | مدة التجميد: {CYCLE_PAUSE_MINUTES} دقيقة.")
                                save_state()

                            elif text == 'إيقاف الدورية':
                                global ENABLE_CYCLE_MODE
                                ENABLE_CYCLE_MODE = False
                                PAUSE_ANALYSIS = False
                                send_telegram("⏹️ تم إيقاف التشغيل المتقطع (الدورية). البوت يعمل بشكل مستمر.")
                                save_state()

                            elif text.startswith('ضبط مدة العمل'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*دقيقة', text)
                                if match:
                                    new_val = float(match.group(1))
                                    if 5 <= new_val <= 1440:
                                        CYCLE_ACTIVE_MINUTES = new_val
                                        send_telegram(f"✅ تم ضبط مدة العمل إلى {new_val:.0f} دقيقة.")
                                        save_state()
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 5 و 1440 دقيقة.")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `ضبط مدة العمل 60 دقيقة`")

                            elif text.startswith('ضبط مدة التجميد'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*دقيقة', text)
                                if match:
                                    new_val = float(match.group(1))
                                    if 1 <= new_val <= 120:
                                        CYCLE_PAUSE_MINUTES = new_val
                                        send_telegram(f"✅ تم ضبط مدة التجميد إلى {new_val:.0f} دقيقة.")
                                        save_state()
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 1 و 120 دقيقة.")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `ضبط مدة التجميد 15 دقيقة`")

                            elif text == 'حالة الدورية':
                                with _cycle_lock:
                                    elapsed = (time.time() - _cycle_timer_start) / 60.0
                                    remaining = (CYCLE_ACTIVE_MINUTES - elapsed) if _cycle_state == 'active' else (CYCLE_PAUSE_MINUTES - elapsed)
                                    status = (
                                        f"🔄 <b>حالة الدورية (التشغيل المتقطع)</b>\n"
                                        f"📌 الحالة: {'🟢 مفعل' if ENABLE_CYCLE_MODE else '🔴 معطل'}\n"
                                        f"⏳ الحالي: {'🟢 فترة عمل' if _cycle_state == 'active' else '⏸️ فترة تجميد'}\n"
                                        f"⏱️ مدة العمل: {CYCLE_ACTIVE_MINUTES:.0f} دقيقة\n"
                                        f"⏱️ مدة التجميد: {CYCLE_PAUSE_MINUTES:.0f} دقيقة\n"
                                        f"⏳ متبقي: {max(0, remaining):.1f} دقيقة"
                                    )
                                    send_telegram(status)
            time.sleep(1)
        except Exception as e:
...
def thread_watchdog():
    essential_threads = {
        ...
        'background_cycle_manager': background_cycle_manager,  # تحديث 5
        'background_connection_reset': background_connection_reset  # تحديث 6
    }
...
def save_state():
    ...
    try:
        ...
        data = {
            ...
            # تحديث 5: إضافة إعدادات الدورية
            'cycle_settings': {
                'enabled': ENABLE_CYCLE_MODE,
                'active_minutes': CYCLE_ACTIVE_MINUTES,
                'pause_minutes': CYCLE_PAUSE_MINUTES,
                'state': _cycle_state,
                'timer_start': _cycle_timer_start
            },
            # تحديث 6: إضافة إعدادات إعادة التعيين
            'connection_reset_settings': {
                'enabled': ENABLE_CONNECTION_RESET,
                'interval_hours': CONNECTION_RESET_INTERVAL_HOURS,
                'last_reset_time': _last_connection_reset_time
            }
        }
...
def load_state():
    ...
    global UPPER_THRESHOLD_GLOBAL
    global POSITION_SIZE_PERCENT
    global ENABLE_CYCLE_MODE, CYCLE_ACTIVE_MINUTES, CYCLE_PAUSE_MINUTES, _cycle_state, _cycle_timer_start
    global ENABLE_CONNECTION_RESET, CONNECTION_RESET_INTERVAL_HOURS, _last_connection_reset_time
    ...
    try:
        ...
        POSITION_SIZE_PERCENT = cust.get('position_size_percent', 0.20)
        ...
        # تحديث 5: استعادة إعدادات الدورية
        cycle = data.get('cycle_settings', {})
        ENABLE_CYCLE_MODE = cycle.get('enabled', False)
        CYCLE_ACTIVE_MINUTES = cycle.get('active_minutes', 60.0)
        CYCLE_PAUSE_MINUTES = cycle.get('pause_minutes', 15.0)
        _cycle_state = cycle.get('state', 'active')
        _cycle_timer_start = cycle.get('timer_start', time.time())
        # تحديث 6: استعادة إعدادات إعادة التعيين
        reset = data.get('connection_reset_settings', {})
        ENABLE_CONNECTION_RESET = reset.get('enabled', False)
        CONNECTION_RESET_INTERVAL_HOURS = reset.get('interval_hours', 1.0)
        _last_connection_reset_time = reset.get('last_reset_time', time.time())
