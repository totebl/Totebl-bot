...
GEMINI_MODEL_FLASH = 'gemini-3.5-flash'                 # ✅ النموذج الأساسي (يعمل)
GEMINI_MODEL_FLASH_LITE = 'gemini-3.1-flash-lite'      # ✅ النموذج الثانوي (يعمل)
        ...
        try:
            if PAPER_TRADING:
                ...
                for sym, p in pos_list:
                    # محاولة الحصول على السعر من WebSocket أولاً
                    ws_data = ws_manager.get_ticker(sym)
                    if ws_data and ws_data.get('price', 0) > 0:
                        price = ws_data['price']
                    else:
                        # محاولة جلب السعر من API مع إعادة محاولات
                        ticker = None
...
def _ask_ai_internal(symbol, df, current_price, sym_type, market_condition, ticker_data, threshold, desc):
    ...
    prompt = f"""أنت خبير تداول. قم بتحليل الرمز {symbol} (نوع: {sym_type}) في ظل ظروف السوق: {market_condition}.
...
السؤال: بناءً على هذه البيانات، هل تنصح بشراء هذا الرمز الآن؟ أجب فقط بـ "نعم" أو "لا" ثم سبب مختصر جداً."""
    ...
    # تم حذف السطر genai.configure(api_key=GEMINI_API_KEY) لأنه تم تكوينه مسبقاً
    ...
    while True:
        ...
        if recovery_attempts_remaining > 0:
            ...
            if recovery_attempts_remaining == 0:
                ...
                try:
                    # 1. إعادة تعيين RateLimiter بنفس القيم الأصلية (60)
...
def analyze_and_trade():
    ...
    try:
        ...
        for sym in active:
            ...
            # استخدم get_cached_features بدلاً من fetch_ohlcv_persistent للحصول على المؤشرات المتقدمة
        ...
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            ...
            pending.append({'symbol': sym, 'score': extra, 'price': cur_price, 'atr': atr_val,
                            ...
                            'pred': score, 'conf': conf, 'regime': regime, 'df_primary': df_primary})   # إضافة df_primary للاستشارة
        if pending:
            pending.sort(key=lambda x: x['score'], reverse=True)
            rem_bal = free_bal
            exp = curr_exp
            # حساب total_eq2 و pos_usdt و max_exp2 مرة واحدة قبل الحلقة (إصلاح تقلص حجم الصفقة)
            ...
            for opp in pending:
                ...
                # استشارة AI قبل الشراء (إذا كان مفعلاً)
                approved = True
                if AI_ENABLED and GEMINI_API_KEY and genai is not None:
                    ...
                    if not approved:
                        ...
                        continue
                # إعادة استخدام pos_usdt المحسوبة مسبقاً
