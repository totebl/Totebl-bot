# ========================================================================
# بوت التداول المتطور v27.2 - إصلاح تعليق خيط جلب الرصيد
# - منع تعليق fetch_balance باستخدام مهلة زمنية قصوى (30 ثانية لكل محاولة)
# - تقليل عدد محاولات جلب الرصيد إلى 2 لمنع الانتظار الطويل
# - ضمان عدم احتجاز الأقفال في حالة تعليق الطلب
# ========================================================================
...
def get_real_balance_usdt():
    """جلب الرصيد الحقيقي مع مهلة قصوى لمنع التعليق"""
    ...
    # مزامنة الوقت (قد تعلق أيضاً، لكن نضع لها مهلة قصيرة)
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(exchange.load_time_difference)
            future.result(timeout=5)  # مهلة 5 ثوانٍ لمزامنة الوقت
            logger.info("🕰️ تم مزامنة الوقت مع Binance.")
    except FuturesTimeoutError:
        logger.warning("⚠️ مهلة مزامنة الوقت (5 ثوانٍ)، نستمر بدون مزامنة.")
    except Exception as e:
        logger.warning(f"⚠️ فشل مزامنة الوقت: {e}")

    # محاولات جلب الرصيد مع مهلة قصوى لكل محاولة
    max_attempts = 2  # تقليل المحاولات إلى 2 فقط
    for attempt in range(max_attempts):
        try:
            # استخدام ThreadPoolExecutor لمنع التعليق الأبدي
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(exchange.fetch_balance, params={'recvWindow': 60000})
                balance = future.result(timeout=25)  # مهلة 25 ثانية لكل محاولة
            ...
            logger.warning(f"⚠️ مهلة جلب الرصيد (محاولة {attempt+1}/{max_attempts}) - استغرقت أكثر من 25 ثانية")
        except Exception as e:
            error_str = str(e)
...
def _get_balance_no_pause(max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(exchange.fetch_balance, params={'recvWindow': 60000})
                balance = future.result(timeout=25)
            usdt = balance.get('USDT', {}).get('free', 0.0)
            if usdt > 0:
                with _state_lock:
                    bot_stats.last_balance = usdt
...
<head><meta charset="UTF-8"><title>بوت v27.2</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v27.2 – إصلاح تعليق خيط جلب الرصيد</h1>
<div id="stats"></div>
...
@csrf.exempt
def ping():
    return jsonify({"status":"pong","version":"27.2"})
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v27.2 – إصلاح تعليق خيط جلب الرصيد", "version":"27.2"})
...
@csrf.exempt
def health():
    return jsonify({"healthy":True,"paused":_is_paused(),"open":len(open_positions),"version":"27.2"})
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v27.2 - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v27.2 يعمل – الوضع: {mode_str}\n✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n✅ تقرير يومي يعرض صافي الربح/الخسارة\n✅ التحويل بين التداول الورقي والحقيقي: `تداول حقيقي` / `تداول ورقي`\n✅ تحسينات الذاكرة والاستقرار\n✅ مراقبة خيط تلغرام وإعادة تشغيله تلقائياً\n✅ إصلاح تعليق خيط جلب الرصيد (مهلة قصوى 30 ثانية لكل محاولة)\n✅ تثبيت تلقائي لخادم Waitress (خادم ويب إنتاجي)")
