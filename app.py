    ...
    for attempt in range(max_retries):
        try:
            ...
            if "418" in error_msg or "banned" in error_msg or "too many" in error_msg or "Way too much" in error_msg:
                logger.critical(f"🚨 IP لا يزال محظورًا. الانتظار 3 دقائق ثم إعادة المحاولة.")  # تم التعديل
                send_telegram(f"⚠️ Binance لا يزال يحظر IP. انتظر 3 دقائق... (محاولة {attempt+1}/{max_retries})")
                time.sleep(180)  # من 300 إلى 180
                continue
            if attempt < max_retries - 1:
                continue
...
def fetch_ticker_with_retry(symbol, max_retries=2):
    ...
    # لا نستخدم REST API كبديل، نعتمد فقط على WebSocket
    return None
...
def analyze_and_trade():
    ...
    # محاولة الحصول على القفل مع مهلة 610 ثانية (حوالي 10 دقائق و10 ثوانٍ)
    lock_acquired = False
    try:
        lock_acquired = _processing_lock.acquire(timeout=610)
    except Exception as e:
        logger.error(f"خطأ أثناء محاولة الحصول على القفل: {e}")

    if not lock_acquired:
        logger.warning("⚠️ لم نتمكن من الحصول على _processing_lock بعد 610 ثانية. محاولة تحرير قسري...")
        try:
            # تحرير القفل بالقوة (قد ينجح)
            while _processing_lock.locked():
                _processing_lock.release()
            logger.info("✅ تم تحرير القفل القسري، إعادة المحاولة للحصول عليه")
            # إعادة محاولة واحدة قصيرة
