# ========================================================================
# بوت التداول المتطور v26.6 - تحكم كامل عن طريق تلغرام + رصيد حقيقي
# - إصلاح جذري لمشكلة جلب الرصيد (مزامنة الوقت، recvWindow موسع، مهلة أطول)
# - تحسينات الذاكرة ومنع التسريب (deque, تنظيف error reports, مكنسة محسنة)
# - دعم أفضل لمنصة Render (PORT, use_reloader=False, تسجيل واضح)
# - المنفذ الافتراضي 8080 (يتوافق مع إعدادات Render)
# ========================================================================
...
# تمكين تسجيل Flask لظهور رسالة "Running on ..."
...
_last_balance_failure_start = None  # متغير جديد لتتبع بداية الفشل الأول
...
def cleanup_error_reports():
    """تنظيف سجل الأخطاء القديم - يمنع التسرب"""
    global _last_error_report_times
    now = time.time()
    # حذف الأخطاء الأقدم من ساعة
...
# --------------------------- دوال التوازن والرصيد (مُقدمة لتجنب NameError) ---------------------------
...
def get_real_balance_usdt():
    ...
    # 1. مزامنة الوقت قبل كل محاولة جلب للرصيد (الحل الجذري للفارق الزمني)
    try:
        exchange.load_time_difference()
        logger.info("🕰️ تم مزامنة الوقت مع Binance.")
    except Exception as e:
        logger.warning(f"⚠️ فشل مزامنة الوقت، ولكن سنحاول المتابعة: {e}")

    # 2. زيادة recvWindow بشكل كبير وعدد محاولات أكثر (5 محاولات)
    for attempt in range(5):
        try:
            # استخدام recvWindow كبير جدًا (60 ثانية) لاستيعاب أي فرق زمني
            balance = exchange.fetch_balance(params={'recvWindow': 60000})
            usdt = balance.get('USDT', {}).get('free', 0.0)
            if usdt is not None:
                with _state_lock:
                    bot_stats.last_balance = usdt
                    _last_successful_balance_time = now
                    # إعادة تعيين مؤشر بدء الفشل
            ...
            error_str = str(e)
            logger.warning(f"محاولة {attempt+1}/5 لجلب الرصيد فشلت: {error_str[:150]}")
            # إذا كان الخطأ متعلقاً بالتوقيت، نعيد المزامنة
            if "Timestamp" in error_str or "-1021" in error_str:
                logger.warning("⚠️ خطأ في التوقيت، سنعيد مزامنة الوقت قبل المحاولة التالية.")
                try:
                    exchange.load_time_difference()
                except:
                    pass
    ...
    # 3. إدارة المهلة بشكل أكثر مرونة (مهلة أطول للتشغيل الأول)
    if _last_successful_balance_time == 0:
        # المرة الأولى: نعطي مهلة 5 دقائق
        if _last_balance_failure_start is None:
            _last_balance_failure_start = now
        if now - _last_balance_failure_start < 300:  # 5 دقائق
            logger.warning("⚠️ جاري المحاولة لجلب الرصيد (المرة الأولى)...")
            return None
    else:
        # سبق أن نجحنا: نعطي مهلة دقيقتين كالمعتاد
...
def _get_balance_no_pause(max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            bal = exchange.fetch_balance(params={'recvWindow': 60000})
...
def _safe_fetch_balance_after_trade(attempts=5, delay=1.0):
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            return bot_stats.last_balance
...
def generate_error_report(error_type, component, details="", traceback_str=""):
    ...
    with _global_state_lock:
        ...
        total_equity = get_total_equity()  # أصبحت معرفة مسبقاً
...
def fetch_orderbook_with_cache(symbol, limit=LIQUIDITY_CHECK_DEPTH, ttl=2):
    ...
    try:
        ...
        with _orderbook_cache_lock:
            # ✅ تنظيف عند الإضافة بدلاً من انتظار خيط منفصل
...
<head><meta charset="UTF-8"><title>بوت v26.6</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v26.6 – تحكم كامل + إصلاح جذري لجلب الرصيد</h1>
<div id="stats"></div>
...
# ======================== مكنسة محسنة (cache_cleanup_thread) ========================
def cache_cleanup_thread():
    """مكنسة محسنة - تنظيف تدريجي بدون تأثير على الأداء"""
    last_full_cleanup = 0
    cleanup_counter = 0
    while True:
        time.sleep(300)  # كل 5 دقائق (بدلاً من ساعة)
        now = time.time()
        cleanup_counter += 1
        
        # تنظيف ohlcv cache - تدريجي (20% فقط)
        with _cache_lock:
            items = list(_ohlcv_cache.items())
            if len(items) > _ohlcv_cache_max:
                for key, _ in items[:20]:  # احذف 20 عنصر فقط
                    if key in _ohlcv_cache:
                        del _ohlcv_cache[key]
        
        # تنظيف features cache - تدريجي
        with _features_cache_lock:
        ...
        # تنظيف manipulation cache - تدريجي
        with _cache_lock:
        ...
        if now - last_full_cleanup > 1800:
            ...
            # تنظيف market cap cache (أقدم من يوم)
            with _market_cap_cache_lock:
                keys_to_delete = []
...
# ======================== مراقبة الذاكرة الاحتياطية ========================
def memory_watchdog():
    """مراقبة خفيفة للذاكرة - تنبيه وتنظيف عند الحاجة"""
    while True:
        ...
        try:
            ...
            if memory_mb > 1024:  # أكثر من 1GB
                ...
                if memory_after > 1500:  # أكثر من 1.5GB بعد التنظيف
...
@csrf.exempt
def ping():
    return jsonify({"status":"pong","version":"26.6"})
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v26.6 – تحكم كامل + إصلاح جذري لجلب الرصيد", "version":"26.6"})
...
@csrf.exempt
def health():
    return jsonify({"healthy":True,"paused":_is_paused(),"open":len(open_positions),"version":"26.6"})
...
def graceful_shutdown(signum=None, frame=None):
    """دالة يتم استدعاؤها عند استلام إشارة إيقاف (مثل SIGTERM أو SIGINT) لحفظ الحالة قبل الخروج"""
...
if __name__ == '__main__':
    # تعيين المنفذ المطلوب - Render يتوقع PORT أو 8080 كافتراضي
    port = int(os.environ.get('PORT', 8080))
