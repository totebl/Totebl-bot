...
GEMINI_MODEL_FLASH_LITE = 'gemini-1.5-flash-lite'
AI_ENABLED = os.environ.get('AI_ENABLED', 'false').lower() == 'true'
CURRENT_AI_MODEL = os.environ.get('CURRENT_AI_MODEL', 'flash')
AI_TIMEOUT = 65
    ...
    while True:
        try:
            ...
            report = (f"<b>📊 تقرير دوري (كل 4 ساعات)</b>\n🕒 الوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                      ...
                      f"🤖 <b>الذكاء الاصطناعي (Gemini):</b> {'✅ مفعل' if AI_ENABLED else '❌ معطل'} | النموذج الحالي: {'Flash' if CURRENT_AI_MODEL == 'flash' else 'Flash-Lite'}")
                    ...
                    for upd in data.get("result", []):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                                ...
                                if is_model_available(GEMINI_MODEL_FLASH):
                                    CURRENT_AI_MODEL = 'flash'
                                    save_state()
                                    send_telegram(f"✅ تم التبديل إلى نموذج **Gemini Flash** ({GEMINI_MODEL_FLASH}).")
                                else:
                                    send_telegram(f"❌ النموذج {GEMINI_MODEL_FLASH} غير متاح. لم يتم التبديل.")
                            elif text == 'تحول إلى النموذج الثاني':
                                if is_model_available(GEMINI_MODEL_FLASH_LITE):
                                    CURRENT_AI_MODEL = 'flash-lite'
                                    save_state()
                                    send_telegram(
                                        f"✅ تم التبديل إلى نموذج **Gemini Flash-Lite** ({GEMINI_MODEL_FLASH_LITE}).")
                                else:
                                    send_telegram(f"❌ النموذج {GEMINI_MODEL_FLASH_LITE} غير متاح. لم يتم التبديل.")
                            ...
                            # ------------------------------------------------------------
                                ...
                                CURRENT_AI_MODEL = 'flash'
                                send_telegram(
                                    "🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (الذكاء الاصطناعي معطل، النموذج الافتراضي Flash)")
                                save_state()
...
def load_state():
    ...
    try:
        for sym, pdata in data.get('open_positions', {}).items():
            ...
            pos.entry_momentum_time = datetime.fromisoformat(pdata['entry_momentum_time']) if pdata.get(
