...
# ========== عتبة مجموع النموذجين (Rule+EMR) - جديدة ==========
RULE_EMR_SUM_THRESHOLD = float(os.environ.get('RULE_EMR_SUM_THRESHOLD', 1.5))
# =============================================================
...
def place_preemptive_stop_loss(symbol, size, entry_price, sym_type, stop_price_override=None):
    ...
    try:
        ...
        # حساب سعر الحد (limitPrice) الذي سيتم به البيع بعد التفعيل
...
class BuyingCommittee:
    ...
    def calculate_weighted_average(self, df, symbol, sym_type='normal', market_condition='normal'):
        ...
        # الأوزان الجديدة
        weights = {'rule': 0.15, 'emr': 0.15, 'cfhm': 0.40, 'timing': 0.15, 'vwap_obv': 0.15}
        weighted_avg = 0.0
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        ...
        # التحقق من عتبة مجموع النموذجين
        global RULE_EMR_SUM_THRESHOLD
        if rule_emr_sum > RULE_EMR_SUM_THRESHOLD:
            reason = f"❌ رفض: مجموع (Rule+EMR) = {rule_emr_sum:.2f} > {RULE_EMR_SUM_THRESHOLD:.2f} (عتبة الرفض)"
...
def telegram_polling():
    ...
    global USE_1H_MULTIPLIER  # <-- إضافة المتغير الجديد
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                                ...
                                if match:
                                    ...
                                    if 0.5 <= hours <= 72:
                                        COOLDOWN_WIN_HOURS = hours  # لم تعد بحاجة إلى global هنا لأنها معلنة في أعلى الدالة
                                        save_state()
                            ...
                            # ------------------------------------------------------------
                            # 13. أمر إعادة ضبط الفلاتر (تم تحديثه ليشمل قيم التبريد الجديدة والعتبة الجديدة)
                            # ------------------------------------------------------------
                                ...
                                with _30d_cache_lock:
                                    _30d_performance_cache.clear()
                                # إعادة ضبط العتبة الجديدة
                                RULE_EMR_SUM_THRESHOLD = 1.5
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (الذكاء الاصطناعي معطل، النموذج الافتراضي Flash، نظام التبريد الذكي مفعّل بـ 12 ساعة للرابح و 5 دقائق للخاسر، وعتبة مجموع النموذجين = 1.5)")
                                save_state()
                            
                            # ------------------------------------------------------------
                            # 14. أمر رسم منحنى الرصيد الكلي
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 15. أوامر إيقاف دخول الصفقات الجديدة
                            # ------------------------------------------------------------
                            ...
                            # ============================================================
                            # 🆕 16. أوامر تشغيل/إيقاف المضاعف الجديد
                            # ============================================================
                            ...
                            # ============================================================
                            # 17. أمر تعيين عتبة مجموع النموذجين (Rule+EMR)
                            # ============================================================
                            elif text.startswith('تعيين عتبة النموذجين'):
                                match = re.search(r'(\d+(?:\.\d+)?)', text)
                                if match:
                                    new_threshold = float(match.group(1))
                                    if 0.0 <= new_threshold <= 2.0:
                                        RULE_EMR_SUM_THRESHOLD = new_threshold
                                        save_state()
                                        send_telegram(f"✅ تم تعيين عتبة مجموع النموذجين (Rule+EMR) إلى {new_threshold:.2f}\n(سيتم رفض الصفقات التي يتجاوز مجموعها هذه القيمة)")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.0 و 2.0")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين عتبة النموذجين 1.5`")
    ...
    try:
        ...
        data = {
            ...
            'filters': {
                ...
                'rule_emr_sum_threshold': RULE_EMR_SUM_THRESHOLD
            },
            'custom_settings': {
...
def load_state():
    ...
    try:
        ...
        RULE_EMR_SUM_THRESHOLD = filters.get('rule_emr_sum_threshold', 1.5)
