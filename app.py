...
# ========================================================================
# تحديث v29.1: إضافة تحكم كامل في طلبات REST عبر PAUSE_ANALYSIS و PAPER_TRADING و TEST_MODE
# لتفادي حظر IP من Binance مع الحفاظ على مراقبة المراكز وإغلاقها بشكل طبيعي.
# تم سد جميع الثغرات التي ترسل طلبات REST غير ضرورية أثناء الإيقاف.
# ========================================================================
...
TAKE_PROFIT_LEVELS_NORMAL = [0.015, 0.025, 0.035, 0.045, 0.055, 0.065, 0.075]
...
class Position:
    ...
    def _calc_initial_momentum(self):
        try:
            df = fetch_ohlcv_retry(self.symbol, TIMEFRAMES['primary'], limit=50, allow_rest=_allow_rest_for_analysis())
            if len(df)<15: return
        ...
        try:
            ...
            with _market_cap_cache_lock:
                if coin in _market_cap_cache:
                    return _market_cap_cache[coin]['value']
        except:
            pass
        return 0
    try:
...
def get_amount_limits(symbol, max_age=3600):
    if PAPER_TRADING or TEST_MODE:
        return 1.0, None, 10.0  # قيم افتراضية لتجنب REST
    now = time.time()
...
def update_preemptive_stop_loss(symbol, pos, new_stop_price):
    if PAUSE_ANALYSIS or _should_simulate():
        return
            ...
            if not is_sim:
                ...
                for attempt in range(MAX_RETRIES_CLOSE):
                    ...
                    if ws_data and ws_data.get('price', 0) > 0:
                        ...
                        # الإغلاق يحتاج REST دائماً (allow_rest=True)
        ...
        try:
            ...
            for sym, pos in snapshot:
                ...
                if ws_data and ws_data.get('price', 0) > 0:
                    ...
                    # لا نستخدم REST هنا، نكتفي بالسعر المخزن لتجنب الطلبات
                    continue
...
def sync_pending_orders():
    if PAPER_TRADING or TEST_MODE or PAUSE_ANALYSIS:
        return
    exchange = get_active_exchange()
    try:
        rest_rate_limiter.wait_if_needed(weight=2)
...
if __name__ == '__main__':
    ...
    send_telegram(f"🚀 بوت v29.0 (النسخة المبسطة بعد التنظيف) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"✅ نظام إيقاف التحليل الذكي: عند تفعيل `إيقاف التحليل`، يتوقف جلب البيانات (REST) نهائياً مع بقاء المراقبة والإغلاق نشطين.")
