# ========================================================================
# بوت التداول v28.0 – النسخة الذهبية النهائية (WebSocket Full)
# جميع الإصلاحات الـ 43 مطبقة + توسيع WebSocket للدفتر، الصفقات، التيكرات
# التعديلات: WebSocket للشموع، depth، trades، all tickers، إدارة العدد الأقصى، تحسينات الذاكرة والاتصال
# ========================================================================
    ...
    def _calc_initial_momentum(self):
        ...
        try:
            if ws_manager is not None and ws_manager.initialized:
                df = ws_manager.get_klines(self.symbol, limit=50)
...
class AdvancedWebSocketManager:
    def __init__(self):
        ...
        # للشموع
        self.klines_cache = {}          # { 'BTC/USDT': pd.DataFrame }
        ...
        self.last_access_time = {}          # symbol -> timestamp
    ...
    def _fetch_rest_klines(self, symbol, timeframe='5m', limit=500):
        ...
        for attempt in range(3):
            try:
                ...
                logger.warning(f"REST جلب {symbol} محاولة {attempt+1} فشلت: {e}")
    ...
    def _initialize_or_repair(self, symbol, limit=500):
        """تهيئة كاش العملة أو إصلاح الفجوات (بدون إرسال اشتراكات)"""
        try:
            logger.info(f"🔄 تهيئة/إصلاح {symbol} من REST...")
            df = self._fetch_rest_klines(symbol, '5m', limit)
            if df is not None and not df.empty:
                with self.klines_lock:
                    self.klines_cache[symbol] = df
                # حساب المؤشرات في الخلفية
                self._update_features_async(symbol, df)
                return df
        except Exception as e:
            logger.error(f"فشل تهيئة {symbol}: {e}")
        return pd.DataFrame()

    def _check_and_fill_gap(self, symbol):
        ...
        with self.klines_lock:
            ...
            # إذا كان فارق التوقيت أكبر من 6 دقائق (360 ثانية)
            if (now - last_time).total_seconds() > 360:
                logger.warning(f"⚠️ فجوة زمنية لـ {symbol}. جاري الإصلاح...")
                # تأخير عشوائي لتجنب إغراق REST
                time.sleep(random.uniform(0.5, 3.0))
                missing = self._fetch_rest_klines(symbol, '5m', limit=10)
                if missing is not None and not missing.empty:
                    ...
                    self.klines_cache[symbol] = df
                    self._update_features_async(symbol, df)
                    logger.info(f"✅ تم إصلاح فجوة {symbol}")
        ...
        def _update():
            try:
                if df is not None and not df.empty and len(df) >= 50:
                    ...
                    with self.klines_lock:
                        self.klines_cache[symbol] = df_copy
    ...
    def _gap_checker(self):
        ...
        while self.running:
            ...
            time.sleep(60)
            with self.klines_lock:
                symbols = list(self.klines_cache.keys())
            for sym in symbols:
                self._check_and_fill_gap(sym)
                time.sleep(0.1)
    ...
    def cleanup_klines_cache(self, max_symbols=100):
        """تنظيف klines_cache بالحفاظ على آخر max_symbols رمز (حسب last_access_time)"""
        with self.klines_lock:
            if len(self.klines_cache) > max_symbols:
                # ترتيب حسب آخر استخدام
                sorted_syms = sorted(self.klines_cache.keys(), key=lambda s: self.last_access_time.get(s, 0))
                to_remove = sorted_syms[:len(self.klines_cache) - max_symbols]
                for s in to_remove:
                    del self.klines_cache[s]
                    self.last_access_time.pop(s, None)
            ...
            if 'stream' in data and 'data' in data:
                ...
                if '@kline' in stream:
                    ...
                    with self.klines_lock:
                        ...
                        # إذا اكتملت الشمعة، نعيد حساب المؤشرات في الخلفية
                        if kline['x']:
                            # تعقيم البيانات (الدالة الآن لا تفعل شيئاً)
                            df = sanitize_dataframe(df) or df
                            self.klines_cache[symbol] = df
                            self._update_features_async(symbol, df)
                            logger.debug(f"📊 شمعة جديدة مغلقة لـ {symbol}")
        ...
        if self.subscribed_symbols:
            ...
            if params:
                for i in range(0, len(params), 1024):
                    ...
                    time.sleep(0.1)
            logger.info(f"✅ تم الاشتراك في الشموع والدفتر والصفقات لـ {len(self.subscribed_symbols)} عملة")
    ...
    def get_klines(self, symbol, timeframe='5m', limit=500):
        """الحصول على بيانات الشموع مع دعم إعادة التجميع للأطر الأخرى"""
        ...
        if timeframe != '5m':
            # للأطر الأخرى، نأخذ 5m ونعيد التجميع
            df_5m = self.get_klines(symbol, '5m', limit=500)
            if df_5m.empty:
                return pd.DataFrame()
            return _resample_klines(df_5m, timeframe)

        with self.klines_lock:
            df = self.klines_cache.get(symbol)
            if df is None or df.empty:
                # تهيئة من REST
                return self._initialize_or_repair(symbol, limit)

            # نعيد نسخة محدثة مع تعقيم (الدالة الآن لا تفعل شيئاً)
            if len(df) > limit:
    ...
    def update_subscriptions(self, new_symbols):
        ...
        if to_remove:
            ...
            with self.klines_lock:
                for sym in to_remove:
                    self.klines_cache.pop(sym, None)
        ...
        # إضافة رموز جديدة
        if to_add:
            # تهيئة الكاش من REST لكل رمز جديد (بدون اشتراك)
            for sym in to_add:
                self._initialize_or_repair(sym, 500)
            # إرسال اشتراكات لجميع القنوات الثلاث
            sub_params = []
            for sym in to_add:
                base = sym.replace('/', '').lower()
            ...
            for i in range(0, len(sub_params), 1024):
                ...
                time.sleep(0.1)
            self.subscribed_symbols = new_set
            logger.info(f"✅ اشتراك في {len(to_add)} رمز جديد (شموع+دفتر+صفقات)")
...
def _resample_klines(df_5m, target_interval):
    """إعادة تجميع بيانات 5m إلى أي إطار زمني آخر مع فرض UTC"""
...
def fetch_ohlcv_retry(symbol, timeframe='5m', limit=500, max_retries=2, ttl_seconds=60):
    ...
    if timeframe == '5m':
        if ws_manager is not None:
            df = ws_manager.get_klines(symbol, limit=limit)
            if df is not None and not df.empty:
                return df
...
<head><meta charset="UTF-8"><title>بوت v28.0 (Full WebSocket)</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v28.0 (Full WebSocket)</h1>
<div id="stats"></div>
...
def classify_symbol(symbol, df=None, ticker_data=None):
    ...
    if current_price < 1.0:
        ...
        return 'meme'
    if df is None:
        df = ws_manager.get_klines(symbol, timeframe='1h', limit=200)
        df = sanitize_dataframe(df) or df
...
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        ...
        # إصلاح 18 و 19: استخدام إعادة التجميع بدلاً من REST
        df_confirm_1 = _resample_klines(df_primary, '15T')
        df_confirm_2 = _resample_klines(df_primary, '1H')
...
def get_current_momentum(symbol, lookback_minutes):
    try:
        df = ws_manager.get_klines(symbol, limit=50)
        df = sanitize_dataframe(df) or df
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # إصلاح 28: استخدام ws_manager.get_klines بدلاً من get_cached_features (إلغاء TTL)
                df = ws_manager.get_klines(sym, limit=100)
                df = sanitize_dataframe(df) or df
                if df is None or df.empty:
                    continue
        ...
        with acquire_timeout(_processing_lock, timeout=60):
            ...
            if not _initial_warmup_done and elapsed_since_start < STARTUP_DELAY_SECONDS + 30:
                ...
                for sym in BASE_SYMBOLS:
                    ws_manager.get_klines(sym, limit=500)  # تهيئة WebSocket
                    time.sleep(0.5)
            ...
            for sym in active:
                ...
                if regime == 'trending_down':
                    continue
                # استخدام ws_manager.get_klines للحصول على الـ 5m (إصلاح 20)
                df_primary = ws_manager.get_klines(sym, limit=500)
                df_primary = sanitize_dataframe(df_primary) or df_primary
            ...
            for sym, sym_type, df_primary, regime in candidates_for_manipulation:
                ...
                if cur_price is None or cur_price <= 0:
                    continue
                # فلاتر الساعة و 4 ساعات باستخدام إعادة التجميع (إصلاح 21)
                if FILTER_HOUR_CANDLE_ENABLED:
                    df_1h = _resample_klines(df_primary, '1H')
                ...
                if FILTER_4H_CANDLE_ENABLED:
                    df_4h = _resample_klines(df_primary, '4H')
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v28.0 (Full WebSocket) - الوضع: {mode_str}")
    send_telegram(
        f"🚀 بوت v28.0 (الإصدار المعدل لـ Binance Spot + Gemini AI) يعمل – الوضع: {mode_str}\n✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n✅ تقرير يومي يعرض صافي الربح/الخسارة\n✅ التحويل بين التداول الورقي والحقيقي: `تداول حقيقي` / `تداول ورقي`\n✅ أمر `منحنى` أو `رسم` لإرسال صورة بيانية لمنحنى الرصيد الكلي\n✅ أمر `عدد الصفقات المسموحة` للتحكم بالحد اليومي للصفقات\n✅ تحسينات كبيرة في الذاكرة ومنع التسريب\n✅ WebSocket خالص مع إعادة تشغيل تلقائي (شموع، دفتر، صفقات، تيكرات)\n✅ مراقبة وإعادة تشغيل جميع الخيوط بما فيها WebSocket (مع حماية watchdog)\n✅ جلب صبور للرصيد الحقيقي (حتى 120 ثانية)\n✅ سرعة مراقبة المراكز 2 ثانية\n✅ التحكم بدرجة الاشتباه: `رفع عتبة الاشتباه 70%` أو `خفض عتبة الاشتباه 50%`\n✅ التحكم بعتبة البيع: `رفع عتبة البيع 70%` أو `خفض عتبة البيع 55%`\n✅ التحكم بعتبات الخسارة الوقائية:\n   - `تعيين عتبة خسارة 25% 0.9`\n   - `تعيين عتبة خسارة 33% 1.5`\n   - `تعيين عتبة خسارة 100% 2.0`\n   - `عتبات الخسارة` لعرض القيم الحالية\n✅ البيع التدريجي الصارم (بدون إغلاق مبكر بسبب المبلغ الصغير)\n✅ نسب البيع: 15%, 15%, 15%, 15%, 20%, 50%, 100%\n✅ دعم أفضل لمنصة Render (منفذ 8080)\n✅ **إصلاح ذاتي كامل - لا حاجة لإعادة تشغيل خارجية**\n✅ **الوقف المسبق الموحد للعادي والميم: stop_loss_limit (مدعوم لجميع أزواج Spot)**\n✅ **خيط auto_recovery خفيف (بدون إعادة تحميل الأسواق) - مع منطق الانتظار 12 دقيقة ثم 3 محاولات قبل الاسترداد**\n✅ **تحسين حساب قيمة المراكز المفتوحة في التداول الورقي والحقيقي مع فاصل زمني 5 ثوانٍ وإعادة محاولات واعتماد على WebSocket**\n✅ **إصلاح تقلص حجم الصفقات المتزامنة (تم تعديل حساب total_eq2 مرة واحدة قبل حلقة الفرص)**\n✅ **التحكم الديناميكي بعدد الرموز وحجم التعرض وحجم الصفقة حسب حالة السوق عبر أوامر تلغرام**\n✅ **منع البيع الجزئي الصغير جداً (أقل من min_cost) تلقائياً**\n✅ **ترقيم أهداف الربح بشكل متسلسل (الهدف الأول، الثاني، ...)**\n✅ **إضافة جميع أوامر v27.7 المفقودة (تشغيل/إيقاف الفلاتر الفردية، تعديل عتبات الشراء، فترة التبريد، مدة المسح)**\n✅ **إضافة أمر تلغرام للتحكم بعدد الصفقات اليومية المسموحة**\n✅ **تحسين تنفيذ الأوامر: هامش ±0.3%، مهلة 10 ثوانٍ، فاصل تحقق 1 ثانية، تحول إلى أمر سوق تلقائي مع إعادة جلب الحالة قبل الإلغاء**\n✅ **إصلاح خطير: استخدام stop_loss_limit بدلاً من stop_loss لضمان التوافق مع سوق Spot**\n✅ **إضافة فلتر Google Gemini مع نموذجين (Flash و Flash-Lite) والتحكم بهما عبر تلغرام**\n✅ **تحسينات شاملة للذكاء الاصطناعي: مهلة 65 ثانية، إعادة محاولة واحدة للأخطاء المؤقتة، إشعارات القبول والرفض مع السبب، اختبار المفتاح عند بدء التشغيل، استخدام بيانات كاملة، تسجيل الموافقة في سجل التداول**\n✅ **الآن يعتمد على WebSocket للدفتر والصفقات والتيكرات مما يقلل طلبات REST بنسبة >80%**")
