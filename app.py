...
def send_telegram(text):
    ...
    for i in range(0, len(clean), 3800):
        part = clean[i:i+3800]
        try:
            resp = telegram_session.post(  # استخدام جلسة تلغرام الخاصة (تتجاوز الوكيل)
...
def send_telegram_photo(buf, caption=""):
    ...
    try:
        ...
        resp = telegram_session.post(url, files=files, data=data, timeout=30)  # استخدام جلسة تلغرام
...
def telegram_polling():
    ...
    while True:
        try:
            ...
            try:
                resp = telegram_session.get(url, params=params, timeout=35)  # استخدام جلسة تلغرام
