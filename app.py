...
_AUTO_RECOVERY_LOCK = threading.Lock()    # <-- تم إضافة هذا السطر لحل الخطأ
...
def place_preemptive_stop_loss(symbol, size, entry_price, sym_type, stop_price_override=None):
    ...
    if stop_price_override is not None:
        ...
        # سعر التنفيذ المباشر من الدخول الأصلي (الحد الأقصى للخسارة ثابت)
...
# --------------------------- خيط auto_recovery خفيف (بدون إعادة تحميل الأسواق) ---------------------------
def auto_recovery_monitor():
    """خيط مستقل لمراقبة الاتصال بـ Binance وإعادة ضبط البوت تلقائياً عند الحظر المتكرر (بدون إعادة تحميل الأسواق)."""
        ...
        try:
            ...
            if failures >= _AUTO_RECOVERY_THRESHOLD:
                ...
                try:
                    ...
                    # 3. مسح ذاكرة التخزين المؤقت (بدون إعادة تحميل الأسواق)
                    with _cache_lock:
                        _ohlcv_cache.clear()
                    with _features_cache_lock:
                    ...
                    # ملاحظة: لا نقوم بإعادة تحميل الأسواق هنا لتجنب الحمل الزائد.
...
def analyze_and_trade():
    ...
    try:
        ...
        free_bal = get_real_balance_usdt(max_retries=5, delay=5, silent=True)   # <--- تم التعديل: 5 محاولات، 5 ثوانٍ بينها = 25 ثانية كحد أقصى
