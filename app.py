...
#   - إضافة فاصل زمني 5 ثوانٍ بين محاولات جلب الأسعار للمراكز المفتوحة.
# ========================================================================
...
MAX_POSITION_PERCENT_GOOD = float(os.environ.get('MAX_POSITION_PERCENT_GOOD', 0.20))   # 20%
...
MAX_EXPOSED_PERCENT = float(os.environ.get('MAX_EXPOSED_PERCENT', 0.80))   # 80%
...
# تم تعديل هذه الدالة بشكل رئيسي لحل مشكلة تناقص أحجام الصفقات في الورقي
        ...
        try:
            if PAPER_TRADING:
                ...
                for sym, p in pos_list:
                    # محاولة جلب السعر الحالي مع فاصل زمني 5 ثوانٍ بين المحاولات الفاشلة
                    ticker = None
                    # أولاً: نستخدم WebSocket إن أمكن (سريع)
                    ws_data = ws_manager.get_ticker(sym)
                    if ws_data and ws_data.get('price', 0) > 0:
                        price = ws_data['price']
                        logger.debug(f"✅ {sym} السعر من WebSocket: {price}")
                    else:
                        # ثانياً: REST API مع إعادة محاولة وفاصل 5 ثوانٍ
                        for retry in range(3):
                            ticker = fetch_ticker_with_retry(sym, max_retries=1)
                            if ticker and ticker.get('last', 0) > 0:
                                price = ticker['last']
                                logger.debug(f"✅ {sym} السعر من API بعد {retry+1} محاولات: {price}")
                                break
                            if retry < 2:
                                logger.debug(f"⚠️ فشل جلب {sym}، انتظار 5 ثوانٍ ثم إعادة المحاولة ({retry+2}/3)")
                                time.sleep(5)
                        else:
                            # فشلت جميع المحاولات، استخدم سعر الدخول
                            price = p.entry_price
                ...
                # الوضع الحقيقي (لم يتغير منطقياً لكن نطبق نفس التحسينات)
...
def analyze_and_trade():
    ...
    try:
        ...
        # تحديد نسبة حجم الصفقة حسب حالة السوق
        if market_cond == 'good':
