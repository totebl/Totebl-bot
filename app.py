...
# - إعادة محاولة جلب الشموع كل 10 ثوانٍ لمدة 50 ثانية كحد أقصى
# - إلغاء شرط السعر الحالي > إغلاق آخر شمعة ساعة (سبب عدم الدخول في صفقات)
# - إلغاء شرط السيولة بالكامل (إزالة check_liquidity وجميع شروطها)
# ========================================================================
...
LIQUIDITY_CHECK_DEPTH = int(os.environ.get('LIQUIDITY_CHECK_DEPTH', 10))  # غير مستخدم حالياً
LIQUIDITY_MAX_PERCENT = float(os.environ.get('LIQUIDITY_MAX_PERCENT', 0.20))  # غير مستخدم
LIQUIDITY_DEPTH_MULTIPLIER = float(os.environ.get('LIQUIDITY_DEPTH_MULTIPLIER', 0.9))  # غير مستخدم
...
MIN_DEPTH_USD_NORMAL = int(os.environ.get('MIN_DEPTH_USD_NORMAL', 1500))   # غير مستخدم
MIN_DEPTH_USD_MEME = int(os.environ.get('MIN_DEPTH_USD_MEME', 2000))        # غير مستخدم
MAX_SPREAD_NORMAL = float(os.environ.get('MAX_SPREAD_NORMAL', 0.005))       # غير مستخدم
MAX_SPREAD_MEME = float(os.environ.get('MAX_SPREAD_MEME', 0.020))           # غير مستخدم
    ...
    try:
        ...
        for sym in active:
            ...
            if regime == 'trending_down':
                continue
            # استخدام الدالة الجديدة للمحاولة المستمرة لجلب الشموع (5 محاولات كل 10 ثوانٍ = 50 ثانية كحد أقصى)
        ...
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            ...
            # تم إلغاء شرط السيولة بالكامل - لا فحص للسيولة
