...
# - فلتر الشمعة الصاعدة على 4 ساعات (متسامح مع فشل البيانات)
# - توسيع شروط الماسح وفحص السيولة وتبريد أقل
# - إزالة شرط الزخم من البيع المبكر المباشر
# ========================================================================
...
COOLDOWN_HOURS_BAD = float(os.environ.get('COOLDOWN_HOURS_BAD', 0.5))
...
BASE_COOLDOWN_HOURS = float(os.environ.get('BASE_COOLDOWN_HOURS', 0.5))

MIN_24H_CHANGE_PERCENT = float(os.environ.get('MIN_24H_CHANGE_PERCENT', 0.5))
MIN_24H_VOLUME_USD = float(os.environ.get('MIN_24H_VOLUME_USD', 75_000))
MIN_MARKET_CAP_USD = float(os.environ.get('MIN_MARKET_CAP_USD', 1_500_000))
...
_last_loss_time = {}
LOSS_COOLDOWN_MINUTES = int(os.environ.get('LOSS_COOLDOWN_MINUTES', 1))

_pending_symbols_time = {}
PENDING_TIMEOUT_SECONDS = int(os.environ.get('PENDING_TIMEOUT_SECONDS', 3600))
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # البيع المبكر المباشر (بدون زخم) - تم إزالة شرط الزخم
                if profit < 0:
                    if profit <= -0.03:
                ...
                # مراقبة الانهيار السريع (بدون زخم)
...
def check_liquidity(symbol, size_usd, sym_type='normal'):
    ...
    ratio_normal = float(os.environ.get('LIQUIDITY_RATIO_NORMAL', 0.30))
    ratio_meme = float(os.environ.get('LIQUIDITY_RATIO_MEME', 0.40))
...
def execute_full_trade(symbol, side, size, price, atr, sl, tp, sym_type, pred, conf, regime):
    ...
    try:
        ...
        _pending_symbols_time[symbol] = time.time()
    finally:
        _global_state_lock.release()
    ...
    try:
        ...
        # إنشاء الوقف مباشرة (بدون تأخير ثانيتين)
        stop_id = place_preemptive_stop_loss(symbol, filled_size, fill_price, sym_type)
        if not stop_id:
            logger.error(f"❌ فشل إنشاء وقف مسبق لـ {symbol} بعد الشراء، سيتم إلغاء الصفقة.")
            try:
                exchange = get_active_exchange()
                if order_id:
                    exchange.cancel_order(order_id, symbol)
            ...
            with _global_state_lock:
                if symbol in open_positions:
                    open_positions[symbol].preemptive_sl_order_id = stop_id
                    sl_percent = PREPLACED_SL_PERCENT_MEME if sym_type == 'meme' else PREPLACED_SL_PERCENT_NORMAL
                    open_positions[symbol].last_trailing_stop_sent = fill_price * (1 - sl_percent)
...
# --------------------------- دالة التحليل الرئيسية (مع فلتر 4 ساعات المتسامح) ---------------------------
def analyze_and_trade():
    ...
    try:
        ...
        for sym in active:
            ...
            # ---------- الفلتر الجديد: الشمعة الحالية على 4 ساعات صاعدة (متسامح مع فشل البيانات) ----------
            try:
                df_4h_raw = fetch_ohlcv_retry_raw(sym, '4h', limit=2, max_retries=2)
                if df_4h_raw is not None and not df_4h_raw.empty:
                    last_candle = df_4h_raw.iloc[-1]
                    open_4h = last_candle['open']
                    close_4h = last_candle['close']
                    if close_4h <= open_4h:
                        logger.info(f"🚫 رفض {sym}: الشمعة الحالية على 4 ساعات ليست صاعدة (close={close_4h:.8f} <= open={open_4h:.8f})")
                        continue
                # إذا لم توجد بيانات أو فشل الجلب، نتخطى الفلتر (لا نستخدم continue)
            except Exception as e:
                logger.debug(f"⚠️ فشل جلب بيانات 4 ساعات لـ {sym}: {e}، تخطي الفلتر")
                # نستمر ولا نمنع الشراء
