# ========================================================================
# بوت التداول v27.13 – إصدار محسن بالكامل يعتمد على 3 أطر زمنية (5م، 15د، 1س) مع أوزان ديناميكية
# التغييرات الجوهرية في v27.13:
#   - استبدال 3 نماذج قديمة (Rule, EMR, VWAP_OBV) بنماذج ذكية جديدة:
#       * LinearTrendPredictor (يحل محل Rule) – انحدار خطي لالتقاط الاتجاه والارتدادات
#       * PsychoCandleModel (يحل محل EMR) – تحليل نفسي للشموع (خوف/جشع)
#       * WaveStrengthModel (يحل محل VWAP_OBV) – قياس قوة الموجة الحقيقية مقابل السابقة
#   - إعادة توزيع أوزان النماذج: Rule=20%, EMR=15%, CFHM=30%, Timing=25%, VWAP_OBV=10%
#   - تحديث أوزان البيع (SELL_WEIGHTS) بنفس التوزيع الجديد
#   - الحفاظ على كامل الوظائف الأخرى (WebSocket، أوامر تلغرام، إدارة المخاطر، إلخ)
#   - إخفاء هوية الطلب (Request Headers Spoofing) لتجنب حظر Binance لعناوين IP السحابية
# ========================================================================
...
primary_exchange = ccxt.binance({
    ...
    # ========== إخفاء هوية الطلب (يجعل البوت يبدو وكأنه متصفح Chrome) ==========
    'headers': {
...
<head><meta charset="UTF-8"><title>بوت v27.13 (Spot + 3 أطر)</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v27.13 (Spot Edition – 3 Timeframes)</h1>
<div id="stats"></div>
...
# ============================================================
# نموذج التنبؤ بالاتجاه الخطي (Linear Trend Predictor)
# ============================================================
class LinearTrendPredictor:
    def get_signal_percent(self, df):
        if len(df) < 20: return 0.5, 0.5
        closes = df['close'].iloc[-20:].values
        x = np.arange(len(closes))
        slope, intercept = np.polyfit(x, closes, 1)
        expected_price_current = slope * 19 + intercept
        current_price = closes[-1]
        residuals = closes - (slope * x + intercept)
        std_residual = np.std(residuals) if np.std(residuals) > 0 else current_price * 0.001
        deviation = (current_price - expected_price_current) / std_residual
        if slope > 0:
            if deviation < 0.5: buy_score = 0.85
            elif deviation < 1.5: buy_score = 0.65
            else: buy_score = 0.25
        elif slope < 0:
            if deviation < -0.5: buy_score = 0.10
            elif deviation < 0.5: buy_score = 0.30
            else: buy_score = 0.50
        else:
            buy_score = 0.70 if std_residual < current_price * 0.01 else 0.50
        buy_score = max(0.0, min(1.0, buy_score))
        return buy_score, 1.0 - buy_score

# ============================================================
# نموذج التحليل النفسي للشموع (PsychoCandleModel)
# ============================================================
class PsychoCandleModel:
    def update(self, symbol, df):
        if len(df) < 10: return {'buy': 0.5, 'sell': 0.5}
        open_prices = df['open'].iloc[-10:].values
        high_prices = df['high'].iloc[-10:].values
        low_prices = df['low'].iloc[-10:].values
        close_prices = df['close'].iloc[-10:].values
        volumes = df['volume'].iloc[-10:].values
        avg_volume = np.mean(volumes)
        fear_signal, greed_signal, accumulation_signal = 0.0, 0.0, 0.0
        for i in range(10):
            body = abs(close_prices[i] - open_prices[i])
            if body < 1e-8: body = 0.0001 * close_prices[i]
            upper_wick = high_prices[i] - max(open_prices[i], close_prices[i])
            lower_wick = min(open_prices[i], close_prices[i]) - low_prices[i]
            if upper_wick > body * 2.5 and close_prices[i] > open_prices[i]:
                fear_signal += 0.15
            if lower_wick > body * 2.5 and close_prices[i] < open_prices[i]:
                greed_signal += 0.20
            if body < (high_prices[i] - low_prices[i]) * 0.15 and volumes[i] < avg_volume * 0.6:
                accumulation_signal += 0.10
            if close_prices[i] < open_prices[i] and body > (high_prices[i] - low_prices[i]) * 0.8:
                fear_signal += 0.25
            if close_prices[i] > open_prices[i] and body > (high_prices[i] - low_prices[i]) * 0.8:
                fear_signal += 0.15
        if len(df) >= 3:
            last_body = abs(df['close'].iloc[-1] - df['open'].iloc[-1])
            last_lower = min(df['open'].iloc[-1], df['close'].iloc[-1]) - df['low'].iloc[-1]
            prev_body = abs(df['close'].iloc[-2] - df['open'].iloc[-2])
            if last_lower > last_body * 2 and prev_body > last_body * 1.5 and df['close'].iloc[-1] > df['open'].iloc[-1]:
                greed_signal += 0.30
        recent_range = (df['high'].iloc[-5:].max() - df['low'].iloc[-5:].min()) / df['close'].iloc[-1]
        recent_vol_avg = df['volume'].iloc[-5:].mean()
        if recent_range < 0.015 and recent_vol_avg < avg_volume * 0.7:
            accumulation_signal += 0.25
        fear_total = min(0.65, fear_signal)
        greed_total = min(0.80, greed_signal + accumulation_signal)
        buy_score = greed_total + (0.5 - fear_total / 2)
        buy_score = max(0.0, min(1.0, buy_score))
        return {'buy': buy_score, 'sell': max(0.0, min(1.0, 1.0 - buy_score))}

# ============================================================
# نموذج قوة الموجة (WaveStrengthModel)
# ============================================================
class WaveStrengthModel:
    def get_score(self, df):
        if len(df) < 30: return 0.5
        lows = df['low'].iloc[-30:].values
        highs = df['high'].iloc[-30:].values
        volumes = df['volume'].iloc[-30:].values
        recent_low_idx = np.argmin(lows[-5:]) + (len(lows) - 5)
        recent_low = lows[recent_low_idx]
        if recent_low_idx < 10: return 0.5
        prior_high_idx = np.argmax(highs[recent_low_idx-15:recent_low_idx]) + (recent_low_idx - 15)
        prior_high = highs[prior_high_idx]
        recent_high_idx = np.argmax(highs[-5:]) + (len(highs) - 5)
        recent_high = highs[recent_high_idx]
        if recent_high_idx < 10: return 0.5
        prior_low_idx = np.argmin(lows[recent_high_idx-15:recent_high_idx]) + (recent_high_idx - 15)
        prior_low = lows[prior_low_idx]
        if prior_high == 0 or recent_low == 0 or recent_high == 0 or prior_low == 0: return 0.5
        down_dist = (prior_high - recent_low) / prior_high
        down_vol = np.sum(volumes[prior_low_idx:recent_low_idx+1]) if recent_low_idx >= prior_low_idx else 1
        up_dist = (recent_high - recent_low) / recent_low
        up_vol = np.sum(volumes[recent_low_idx:recent_high_idx+1]) if recent_high_idx >= recent_low_idx else 1
        down_power = down_dist * (down_vol / 1_000_000)
        up_power = up_dist * (up_vol / 1_000_000)
        if down_power < 0.0001: down_power = 0.0001
        strength_ratio = up_power / down_power
        if strength_ratio >= 1.5: buy_score = 0.90
        elif strength_ratio >= 1.2: buy_score = 0.70
        elif strength_ratio < 1.0: buy_score = 0.20
        else: buy_score = 0.50
        return max(0.0, min(1.0, buy_score))

# ======================= نموذج التدفق النقدي والزخم (CFHM) – باقٍ كما هو =======================
class CFHMModel:
...
# ======================= نموذج توقيت الدخول (Timing) – باقٍ كما هو =======================
...
# ======================= لجنة الشراء الجديدة (v27.13) =======================
class BuyingCommittee:
    def __init__(self):
        self.models = {
            'rule': LinearTrendPredictor(),
            'emr': PsychoCandleModel(),
            'cfhm': CFHMModel(),
            'timing': EntryTimingModel(),
            'vwap_obv': WaveStrengthModel()
        }
        self.weights = {
            'rule': 0.20,
            'emr': 0.15,
        ...
        self.DEFAULT_THRESHOLDS = {
            'normal': {
                'bad': {'min_conf':0.52, 'thresh3':0.60, 'thresh2':0.56, 'thresh1':0.52, 'weak_penalty_15m':0.15, 'strong_bonus_15m':0.10, 'weak_penalty_1h':0.15, 'strong_bonus_1h':0.10},
                'normal': {'min_conf':0.52, 'thresh3':0.60, 'thresh2':0.56, 'thresh1':0.52, 'weak_penalty_15m':0.12, 'strong_bonus_15m':0.12, 'weak_penalty_1h':0.12, 'strong_bonus_1h':0.12},
                'good': {'min_conf':0.52, 'thresh3':0.60, 'thresh2':0.56, 'thresh1':0.52, 'weak_penalty_15m':0.10, 'strong_bonus_15m':0.15, 'weak_penalty_1h':0.10, 'strong_bonus_1h':0.15}
            },
            'meme': {
                'bad': {'min_conf':0.52, 'thresh3':0.60, 'thresh2':0.56, 'thresh1':0.52, 'weak_penalty_15m':0.12, 'strong_bonus_15m':0.08, 'weak_penalty_1h':0.12, 'strong_bonus_1h':0.08},
                'normal': {'min_conf':0.52, 'thresh3':0.60, 'thresh2':0.56, 'thresh1':0.52, 'weak_penalty_15m':0.10, 'strong_bonus_15m':0.10, 'weak_penalty_1h':0.10, 'strong_bonus_1h':0.10},
                'good': {'min_conf':0.52, 'thresh3':0.60, 'thresh2':0.56, 'thresh1':0.52, 'weak_penalty_15m':0.08, 'strong_bonus_15m':0.12, 'weak_penalty_1h':0.08, 'strong_bonus_1h':0.12}
            }
        }
    ...
    def calculate_weighted_average(self, df_5m, df_15m, df_1h, symbol, sym_type='normal', market_condition='normal'):
        ...
        return {'avg_scores': avg_scores, 'weighted_avg': weighted_avg, 'rule_emr_sum': rule_emr_sum}
    ...
    def decide(self, df_5m, df_15m, df_1h, symbol, sym_type='normal', market_condition='normal'):
        ...
        if RULE_EMR_SUM_FILTER_ENABLED:
            if rule_emr_sum > RULE_EMR_SUM_UPPER:
                return 'neutral', 0, 0, f"❌ رفض: مجموع (Rule+EMR) = {rule_emr_sum:.2f} > الحد الأعلى {RULE_EMR_SUM_UPPER:.2f}", avg_scores
            if rule_emr_sum < RULE_EMR_SUM_LOWER:
                return 'neutral', 0, 0, f"❌ رفض: مجموع (Rule+EMR) = {rule_emr_sum:.2f} < الحد الأدنى {RULE_EMR_SUM_LOWER:.2f}", avg_scores
...
def get_current_momentum(symbol, lookback_minutes):
    try:
        ...
        lookback = max(1, int(lookback_minutes / 15))
...
def analyze_and_trade():
    ...
    try:
        ...
        for sym, sym_type, df_5m, df_15m, df_1h, regime in candidates_for_manipulation:
            ...
            final_score = avg_score * multiplier if USE_1H_MULTIPLIER else avg_score
                    ...
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                                ...
                                buying_committee.weights = {'rule':0.20, 'emr':0.15, 'cfhm':0.30, 'timing':0.25, 'vwap_obv':0.10}
                                SELL_WEIGHTS = {'rule':0.20, 'emr':0.15, 'cfhm':0.30, 'timing':0.25, 'vwap_obv':0.10}
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية (مع أوزان النماذج الجديدة: 20%،15%،30%،25%،10% وأوزان الأطر 10%،30%،60% وعتبة 0.52)")
                                save_state()
...
def save_state():
    ...
    try:
        ...
        model_weights = buying_committee.weights if 'buying_committee' in globals() else {'rule':0.20, 'emr':0.15, 'cfhm':0.30, 'timing':0.25, 'vwap_obv':0.10}
        data = {
...
def load_state():
    ...
    try:
        ...
        model_weights = weights.get('models', {'rule':0.20, 'emr':0.15, 'cfhm':0.30, 'timing':0.25, 'vwap_obv':0.10})
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v27.13 (Spot + Gemini AI + تبريد ذكي + 3 أطر حقيقية + أوزان ديناميكية + نماذج جديدة) - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v27.13 (الإصدار المحسن بـ 3 أطر زمنية 5م، 15د، 1س مع أوزان ديناميكية ونماذج ذكية) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"✅ **استبدال النماذج القديمة (Rule, EMR, VWAP_OBV) بنماذج ذكية جديدة (LinearTrendPredictor, PsychoCandleModel, WaveStrengthModel)**\n"
                  f"✅ **أوزان النماذج الجديدة: Rule=20%, EMR=15%, CFHM=30%, Timing=25%, VWAP_OBV=10%**")
