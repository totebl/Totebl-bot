...
# النماذج الثابتة (لا يتم البحث عن الأحدث)
DEFAULT_FLASH_MODEL = "gemini-2.0-flash"
DEFAULT_FLASH_LITE_MODEL = "gemini-2.0-flash-lite"
...
def monitor_positions():
    ...
    global TRAILING_DISTANCE_PERCENT, SELL_15M_THRESHOLD, SELL_15M_CHECK_INTERVAL, _last_15m_sell_check
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # =========================== التحديث 3: نظام البيع الهرمي الجديد ===========================
                now = time.time()
                ...
                if now - last_check >= SELL_15M_CHECK_INTERVAL:
                    try:
                        ...
                        sell_score = (
                            (1 - weighted_5m) * f_sell.get('5m', 0) +
                            (1 - weighted_15m) * f_sell.get('15m', 0) +
                            (1 - weighted_1h) * f_sell.get('1h', 0)
                        ) / total_frame_weight
                        if sell_score >= SELL_PARTIAL_THRESHOLD:
                            reason = f"📉 بيع 50% (جزئي): درجة البيع {sell_score:.3f} ≥ {SELL_PARTIAL_THRESHOLD:.2f}"
                            close_partial(sym, 0.5, cur_price, reason)
                        elif sell_score >= SELL_FULL_THRESHOLD:
                            reason = f"📉 بيع 100% (كامل): درجة البيع {sell_score:.3f} ≥ {SELL_FULL_THRESHOLD:.2f}"
...
def analyze_and_trade():
    ...
    try:
        ...
        # تحميل الكاش الأولي للرموز الأساسية تم إزالته بالكامل (BASE_SYMBOLS لم تعد تُحلل تلقائياً)
    ...
    try:
        ...
        data = {
            ...
            'filters': {
                ...
                'sell_15m_threshold': SELL_15M_THRESHOLD,
                # ===== التحديث 3: عتبات البيع =====
...
def load_state():
    ...
    try:
        ...
        SELL_15M_THRESHOLD = filters.get('sell_15m_threshold', 0.42)
        # === قراءة عتبات البيع ===
...
if __name__ == '__main__':
    ...
    # ========== تم النقل إلى هنا (قبل load_state) لحل خطأ buying_committee is not defined ==========
    buying_committee = BuyingCommittee()
    # ========== تعيين أوزان النماذج الافتراضية ==========
