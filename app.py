...
ACTIVE_SYMBOLS_LIMIT = int(os.environ.get('ACTIVE_SYMBOLS_LIMIT', 20))  # تم الرفع من 10 إلى 20
MAX_DAILY_TRADES = int(os.environ.get('MAX_DAILY_TRADES', 30))
BASE_COOLDOWN_HOURS = float(os.environ.get('BASE_COOLDOWN_HOURS', 1.0))

MIN_24H_CHANGE_PERCENT = float(os.environ.get('MIN_24H_CHANGE_PERCENT', 1.0))  # تم الرفع من 0.5 إلى 1.0
MIN_24H_VOLUME_USD = float(os.environ.get('MIN_24H_VOLUME_USD', 100_000))  # تم التخفيض من 150_000 إلى 100_000
MIN_MARKET_CAP_USD = float(os.environ.get('MIN_MARKET_CAP_USD', 2_000_000))  # لم يتغير
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                if profit < 0:
                    current_mom = get_current_momentum(sym, pos.momentum_check_minutes)
                    if current_mom is not None and pos.initial_momentum is not None and pos.initial_momentum != 0:
                        mom_ratio = current_mom / pos.initial_momentum
                        if profit <= -0.015 and mom_ratio <= 0.7:
                            close_partial(sym, 0.5, cur_price, f"⚠️ وقاية: خسارة {abs(profit):.2%} و زخم {mom_ratio:.1%} من الابتدائي (بيع 50%)")
                            continue
                        elif profit <= -0.03 and mom_ratio <= 0.6:
                            close_partial(sym, 1.0, cur_price, f"💀 وقاية: خسارة {abs(profit):.2%} و زخم {mom_ratio:.1%} من الابتدائي (إغلاق كامل)")
                            continue

                if profit <= -0.005 and profit <= 0.013:
                    current_mom = get_current_momentum(sym, pos.momentum_check_minutes)
                    if current_mom is not None and pos.initial_momentum is not None and pos.initial_momentum > 0:
                        mom_ratio = current_mom / pos.initial_momentum
                        if mom_ratio <= 0.1:
                            close_partial(sym, 1.0, cur_price, f"🐻 زخم منهار ({mom_ratio:.1%}) - إغلاق كامل")
                            continue
                        elif mom_ratio <= 0.3:
                            if profit <= -0.015:
                                close_partial(sym, 1.0, cur_price, f"🐻 زخم ضعيف جداً ({mom_ratio:.1%}) + خسارة {abs(profit):.1%} - إغلاق كامل")
                            else:
                                close_partial(sym, 0.5, cur_price, f"🐻 زخم ضعيف ({mom_ratio:.1%}) + خسارة {abs(profit):.1%} - إغلاق نصف")
                            continue
