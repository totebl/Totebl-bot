# ========================================================================
# بوت التداول v27.8 – إصدار مستقر معدل لـ Binance Spot + DeepSeek Filter (معالج القيم المفقودة)
# التغييرات: إضافة فلتر DeepSeek النهائي مع تحكم عبر تلغرام، مع تصحيح خطأ القيم المفقودة
...
DEEPSEEK_FALLBACK_ACTION = os.environ.get('DEEPSEEK_FALLBACK_ACTION', 'allow')  # allow or reject
...
# --------------------------- دالة استشارة DeepSeek (المصححة) ---------------------------
def ask_deepseek_for_approval(symbol, df, current_price, sym_type, market_condition, ticker_data=None):
    if not DEEPSEEK_ENABLED or not DEEPSEEK_API_KEY:
        return True
    try:
        if df.empty or len(df) < 20:
            logger.warning(f"⚠️ بيانات غير كافية لـ {symbol} لاستشارة DeepSeek")
            return DEEPSEEK_FALLBACK_ACTION == 'allow'

        # دالة مساعدة لتنسيق القيم مع التعامل مع None و NaN
...
السؤال: بناءً على هذه البيانات، هل تنصح بشراء هذا الرمز الآن؟ أجب فقط بـ "نعم" أو "لا" ثم سبب مختصر جداً."""
        ...
        payload = {"model": DEEPSEEK_MODEL, "messages": [{"role": "user", "content": prompt}], "temperature": 0.2, "max_tokens": 80}
        response = requests.post(f"{DEEPSEEK_BASE_URL}/chat/completions", json=payload, headers=headers, timeout=DEEPSEEK_TIMEOUT)
        if response.status_code == 200:
            answer = response.json()['choices'][0]['message']['content'].strip().lower()
            if "نعم" in answer or "yes" in answer:
                logger.info(f"✅ DeepSeek وافق على شراء {symbol}")
                send_telegram(f"🤖 DeepSeek وافق على شراء {symbol}\nالرد: {answer[:200]}")
                return True
            else:
                logger.info(f"❌ DeepSeek رفض شراء {symbol}")
                send_telegram(f"🤖 DeepSeek رفض شراء {symbol}\nالرد: {answer[:200]}")
                return False
        else:
            logger.warning(f"⚠️ فشل استدعاء DeepSeek API (HTTP {response.status_code})")
            send_telegram(f"⚠️ فشل استشارة DeepSeek لـ {symbol}، سقوط إلى الإجراء الافتراضي: {DEEPSEEK_FALLBACK_ACTION}")
            return DEEPSEEK_FALLBACK_ACTION == 'allow'
    except Exception as e:
        logger.error(f"خطأ في استدعاء DeepSeek: {e}")
        send_telegram(f"⚠️ خطأ في استشارة DeepSeek لـ {symbol}: {str(e)[:100]}، سقوط إلى {DEEPSEEK_FALLBACK_ACTION}")
        return DEEPSEEK_FALLBACK_ACTION == 'allow'
    ...
    try:
        ...
        if pending:
            ...
            for opp in pending:
                ...
                # استشارة DeepSeek قبل الشراء
                if DEEPSEEK_ENABLED:
                    ticker_data = all_tickers.get(opp['symbol'])
                    approved = ask_deepseek_for_approval(opp['symbol'], opp['df_primary'], opp['price'], opp['type'], market_cond, ticker_data)
                    if not approved:
                        logger.info(f"🚫 تم رفض شراء {opp['symbol']} بواسطة DeepSeek")
                        continue
...
# --------------------------- دالة telegram_polling المعدلة (بإضافة جميع الأوامر المفقودة + أمر منحنى + أمر عدد الصفقات المسموحة) ---------------------------
                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                            # 11. أوامر DeepSeek (تفعيل/إيقاف)
                            # ------------------------------------------------------------
                            elif text == 'شغل ديب سيك' or text == 'تفعيل ديب سيك':
                                DEEPSEEK_ENABLED = True
                                save_state()
                                send_telegram("🤖 تم **تفعيل** استشارة DeepSeek قبل الشراء. سأستشير النموذج قبل كل صفقة.")
                            elif text == 'أوقف ديب سيك' or text == 'تعطيل ديب سيك':
                                DEEPSEEK_ENABLED = False
                                save_state()
                                send_telegram("🤖 تم **إيقاف** استشارة DeepSeek. سأعمل بشكل طبيعي بدون استشارة.")
                            ...
                            # ------------------------------------------------------------
                                ...
                                DEEPSEEK_ENABLED = False  # إعادة ضبط DeepSeek إلى معطل افتراضياً
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (DeepSeek معطل افتراضياً)")
                                save_state()
                            ...
                            # ------------------------------------------------------------
                                ...
                                try:
                                    # تثبيت matplotlib إذا لم يكن موجوداً
                                    try:
                                        import matplotlib
                                        matplotlib.use('Agg')
    ...
    try:
        ...
        data = {
            ...
            'filters': {
                ...
                'stop_loss_33': STOP_LOSS_PARTIAL_2_PERCENT,
                'stop_loss_100': STOP_LOSS_FULL_PERCENT,
                'deepseek_enabled': DEEPSEEK_ENABLED   # حفظ حالة DeepSeek
            },
            'custom_settings': {
...
def load_state():
    ...
    try:
        ...
        DEEPSEEK_ENABLED = filters.get('deepseek_enabled', DEEPSEEK_ENABLED)   # استعادة حالة DeepSeek
