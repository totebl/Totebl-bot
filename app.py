# ========================================================================
# بوت التداول المتطور v28.0 - إصدار نهائي مع إصلاحات جذرية
# - إصلاح جلب الرصيد الحقيقي (5 محاولات، مهلة 60 ثانية، recvWindow=120000)
# - إصلاح الرصيد الورقي (يعرض الرصيد الصحيح PAPER_INITIAL_BALANCE)
# - إلغاء التحقق القاسي من API في بدء التشغيل (لمنع الحظر)
# - تحسينات الاستقرار: مراقب خيوط، تحرير القفل، Waitress، as_completed
# ========================================================================
...
# محاولة استيراد waitress مع تثبيت تلقائي
try:
    from waitress import serve
    WAITRESS_AVAILABLE = True
except ImportError:
    if _auto_install("waitress"):
        try:
            from waitress import serve
            WAITRESS_AVAILABLE = True
        except ImportError:
            WAITRESS_AVAILABLE = False
            print("⚠️ waitress لا زالت غير مثبتة، سيتم استخدام Flask التطويري")
    else:
        WAITRESS_AVAILABLE = False
        print("⚠️ waitress غير مثبتة، سيتم استخدام Flask التطويري")
...
_last_balance_failure_start = None
...
STARTUP_REDUCED_LIMIT_DURATION = 600  # زيادة إلى 10 دقائق (600 ثانية)
STARTUP_REDUCED_SYMBOLS = 3
...
# --------------------------- فلاتر قابلة للتحكم عن طريق تلغرام ---------------------------
...
# ======================== دالة تنظيف error reports ========================
def cleanup_error_reports():
    global _last_error_report_times
    now = time.time()
    keys_to_delete = [k for k, t in _last_error_report_times.items() 
                      if now - t > 3600]
...
# الدالة المحسنة لجلب الرصيد (5 محاولات، مهلة 60 ثانية، recvWindow موسع)
def get_real_balance_usdt():
    global _last_successful_balance_time, _balance_failure_paused, _balance_failure_start_time, _balance_retry_count, _last_balance_failure_start
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            return bot_stats.last_balance
    ...
    # مزامنة الوقت (مع مهلة 10 ثوانٍ)
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(exchange.load_time_difference)
            future.result(timeout=10)
            logger.info("🕰️ تم مزامنة الوقت مع Binance.")
    except FuturesTimeoutError:
        logger.warning("⚠️ مهلة مزامنة الوقت (10 ثوانٍ)، نستمر بدون مزامنة.")
    except Exception as e:
        logger.warning(f"⚠️ فشل مزامنة الوقت: {e}")

    max_attempts = 5
    timeout_per_attempt = 60  # 60 ثانية لكل محاولة كما طلبت

    for attempt in range(max_attempts):
        try:
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(exchange.fetch_balance, params={'recvWindow': 120000})
                balance = future.result(timeout=timeout_per_attempt)
            usdt = balance.get('USDT', {}).get('free', 0.0)
            if usdt is not None:
                with _state_lock:
                    bot_stats.last_balance = usdt
                    _last_successful_balance_time = now
                    _last_balance_failure_start = None
                    if _balance_failure_paused:
                        ...
                        send_telegram(f"✅ استعادة الرصيد الحقيقي: ${usdt:.2f} USDT")
                return usdt
        except FuturesTimeoutError:
            logger.warning(f"⚠️ مهلة جلب الرصيد (محاولة {attempt+1}/{max_attempts}) - استغرقت أكثر من {timeout_per_attempt} ثانية")
        except Exception as e:
            error_str = str(e)
            logger.warning(f"محاولة {attempt+1}/{max_attempts} لجلب الرصيد فشلت: {error_str[:150]}")
            if "Timestamp" in error_str or "-1021" in error_str:
                logger.warning("⚠️ خطأ في التوقيت، نحاول مرة أخرى")
                try:
                    exchange.load_time_difference()
                except:
                    pass
        time.sleep(5)  # انتظار 5 ثوانٍ بين المحاولات

    # إدارة المهلة الطويلة الأمد
    if _last_successful_balance_time == 0:
        if _last_balance_failure_start is None:
            _last_balance_failure_start = now
        if now - _last_balance_failure_start < 600:  # 10 دقائق للمرة الأولى
            logger.warning("⚠️ جاري المحاولة لجلب الرصيد (المرة الأولى)...")
            return None
    else:
        if now - _last_successful_balance_time <= 120:
            return bot_stats.last_balance
    ...
    with _state_lock:
        if not _balance_failure_paused:
            ...
            _set_paused(True)
            send_telegram("⚠️ فشل جلب الرصيد بشكل مستمر. تم إيقاف التداول مؤقتًا.")
            generate_error_report("فشل_اتصال", "رصيد", "فشل get_real_balance_usdt لأكثر من المهلة")
        return None
...
def _get_balance_no_pause(max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(exchange.fetch_balance, params={'recvWindow': 120000})
                balance = future.result(timeout=60)
            usdt = balance.get('USDT', {}).get('free', 0.0)
            if usdt > 0:
                with _state_lock:
                    bot_stats.last_balance = usdt
            ...
            logger.warning(f"⚠️ مهلة جلب الرصيد (محاولة {attempt+1}/{max_retries})")
        except Exception as e:
            logger.warning(f"محاولة {attempt+1} لجلب الرصيد (بدون إيقاف) فشلت: {e}")
        time.sleep(1)
    return None

def _safe_fetch_balance_after_trade(attempts=3, delay=1.0):
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            return bot_stats.last_balance
...
primary_exchange = ccxt.binance({
    ...
    'rateLimit': 3000,  # زيادة من 2000 إلى 3000 مللي ثانية
    'timeout': 60000,
    'options': {
        'defaultType': 'spot',
        'recvWindow': 120000,  # موسع إلى 120 ثانية
...
def fetch_ohlcv_persistent(symbol, timeframe, limit=500, max_attempts=5, retry_interval=10):
    ...
    logger.warning(f"❌ فشل جلب {symbol} {timeframe} بعد {max_attempts} محاولات.")
    return pd.DataFrame()
...
<head><meta charset="UTF-8"><title>بوت v28.0</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v28.0 – إصدار نهائي مع إصلاحات جذرية</h1>
<div id="stats"></div>
...
def update_preemptive_stop_loss(symbol, pos, new_stop_price):
    ...
    if new_oid:
        ...
        logger.debug(f"⚠️ فشل تحديث الوقف المسبق لـ {symbol} (يتوقع في وضع المحاكاة)")
...
def save_state():
    ...
    try:
        ...
        if len(bot_stats.symbol_performance) > 500:
            sorted_items = sorted(bot_stats.symbol_performance.items(), 
...
def close_partial(symbol, percent, price, reason, pnl_usdt=None):
    ...
    try:
        ...
        try:
            ...
            if not is_sim:
                new_balance = _safe_fetch_balance_after_trade(attempts=5, delay=0.5)
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                if profit < 0:
                    if profit <= -0.015 and not pos.sold_at_15:
                        sell_percent = 0.25
                        close_partial(sym, sell_percent, cur_price, f"⚠️ بيع وقائي: خسارة {abs(profit):.2%} (بيع 25%)")
                        pos.sold_at_15 = True
                        save_state()
                    elif profit <= -0.02 and not pos.sold_at_20:
                        sell_amount = pos.total_size * 0.25
                        if sell_amount > pos.remaining_size:
                            sell_amount = pos.remaining_size
                        sell_percent_of_remaining = sell_amount / pos.remaining_size if pos.remaining_size > 0 else 0
                        if sell_percent_of_remaining > 0:
                            close_partial(sym, sell_percent_of_remaining, cur_price, f"⚠️ بيع وقائي: خسارة {abs(profit):.2%} (بيع 25% إضافية)")
                        pos.sold_at_20 = True
                        save_state()
                    elif profit <= -0.03:
                        if pos.remaining_size > 0:
                            close_partial(sym, 1.0, cur_price, f"💀 بيع وقائي كامل: خسارة {abs(profit):.2%} (إغلاق كامل)")
        ...
        if _should_simulate():
            if TEST_MODE or PAPER_TRADING:
                ...
                with _global_state_lock:
                    ...
                    logger.debug(f"تحديث الرصيد الورقي: -{total_cost:.4f} USDT -> الرصيد الجديد: {bot_stats.last_balance:.2f}")
...
def analyze_and_trade():
    ...
    try:
        ...
        if not lock_acquired:
            if _processing_lock.locked() and time.time() - _last_processing_lock_released > 300:
                logger.error("❗ قفل التحليل معلق لأكثر من 5 دقائق، إعادة تعيينه...")
        ...
        for sym in active:
            ...
            if df_primary.empty or len(df_primary) < 50:
                logger.debug(f"تخطي {sym}: فشل جلب بيانات {TIMEFRAMES['primary']} بعد محاولات متعددة")
                continue
            time.sleep(0.3)
        ...
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            if FILTER_MANIPULATION_ENABLED:
                ...
                logger.debug(f"⏩ فلتر التلاعب معطل، تم تخطي فحص {sym}")
            ...
            if FILTER_HOUR_CANDLE_ENABLED:
                ...
                if df_1h.empty or len(df_1h) < 2:
                    logger.debug(f"تخطي {sym}: لا توجد بيانات كافية لشمعة ساعة")
                    continue
                last_completed_close = df_1h['close'].iloc[-2]
                if cur_price <= last_completed_close:
                    logger.debug(f"تخطي {sym}: السعر الحالي {cur_price} <= إغلاق آخر ساعة {last_completed_close} (الفلتر مفعل)")
                    continue
            else:
                logger.debug(f"⏩ فلتر شمعة الساعة معطل، تم تخطي شرط السعر لـ {sym}")
            ...
            if FILTER_4H_CANDLE_ENABLED:
                ...
                if df_4h.empty or len(df_4h) < 2:
                    logger.debug(f"تخطي {sym}: لا توجد بيانات كافية لشمعة 4 ساعات")
                    continue
                last_completed_close_4h = df_4h['close'].iloc[-2]
                if cur_price <= last_completed_close_4h:
                    logger.debug(f"تخطي {sym}: السعر الحالي {cur_price} <= إغلاق آخر شمعة 4 ساعات {last_completed_close_4h} (الفلتر مفعل)")
                    continue
            else:
                logger.debug(f"⏩ فلتر شمعة 4 ساعات معطل، تم تخطي شرط السعر لـ {sym}")
            ...
            if FILTER_LIQUIDITY_ENABLED:
                ...
                logger.debug(f"⏩ فلتر السيولة معطل، تم تخطي فحص السيولة لـ {sym}")
        ...
        if pending:
            ...
            for opp in pending:
                ...
                succ, actual_size = execute_full_trade(opp['symbol'], 'buy', sz, opp['price'], opp['atr'],
                                                       ...
                                                       opp['conf'], opp['regime'])
                if succ:
                    new_rem = _safe_fetch_balance_after_trade(attempts=5, delay=1.0) if not PAPER_TRADING else bot_stats.last_balance
                    if new_rem is None:
...
def cache_cleanup_thread():
    ...
    while True:
        ...
        if now - last_full_cleanup > 1800:
            ...
            with _market_cap_cache_lock:
                keys_to_delete = []
                for coin, entry in _market_cap_cache.items():
                    if now - entry['timestamp'] > 86400:
        ...
        try:
            ...
            if memory_mb > 1024:
                ...
                if memory_after > 1500:
                    logger.critical(f"❌ ذاكرة عالية جداً: {memory_after:.0f}MB - حفظ الحالة والخروج")
                    send_telegram(f"🚨 ذاكرة عالية جداً ({memory_after:.0f}MB) - حفظ الحالة وإعادة تشغيل آمن")
                    save_state()
                    sys.exit(1)
        except ImportError:
            pass
...
def telegram_thread_watchdog():
    while True:
        time.sleep(30)
        if 'telegram_polling' in _essential_threads:
            t = _essential_threads['telegram_polling']['thread']
            if not t.is_alive():
                logger.warning("⚠️ خيط تلغرام توقف! جاري إعادة التشغيل...")
                send_telegram("🔄 إعادة تشغيل اتصال تلغرام تلقائياً...")
                new_t = threading.Thread(target=telegram_polling, daemon=True, name="telegram_polling")
                new_t.start()
                _essential_threads['telegram_polling']['thread'] = new_t
...
@csrf.exempt
def ping():
    return jsonify({"status":"pong","version":"28.0"})
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v28.0 – إصدار نهائي مع إصلاحات جذرية لجلب الرصيد والاستقرار", "version":"28.0"})
...
@csrf.exempt
def health():
    return jsonify({"healthy":True,"paused":_is_paused(),"open":len(open_positions),"version":"28.0"})
...
def thread_watchdog():
    essential_threads = {
        ...
        'telegram_thread_watchdog': telegram_thread_watchdog
    }
                            ...
                            if text == "توقف عن التداول":
                                ...
                                try:
                                    if PAPER_TRADING or TEST_MODE:
                                        ...
                                        real_bal = get_real_balance_usdt()
                                        if real_bal is None:
                                            free_balance = bot_stats.last_balance
                                    ...
                                    msg += f"━━━━━━━━━━━━━━━━━━━━\n"
                                    msg += f"💵 <b>الرصيد الحر:</b> ${free_balance:,.2f}\n"
                                    ...
                                    msg += f"━━━━━━━━━━━━━━━━━━━━\n"
                                    msg += f"🏦 <b>الإجمالي الكلي:</b> ${total:,.2f}"
                                    send_telegram(msg)
                                except Exception as e:
                                ...
                                status = (
                                    f"📊 <b>حالة الفلاتر والقيم الحالية</b>\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                ...
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والعتبات إلى القيم الافتراضية الأصلية")
                                save_state()
                                ...
                                if match:
                                    ...
                                    send_telegram(f"✅ تم ضبط فترة التبريد إلى {value} {unit}\n"
                                                 f"📌 تبريد عادي: {BASE_COOLDOWN_HOURS} ساعة\n"
                                                 f"📌 تبريد سوق جيد: {COOLDOWN_HOURS_GOOD} ساعة\n"
                                                 f"📌 تبريد سوق سيء: {COOLDOWN_HOURS_BAD} ساعة")
                                    save_state()
                                else:
                                ...
                                if match:
                                    ...
                                    send_telegram(f"✅ تم رفع عتبات لجنة الشراء بنسبة {percent}%\n"
                                                 f"📊 المضاعف الجديد: {new_multiplier:.2f} (أي عتبات ×{new_multiplier:.2f})")
                                else:
                                ...
                                if not ENABLE_TRADING:
                                    ...
                                    if open_positions:
                                        send_telegram("🔄 جاري إغلاق جميع المراكز الورقية قبل التحويل إلى التداول الحقيقي...")
                                    ...
                                    with _global_state_lock:
                                        ...
                                        bot_stats.weekly_wins = 0
                                        bot_stats.weekly_losses = 0
                                        bot_stats.last_balance = 10000.0
                                        bot_stats.equity_curve = deque(maxlen=1000)
                                        bot_stats.symbol_performance = {}
                                        _daily_trades_date = datetime.now().date()
                                    PAPER_TRADING = False
                                    TEST_MODE = False
                                    send_telegram("✅ تم التبديل إلى **التداول الحقيقي**. سيتم جلب الرصيد الحقيقي من المنصة فوراً. تأكد من أن API keys صالحة ولديها أذونات التداول.")
                                    real_bal = get_real_balance_usdt()
                                    if real_bal:
                                        send_telegram(f"💰 الرصيد الحقيقي المتاح: ${real_bal:.2f} USDT")
                                    else:
                                        send_telegram("⚠️ فشل جلب الرصيد الحقيقي. تأكد من الاتصال بالمنصة.")
                                    save_state()
                                else:
...
if __name__ == '__main__':
    ...
    # إلغاء التحقق القاسي من API الذي يسبب حظراً غير ضروري
    # بدلاً من ذلك، سنثق في أن المستخدم أدخل مفاتيح صحيحة وسيحاول البوت الاتصال بشكل طبيعي
    if not TEST_MODE and not PAPER_TRADING and ENABLE_TRADING:
        logger.info("🔑 تم استخدام مفاتيح API حقيقية (بدون اختبار مسبق لتجنب الحظر)")
    ...
    logger.info(f"🚀 بدء البوت v28.0 - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v28.0 يعمل – الوضع: {mode_str}\n✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n✅ تقرير يومي يعرض صافي الربح/الخسارة\n✅ التحويل بين التداول الورقي والحقيقي\n✅ إصلاحات جذرية لجلب الرصيد (5 محاولات، مهلة 60 ثانية، recvWindow=120000)\n✅ تثبيت تلقائي لخادم Waitress\n✅ مراقبة الخيوط وإعادة التشغيل التلقائي")
    ...
    try:
        if WAITRESS_AVAILABLE:
            logger.info("🌟 بدء تشغيل خادم الويب باستخدام Waitress (إنتاجي)...")
            serve(app, host='0.0.0.0', port=port, threads=4)
        else:
            logger.info("🌟 بدء تشغيل خادم الويب باستخدام Flask التطويري...")
            app.run(host='0.0.0.0', port=port, debug=False, threaded=True, use_reloader=False)
    except Exception as e:
        logger.critical(f"❌ فشل تشغيل خادم الويب: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)