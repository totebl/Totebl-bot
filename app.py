# ========================================================================
# بوت التداول v27.6 – إصدار نهائي مستقر (مع تعديلات السرعة والأمان + Heartbeat)
# ========================================================================
...
LOCK_TIMEOUT_SECONDS = 300   # 5 دقائق
_manipulation_cache = {}
...
# --------------------------- WebSocket Manager مع آلية إعادة تشغيل قوية + Heartbeat ---------------------------
class SimpleWebSocketManager:
    def __init__(self):
        ...
        self.last_message_time = time.time()  # Heartbeat
    ...
    def on_open(self, ws):
        ...
        self.reconnect_delay = 5
        self.last_message_time = time.time()  # إعادة تعيين عند الفتح
...
def thread_watchdog():
    ...
    while True:
        try:
            ...
            # Heartbeat لـ WebSocket
            if ws_manager.is_alive():
                time_since_last_msg = time.time() - ws_manager.last_message_time
                if time_since_last_msg > 90:  # أكثر من 90 ثانية بدون رسائل
                    logger.warning(f"⚠️ WebSocket Manager حي لكن لا تصل رسائل منذ {time_since_last_msg:.0f} ثانية (تعليق صامت). إعادة تشغيل...")
                    send_telegram(f"🔄 WebSocket Manager حي لكن لا يستقبل بيانات - إعادة تشغيل تلقائي.")
                    ws_manager.restart()
                    time.sleep(5)
