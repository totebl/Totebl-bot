...
# ========================== دالة تعقيم البيانات (إضافة جديدة) ==========================
def sanitize_dataframe(df, required_cols=None):
    """تأكد من أن DataFrame خالٍ من NaN و Inf في الأعمدة المطلوبة"""
...
class AdvancedWebSocketManager:
    ...
    def _perform_cleanup(self, now):
        ...
        if now - self._last_medium_cleanup > 300:
            self.cleanup_all_tickers_cache(max_age=600)
    ...
    def cleanup_all_tickers_cache(self, max_age=600):
        """تنظيف all_tickers_cache (البيانات الأقدم من max_age ثانية)"""
        now = time.time()
...
# ======================= دالة add_advanced_features المعدلة (إزالة fillna(0)) =======================
def add_advanced_features(df):
    ...
    # إزالة fillna(0) والاكتفاء بـ ffill و bfill
    df = df.replace([np.inf, -np.inf], np.nan)
    df = df.ffill().bfill()
    # تحويل إلى float64 مع الاحتفاظ بـ NaN في البداية
...
def cache_cleanup_thread():
    ...
    while True:
        ...
        if ws_manager:
            ...
            ws_manager.cleanup_all_tickers_cache(max_age=600)
