...
# - تم التعديل لحل مشكلة جلب الرصيد الحقيقي على Render
# - إضافة recvWindow و adjustForTimeDifference
# - تحسين منطق إعادة محاولة جلب الرصيد
# ========================================================================
...
# التحقق من صحة مفاتيح API عند بدء التشغيل في الوضع الحقيقي
if not PAPER_TRADING and ENABLE_TRADING and not BINANCE_SANDBOX:
    if not BINANCE_API_KEY or not BINANCE_SECRET_KEY:
        logger.critical("❌ مفاتيح API مفقودة للوضع الحقيقي. تأكد من تعيين BINANCE_API_KEY و BINANCE_SECRET_KEY")
        sys.exit(1)
    else:
        logger.info("✅ تم العثور على مفاتيح API للوضع الحقيقي")
...
# تم التعديل: إضافة recvWindow و adjustForTimeDifference
primary_exchange = ccxt.binance({
    ...
    'options': {
        'defaultType': 'spot',
        'recvWindow': 60000,                     # توسيع النافذة إلى 60 ثانية
        'adjustForTimeDifference': True          # تصحيح فارق التوقيت تلقائياً
    },
})
...
def fetch_ohlcv_persistent(symbol, timeframe, limit=500, max_attempts=5, retry_interval=10):
    """يحاول جلب البيانات بشكل متكرر كل 10 ثوانٍ لمدة 50 ثانية كحد أقصى (5 محاولات)"""
...
def get_real_balance_usdt():
    """جلب الرصيد الحقيقي مع إعادة محاولة ذكية وتجنب القفل الدائم"""
    global _last_successful_balance_time, _balance_failure_paused, _balance_failure_start_time, _balance_retry_count

    # في حالة المحاكاة، نرجع الرصيد المخزن
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            return bot_stats.last_balance

    # إذا كنا في حالة فشل سابقة ولم يمر وقت كافٍ، لا نحاول مرة أخرى
    now = time.time()
    if _balance_failure_paused:
        if now - _balance_failure_start_time < 120:  # انتظر دقيقتين قبل إعادة المحاولة التلقائية
            logger.debug("⚠️ مؤقت فشل الرصيد لا يزال نشطًا، لن نحاول جلب الرصيد الآن")
            return None
        else:
            # بعد 120 ثانية، نعيد المحاولة تلقائيًا
            _balance_failure_paused = False
            _balance_retry_count = 0
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            if text == "توقف عن التداول":
                                ...
                                if not ENABLE_TRADING:
                                    ...
                                    TEST_MODE = False
                                    # إعادة تعيين حالة فشل الرصيد
                                    global _balance_failure_paused, _balance_retry_count, _last_successful_balance_time
                                    _balance_failure_paused = False
                                    _balance_retry_count = 0
                                    _last_successful_balance_time = 0
                                    _set_paused(False)
