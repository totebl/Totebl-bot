# ========================================================================
# بوت التداول v29.2 – النسخة المبسطة (بعد تنظيف شامل + تحكم REST)
# تم حذف: التلاعب، الأداء 30 يوم، الذكاء الاصطناعي، التوصيات، لجنة البيع، حالة السوق
...
# ========================================================================
# التحديثات الجديدة (v29.1 & v29.2):
# - إضافة _allow_rest_for_analysis() للتحكم في طلبات REST أثناء التحليل.
# - إيقاف جميع طلبات REST غير الضرورية عند PAUSE_ANALYSIS أو PAPER_TRADING أو TEST_MODE.
# - الحفاظ على WebSocket و monitor_positions و close_partial (باستخدام REST) دون تأثر.
# - إعادة كتابة fetch_tickers_with_retry بصلاحية أطول (4 دقائق) وتأخير عشوائي.
# - إعادة ضبط RateLimiter ووقت المسح عند تشغيل التحليل.
# ========================================================================
...
# ------------------- متغيرات جديدة للتحكم في REST أثناء التحليل -------------------
_analysis_resume_time = 0  # وقت استئناف التحليل (للاستخدام المستقبلي)
...
# ----------------------------------------------------------------------
# دالة التحكم في REST أثناء التحليل
# ----------------------------------------------------------------------
def _allow_rest_for_analysis():
    """تحدد ما إذا كان مسموحاً باستخدام REST لجلب بيانات التحليل (وليس للإغلاق)."""
    return not (PAUSE_ANALYSIS or PAPER_TRADING or TEST_MODE)
                ...
                for sym, p in pos_list:
                    ...
                    if ws_data and ws_data.get('price', 0) > 0:
                        ...
                        for retry in range(3):
                            ticker = fetch_ticker_with_retry(sym, max_retries=1, allow_rest=True)  # الإغلاق يحتاج REST
...
def get_real_balance_usdt(max_retries=5, delay=3.0, silent=True):
    ...
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            return bot_stats.last_balance
    # إذا كان التحليل موقفاً، لا نرسل REST
    if PAUSE_ANALYSIS:
        with _global_state_lock:
            return bot_stats.last_balance
    exchange = get_active_exchange()
...
def _safe_fetch_balance_after_trade(attempts=10, delay=3.0, silent=True):
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            return bot_stats.last_balance
    # الإغلاق لا يتأثر بـ PAUSE_ANALYSIS، نسمح بـ REST
...
# ----------------------------------------------------------------------
def fetch_ticker_with_retry(symbol, max_retries=2, allow_rest=True):
...
def fetch_tickers_with_retry(max_retries=2):
    # 1. إذا كان التحليل موقفاً، استخدم البيانات المخزنة فقط (بدون REST) مع صلاحية أطول
    if PAUSE_ANALYSIS or PAPER_TRADING or TEST_MODE:
        with _global_state_lock:
            if bot_stats.last_valid_tickers:
                age = time.time() - bot_stats.last_tickers_time
                if age < STALE_DATA_MAX_AGE * 2:  # 4 دقائق
                    return dict(bot_stats.last_valid_tickers)
        return {}

    # 2. محاولة WebSocket أولاً (بيانات لحظية)
    tickers = ws_manager.get_all_tickers()
    if tickers and len(tickers) > 50:  # خفض العتبة إلى 50
        with _global_state_lock:
            bot_stats.last_valid_tickers = dict(tickers)
            bot_stats.last_tickers_time = time.time()
        return tickers

    # 3. استخدام البيانات المخزنة إذا كانت حديثة نسبياً (لمنع REST غير الضروري)
    with _global_state_lock:
        if bot_stats.last_valid_tickers:
            age = time.time() - bot_stats.last_tickers_time
            if age < STALE_DATA_MAX_AGE * 2:  # 4 دقائق
                logger.info(f"⏳ استخدام بيانات مخزنة (عمرها {age:.0f} ثانية)")
                return dict(bot_stats.last_valid_tickers)

    # 4. اللجوء إلى REST مع تأخير عشوائي ومحاولات محدودة
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(f"⚠️ فشل جلب REST (محاولة {attempt+1}): {e}")
            if "418" in str(e) or "banned" in str(e):
                wait_time = 60 + random.randint(0, 30)
                logger.warning(f"🚨 حظر مؤقت، انتظار {wait_time} ثانية")
                time.sleep(wait_time)
                continue
            if attempt < max_retries - 1:
                time.sleep(2 ** attempt + random.uniform(0, 1))

    # 5. في حال فشل كل شيء، نعيد آخر بيانات مخزنة (حتى لو قديمة) لتجنب الفشل الكامل
    with _global_state_lock:
        if bot_stats.last_valid_tickers:
            logger.warning("⚠️ فشل REST، استخدام آخر بيانات مخزنة (قد تكون قديمة)")
            return dict(bot_stats.last_valid_tickers)
    return {}

def get_market_cap_from_coingecko(symbol, ticker_data=None, allow_rest=True):
    if not allow_rest:
        return 0
    try:
...
# ----------------------------------------------------------------------
# دوال جلب OHLCV (مع Rate Limiter والتحكم في REST)
# ----------------------------------------------------------------------
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=500, max_retries=4, allow_rest=True):
    if not allow_rest:
        return pd.DataFrame()
    time.sleep(0.1)
    exchange = get_active_exchange()
...
def fetch_ohlcv_persistent(symbol, timeframe, limit=500, max_attempts=5, retry_interval=10, allow_rest=True):
    if not allow_rest:
        return pd.DataFrame()
    for attempt in range(max_attempts):
        df = fetch_ohlcv_retry_raw(symbol, timeframe, limit, max_retries=1, allow_rest=allow_rest)
        if not df.empty:
            return df
...
def fetch_ohlcv_retry(symbol, timeframe, limit=500, max_retries=2, ttl_seconds=60, allow_rest=True):
    if not allow_rest:
        return pd.DataFrame()
    ...
    df = fetch_ohlcv_retry_raw(symbol, timeframe, limit, max_retries, allow_rest=allow_rest)
    if not df.empty:
        with _cache_lock:
...
def get_cached_features(symbol, timeframe, limit=500, ttl=60, allow_rest=True):
    if not allow_rest:
        return pd.DataFrame()
    cache_key = f"{symbol}_{timeframe}_{limit}"
    now = time.time()
    ...
    df = fetch_ohlcv_retry(symbol, timeframe, limit=limit, allow_rest=allow_rest)
    if df.empty:
        return pd.DataFrame()
    if len(df) >= 20:
...
def fetch_orderbook_with_cache(symbol, limit=LIQUIDITY_CHECK_DEPTH, ttl=5, allow_rest=True):
    if not allow_rest:
        return None
    now = time.time()
...
<head><meta charset="UTF-8"><title>بوت v29.2 المبسط</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v29.2 (نسخة مبسطة)</h1>
<div id="stats"></div>
...
# ----------------------------------------------------------------------
def classify_symbol(symbol, df=None, ticker_data=None, allow_rest=True):
    ...
    if ticker_data:
        ...
        ticker = fetch_ticker_with_retry(symbol, allow_rest=allow_rest)
        current_price = ticker.get('last',0) if ticker else 0
        volume_24h = ticker.get('quoteVolume',0) if ticker else 0
    if current_price < 1.0:
        if volume_24h > 2_000_000:
            return 'normal'
        cap = get_market_cap_from_coingecko(symbol, ticker_data, allow_rest=allow_rest)
        if cap > 50_000_000:
            return 'normal'
        return 'meme'
    if df is None:
        df = fetch_ohlcv_retry(symbol, '1h', limit=200, allow_rest=allow_rest)
...
def detect_market_regime(symbol, allow_rest=True):
    if not allow_rest:
        return 'neutral'
    df = fetch_ohlcv_retry(symbol, '1h', limit=200, allow_rest=allow_rest)
    if df.empty:
        return 'neutral'
    df_f = get_cached_features(symbol, '1h', limit=200, ttl=120, allow_rest=allow_rest)
    if df_f.empty:
        return 'neutral'
...
# ----------------------------------------------------------------------
# دوال تنفيذ الأوامر (تستخدم REST دائماً عند التداول الحقيقي)
# ----------------------------------------------------------------------
...
def validate_restored_position(symbol, pos):
    if TEST_MODE or PAPER_TRADING or not ENABLE_TRADING:
        return True
    # هذه الدالة تستخدم REST للتحقق من الرصيد، ونسمح بذلك دائماً (حتى أثناء PAUSE_ANALYSIS) لأنها ضرورية للتحقق من سلامة المراكز
    for attempt in range(3):
        try:
            exchange = get_active_exchange()
            rest_rate_limiter.wait_if_needed(weight=20)
        ...
        try:
            if not is_sim:
                ...
                for attempt in range(MAX_RETRIES_CLOSE):
                    # نسمح بـ REST دائماً للإغلاق (allow_rest=True)
                    ws_data = ws_manager.get_ticker(symbol)
                    if ws_data and ws_data.get('price', 0) > 0:
                        curr = ws_data['price']
                    else:
                        ticker = fetch_ticker_with_retry(symbol, max_retries=1, allow_rest=True)
...
def close_all_positions(reason="أمر إداري"):
    ...
    for sym in syms:
        ...
        if ws_data and ws_data.get('price', 0) > 0:
            ...
            ticker = fetch_ticker_with_retry(sym, max_retries=1, allow_rest=True)  # نسمح بـ REST للإغلاق
        ...
        try:
            ...
            for sym, pos in stuck.items():
                ...
                if ws_data and ws_data.get('price', 0) > 0:
                    ...
                    ticker = fetch_ticker_with_retry(sym, max_retries=1, allow_rest=True)  # نسمح بـ REST
...
def check_liquidity(symbol, size_usd, sym_type='normal', allow_rest=True):
    if not allow_rest:
        return False, "REST غير مسموح"
    ob = fetch_orderbook_with_cache(symbol, limit=LIQUIDITY_CHECK_DEPTH, allow_rest=allow_rest)
...
# ----------------------------------------------------------------------
# خيط auto_recovery_monitor (يتوقف عند PAUSE_ANALYSIS)
# ----------------------------------------------------------------------
...
def analyze_and_trade():
    ...
    try:
        ...
        # تحديد ما إذا كان مسموحاً بـ REST في هذه الدورة
        allow_rest = _allow_rest_for_analysis()
        if not allow_rest:
            logger.info("⏸️ التحليل موقف (REST غير مسموح)، الخروج من الدورة.")
            return
        ...
        all_tickers = fetch_tickers_with_retry()  # هذه الدالة تتعامل مع التحكم في REST داخلياً
        if not all_tickers:
        ...
        for sym in active:
            ...
            regime = detect_market_regime(sym, allow_rest=allow_rest)
            ...
            df_5m = get_cached_features(sym, '5m', limit=200, ttl=ttl_used, allow_rest=allow_rest)
            df_15m = get_cached_features(sym, '15m', limit=500, ttl=ttl_used, allow_rest=allow_rest)
            df_1h = get_cached_features(sym, '1h', limit=500, ttl=ttl_used, allow_rest=allow_rest)
            ...
            sym_type = classify_symbol(sym, df=df_15m, ticker_data=all_tickers.get(sym), allow_rest=allow_rest)
            ...
            if FILTER_HOUR_CANDLE_ENABLED:
                df_1h_filter = fetch_ohlcv_persistent(sym, '1h', limit=2, max_attempts=5, retry_interval=10, allow_rest=allow_rest)
            ...
            if FILTER_LIQUIDITY_ENABLED:
                liquidity_ok, liq_msg = check_liquidity(sym, size * cur_price, sym_type, allow_rest=allow_rest)
                if not liquidity_ok:
...
def background_scanner():
    ...
    while True:
        ...
        if not ws_manager.is_alive():
            logger.warning("⚠️ WebSocket غير متصل، جاري إعادة التشغيل...")
            ws_manager.restart()
        if scanner and scanner.should_scan():
            scanner.scan()
        time.sleep(10)
            ...
            if positions_count > 0:
                ...
                for sym, pos in open_positions.items():
                    ...
                    if ws_data and ws_data.get('price', 0) > 0:
                        ...
                        ticker = fetch_ticker_with_retry(sym, max_retries=1, allow_rest=True)
...
# ----------------------------------------------------------------------
# دالة telegram_polling المبسطة (مع تعديل أمر تشغيل التحليل)
# ----------------------------------------------------------------------
                    ...
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ========== أوامر إيقاف/تشغيل التحليل ==========
                                ...
                                PAUSE_ANALYSIS = False
                                _analysis_resume_time = time.time()
                                # إعادة ضبط Rate Limiter ووقت المسح لبدء سلس
                                rest_rate_limiter.calls.clear()
                                if scanner:
                                    scanner.last_scan = 0
                                    scanner.next_scan_time = time.time() + 5
                                _last_analyzed_candidates = []
                                send_telegram("▶️ تم تشغيل التحليل مع بدء بطيء.")
                            ...
                            # ========== أمر إعادة الضبط ==========
                                ...
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية.")
                                save_state()
...
# ----------------------------------------------------------------------
# كلاس MarketScanner (مع تعديل scan لتأجيل المسحة عند الفشل)
# ----------------------------------------------------------------------
class MarketScanner:
    def __init__(self):
        self.last_scan = 0
    ...
    def scan(self):
        ...
        if not tickers:
            logger.warning("⚠️ فشل جلب بيانات الأسواق أثناء المسح.")
            self.next_scan_time = time.time() + 60  # تأجيل المسحة التالية
            return
        filtered = []
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v29.2 (نسخة مبسطة) - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v29.2 (النسخة المبسطة بعد التنظيف والتحكم في REST) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"✅ تم إيقاف طلبات REST غير الضرورية أثناء إيقاف التحليل، مع الحفاظ على الإغلاق والمراقبة.")
