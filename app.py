...
DEFAULT_FLASH_LITE_MODEL = "gemini-3.1-flash-lite"
...
SELL_WEIGHTS = {
    'rule': 0.1567,
    'emr': 0.1567,
    'cfhm': 0.38,     # النموذج الأقوى في استمرار الاتجاه
    'timing': 0.15,   # تم تخفيضه لتجنب البيع المبكر
    'vwap_obv': 0.1566
}
# ============================================================
...
class RuleBasedModel:
    ...
    def get_signal_percent(self, df):
        ...
        # زيادة فترة الزخم من 6 إلى 12 شمعة (من 1.5 ساعة إلى 3 ساعات)
        mom = (df['close'].iloc[-1] - df['close'].iloc[-12]) / df['close'].iloc[-12] if len(df)>=12 else 0
...
# ======================= النموذج الهجين الجديد (يحل محل EMR) - معدل لعلاقة RSI غير الخطية =======================
class HybridMomentumRSIModel:
    """
    نموذج هجين (جمع بين 3 مؤشرات في صوت واحد):
    1. الزخم المُكيَّف مع التقلب (VAMM) - الوزن 50%
    2. مؤشر القوة النسبية (RSI) - الوزن 30% (مع علاقة غير خطية لتجنب القمم)
    3. حجم التداول النسبي (Volume Ratio) - الوزن 20%
    
    هذا النموذج يحل محل EMR ويعطي درجة شراء وبيع واحدة متكاملة.
    """
    def __init__(self):
        pass  # لا يحتاج إلى تخزين أي شيء

    def update(self, symbol, df):
        """
        تعيد درجة شراء ودرجة بيع بناءً على المؤشرات الثلاثة مجتمعة.
        """
        # إذا كانت البيانات غير كافية، نعطي صوت محايد (0.5)
        if df.empty or len(df) < 20:
            return {'buy': 0.5, 'sell': 0.5}

        # ============================================================
        # 1. حساب الزخم المُكيَّف مع التقلب (VAMM) - الوزن 50%
        # ============================================================
        price_now = df['close'].iloc[-1]
        price_10_ago = df['close'].iloc[-11]  # قبل 10 شموع (150 دقيقة)
        raw_momentum = (price_now - price_10_ago) / price_10_ago if price_10_ago != 0 else 0

        # حساب ATR%
        if 'atr_percent' in df.columns and not pd.isna(df['atr_percent'].iloc[-1]):
        ...
        if atr_pct > 0:
            adjusted_momentum = raw_momentum / atr_pct
        else:
            adjusted_momentum = raw_momentum * 50

        vamm_score = 1 / (1 + np.exp(-adjusted_momentum * 2.5))
        vamm_score = max(0.0, min(1.0, vamm_score))

        # ============================================================
        # 2. حساب مؤشر القوة النسبية (RSI) - علاقة غير خطية لتجنب القمم
        # ============================================================
        if 'rsi' in df.columns and not pd.isna(df['rsi'].iloc[-1]):
            rsi_value = df['rsi'].iloc[-1]
            # تحويل RSI إلى درجة بطريقة غير خطية لتجنب القمم
            if rsi_value < 30:
                # منطقة تشبع بيعي (قد تكون فرصة للارتداد)
                rsi_score = 0.6 + (30 - rsi_value) / 150  # بين 0.6 و 0.8
            elif 30 <= rsi_value <= 70:
                # المنطقة الطبيعية: تزداد الدرجة تدريجياً من 0.4 إلى 0.8
                rsi_score = 0.4 + (rsi_value - 30) / 100
            else:  # rsi_value > 70
                # منطقة تشبع شرائي (خطر قمة) → تنخفض الدرجة بشدة
                rsi_score = max(0.0, 0.8 - (rsi_value - 70) / 25)
        else:
            rsi_score = 0.5

        # ============================================================
        # 3. حساب حجم التداول النسبي - الوزن 20%
        # ============================================================
        if 'volume_ratio' in df.columns and not pd.isna(df['volume_ratio'].iloc[-1]):
            vol_ratio = df['volume_ratio'].iloc[-1]
            if vol_ratio > 1.5:
                vol_score = min(1.0, 0.5 + (vol_ratio - 1.5) * 0.3)
            elif vol_ratio < 0.5:
                vol_score = max(0.0, 0.5 - (0.5 - vol_ratio) * 0.5)
            else:
                vol_score = 0.5
        else:
            vol_score = 0.5
        ...
        # ============================================================
        # 4. دمج المؤشرات الثلاثة في درجة واحدة (هجين)
        # ============================================================
        hybrid_buy_score = (vamm_score * 0.50) + (rsi_score * 0.30) + (vol_score * 0.20)
        hybrid_buy_score = max(0.0, min(1.0, hybrid_buy_score))
        hybrid_sell_score = 1 - hybrid_buy_score

        return {'buy': hybrid_buy_score, 'sell': hybrid_sell_score}
...
class BuyingCommittee:
    def __init__(self):
        self.models = {
            'rule': RuleBasedModel(),
            'emr': HybridMomentumRSIModel(),   # تم استبدال EMRModel بالنموذج الهجين
            'cfhm': CFHMModel(),
    ...
    def calculate_weighted_average(self, df, symbol, sym_type='normal', market_condition='normal'):
        ...
        # ===== الأوزان الجديدة (تم التعديل) =====
        weights = {'rule': 0.10, 'emr': 0.15, 'cfhm': 0.30, 'timing': 0.30, 'vwap_obv': 0.15}
        weighted_avg = 0.0
