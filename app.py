...
WEIGHT_5M = float(os.environ.get('WEIGHT_5M', 33.33))
WEIGHT_15M = float(os.environ.get('WEIGHT_15M', 33.33))
WEIGHT_1H = float(os.environ.get('WEIGHT_1H', 33.34))
...
class BuyingCommittee:
    ...
    def calculate_weighted_average(self, df_5m, df_15m, df_1h, symbol, sym_type='normal', market_condition='normal'):
        ...
        # أوزان النماذج الثابتة (كما كانت)
        weights = {'rule': 0.15, 'emr': 0.15, 'cfhm': 0.28, 'timing': 0.28, 'vwap_obv': 0.14}
        weighted_avg = sum(avg_scores[m] * weights[m] for m in weights)
                    ...
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                                ...
                                PAUSE_ANALYSIS = False
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (الذكاء الاصطناعي معطل، النموذج الافتراضي Flash، نظام التبريد الذكي مفعّل بـ 12 ساعة للرابح و 5 دقائق للخاسر، وعتبات النموذجين: الأعلى 1.5 والأدنى 1.3، وتحديث الوقف مفعّل، نسبة تتبع الأرباح 1%، عتبة بيع 15 دقيقة 0.42، أوزان الأطر 33.33% لكل إطار، التحليل مفعّل)")
                                save_state()
...
def load_state():
    ...
    try:
        ...
        WEIGHT_5M = weights.get('5m', 33.33)
        WEIGHT_15M = weights.get('15m', 33.33)
        WEIGHT_1H = weights.get('1h', 33.34)
