# ========================================================================
# بوت التداول v27.8 – إصدار مستقر معدل لـ Binance Spot + Google Gemini AI
# التغييرات: إضافة فلتر Google Gemini مع نموذجين (Flash و Flash-Lite) والتحكم عبر تلغرام
...
import zipfile  # <--- إضافة لدعم الملفات المضغوطة
import gc  # لجمع القمامة يدوياً
...
_30d_performance_cache_max = 80   # حد أقصى جديد
...
# ========== التعديل الرئيسي: تغيير مسار الملف إلى الجذر ==========
MARKETS_CACHE_FILE = "markets_cache"   # سيتم إضافة .json أو .zip حسب الحاجة
# ================================================================
...
# ========== متغيرات حدود المخابئ الإضافية ==========
AMOUNT_LIMITS_CACHE_MAX = 80   # حد أقصى لـ _amount_limits_cache
_ORDERBOOK_CACHE_MAX = 80      # حد أقصى لـ orderbook cache (تم التخفيض من 500)
# =================================================
...
def send_telegram_file(file_path, caption=""):
    """إرسال ملف عبر تلغرام (sendDocument)"""
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        return
    if not os.path.exists(file_path):
        logger.error(f"⚠️ الملف غير موجود: {file_path}")
        return
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            resp = requests.post(url, files=files, data=data, timeout=30)
        if resp.status_code == 200:
            logger.info(f"✅ تم إرسال الملف {file_path} إلى تلغرام")
        else:
            logger.warning(f"⚠️ فشل إرسال الملف: {resp.text}")
    except Exception as e:
        logger.error(f"❌ خطأ في إرسال الملف: {e}")

# ==================== التعديل الرئيسي: دالة تحميل الأسواق من ZIP أو JSON (الجذر أولاً) ====================
def load_markets_from_cache():
    """تحميل الأسواق من الملف المحلي (يدعم JSON و ZIP) - يبحث في الجذر أولاً"""
    # 1. محاولة تحميل الملف المضغوط من الجذر
    zip_root = MARKETS_CACHE_FILE + ".zip"   # markets_cache.zip
    if os.path.exists(zip_root):
        try:
            with zipfile.ZipFile(zip_root, 'r') as zf:
                # البحث عن أول ملف JSON داخل ZIP
                json_files = [f for f in zf.namelist() if f.endswith('.json')]
                if json_files:
                    with zf.open(json_files[0]) as f:
                        data = json.load(f)
                    primary_exchange.markets = data['markets']
                    primary_exchange.markets_by_id = data['markets_by_id']
                    primary_exchange.symbols = data['symbols']
                    logger.info("✅ تم تحميل الأسواق من الكاش المضغوط في الجذر (markets_cache.zip)")
                    return True
                else:
                    logger.warning("⚠️ ملف ZIP في الجذر لا يحتوي على ملف JSON")
        except Exception as e:
            logger.warning(f"⚠️ فشل تحميل الكاش المضغوط من الجذر: {e}")

    # 2. محاولة تحميل الملف المضغوط من مجلد data/ (احتياطي)
    zip_data = "data/" + MARKETS_CACHE_FILE + ".zip"
        ...
        try:
            with zipfile.ZipFile(zip_data, 'r') as zf:
                ...
                if json_files:
                    ...
                    primary_exchange.markets_by_id = data['markets_by_id']
                    primary_exchange.symbols = data['symbols']
                    logger.info("✅ تم تحميل الأسواق من الكاش المضغوط في data/ (markets_cache.zip)")
                    return True
                else:
                    logger.warning("⚠️ ملف ZIP في data/ لا يحتوي على ملف JSON")
        except Exception as e:
            logger.warning(f"⚠️ فشل تحميل الكاش المضغوط من data/: {e}")

    # 3. محاولة تحميل الملف غير المضغوط (JSON) من الجذر (احتياطي)
    json_root = MARKETS_CACHE_FILE + ".json"
    if os.path.exists(json_root):
        try:
            ...
            primary_exchange.markets_by_id = data['markets_by_id']
            primary_exchange.symbols = data['symbols']
            logger.info("✅ تم تحميل الأسواق من الكاش JSON في الجذر (markets_cache.json)")
            return True
        except Exception as e:
            logger.warning(f"⚠️ فشل تحميل JSON من الجذر: {e}")

    # 4. محاولة تحميل JSON من data/ (احتياطي أخير)
    json_data = "data/" + MARKETS_CACHE_FILE + ".json"
    if os.path.exists(json_data):
        try:
            ...
            primary_exchange.markets_by_id = data['markets_by_id']
            primary_exchange.symbols = data['symbols']
            logger.info("✅ تم تحميل الأسواق من الكاش JSON في data/ (markets_cache.json)")
            return True
        except Exception as e:
            logger.warning(f"⚠️ فشل تحميل JSON من data/: {e}")

    return False
# =============================================================================================

def save_markets_cache():
    """حفظ الأسواق الحالية في ملف JSON في الجذر"""
    try:
        data = {
            'markets': primary_exchange.markets,
            'markets_by_id': primary_exchange.markets_by_id,
            'symbols': primary_exchange.symbols
        }
        # حفظ في الجذر
        file_path = MARKETS_CACHE_FILE + ".json"
        with open(file_path, 'w') as f:
            json.dump(data, f)
        logger.info(f"💾 تم حفظ الأسواق في الكاش المحلي (الجذر) - {file_path}")
    except Exception as e:
        logger.warning(f"⚠️ فشل حفظ الكاش: {e}")
...
# --------------------------- كلاس BotStats ---------------------------
class BotStats:
    __slots__ = ('total_pnl_usdt','winning_trades','losing_trades','total_trades','daily_pnl','weekly_pnl','weekly_wins','weekly_losses','last_balance','start_time','last_valid_tickers','last_tickers_time','equity_curve','symbol_performance','last_week_number')
    def __init__(self):
        ...
        self.equity_curve = deque(maxlen=200)   # تم التخفيض من 1000
...
# --------------------------- كلاس Position ---------------------------
class Position:
    __slots__ = ('symbol','side','total_size','remaining_size','entry_price','highest_price','lowest_price','stop_loss','symbol_type','take_profit_levels','trailing_stop','trailing_activated','atr','open_time','closed_pnl','pred','confidence','regime','initial_momentum','entry_momentum_time','momentum_decay_threshold','momentum_check_minutes','max_no_profit_minutes','last_fail_time','retry_count','crash_monitor_start','lowest_drop','_closing','preemptive_sl_order_id','last_trailing_stop_sent','last_target_hit_time','last_target_hit_index','sold_at_15','sold_at_20','tp_hit_count','ai_approved')
...
# ==================== دالة تحميل الأسواق مع إعادة المحاولة (معدلة) ====================
def load_markets_with_retry(max_retries=10, initial_delay=60):
    # 1. محاولة التحميل من الملف المحلي (يدعم ZIP و JSON)
    if load_markets_from_cache():
        logger.info("📂 استخدام ملف الأسواق المخزن محلياً (من GitHub)")
        return True

    # 2. إذا لم يوجد الملف، نحمّل من Binance
    logger.info("🌐 لم يتم العثور على ملف أسواق محلي، جاري التحميل من Binance...")
    for attempt in range(max_retries):
        if attempt == 0:
            wait = initial_delay
        else:
            wait = min(300, 30 * (1.5 ** (attempt - 1)))   # زيادة الحد الأقصى إلى 300 ثانية ونمو أقل حدة
        logger.info(f"⏳ انتظار {wait} ثانية قبل المحاولة {attempt+1}/{max_retries} لتحميل الأسواق...")
        time.sleep(wait)

        # استخدام نظام الوزن المخصص
        rest_rate_limiter.wait_if_needed(weight=20)

        try:
            primary_exchange.load_markets()
            if 'BTC/USDT' in primary_exchange.markets:
                logger.info("✅ تم تحميل الأسواق بنجاح من Binance")
                save_markets_cache()   # حفظ الملف لاستخدامه لاحقاً
                send_telegram("📂 تم حفظ ملف الأسواق محلياً (يمكنك طلبه بأمر `ارسل ملف الأسواق`)")
                return True
            else:
            ...
            if "418" in error_msg or "banned" in error_msg or "too many" in error_msg or "Way too much" in error_msg:
                logger.critical(f"🚨 IP لا يزال محظوراً. الانتظار 5 دقائق ثم إعادة المحاولة.")
...
class SimpleWebSocketManager:
    ...
    def cleanup_ticker_cache(self, max_age=300):
        """حذف المدخلات الأقدم من max_age ثانية"""
        now = time.time()
        with self.lock:
            keys_to_delete = [k for k, v in self.ticker_cache.items() if now - v.get('timestamp', 0) > max_age]
            for k in keys_to_delete:
                del self.ticker_cache[k]
            if len(keys_to_delete) > 0:
                logger.debug(f"🧹 تنظيف ticker_cache: حذف {len(keys_to_delete)} مدخلاً")
            ...
            if resp.status_code == 200:
                ...
                if cap > 0:
                    with _market_cap_cache_lock:
                        ...
                        # تطبيق الحد الأقصى للكاش - إصلاح: استخدام oldest مباشرة
                        if len(_market_cap_cache) > MARKET_CAP_CACHE_MAX:
                            oldest = min(_market_cap_cache.items(), key=lambda x: x[1]['timestamp'])[0]
                            del _market_cap_cache[oldest]
                    return cap
        except:
            pass
...
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=500, max_retries=4):
    ...
    while attempt < max_retries:
        try:
            ...
            actual_limit = min(limit, 200)   # تحديد حد أقصى 200 شمعة لتوفير الذاكرة
            if timeframe == '4h':
                actual_limit = min(actual_limit, 200)
...
def get_cached_features(symbol, timeframe, limit=500, ttl=20):   # ttl مخفض إلى 20 ثانية
    if limit > 200:
        limit = 200   # تحديد حد أقصى 200
    cache_key = f"{symbol}_{timeframe}_{limit}"
    now = time.time()
...
def get_30d_performance(symbol, days=None):
    ...
    try:
        ...
        with _30d_cache_lock:
            ...
            # إصلاح: استخدام oldest مباشرة
            if len(_30d_performance_cache) > _30d_performance_cache_max:
...
<head><meta charset="UTF-8"><title>بوت v27.8 (معدل لـ Spot)</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v27.8 (Spot Edition)</h1>
<div id="stats"></div>
...
def get_amount_limits(symbol, max_age=3600):
    ...
    for attempt in range(2):
        try:
            ...
            # تطبيق الحد الأقصى للكاش
            if len(_amount_limits_cache) > AMOUNT_LIMITS_CACHE_MAX:
                sorted_keys = sorted(_amount_limits_cache_time.items(), key=lambda x: x[1])
                for k, _ in sorted_keys[:len(_amount_limits_cache) - AMOUNT_LIMITS_CACHE_MAX]:
                    del _amount_limits_cache[k]
                    del _amount_limits_cache_time[k]
...
def cleanup_amount_limits_cache():
    """تنظيف _amount_limits_cache من المدخلات القديمة والتي تزيد عن الحد"""
    now = time.time()
    # حذف المدخلات الأقدم من ساعة
    keys_to_delete = [k for k, t in _amount_limits_cache_time.items() if now - t > 3600]
    for k in keys_to_delete:
        _amount_limits_cache.pop(k, None)
        _amount_limits_cache_time.pop(k, None)
    # إذا زاد الحجم عن الحد، احذف الأقدم
    if len(_amount_limits_cache) > AMOUNT_LIMITS_CACHE_MAX:
        sorted_keys = sorted(_amount_limits_cache_time.items(), key=lambda x: x[1])
        for k, _ in sorted_keys[:len(_amount_limits_cache) - AMOUNT_LIMITS_CACHE_MAX]:
            _amount_limits_cache.pop(k, None)
...
class OrderBookManipulationDetector:
    __slots__ = ('order_snapshots','spoofing_history','wall_history','last_cleanup')
    def __init__(self):
    ...
    def cleanup(self, max_age=300):
        """حذف المفاتيح التي لم تُستخدم منذ max_age ثانية"""
        now = time.time()
        keys_to_delete = []
        for sym in list(self.order_snapshots.keys()):
            if now - self.order_snapshots[sym].get('timestamp', 0) > max_age:
                keys_to_delete.append(sym)
        for sym in keys_to_delete:
            del self.order_snapshots[sym]
            if sym in self.spoofing_history:
                del self.spoofing_history[sym]
            if sym in self.wall_history:
                del self.wall_history[sym]
        if len(keys_to_delete) > 0:
            logger.debug(f"🧹 تنظيف OrderBookManipulationDetector: حذف {len(keys_to_delete)} رمزاً")
...
class WhaleManipulationDetector:
    __slots__ = ('trade_history','whale_trades','last_price','last_price_time','volume_history')
    def __init__(self):
    ...
    def cleanup(self, max_age=300):
        """حذف المفاتيح التي لا تحتوي على بيانات حديثة"""
        now = time.time()
        keys_to_delete = []
        for sym in list(self.trade_history.keys()):
            # نتحقق من وجود أي عنصر في أي من الـ deques
            has_recent = False
            if self.trade_history[sym]:
                # نتحقق من آخر عنصر في trade_history
                if now - self.trade_history[sym][-1].get('time', 0) < max_age:
                    has_recent = True
            if not has_recent and self.whale_trades[sym]:
                if now - self.whale_trades[sym][-1].get('time', 0) < max_age:
                    has_recent = True
            if not has_recent and self.volume_history[sym]:
                # volume_history لا يحتوي على طابع زمني، نعتمد على وجود عناصر
                pass
            if not has_recent:
                keys_to_delete.append(sym)
        for sym in keys_to_delete:
            del self.trade_history[sym]
            del self.whale_trades[sym]
            del self.volume_history[sym]
        if len(keys_to_delete) > 0:
            logger.debug(f"🧹 تنظيف WhaleManipulationDetector: حذف {len(keys_to_delete)} رمزاً")
        ...
        if recovery_attempts_remaining > 0:
            ...
            if recovery_attempts_remaining == 0:
                ...
                try:
                    ...
                    _amount_limits_cache.clear()
                    _amount_limits_cache_time.clear()
                    ws_manager.cleanup_ticker_cache(max_age=0)  # تفريغ الكل
                    order_book_detector.cleanup(max_age=0)
                    whale_detector.cleanup(max_age=0)
...
def cache_cleanup_thread():
    last_full_cleanup = 0
    while True:
        time.sleep(90)  # أصبح 90 ثانية بدلاً من 300
        now = time.time()
        with _cache_lock:
        ...
        # تنظيف المخابئ الإضافية
        cleanup_amount_limits_cache()
        ws_manager.cleanup_ticker_cache(max_age=300)  # 5 دقائق
        order_book_detector.cleanup(max_age=300)
        whale_detector.cleanup(max_age=300)
        # تنظيف _30d_performance_cache - إصلاح
        with _30d_cache_lock:
            if len(_30d_performance_cache) > _30d_performance_cache_max:
                oldest = min(_30d_performance_cache.items(), key=lambda x: x[1]['timestamp'])[0]
                del _30d_performance_cache[oldest]
        # تنظيف _market_cap_cache - إصلاح
        with _market_cap_cache_lock:
            if len(_market_cap_cache) > MARKET_CAP_CACHE_MAX:
                oldest = min(_market_cap_cache.items(), key=lambda x: x[1]['timestamp'])[0]
                del _market_cap_cache[oldest]
...
def memory_watchdog():
    while True:
        ...
        try:
            ...
            if memory_mb > 260:  # أصبح 260 ميجابايت بدلاً من 1024
                logger.warning(f"⚠️ استهلاك الذاكرة: {memory_mb:.0f}MB - تنظيف احتياطي")
                with _global_state_lock:
                    if len(bot_stats.equity_curve) > 200:
                        bot_stats.equity_curve = deque(list(bot_stats.equity_curve)[-200:], maxlen=200)
                with _cache_lock:
                    _ohlcv_cache.clear()
                    _manipulation_cache.clear()
                with _features_cache_lock:
                ...
                if memory_after > 350:
                    ...
                    send_telegram(f"🚨 ذاكرة عالية جداً ({memory_after:.0f}MB) - تم تنظيف جميع المخابئ")
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                                ...
                                if not GEMINI_API_KEY:
                                    ...
                                    # تحديث النماذج أولاً (ثابتة، لا تبحث عن الأحدث) - الآن لا تفعل شيئاً
                                    # refresh_gemini_models()  # تم إزالتها
                                ...
                                # لا حاجة لـ refresh_gemini_models() لأن النماذج ثابتة
                            ...
                            # ------------------------------------------------------------
                                ...
                                STOP_LOSS_PARTIAL_2_PERCENT = 0.012
                                STOP_LOSS_FULL_PERCENT = 0.017
                                # إعادة ضبط الإعدادات الديناميكية إلى القيم الافتراضية (من المتغيرات البيئية أو الثوابت)
                                CUSTOM_ACTIVE_SYMBOLS_LIMIT = ACTIVE_SYMBOLS_LIMIT
                                CUSTOM_MAX_EXPOSED_PERCENT = MAX_EXPOSED_PERCENT
                                CUSTOM_POSITION_PERCENT_GOOD = MAX_POSITION_PERCENT_GOOD
                                ...
                                AI_ENABLED = False
                                CURRENT_AI_MODEL = 'flash'
                                # إعادة ضبط إعدادات الفلتر الجديد
                                ...
                                with _30d_cache_lock:
                                    _30d_performance_cache.clear()
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (الذكاء الاصطناعي معطل، النموذج الافتراضي Flash)")
                                save_state()
                            ...
                            # ==================== أمر تلغرام الجديد المحدث ====================
                            elif text == 'ارسل ملف الأسواق' or text == 'ارسل الكاش':
                                # محاولة إرسال الملف المضغوط من الجذر أولاً
                                zip_root = MARKETS_CACHE_FILE + ".zip"
                                if os.path.exists(zip_root):
                                    send_telegram_file(zip_root, "📂 ملف الأسواق المضغوط (الجذر - markets_cache.zip)")
                                elif os.path.exists("data/" + MARKETS_CACHE_FILE + ".zip"):
                                    send_telegram_file("data/" + MARKETS_CACHE_FILE + ".zip", "📂 ملف الأسواق المضغوط (data/)")
                                elif os.path.exists(MARKETS_CACHE_FILE + ".json"):
                                    send_telegram_file(MARKETS_CACHE_FILE + ".json", "📂 ملف الأسواق (JSON) في الجذر")
                                elif os.path.exists("data/" + MARKETS_CACHE_FILE + ".json"):
                                    send_telegram_file("data/" + MARKETS_CACHE_FILE + ".json", "📂 ملف الأسواق (JSON) في data/")
                                else:
                                    send_telegram("⚠️ لا يوجد ملف أسواق مخزن حالياً. سيتم إنشاؤه عند أول تحميل ناجح من Binance.")
                            # =================================================================================

                            # ==================== أمر تلغرام لعرض الملفات الضخمة ====================
                            elif text == 'الذاكرة' or text == 'حجم الملفات' or text == 'ملفات':
                                try:
                                    files_info = []
                                    dirs_to_scan = ['.', 'data']
                                        ...
                                        for root, dirs, files in os.walk(dir_name):
                                            ...
                                            for file in files:
                                                ...
                                                try:
                                                    ...
                                                    if size > 1024 * 1024:  # أكبر من 1 ميجابايت
                                                        files_info.append((file_path, size))
                                                except:
                                                    continue
                                    files_info.sort(key=lambda x: x[1], reverse=True)
                                    top_files = files_info[:15]
                                    if not top_files:
                                        send_telegram("📂 لا توجد ملفات كبيرة (أكبر من 1 ميجابايت) في المجلدات المهمة.")
                                    else:
                                        msg = "<b>📊 الملفات الأكبر حجماً (بالميجابايت)</b>\n"
                                        total_size = 0
                                        for path, size in top_files:
                                            size_mb = size / (1024 * 1024)
                                            msg += f"• {path}: {size_mb:.2f} MB\n"
                                            total_size += size
                                        total_mb = total_size / (1024 * 1024)
                                        msg += f"\n<b>📦 إجمالي حجم هذه الملفات: {total_mb:.2f} MB</b>"
                                        send_telegram(msg)
                                except Exception as e:
                                    send_telegram(f"⚠️ حدث خطأ أثناء جلب أحجام الملفات: {e}")
                            # ===================================================================
...
# --------------------------- كلاس MarketScanner ---------------------------
class MarketScanner:
    __slots__ = ('last_scan','candidates','scores')
    def __init__(self):
        self.last_scan = 0
        self.candidates = []
        self.scores = {}
...
def save_state():
    if not _global_state_lock.acquire(timeout=5):
        ...
        return
    try:
        # تقليل حجم equity_curve إلى 200
        if len(bot_stats.equity_curve) > 200:
            bot_stats.equity_curve = deque(list(bot_stats.equity_curve)[-200:], maxlen=200)
        # تقليل حجم symbol_performance إلى 100
        if len(bot_stats.symbol_performance) > 100:
            sorted_items = sorted(bot_stats.symbol_performance.items(), key=lambda x: x[1], reverse=True)
            bot_stats.symbol_performance = dict(sorted_items[:100])
        pos_dict = {}
        for sym, p in open_positions.items():
        ...
        data = {
            ...
            'bot_stats':{
                ...
                'equity_curve':list(bot_stats.equity_curve)[-200:],
...
def load_state():
    ...
    try:
        ...
        bot_stats.equity_curve = deque(equity[-200:], maxlen=200) if equity else deque(maxlen=200)
...
if __name__ == '__main__':
    ...
    # ==================== تحميل الأسواق (يدعم ZIP و JSON) ====================
    if not load_markets_with_retry(max_retries=10, initial_delay=60):
    ...
    # تأكيد وجود ملف الكاش (في حال لم يتم حفظه داخل الدالة)
    # إذا كان ZIP موجوداً، لا نحتاج لحفظ JSON جديد
    if not os.path.exists(MARKETS_CACHE_FILE + ".zip") and not os.path.exists(MARKETS_CACHE_FILE + ".json"):
        save_markets_cache()
    # ========================================================================
