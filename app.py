...
#   - التحديث 12: إضافة فلتر سادس (حد أدنى) نموذج VWAP_OBV على 1h عتبة 0.44.
#   - التحديث 13: إضافة فلتر سابع (حد أدنى) نموذج Timing على 15m عتبة 0.35.
...
SECOND_MODEL_FILTER_THRESHOLD = float(os.environ.get('SECOND_MODEL_FILTER_THRESHOLD', 0.59))
...
SIXTH_MODEL_FILTER_THRESHOLD = float(os.environ.get('SIXTH_MODEL_FILTER_THRESHOLD', 0.44))
...
SEVENTH_MODEL_FILTER_THRESHOLD = float(os.environ.get('SEVENTH_MODEL_FILTER_THRESHOLD', 0.35))
                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                            # 13. أوامر الفلاتر (الأول، الثاني، الثالث، الرابع، الخامس، السادس، السابع)
                            # ------------------------------------------------------------
                            # === فلتر النموذج السابع (حد أدنى - جديد) ===
                            ...
                            # ------------------------------------------------------------
                                ...
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية (مع فلاتر النماذج الأول والثاني والثالث والرابع والخامس والسادس والسابع).")
                                save_state()
...
def load_state():
    ...
    try:
        ...
        # === قراءة فلتر النموذج السادس (جديد) ===
        SIXTH_MODEL_FILTER_MODEL = filters.get('sixth_model_filter_model', 'vwap_obv')
        SIXTH_MODEL_FILTER_THRESHOLD = filters.get('sixth_model_filter_threshold', 0.44)
        ...
        # === قراءة فلتر النموذج السابع (جديد) ===
        SEVENTH_MODEL_FILTER_MODEL = filters.get('seventh_model_filter_model', 'timing')
        SEVENTH_MODEL_FILTER_THRESHOLD = filters.get('seventh_model_filter_threshold', 0.35)
...
if __name__ == '__main__':
    ...
    send_telegram(f"🚀 بوت v28.0 (الإصدار المحسن بـ 3 أطر زمنية 5م، 15د، 1س مع أوزان ديناميكية وتوصيات ذكية) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"✅ **التحديث 12: إضافة فلتر سادس (حد أدنى) نموذج VWAP_OBV على 1h عتبة 0.44**\n"
                  f"✅ **التحديث 13: إضافة فلتر سابع (حد أدنى) نموذج Timing على 15m عتبة 0.35**")
