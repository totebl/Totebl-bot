...
def calculate_suggested_weights():
    """تحليل آخر 100 صفقة واقتراح الأوزان المثلى للنماذج والأطر باستخدام معامل الارتباط"""
    ...
    if len(df) < 5:
        return None, f"⚠️ عدد الصفقات قليل جداً ({len(df)}). احتاج 5 صفقات على الأقل."

    # اقتصار التحليل على آخر 100 صفقة لضمان الحداثة
    df = df.tail(100)

    # أنماط الاستخراج
    pattern_models = r"Rule=([\d.]+)\s*\|\s*EMR=([\d.]+)\s*\|\s*CFHM=([\d.]+)\s*\|\s*Timing=([\d.]+)\s*\|\s*VWAP=([\d.]+)"
    
    # قوائم لتخزين الدرجات مع الأرباح المقابلة
    model_data = {m: {'scores': [], 'pnls': []} for m in ['rule','emr','cfhm','timing','vwap_obv']}
    frame_data = {'5m': {'scores': [], 'pnls': []}, '15m': {'scores': [], 'pnls': []}, '1h': {'scores': [], 'pnls': []}}
    ...
    for idx, row in df.iterrows():
        ...
        # 1. استخراج درجات النماذج
        match_m = re.search(pattern_models, reason)
        if match_m:
            vals = list(map(float, match_m.groups()))
            names = ['rule', 'emr', 'cfhm', 'timing', 'vwap_obv']
            for i, name in enumerate(names):
                model_data[name]['scores'].append(vals[i])
                model_data[name]['pnls'].append(pnl)

        # 2. استخراج درجات الأطر (نأخذ متوسط درجات الأعضاء الخمسة لكل إطار)
        try:
            # إطار 5 دقائق
            match_5m = re.search(r"📊 إطار 5 دقائق:\n\s+Rule=([\d.]+)\s*\|\s*EMR=([\d.]+)\s*\|\s*CFHM=([\d.]+)\s*\|\s*Timing=([\d.]+)\s*\|\s*VWAP=([\d.]+)", reason)
            if match_5m:
                vals = list(map(float, match_5m.groups()))
                avg_score = sum(vals) / len(vals)
                frame_data['5m']['scores'].append(avg_score)
                frame_data['5m']['pnls'].append(pnl)
            
            match_15m = re.search(r"📊 إطار 15 دقيقة:\n\s+Rule=([\d.]+)\s*\|\s*EMR=([\d.]+)\s*\|\s*CFHM=([\d.]+)\s*\|\s*Timing=([\d.]+)\s*\|\s*VWAP=([\d.]+)", reason)
            if match_15m:
                vals = list(map(float, match_15m.groups()))
                avg_score = sum(vals) / len(vals)
                frame_data['15m']['scores'].append(avg_score)
                frame_data['15m']['pnls'].append(pnl)
            
            match_1h = re.search(r"📊 إطار 1 ساعة:\n\s+Rule=([\d.]+)\s*\|\s*EMR=([\d.]+)\s*\|\s*CFHM=([\d.]+)\s*\|\s*Timing=([\d.]+)\s*\|\s*VWAP=([\d.]+)", reason)
            if match_1h:
                vals = list(map(float, match_1h.groups()))
                avg_score = sum(vals) / len(vals)
                frame_data['1h']['scores'].append(avg_score)
                frame_data['1h']['pnls'].append(pnl)
        except:
            continue

    # --- حساب الأوزان للنماذج باستخدام معامل الارتباط ---
    model_corr = {}
    for m in model_data:
        if len(model_data[m]['scores']) > 5:
            corr = np.corrcoef(model_data[m]['scores'], model_data[m]['pnls'])[0, 1]
            model_corr[m] = max(0, corr if not np.isnan(corr) else 0)
        else:
            model_corr[m] = 0

    total_corr = sum(model_corr.values())
    if total_corr == 0:
        model_weights = {m: 0 for m in model_corr}
        model_msg = "⚠️ لا يوجد ارتباط إيجابي. يوصى باستخدام Timing و CFHM فقط."
    else:
        model_weights = {m: (v / total_corr) for m, v in model_corr.items()}
        # نضمن حداً أدنى 5%
        for m in model_weights:
            if model_weights[m] < 0.05 and model_weights[m] > 0:
                model_weights[m] = 0.05
        total = sum(model_weights.values())
        model_weights = {m: v/total for m, v in model_weights.items()}
        model_msg = "✅ بناءً على معامل الارتباط مع الربح:"

    # --- حساب الأوزان للأطر ---
    frame_corr = {}
    for f in frame_data:
        if len(frame_data[f]['scores']) > 5:
            corr = np.corrcoef(frame_data[f]['scores'], frame_data[f]['pnls'])[0, 1]
            frame_corr[f] = max(0, corr if not np.isnan(corr) else 0)
        else:
            frame_corr[f] = 0

    total_frame_corr = sum(frame_corr.values())
    if total_frame_corr == 0:
        frame_weights = {'5m': 0, '15m': 0, '1h': 0}
        frame_msg = "⚠️ لا يوجد ارتباط. يُنصح بالاحتفاظ بالأوزان الحالية (10، 30، 60)."
    else:
        frame_weights = {f: (v / total_frame_corr) for f, v in frame_corr.items()}
        frame_msg = "✅ بناءً على معامل الارتباط مع الربح:"

    # --- بناء الرسالة النهائية ---
    result_msg = f"<b>📊 توصية الأوزان المثلى</b>\n(تحليل آخر {len(df)} صفقة)\n\n"
    
    result_msg += f"<b>🤖 أوزان النماذج:</b>\n"
    if total_corr > 0:
        name_map = {'rule':'Rule','emr':'EMR','cfhm':'CFHM','timing':'Timing','vwap_obv':'VWAP'}
        for m in ['rule', 'emr', 'cfhm', 'timing', 'vwap_obv']:
            pct = model_weights.get(m, 0) * 100
            corr_val = model_corr.get(m, 0)
            result_msg += f"   {name_map[m]}: {pct:.1f}% (ارتباط {corr_val:+.3f})\n"
        tg_cmd = f"تعيين أوزان النماذج Rule={int(model_weights.get('rule',0)*100)} EMR={int(model_weights.get('emr',0)*100)} CFHM={int(model_weights.get('cfhm',0)*100)} Timing={int(model_weights.get('timing',0)*100)} VWAP={int(model_weights.get('vwap_obv',0)*100)}"
        result_msg += f"\n📨 <code>{tg_cmd}</code>\n"
    else:
        result_msg += "   ⚠️ يوصى باستخدام نموذجي Timing و CFHM مؤقتاً.\n"
        result_msg += f"\n📨 <code>تعيين أوزان النماذج Rule=0 EMR=0 CFHM=40 Timing=60 VWAP=0</code>\n"

    result_msg += f"\n<b>⏱️ أوزان الأطر الزمنية (5م، 15د، 1س):</b>\n"
    if total_frame_corr > 0:
        for f in ['5m', '15m', '1h']:
            pct = frame_weights.get(f, 0) * 100
            corr_val = frame_corr.get(f, 0)
            result_msg += f"   {f}: {pct:.1f}% (ارتباط {corr_val:+.3f})\n"
        tg_cmd_frames = f"تعيين أوزان الأطر {int(frame_weights.get('5m',0)*100)} {int(frame_weights.get('15m',0)*100)} {int(frame_weights.get('1h',0)*100)}"
        result_msg += f"\n📨 <code>{tg_cmd_frames}</code>\n"
    else:
        result_msg += "   ⚠️ يُنصح بالاحتفاظ بالأوزان الحالية (10، 30، 60).\n"
