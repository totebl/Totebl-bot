...
class SimpleWebSocketManager:
    ...
    def start(self):
        ...
        # 🔥 إعداد الوكيل لـ WebSocket باستخدام setdefaultproxy
        proxy_url = os.environ.get('HTTP_PROXY') or os.environ.get('HTTPS_PROXY')
        if proxy_url:
            # استخراج المضيف والمنفذ من الرابط
            proxy_url_clean = proxy_url.replace('http://', '').replace('https://', '')
            if ':' in proxy_url_clean:
                parts = proxy_url_clean.split(':')
                proxy_host = parts[0]
                proxy_port = int(parts[1]) if parts[1].isdigit() else 3128
            else:
                proxy_host = proxy_url_clean
                proxy_port = 3128
            websocket.setdefaultproxy(proxy_type="http", host=proxy_host, port=proxy_port)
            logger.info(f"✅ سيتم استخدام وكيل WebSocket: {proxy_host}:{proxy_port}")
        else:
            # إلغاء تعيين الوكيل
            websocket.setdefaultproxy(proxy_type=None, host=None, port=None)
