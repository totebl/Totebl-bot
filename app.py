...
# تم تعديل آلية التعامل مع حظر Binance: تأخير متزايد بحد أقصى 60 ثانية بدون إيقاف التداول
# تم إضافة أمرين تلغرام: إيقاف التحليل / استئناف التحليل (بدون إغلاق المراكز)
# تم إضافة Slow Mode في RateLimiter بعد الاستئناف اليدوي
# ========================================================================
...
# --------------------------- المتغيرات العامة ---------------------------
PAUSED = False
analysis_active = True          # متغير جديد للتحكم في تشغيل/إيقاف التحليل (بدون إغلاق المراكز)
...
# --------------------------- Rate Limiter معدل لدعم Slow Mode ---------------------------
class RateLimiter:
    def __init__(self, max_calls=50, period=60):
        ...
        self.period = period
        self.lock = threading.Lock()
        self.slow_mode_until = 0

    def set_slow_mode(self, duration_seconds, slow_max_calls=10):
        with self.lock:
            self.slow_mode_until = time.time() + duration_seconds
            self.max_calls = slow_max_calls
            self.calls.clear()
            logger.info(f"🐢 تفعيل الوضع الهادئ: {slow_max_calls} طلب/دقيقة لمدة {duration_seconds} ثانية")
            send_telegram(f"🐢 تم تفعيل الوضع الهادئ ({slow_max_calls} طلب/دقيقة) لمدة {duration_seconds//60} دقيقة")

    def _update_mode(self):
        now = time.time()
        if self.slow_mode_until and now >= self.slow_mode_until:
            self.max_calls = self.default_max_calls
            self.slow_mode_until = 0
            logger.info("✅ العودة إلى الوضع العادي لـ RateLimiter (60 طلب/دقيقة)")
...
# --------------------------- دوال جلب OHLCV (مع Rate Limiter وتأخير متزايد عند الحظر) ---------------------------
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=500, max_retries=100):
    time.sleep(0.1)
    exchange = get_active_exchange()
    last_err = ""
    for attempt in range(max_retries):
        try:
            ...
            # الكشف عن حظر Binance (418 أو banned)
            if "418" in str(e) or "banned" in str(e).lower():
                wait = min(60, 2 ** (attempt + 1))   # تأخير متزايد بحد أقصى 60 ثانية
                if attempt == 0:
                    send_telegram(f"⚠️ حظر Binance مؤقت لـ {symbol} - انتظار {wait} ثانية ثم إعادة المحاولة تلقائياً.")
                logger.info(f"🚫 حظر Binance مؤقت، انتظار {wait} ثانية (محاولة {attempt+1})")
                time.sleep(wait)
            else:
                # أخطاء أخرى: تأخير عادي
...
def fetch_ohlcv_persistent(symbol, timeframe, limit=500, max_attempts=5, retry_interval=10):
    for attempt in range(max_attempts):
        df = fetch_ohlcv_retry_raw(symbol, timeframe, limit, max_retries=100)
        if not df.empty:
            return df
...
def execute_limit_order(symbol, side, size, price_ref, sym_type='normal'):
    ...
    for attempt in range(2):
        try:
            ...
            if "418" in str(e) or "banned" in str(e).lower():
                # حظر Binance أثناء تنفيذ الأمر: تطبيق تأخير متزايد وإعادة المحاولة (حتى 100 مرة)
                logger.warning(f"🚫 حظر Binance أثناء تنفيذ أمر {symbol} - سيتم إعادة المحاولة لاحقاً")
                send_telegram(f"⚠️ حظر Binance أثناء تنفيذ أمر {symbol} - سيتم إعادة المحاولة تلقائياً.")
                time.sleep(60)  # ننتظر دقيقة قبل إعادة المحاولة (يمكن تعديلها)
                return False, None, 0, None
...
def manual_resume():
    global daily_loss_cooldown_until, _daily_loss_tracker, analysis_active
    with _global_state_lock:
        _set_paused(False)
        daily_loss_cooldown_until = None
        analysis_active = True   # إعادة تفعيل التحليل
        # تفعيل Slow Mode لمدة 3 دقائق (180 ثانية) بـ 10 طلبات/دقيقة
        rest_rate_limiter.set_slow_mode(duration_seconds=180, slow_max_calls=10)
        warnings_list = []
...
def analyze_and_trade():
    ...
    global _daily_total_holding_time_win, _daily_total_holding_time_loss, _daily_holding_count_win, _daily_holding_count_loss
    global _daily_biggest_win, _daily_biggest_loss, _daily_most_traded
    global buying_committee, _analysis_failures, analysis_active

    # إذا كان التحليل متوقفاً (بأمر تلغرام)، لا نفعل شيئاً
                    ...
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            if text.startswith('تعيين عتبة خسارة 25%'):
                                ...
                                manual_resume()
                            # الأوامر الجديدة للتحكم في التحليل
                            elif text == "إيقاف التحليل":
                                with _global_state_lock:
                                    analysis_active = False
                                    send_telegram("⏸️ تم إيقاف التحليل (البحث عن صفقات جديدة).\n✅ مراقبة المراكز المفتوحة مستمرة.")
                                    save_state()
                            elif text == "استئناف التحليل":
                                with _global_state_lock:
                                    analysis_active = True
                                    send_telegram("▶️ تم استئناف التحليل (البحث عن صفقات جديدة).")
                                    save_state()
...
def save_state():
    ...
    try:
        ...
        data = {
            ...
            'analysis_active': analysis_active,   # حفظ حالة التحليل
            'bot_stats':{
...
def load_state():
    ...
    global MANIPULATION_REJECT_THRESHOLD, SELL_DECISION_THRESHOLD
    global STOP_LOSS_PARTIAL_1_PERCENT, STOP_LOSS_PARTIAL_2_PERCENT, STOP_LOSS_FULL_PERCENT
    global buying_committee, analysis_active
    ...
    try:
        ...
        analysis_active = data.get('analysis_active', True)  # استعادة حالة التحليل
        ...
        logger.info(f"تم استعادة {len(open_positions)} مركزاً، التحليل مفعل: {analysis_active}")
        try:
