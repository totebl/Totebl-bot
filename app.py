...
#   - نظام تناوب بروكسيات متعدد المصادر (ProxyScrape, Proxy-List.Download, Geonode, OpenProxyList, Spys.me)
#     مع اختبار تلقائي وتحديث دوري كل 5 دقائق.
#   - إعادة ترتيب بدء التشغيل: تشغيل Flask أولاً لفتح المنفذ فوراً، ثم المهام الأخرى في الخلفية
#   - إلغاء الاختبار المسبق للبروكسيات، واعتماد التبديل العشوائي الفوري عند الفشل
#   - إزالة جميع فترات الانتظار الطويلة (5 دقائق، 10 دقائق) عند حظر IP، واستبدالها بالتبديل الفوري
...
# --------------------------- إعدادات التسجيل (نقلت إلى الأعلى) ---------------------------
log_file = "bot.log"
...
# =====================================================================
# 0. إعدادات البروكسي (توضع فوراً بعد الاستيرادات لتكون جاهزة قبل أي اتصال)
# =====================================================================

# منع توجيه الطلبات الداخلية (لخدمة الإيقاظ) عبر البروكسي
os.environ['NO_PROXY'] = 'localhost,127.0.0.1,.onrender.com'
...
# =====================================================================
# 1. دوال جلب البروكسيات من مصادر متعددة
# =====================================================================

def fetch_proxies_from_proxyscrape():
    """المصدر 1: ProxyScrape (HTTP/HTTPS)"""
    url = "https://api.proxyscrape.com/v2/?request=getproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all"
    try:
        resp = requests.get(url, timeout=10)
        if resp.status_code == 200:
            return [f"http://{p.strip()}" for p in resp.text.split('\r\n') if ':' in p.strip()]
    except Exception as e:
        logger.warning(f"⚠️ ProxyScrape فشل: {e}")
    return []

def fetch_proxies_from_proxy_list_download():
    ...
    try:
        ...
        logger.warning(f"⚠️ Proxy-List.Download فشل: {e}")
    return []

def fetch_proxies_from_geonode():
    """المصدر 3: Geonode (يدعم HTTP/HTTPS)"""
    url = "https://api.geonode.com/api/v1/proxy?limit=50&protocols=http%2Chttps"
    try:
        resp = requests.get(url, timeout=10)
        if resp.status_code == 200:
            data = resp.json()
        ...
        logger.warning(f"⚠️ Geonode فشل: {e}")
    return []

def fetch_proxies_from_openproxy():
    """المصدر 4: OpenProxyList (نصي)"""
    ...
    try:
        ...
        logger.warning(f"⚠️ OpenProxyList فشل: {e}")
    return []

def fetch_proxies_from_spys():
    """المصدر 5: Spys.me (نصي، يحتاج معالجة بسيطة)"""
    ...
    try:
        ...
        logger.warning(f"⚠️ Spys.me فشل: {e}")
    return []

PROXY_SOURCES = [
    fetch_proxies_from_proxyscrape,
    fetch_proxies_from_proxy_list_download,
    fetch_proxies_from_geonode,
    fetch_proxies_from_openproxy,
    fetch_proxies_from_spys,
]

def fetch_all_free_proxies():
    """جلب البروكسيات من جميع المصادر، مع إزالة التكرارات."""
    all_proxies = set()
    for source_func in PROXY_SOURCES:
        try:
            proxies = source_func()
            if proxies:
                logger.info(f"✅ [{source_func.__name__}] جلب {len(proxies)} بروكسي")
                all_proxies.update(proxies)
            else:
                logger.warning(f"⚠️ [{source_func.__name__}] لم يعثر على بروكسيات")
        except Exception as e:
            logger.error(f"❌ خطأ في {source_func.__name__}: {e}")
    result = list(all_proxies)
    logger.info(f"📊 إجمالي البروكسيات الفريدة: {len(result)}")
    return result

# =====================================================================
# 2. كلاس ProxyRotator (إدارة وتناوب البروكسيات) - معدل بالكامل
# =====================================================================

class ProxyRotator:
    def __init__(self):
        self.all_proxies = []          # القائمة الكاملة للبروكسيات (بدون اختبار)
        self.last_fetch_time = 0
        self.lock = threading.Lock()
        self.current_proxy = None

    def _fetch_proxies(self):
        """جلب البروكسيات من جميع المصادر (بدون اختبار)"""
        logger.info("🔄 جلب البروكسيات من جميع المصادر (بدون اختبار)...")
        all_new_proxies = fetch_all_free_proxies()
        if not all_new_proxies:
            logger.warning("⚠️ لم يتم العثور على أي بروكسي من أي مصدر.")
            return False

        with self.lock:
            self.all_proxies = all_new_proxies
            self.last_fetch_time = time.time()
            # اختر بروكسي عشوائي كبداية
            if self.all_proxies:
                self.current_proxy = random.choice(self.all_proxies)
                logger.info(f"✅ تم تجهيز {len(self.all_proxies)} بروكسي (بدون اختبار). البروكسي الأول: {self.current_proxy}")
                return True
            else:
                logger.warning("⚠️ لا توجد بروكسيات.")
                return False

    def get_proxy(self):
        """إعادة بروكسي عشوائي صالح (جديد أو من القائمة)"""
        with self.lock:
            now = time.time()
            # إذا كانت القائمة فارغة أو مرت 10 دقائق، قم بتحديثها
            if (not self.all_proxies) or (now - self.last_fetch_time > 600):
                logger.info("⏳ انتهت صلاحية القائمة أو فارغة، جاري التحديث...")
                self._fetch_proxies()
                if not self.all_proxies:
                    return None

            # اختر بروكسي عشوائي من القائمة (بدون ترتيب)
...
# إنشاء كائن عالمي واحد للبروكسي (يُستخدم في جميع الطلبات)
proxy_rotator = ProxyRotator()

# =====================================================================
# 3. دالة تحديث البروكسيات في الخلفية (تمت إضافتها للإصلاح)
# =====================================================================

def background_proxy_updater():
    """خيط خلفي لتحديث قائمة البروكسيات بشكل دوري قبل انتهاء صلاحيتها."""
    while True:
        time.sleep(480)  # كل 8 دقائق (أقل من 10 دقائق)
        try:
            proxy_rotator.get_proxy()  # يُجبر على التحديث إذا لزم الأمر
            logger.debug("🔄 تم تحديث البروكسيات في الخلفية")
        except Exception as e:
            logger.warning(f"⚠️ فشل تحديث البروكسيات في الخلفية: {e}")
...
COOLDOWN_WIN_HOURS = float(os.environ.get('COOLDOWN_WIN_HOURS', 12.0))       # تبريد الصفقة الرابحة (ساعات)
COOLDOWN_LOSS_MINUTES = float(os.environ.get('COOLDOWN_LOSS_MINUTES', 5.0))  # تبريد الصفقة الخاسرة (دقائق)
...
USE_1H_MULTIPLIER = True   # تم تغيير معناه: أصبح يُستخدم أم لا، لكن مصدر المضاعف أصبح 5 دقائق (انظر الشرح)

# --------------------------- فلتر أداء X يوم (عبر ccxt) ---------------------------
FILTER_30D_PROFIT_ENABLED = os.environ.get('FILTER_30D_PROFIT_ENABLED', 'false').lower() == 'true'
MIN_30D_PROFIT_PERCENT = float(os.environ.get('MIN_30D_PROFIT_PERCENT', 0.0))   # 0 = الربح موجب
FILTER_30D_DAYS = int(os.environ.get('FILTER_30D_DAYS', 30))                    # عدد الأيام الافتراضي
...
CUSTOM_PREEMPTIVE_SL_TRIGGER = PREPLACED_SL_TRIGGER_PERCENT   # نسبة التفعيل (افتراضي 1.7%)
CUSTOM_PREEMPTIVE_SL_LIMIT = PREPLACED_SL_LIMIT_PERCENT       # نسبة التنفيذ (افتراضي 2.1%)
# ============================================================

# ========== نظام تتبع الأرباح الجديد (يفعل فوراً) ==========
TRAILING_DISTANCE_PERCENT = float(os.environ.get('TRAILING_DISTANCE_PERCENT', 1.0)) / 100.0  # افتراضي 1%
# ======================================================================
# ========== نظام البيع الجديد (يعتمد على متوسط 5م + 15د) ==========
SELL_15M_THRESHOLD = float(os.environ.get('SELL_15M_THRESHOLD', 0.42))   # عتبة البيع الجديدة
...
CUSTOM_POSITION_PERCENT_NORMAL = 0.25   # قيمة افتراضية جديدة (قابل للتعديل عبر تلغرام)
CUSTOM_POSITION_PERCENT_BAD = 0.20      # قيمة افتراضية جديدة
CUSTOM_MAX_DAILY_TRADES = MAX_DAILY_TRADES   # جديد: عدد الصفقات المسموحة يومياً (قابل للتعديل عبر تلغرام)
CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP = 60   # جديد: مدة الانتظار قبل البيع الكامل إذا لم يحقق الهدف (دقائق)
...
def get_active_exchange():
    # تطبيق البروكسي الحالي إذا كان متوفراً
    proxy = proxy_rotator.get_current_proxy()
    if proxy:
        primary_exchange.proxies = {'http': proxy, 'https': proxy}
    else:
        primary_exchange.proxies = None
    return primary_exchange

# --------------------------- تحميل الأسواق مع صبر شديد (معدل لدعم البروكسي) ---------------------------
def load_markets_with_retry(max_retries=5, initial_delay=2):
    for attempt in range(max_retries):
        try:
            ...
            rest_rate_limiter.wait_if_needed(weight=20)
            if attempt > 0:
                time.sleep(1)  # تأخير بسيط جداً بين المحاولات
            logger.info(f"⏳ محاولة تحميل الأسواق #{attempt+1} عبر بروكسي {proxy if proxy else 'مباشر'}...")
            ...
            error_msg = str(e)
            logger.warning(f"⚠️ فشل تحميل الأسواق (محاولة {attempt+1}/{max_retries}): {e}")
            # إذا كان خطأ 418/429/403، فقط انتقل إلى البروكسي التالي (التبديل فوري)
            if "418" in error_msg or "429" in error_msg or "403" in error_msg or "banned" in error_msg or "too many" in error_msg or "Way too much" in error_msg:
                logger.info(f"🔄 البروكسي {proxy} محظور، التبديل إلى بروكسي آخر...")
                continue  # سيحصل على بروكسي جديد في المحاولة التالية
            if attempt >= max_retries - 1:
                break
...
def fetch_ticker_with_retry(symbol, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "418" in str(e) or "429" in str(e) or "403" in str(e) or "banned" in str(e):
                logger.info(f"🔄 البروكسي محظور، التبديل إلى التالي...")
                continue
            time.sleep(0.5)
    return None

def fetch_tickers_with_retry(max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "418" in str(e) or "429" in str(e) or "403" in str(e):
                logger.info(f"🔄 البروكسي محظور، التبديل إلى التالي...")
                continue
            time.sleep(0.5)
    with _global_state_lock:
...
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=500, max_retries=5):
    time.sleep(0.1)
    exchange = get_active_exchange()
    last_err = ""
    attempt = 0
    while attempt < max_retries:
        try:
            ...
            if "418" in str(e) or "429" in str(e) or "403" in str(e):
                ...
                continue
            if "banned" in str(e):
                logger.warning("⚠️ تم حظر IP (بينانس)، التبديل إلى بروكسي آخر...")
                continue
            time.sleep(0.5)
            attempt += 1
...
def fetch_ohlcv_persistent(symbol, timeframe, limit=500, max_attempts=5, retry_interval=2):
    for attempt in range(max_attempts):
        df = fetch_ohlcv_retry_raw(symbol, timeframe, limit, max_retries=2)
        if not df.empty:
            return df
...
def fetch_ohlcv_retry(symbol, timeframe, limit=500, max_retries=3, ttl_seconds=60):
...
def get_multiplier_from_score(score_5m):
    """
    جدول المضاعف المستخرج من درجة إطار 5 دقائق (معدل ليناسب التقلب الأعلى)
    """
    if score_5m < 0.35:
        return 0.75
    elif score_5m < 0.40:
...
def background_scanner():
    ...
    while True:
        if not PAUSE_ANALYSIS and scanner:
            scanner.scan()
...
def background_balance_updater():
    ...
    while True:
        if ENABLE_TRADING and not TEST_MODE and not PAPER_TRADING:
            ...
            get_total_equity()
        time.sleep(300)  # تم التمديد إلى 5 دقائق
...
def thread_watchdog():
    essential_threads = {
        ...
        'proxy_updater': background_proxy_updater,   # الخيط الجديد لتحديث البروكسيات
    }
...
if __name__ == '__main__':
    ...
    send_telegram(f"🚀 بوت v28.0 (الإصدار المحسن بـ 3 أطر زمنية 5م، 15د، 1س مع أوزان ديناميكية وتوصيات ذكية) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"✅ **نظام تناوب بروكسيات متعدد المصادر (ProxyScrape, Proxy-List.Download, Geonode, OpenProxyList, Spys.me)**\n"
                  f"   مع اختبار تلقائي وتحديث دوري كل 5 دقائق.")
