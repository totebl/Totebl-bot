...
_processing_lock = threading.Lock()  # تم التعديل: RLock -> Lock ليعمل التحرير القسري
...
def force_unlock():
    ...
    try:
        # باستخدام القفل العادي Lock يمكن تحريره من أي خيط
        while _processing_lock.locked():
            try:
                _processing_lock.release()
                logger.info("✅ تم تحرير القفل بنجاح")
            except RuntimeError:
                # قد يحدث إذا لم يكن القفل محتجزاً من الأساس
                break
            except Exception as e:
