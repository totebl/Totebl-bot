...
POSITION_MONITOR_INTERVAL = float(os.environ.get('POSITION_MONITOR_INTERVAL', 2.0))  # 2 ثانية
...
class SellingCommittee:
    def __init__(self):
        self.models = {
            'rule': RuleBasedModel(), 
            'emr': HybridMomentumRSIModel(),  # استخدام النموذج الهجين أيضاً في لجنة البيع
            'cfhm': CFHMModel(),
            'overbought': OverboughtModel(), 
            'engulfing': EngulfingModel(),
            'liquidity_exit': LiquidityExitModel(), 
            'atr_ext': ExtremeATRModel(),
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                # استخدام البيانات المجلوبة دفعة واحدة
                ...
                if profit > 0.011:
                    ...
                    if pos.last_target_hit_index == -1:
                        # استخدام المتغير الديناميكي CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP
                        if (now_time - pos.open_time).total_seconds() > (CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP * 60):
                            ...
                            if first_target and cur_price < first_target:
                                close_partial(sym, 1.0, cur_price, f"🕒 انقضت {CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP} دقيقة دون تحقيق الهدف الأول – جني أرباح")
                                continue
                    else:
                        ...
                        if next_target_idx < len(pos.take_profit_levels):
                            ...
                            if (now_time - pos.last_target_hit_time).total_seconds() > (CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP * 60) and cur_price < next_target:
                                close_partial(sym, 1.0, cur_price, f"🕒 انقضت {CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP} دقيقة بعد تحقيق الهدف {pos.last_target_hit_index+1} دون الوصول للهدف التالي – جني أرباح")
                                continue
                tp_tolerance = 0.001
                ...
                if profit <= -0.06:
                    ...
                    continue
            time.sleep(POSITION_MONITOR_INTERVAL)  # تم التعديل إلى 2 ثانية
        except Exception as e:
                    ...
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                                ...
                                status = (f"📊 <b>حالة الفلاتر والقيم الحالية</b>\n━━━━━━━━━━━━━━━━━━━━\n🛡️ <b>فلتر التلاعب:</b> {'✅ مفعل' if FILTER_MANIPULATION_ENABLED else '❌ معطل'}\n💧 <b>فلتر السيولة:</b> {'✅ مفعل' if FILTER_LIQUIDITY_ENABLED else '❌ معطل'}\n💰 <b>فلتر القيمة السوقية:</b> {'✅ مفعل' if FILTER_MARKET_CAP_ENABLED else '❌ معطل'} (حد: ${MIN_MARKET_CAP_USD:,.0f})\n📊 <b>فلتر حجم التداول 24ساعة:</b> {'✅ مفعل' if FILTER_VOLUME_24H_ENABLED else '❌ معطل'} (حد: ${MIN_24H_VOLUME_USD:,.0f})\n📈 <b>فلتر التغيير 24ساعة:</b> {'✅ مفعل' if FILTER_CHANGE_24H_ENABLED else '❌ معطل'} (حد: {MIN_24H_CHANGE_PERCENT:.1f}%)\n🕐 <b>فلتر شمعة الساعة:</b> {'✅ مفعل' if FILTER_HOUR_CANDLE_ENABLED else '❌ معطل'}\n🕓 <b>فلتر شمعة 4 ساعات:</b> {'✅ مفعل' if FILTER_4H_CANDLE_ENABLED else '❌ معطل'}\n📊 <b>فلتر أداء {FILTER_30D_DAYS} يوم:</b> {'✅ مفعل' if FILTER_30D_PROFIT_ENABLED else '❌ معطل'} (العتبة: {MIN_30D_PROFIT_PERCENT:.2f}%)\n⏸️ <b>دخول الصفقات الجديدة:</b> {'🟢 مفعل (مسموح)' if not PAUSE_NEW_ENTRIES else '🔴 موقف (ممنوع)'}\n🔁 <b>المضاعف (1 ساعة):</b> {'✅ مفعل' if USE_1H_MULTIPLIER else '❌ معطل'}\n━━━━━━━━━━━━━━━━━━━━\n⚙️ <b>الإعدادات الديناميكية:</b>\n🔢 <b>عدد رموز التحليل:</b> {CUSTOM_ACTIVE_SYMBOLS_LIMIT}\n📊 <b>عدد الصفقات المسموحة يومياً:</b> {CUSTOM_MAX_DAILY_TRADES}\n📉 <b>الحد الأقصى للتعرض الكلي:</b> {CUSTOM_MAX_EXPOSED_PERCENT:.1%}\n📈 <b>حجم الصفقة في السوق الجيد:</b> {CUSTOM_POSITION_PERCENT_GOOD:.1%}\n📊 <b>حجم الصفقة في السوق العادي:</b> {CUSTOM_POSITION_PERCENT_NORMAL:.1%}\n📉 <b>حجم الصفقة في السوق السيئ:</b> {CUSTOM_POSITION_PERCENT_BAD:.1%}\n⏱️ <b>فترة الانتظار قبل البيع الكامل:</b> {CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP} دقيقة\n━━━━━━━━━━━━━━━━━━━━\n⚙️ <b>عتبات لجنة الشراء:</b> مضاعف {CURRENT_BUY_COMMITTEE_MULTIPLIER:.2f}×\n🎯 <b>عتبة شراء عادي:</b> {STRENGTH_THRESHOLD:.2f}\n🎯 <b>عتبة شراء ميم:</b> {SCALP_MIN_PROFIT:.2f}\n🎯 <b>عتبة الاشتباه (التلاعب):</b> {MANIPULATION_REJECT_THRESHOLD:.2%}\n🎯 <b>عتبة البيع:</b> {SELL_DECISION_THRESHOLD:.2%}\n🛑 <b>عتبات الخسارة الوقائية:</b>\n   - بيع 25% عند {STOP_LOSS_PARTIAL_1_PERCENT:.2%}\n   - بيع 33% عند {STOP_LOSS_PARTIAL_2_PERCENT:.2%}\n   - بيع كامل عند {STOP_LOSS_FULL_PERCENT:.2%}\n🔄 <b>مدة المسح:</b> {SCAN_INTERVAL_MINUTES} دقيقة\n🤖 <b>الذكاء الاصطناعي (Gemini):</b> {'✅ مفعل' if AI_ENABLED else '❌ معطل'} | النموذج الحالي: {model_display}\n━━━━━━━━━━━━━━━━━━━━\n⏳ <b>نظام التبريد الذكي (الجديد):</b>\n   ✅ تبريد الصفقة الرابحة: {COOLDOWN_WIN_HOURS:.1f} ساعة\n   🔴 تبريد الصفقة الخاسرة: {COOLDOWN_LOSS_MINUTES:.0f} دقائق\n🎯 <b>عتبات مجموع النموذجين (Rule+EMR):</b>\n   - الحد الأعلى: {RULE_EMR_SUM_UPPER:.2f}\n   - الحد الأدنى: {RULE_EMR_SUM_LOWER:.2f}\n   - حالة الفلتر: {'✅ مفعل' if RULE_EMR_SUM_FILTER_ENABLED else '❌ معطل'}\n⚙️ <b>تحديث الوقف المسبق الحقيقي:</b> {'✅ مفعل' if PREEMPTIVE_SL_UPDATE_ENABLED else '❌ معطل'}\n   - نسبة تفعيل الوقف: {CUSTOM_PREEMPTIVE_SL_TRIGGER:.2%}\n   - حد تنفيذ الوقف: {CUSTOM_PREEMPTIVE_SL_LIMIT:.2%}")
                            ...
                            # ------------------------------------------------------------
                            # 8. أوامر رفع/خفض عتبات الاشتباه والبيع
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 12. أوامر نظام التبريد الذكي الجديد (قابل للتعديل عبر تلغرام)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                                ...
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (الذكاء الاصطناعي معطل، النموذج الافتراضي Flash، نظام التبريد الذكي مفعّل بـ 12 ساعة للرابح و 5 دقائق للخاسر، وعتبات النموذجين: الأعلى 1.5 والأدنى 1.3، وتحديث الوقف مفعّل)")
                                save_state()
                            
                            # ------------------------------------------------------------
                            # 16. أمر رسم منحنى الرصيد الكلي
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 17. أوامر إيقاف دخول الصفقات الجديدة
                            # ------------------------------------------------------------
...
# --------------------------- دوال حفظ واستعادة الحالة (معدلة لتشمل USE_1H_MULTIPLIER) ---------------------------
def save_state():
    ...
    try:
        ...
        data = {
            ...
            # 'last_close_time' تم إلغاؤه نهائياً
            ...
            'use_1h_multiplier': USE_1H_MULTIPLIER  # <-- إضافة المتغير الجديد
        }
