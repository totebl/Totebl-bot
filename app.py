...
# سيتم تعيين النماذج ديناميكياً مع وجود قيم افتراضية
...
# دوال البحث عن أحدث نماذج Gemini (مع قيم افتراضية)
...
def get_latest_flash_lite_model():
    if not GEMINI_API_KEY or genai is None:
        return DEFAULT_FLASH_LITE_MODEL
    try:
        genai.configure(api_key=GEMINI_API_KEY)
        models = genai.list_models()
...
def refresh_gemini_models():
    ...
    logger.info(f"✅ نموذج Flash المستخدم: {GEMINI_MODEL_FLASH}")
    logger.info(f"✅ نموذج Flash-Lite المستخدم: {GEMINI_MODEL_FLASH_LITE}")
...
السؤال: بناءً على هذه البيانات، هل تنصح بشراء هذا الرمز الآن؟ أجب فقط بـ "نعم" أو "لا" ثم سبب مختصر جداً."""
    ...
    # استخدام النماذج الديناميكية مع احتياطي
    ...
    # محاولة إنشاء النموذج مع احتياطي
    try:
        model = genai.GenerativeModel(model_name)
    except Exception as e:
        logger.warning(f"فشل إنشاء النموذج {model_name}: {e}. محاولة استخدام النموذج الآخر...")
        other = GEMINI_MODEL_FLASH_LITE if CURRENT_AI_MODEL == 'flash' else GEMINI_MODEL_FLASH
        if other:
            try:
                model = genai.GenerativeModel(other)
                model_name = other
                logger.info(f"تم استخدام النموذج البديل: {model_name}")
            except Exception as e2:
                logger.error(f"فشل إنشاء النموذج البديل: {e2}")
                model = genai.GenerativeModel('gemini-pro')
                model_name = 'gemini-pro'
                logger.warning(f"استخدام النموذج الافتراضي 'gemini-pro'")
        else:
            model = genai.GenerativeModel('gemini-pro')
            model_name = 'gemini-pro'
            logger.warning(f"استخدام النموذج الافتراضي 'gemini-pro'")
