# ========================================================================
# بوت التداول v27.6 – إصدار نهائي مستقر (مع تعديلات السرعة والأمان)
# ========================================================================
...
STOP_LOSS_PARTIAL_1_PERCENT = 0.008    # 0.8% → بيع 25%
STOP_LOSS_PARTIAL_2_PERCENT = 0.012    # 1.2% → بيع 33%
STOP_LOSS_FULL_PERCENT = 0.0172        # 1.72% → بيع كامل
...
# متغيرات مراقبة صحة خيط التحليل
_last_analysis_activity = time.time()
_analyzer_health_lock = threading.Lock()
...
LOCK_TIMEOUT_SECONDS = 300   # تم التعديل إلى 5 دقائق (300 ثانية)
_manipulation_cache = {}
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "418" in error_msg or "banned" in error_msg or "too many" in error_msg or "Way too much" in error_msg:
                logger.critical(f"🚨 IP لا يزال محظورًا. الانتظار 3 دقائق ثم إعادة المحاولة.")
                send_telegram(f"⚠️ Binance لا يزال يحظر IP. انتظر 3 دقائق... (محاولة {attempt+1}/{max_retries})")
                time.sleep(180)  # تم التعديل إلى 3 دقائق
                continue
            if attempt < max_retries - 1:
                continue
...
def fetch_ticker_with_retry(symbol, max_retries=2):
    ...
    # لا نستخدم REST API كبديل
    return None

def fetch_tickers_with_retry(max_retries=2):
    # فقط WebSocket + آخر بيانات مخزنة، لا REST API أبداً
    ...
    # استخدام البيانات القديمة إذا كانت موجودة وحديثة (أقل من 13 دقيقة)
    with _global_state_lock:
        if bot_stats.last_valid_tickers and (time.time() - bot_stats.last_tickers_time) < 780:
            logger.warning(f"⚠️ استخدام بيانات قديمة (عمرها {time.time()-bot_stats.last_tickers_time:.0f} ثانية) - لا يمكن جلب جديدة بسبب تعطيل REST API")
            return dict(bot_stats.last_valid_tickers)
    
    logger.error("❌ لا توجد بيانات أسعار صالحة (WebSocket ميت والبيانات المخزنة قديمة) - سيتم تخطي التحليل مؤقتاً")
    return {}
    ...
    while attempt < max_retries:
        try:
            ...
            if "418" in str(e) or "banned" in str(e):
                logger.critical("⚠️ تم حظر IP! انتظر 3 دقائق")
                send_telegram("🚨 تم حظر IP من Binance. إيقاف الطلبات لمدة 3 دقائق.")
                time.sleep(180)  # تم التعديل إلى 3 دقائق
...
def monitor_positions():
    ...
    while True:
        try:
            ...
            if not all_tickers: time.sleep(3); continue   # تم التعديل إلى 3 ثوانٍ
            with _market_condition_lock:
...
def analyze_and_trade():
    ...
    global _last_analysis_activity

    # تحديث نشاط الخيط
    with _analyzer_health_lock:
        _last_analysis_activity = time.time()

    # محاولة الحصول على القفل مع مهلة 300 ثانية (5 دقائق)
    lock_acquired = False
    try:
        lock_acquired = _processing_lock.acquire(timeout=300)
    except Exception as e:
        logger.error(f"خطأ أثناء محاولة الحصول على القفل: {e}")

    if not lock_acquired:
        logger.warning("⚠️ لم نتمكن من الحصول على _processing_lock بعد 300 ثانية. محاولة تحرير قسري...")
        try:
            while _processing_lock.locked():
                _processing_lock.release()
            logger.info("✅ تم تحرير القفل القسري، إعادة المحاولة للحصول عليه")
            lock_acquired = _processing_lock.acquire(timeout=30)
            if not lock_acquired:
                logger.warning("لا يزال القفل محتجزاً، تخطي هذه الدورة")
                return
        except Exception as e2:
            logger.error(f"فشل التحرير القسري: {e2}")
            return
    ...
    try:
        ...
        if lock_acquired:
            ...
            if _processing_lock.locked():
                logger.critical("❗ قفل _processing_lock لا يزال محتجزاً بعد محاولة التحرير!")
                generate_error_report("خطأ_حرج", "أقفال", "قفل _processing_lock لم يتحرر بعد finally")
        # تحديث نشاط الخيط مرة أخرى عند الانتهاء
        with _analyzer_health_lock:
            _last_analysis_activity = time.time()
    ...
    while True:
        ...
        if _processing_lock.locked():
            ...
            if elapsed > LOCK_TIMEOUT_SECONDS:
                logger.critical(f"❗ قفل _processing_lock معلق لمدة {elapsed:.0f} ثانية! محاولة تحرير قسري (بدون إعادة تشغيل).")
                send_telegram(f"⚠️ قفل التحليل معلق لمدة {elapsed:.0f} ثانية - جارٍ محاولة التحرير القسري.")
                try:
                    while _processing_lock.locked():
                        _processing_lock.release()
                    logger.info("✅ تم تحرير القفل القسري بواسطة lock_health_monitor")
                except Exception as e:
                    ...
                    with _global_state_lock:
                        ...
                        if current_oid and current_oid not in order_map.values():
                            ...
                            if pos.trailing_activated and pos.trailing_stop is not None:
                                ...
                                new_stop = pos.entry_price * (1 - sl_percent)
                            new_oid = place_preemptive_stop_loss(sym, pos.remaining_size, pos.entry_price, pos.symbol_type, stop_price_override=new_stop)
                            if new_oid: pos.preemptive_sl_order_id = new_oid; pos.last_trailing_stop_sent = new_stop
                        elif not current_oid and sym not in order_map:
                            if pos.trailing_activated and pos.trailing_stop is not None:
                                new_stop = pos.trailing_stop
                            else:
                            ...
                            logger.info(f"🔄 تحديث معرف الوقف المسبق لـ {sym} من {current_oid} إلى {order_map[sym]}")
                            pos.preemptive_sl_order_id = order_map[sym]
                except Exception as e: logger.warning(f"فشل مزامنة الوقف المسبق لـ {sym}: {e}")
        except Exception as e: logger.error(f"خطأ في sync_preemptive_stop_orders: {e}")
        time.sleep(60)
...
def analyzer_health_monitor():
    """مراقب صحة خيط التحليل: إذا توقف عن تحديث النشاط لأكثر من 7 دقائق (مع بقائه حياً)، يعيد إنشاؤه."""
    global _last_analysis_activity
    while True:
        time.sleep(120)  # فحص كل دقيقتين لتقليل الحمل
        # التحقق من وجود خيط التحليل الأصلي
        analyzer_thread = _essential_threads.get('background_analyzer', {}).get('thread')
        if analyzer_thread and analyzer_thread.is_alive():
            with _analyzer_health_lock:
                last_activity = _last_analysis_activity
            inactivity = time.time() - last_activity
            # إذا كان الخيط حياً ولكن لم يحدث نشاط لأكثر من 7 دقائق (420 ثانية)
            if inactivity > 420:
                logger.critical(f"🚨 خيط التحليل حي لكنه عالق لمدة {inactivity:.0f} ثانية (أكثر من 7 دقائق). محاولة تحرير القفل وإنشاء خيط جديد...")
                send_telegram(f"⚠️ خيط التحليل عالق منذ {inactivity/60:.1f} دقيقة، جاري إنشاء خيط جديد وتحويل المهام.")
                # محاولة تحرير القفل القسري
                try:
                    if _processing_lock.locked():
                        while _processing_lock.locked():
                            _processing_lock.release()
                        logger.info("✅ تم تحرير القفل القسري بواسطة analyzer_health_monitor")
                except Exception as e:
                    logger.error(f"فشل تحرير القفل القسري: {e}")
                # إنشاء خيط تحليل جديد (بديل)
                new_thread = threading.Thread(target=analyze_and_trade, daemon=False, name="background_analyzer_replacement")
                new_thread.start()
                # تحديث المرجع في _essential_threads
                _essential_threads['background_analyzer']['thread'] = new_thread
                # تحديث طابع النشاط لمنع التكرار السريع
                with _analyzer_health_lock:
                    _last_analysis_activity = time.time()
        time.sleep(420)  # انتظر 7 دقائق قبل الفحص التالي (بعد التحرير)
    ...
    while True:
        try:
            ...
            if not ws_manager.is_alive():
                logger.warning("⚠️ WebSocket Manager توقف. بدء إعادة التشغيل القسرية (5 محاولات كل 7 ثوانٍ)...")
                send_telegram("🔄 WebSocket Manager توقف. جاري إعادة التشغيل (5 محاولات).")
                
                for attempt in range(1, 6):
                    logger.info(f"محاولة إعادة تشغيل WebSocket Manager رقم {attempt}/5")
                    ws_manager.restart()
                    time.sleep(7)
                    
                    if ws_manager.is_alive():
                        logger.info(f"✅ نجحت إعادة تشغيل WebSocket Manager بعد المحاولة {attempt}")
                        send_telegram(f"✅ WebSocket Manager يعمل مجدداً (بعد {attempt} محاولة).")
                        break
                else:
                    logger.critical("❌ فشل إعادة تشغيل WebSocket Manager بعد 5 محاولات. سيتم إعادة المحاولة لاحقاً.")
                    send_telegram("❌ فشل إعادة تشغيل WebSocket Manager. قد تحتاج إلى إعادة تشغيل البوت يدوياً.")
            ...
            for name in list(_essential_threads.keys()):
                ...
                if not t.is_alive():
                    ...
                    send_telegram(f"🔄 خيط {name} توقف وأعيد تشغيله.")
            ...
            if _processing_lock.locked():
                ...
                if lock_age > 720:  # 12 دقيقة كحد أقصى، لكن تحرير القفل سيحدث قبلها
                    logger.critical(f"🚨 قفل _processing_lock عالق لمدة {lock_age:.0f} ثانية! محاولة تحرير قسري (بدون إعادة تشغيل).")
                    send_telegram(f"🚨 قفل التحليل عالق لمدة {lock_age:.0f} ثانية - جارٍ محاولة التحرير القسري.")
                    try:
                        while _processing_lock.locked():
                            _processing_lock.release()
                        logger.info("✅ تم تحرير القفل القسري بواسطة thread_watchdog")
                    except Exception as e:
                        logger.error(f"فشل التحرير القسري: {e}")
        except Exception as e:
...
def load_state():
    ...
    try:
        ...
        STOP_LOSS_FULL_PERCENT = filters.get('stop_loss_100', 0.0172)
