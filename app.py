# ========================================================================
# بوت التداول v27.9 – إصدار مستقر معدل لـ Binance Spot + Google Gemini AI
...
<head><meta charset="UTF-8"><title>بوت v27.9 (معدل لـ Spot)</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v27.9 (Spot Edition)</h1>
<div id="stats"></div>
...
# --------------------------- لجنة الشراء (محسّنة مع العضو الرابع واستبدال EMR بالنموذج الهجين) ---------------------------
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        ...
        # جلب درجة توقيت الدخول من النموذج الجديد
        buy_pct_timing = self.models['timing'].get_entry_score(df_primary)

        raw_values = [buy_pct_rule, buy_pct_emr, buy_pct_cfhm, buy_pct_timing]
        valid = [v for v in raw_values if v is not None]
        if not valid:
            return 'neutral', 0.0, 0.0, "⚠️ بيانات غير كافية من النماذج"

        # جلب البيانات التأكيدية مع تغيير نموذج التأكيد من rule إلى emr (الآن يستخدم النموذج الهجين)
        ...
        # تأكيد باستخدام نموذج EMR الهجين للإطار 5 دقائق
        ...
        # تأكيد باستخدام نموذج EMR الهجين للإطار 1 ساعة
        ...
        # تطبيق الأوزان المرجحة (4 نماذج الآن) - معدلة حسب التحليل
        # CFHM: 40%، Rule: 30%، Hybrid: 20%، Timing: 10% (لجميع الأنواع)
        if sym_type == 'meme':
            weights = {'rule': 0.30, 'emr': 0.20, 'cfhm': 0.40, 'timing': 0.10}
        else:
            weights = {'rule': 0.30, 'emr': 0.20, 'cfhm': 0.40, 'timing': 0.10}
        
        # ترتيب القيم حسب ترتيب النماذج: rule, emr, cfhm, timing
        weighted_avg = 0.0
        weight_sum = 0.0
        for i, model_key in enumerate(['rule', 'emr', 'cfhm', 'timing']):
            if i < len(adjusted):
        ...
        reason = (f"المتوسط={avg:.3f} | العتبة={thresh:.3f} | المضاعف={multiplier:.2f}x | "
                  f"الدرجات: {', '.join([f'{v:.3f}' for v in adjusted])}")
...
def close_partial(symbol, percent, price, reason, pnl_usdt=None):
    ...
    with _global_state_lock:
        ...
        if not validate_restored_position(symbol, pos):
            ...
            # last_close_time تم إلغاؤه، نضيف تبريداً افتراضياً عند الحذف اليدوي
            with _cooldown_lock:
...
# --------------------------- دالة telegram_polling المعدلة (بإضافة أوامر التبريد الجديدة) ---------------------------
                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                            # 6. أوامر ضبط فترة التبريد ومدة المسح ووقت الانتظار (تم استبدال "فلتر التبريد" بجديد)
                            # ------------------------------------------------------------
                            # تم حذف الأمر القديم "فلتر التبريد" واستبداله بالأوامر الجديدة أدناه
