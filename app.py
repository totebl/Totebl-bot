...
# التحديثات: WebSocket لـ 5م، 1س، 4س + فلتر الكمي + تحسينات الذاكرة
# ========================================================================
...
_ohlcv_cache_max = 100  # تم التخفيض من 150
_features_cache = OrderedDict()
_features_cache_max = 150  # تم التخفيض من 200
...
# ========================== دالة تعقيم البيانات (معدلة: إلغاء عملها) ==========================
def sanitize_dataframe(df, required_cols=None):
    """تم إلغاء عمل هذه الدالة بناءً على طلب المستخدم للعودة إلى منطق الإصدار القديم v27.8"""
    if df is None or df.empty:
        return None
    # نعيد البيانات كما هي دون أي تعديل (لا حذف، ولا تعبئة)
    # التنظيف سيكون فقط داخل add_advanced_features كما في الإصدار القديم
    return df
...
class Position:
    ...
    def _calc_initial_momentum(self):
        # تأجيل حساب الزخم إلى ما بعد تهيئة WebSocket مع التحقق من ws_manager
        self.momentum_calculated = False
        try:
            if ws_manager is not None and ws_manager.initialized:
                df = ws_manager.get_klines(self.symbol, '5m')
                # تعقيم البيانات (الدالة الآن لا تفعل شيئاً)
    ...
    def calculate_momentum_later(self):
        """حساب الزخم بعد تهيئة WebSocket (الإصلاح 42)"""
...
class AdvancedWebSocketManager:
    def __init__(self):
        ...
        self.klines_cache = {}  # { 'BTC/USDT_5m': pd.DataFrame, 'BTC/USDT_1h': pd.DataFrame, ... }
        ...
        # قفل إعادة التشغيل (تم إضافته)
        self._restart_lock = threading.Lock()

        # ===== الإضافات الجديدة (الإصلاحات 5-13) =====
        self.max_symbols = int(os.environ.get('MAX_WS_SYMBOLS', 100))
        self.last_access_time = {}          # مفتاح الكاش -> timestamp
        self.recently_removed = {}          # symbol -> timestamp (مدة 10 دقائق)
        self.all_tickers_cache = {}         # {symbol: {...}, '_timestamp': ...}
        self.orderbook_cache = {}           # {symbol: {'bids': [], 'asks': [], 'timestamp': ...}}
        self.trades_cache = {}              # {symbol: {'trades': [], 'timestamp': ...}}
        self.all_tickers_lock = threading.Lock()
        self.orderbook_lock = threading.Lock()
        self.trades_lock = threading.Lock()
        self._id_counter = 0

        # إضافة متغيرات لتتبع آخر تنظيف
    ...
    def _fetch_rest_klines(self, symbol, timeframe='5m', limit=500):
        """جلب البيانات من REST (مرة واحدة عند التهيئة أو إصلاح الفجوات)"""
        exchange = get_active_exchange()
        for attempt in range(3):
            try:
    ...
    def _safe_send(self, message):
        """إرسال رسالة WebSocket مع قفل لمنع التضارب"""
        with self.send_lock:
    ...
    def _initialize_or_repair(self, symbol, timeframe='5m', limit=500):
        """تهيئة كاش العملة أو إصلاح الفجوات (بدون إرسال اشتراكات)"""
        try:
    ...
    def _check_and_fill_gap(self, symbol, timeframe='5m'):
        """فحص وإصلاح الفجوات الزمنية مع تأخير عشوائي"""
        cache_key = f"{symbol}_{timeframe}"
        with self.klines_lock:
    ...
    def _update_features_async(self, symbol, df):
        """حساب المؤشرات في خيط منفصل (لمنع تعطل WebSocket)"""
        def _update():
            try:
                if df is not None and not df.empty and len(df) >= 50:
                    df_copy = df.copy(deep=True)
                    df_copy = add_advanced_features(df_copy)
                    # تعقيم النتيجة (الدالة الآن لا تفعل شيئاً)
    ...
    def _gap_checker(self):
        """خيط دوري لفحص الفجوات وتنظيف الكاشات في جميع الرموز"""
        while self.running:
            now = time.time()
            # فحص الفجوات كل 60 ثانية
            time.sleep(60)
            with self.klines_lock:
            ...
            # تنظيف الكاشات حسب التوقيت المناسب
            self._perform_cleanup(now)

    def _perform_cleanup(self, now):
        """تنفيذ التنظيف الدوري للكاشات حسب الفترات الزمنية"""
        # كل 30 ثانية: تنظيف سريع (ticker, orderbook, trades)
        if now - self._last_fast_cleanup > 30:
            self.cleanup_ticker_cache(max_age=10)
            self.cleanup_orderbook_cache(max_age=2)
            self.cleanup_trades_cache(max_age=120)
            self._last_fast_cleanup = now

        # كل 5 دقائق: تنظيف متوسط (all_tickers)
        if now - self._last_medium_cleanup > 300:
            # التعديل الثاني: رفع تنظيف التيكرات إلى 12 ساعة (43200 ثانية) بدلاً من 10 دقائق (600)
            self.cleanup_all_tickers_cache(max_age=43200)
            self._last_medium_cleanup = now

        # كل 15 دقيقة: تنظيف بطيء (klines حسب الحجم)
        if now - self._last_slow_cleanup > 900:
            self.cleanup_klines_cache(max_symbols=100)
            self._last_slow_cleanup = now

    def cleanup_ticker_cache(self, max_age=10):
        """تنظيف ticker_cache (البيانات الأقدم من max_age ثانية)"""
        now = time.time()
        with self.ticker_lock:
            stale = [s for s, d in self.ticker_cache.items() if now - d.get('timestamp', 0) > max_age]
            for s in stale:
                del self.ticker_cache[s]

    def cleanup_orderbook_cache(self, max_age=2):
        """تنظيف orderbook_cache (البيانات الأقدم من max_age ثانية)"""
        now = time.time()
        with self.orderbook_lock:
            stale = [s for s, d in self.orderbook_cache.items() if now - d.get('timestamp', 0) > max_age]
            for s in stale:
                del self.orderbook_cache[s]

    def cleanup_trades_cache(self, max_age=120):
        """تنظيف trades_cache (البيانات الأقدم من max_age ثانية)"""
        now = time.time()
        with self.trades_lock:
            stale = [s for s, d in self.trades_cache.items() if now - d.get('timestamp', 0) > max_age]
            for s in stale:
                del self.trades_cache[s]

    def cleanup_all_tickers_cache(self, max_age=43200):
        """تنظيف all_tickers_cache (البيانات الأقدم من max_age ثانية) - تم رفع المدة إلى 12 ساعة"""
        now = time.time()
        with self.all_tickers_lock:
            stale = [s for s in self.all_tickers_cache.keys() if s != '_timestamp' and now - self.all_tickers_cache[s].get('timestamp', 0) > max_age]
            for s in stale:
                del self.all_tickers_cache[s]
            # إذا كانت البيانات العامة قديمة، امسح الكاش بالكامل
            if '_timestamp' in self.all_tickers_cache and now - self.all_tickers_cache['_timestamp'] > max_age:
                self.all_tickers_cache.clear()

    def cleanup_klines_cache(self, max_symbols=100):
        """تنظيف klines_cache بالحفاظ على آخر max_symbols رمز (حسب last_access_time) مع الأطر الثلاثة"""
        with self.klines_lock:
            # نحتاج إلى الاحتفاظ بـ max_symbols لكل رمز ولكل إطار
            # لكن الكاش يحتوي على مفاتيح مثل symbol_5m, symbol_1h, symbol_4h
            # سنقوم بحساب عدد الرموز الفريدة
            symbols = set()
            for key in self.klines_cache.keys():
                if '_' in key:
                    sym = key.rsplit('_', 1)[0]
                    symbols.add(sym)
            if len(symbols) > max_symbols:
                # نرتب الرموز حسب آخر استخدام (نأخذ أحدث الأوقات)
                sorted_symbols = sorted(symbols, key=lambda s: self.last_access_time.get(s, 0))
                to_remove_symbols = sorted_symbols[:len(symbols) - max_symbols]
                for sym in to_remove_symbols:
                    # حذف جميع أطر هذا الرمز
                    for tf in ['5m', '1h', '4h']:
                        key = f"{sym}_{tf}"
                        if key in self.klines_cache:
                            del self.klines_cache[key]
                    # حذف من last_access_time
                    self.last_access_time.pop(sym, None)

    def on_message(self, ws, message):
        # تعيين initialized عند استلام أول رسالة
        ...
        try:
            ...
            # 1. معالجة التيكرات العامة (!ticker@arr)
            if 'stream' not in data and isinstance(data, list):
                # رسالة !ticker@arr (بدون stream)
                with self.all_tickers_lock:
                    now = time.time()
                    for item in data:
            ...
            # 2. معالجة القنوات الفردية (شموع، دفتر، صفقات)
            if 'stream' in data and 'data' in data:
                ...
                if '@kline' in stream:
                    ...
                    with self.klines_lock:
                        df = self.klines_cache.get(cache_key)
                        if df is None:
                            # إذا لم تكن في الكاش، ننشئها من REST (لن يحدث عادة)
                            df = self._fetch_rest_klines(symbol, tf, limit=500)
                            if df is None or df.empty:
                                return
                            self.klines_cache[cache_key] = df
                        ...
                        # إضافة الصف والحفاظ على الحجم
                        df = pd.concat([df, new_row])
                        if len(df) > 500:
                            df = df.iloc[-500:]

                        # إذا اكتملت الشمعة، نعيد حساب المؤشرات في الخلفية (لـ 5م فقط)
                        if kline['x']:
                            # تعقيم البيانات (الدالة الآن لا تفعل شيئاً)
                            df = sanitize_dataframe(df) or df
                            self.klines_cache[cache_key] = df
    ...
    def on_open(self, ws):
        ...
        self.reconnect_delay = 5
        # 1. اشتراك عام للتيكرات
        self._safe_send({"method": "SUBSCRIBE", "params": ["!ticker@arr"], "id": self._get_next_id()})
        # 2. اشتراكات فردية لكل رمز في القائمة (شموع + دفتر + صفقات)
        if self.subscribed_symbols:
            ...
            # تقسيم params إلى مجموعات بحجم 1024 (تجنب حد Binance)
            if params:
                for i in range(0, len(params), 1024):
                    chunk = params[i:i+1024]
                    self._safe_send({"method": "SUBSCRIBE", "params": chunk, "id": self._get_next_id()})
                    time.sleep(0.1)
    ...
    def _run_forever(self):
        while self.running:
            try:
                # إنشاء كائن WebSocketApp جديد في كل دورة (لتجنب تسرب الذاكرة)
                self.ws = websocket.WebSocketApp(
                    "wss://stream.binance.com:9443/ws",
                    on_open=self.on_open,
                    on_message=self.on_message,
    ...
    def stop(self):
        ...
        # إيقاف ThreadPoolExecutor (إصلاح 33)
    ...
    # ===== دوال الحصول على البيانات من الكاش =====
            ...
            for sym, data in self.ticker_cache.items():
                if now - data.get('timestamp', 0) < 30:
                    result[sym] = {
                        ...
                        'percentage': data.get('change', 0),
                        'quoteVolume': data.get('volume', 0)
                    }
            return result

    def get_klines(self, symbol, timeframe='5m', limit=None):
        """الحصول على بيانات الشموع مع دعم الأطر المباشرة من الكاش"""
        # منع إعادة تهيئة الرموز المزالة حديثاً
        ...
        with self.klines_lock:
            ...
            if df is None or df.empty:
                # تهيئة من REST
                df = self._initialize_or_repair(symbol, timeframe, limit=500 if limit is None else limit)
                if df is None or df.empty:
                    return pd.DataFrame()
            # إذا كان limit محدداً، نأخذ آخر limit شمعة
            if limit is not None and len(df) > limit:
                return df.iloc[-limit:].copy()
            return df.copy() if not df.empty else pd.DataFrame()

    def update_subscriptions(self, new_symbols):
        """تحديث الاشتراكات ديناميكياً (إضافة/إزالة) مع تطبيق الحد الأقصى وإدارة الكاشات"""
        # حماية الرموز الأساسية والمفتوحة
        ...
        if len(new_set) > self.max_symbols:
            ...
            sorted_removable = sorted(
                removable,
                key=lambda s: (scanner.scores.get(s, 0), -self.last_access_time.get(s, 0))
            )
            to_remove_extra = set(sorted_removable[:len(new_set) - self.max_symbols])
            new_set -= to_remove_extra
            # تسجيل الرموز المزالة في recently_removed
        ...
        if to_remove:
            ...
            # تقسيم إلغاء الاشتراكات أيضاً لتجنب الحد
            for i in range(0, len(unsub_params), 1024):
                chunk = unsub_params[i:i+1024]
                self._safe_send({"method": "UNSUBSCRIBE", "params": chunk, "id": self._get_next_id()})
                time.sleep(0.1)
            # حذف الكاشات
            with self.klines_lock:
                for sym in to_remove:
        ...
        # إضافة رموز جديدة
        if to_add:
            # تهيئة الكاش من REST لكل رمز جديد (بدون اشتراك) للأطر الثلاثة
            for sym in to_add:
                for tf in ['5m', '1h', '4h']:
                    self._initialize_or_repair(sym, tf, 500)
            # إرسال اشتراكات لجميع القنوات
            sub_params = []
            for sym in to_add:
                base = sym.replace('/', '').lower()
    ...
    # ===== دوال الحصول على البيانات من الكاشات الجديدة =====
    def get_cached_all_tickers(self, max_age=5):
        with self.all_tickers_lock:
            if '_timestamp' in self.all_tickers_cache and (time.time() - self.all_tickers_cache['_timestamp']) <= max_age:
                return {k:v for k,v in self.all_tickers_cache.items() if k != '_timestamp'}
        return None
...
# ========================== دوال إعادة التجميع ==========================
def _resample_klines(df_5m, target_interval):
    """إعادة تجميع بيانات 5m إلى أي إطار زمني آخر مع فرض UTC (احتياطي)"""
    ...
    # إعادة التجميع مع منع Look-Ahead Bias (إصلاح 38)
    resampled = df_5m.resample(target_interval, closed='right', label='right').agg({
        'open': 'first',
        'high': 'max',
        'low': 'min',
        'close': 'last',
    ...
    # إعادة حساب المؤشرات
    if len(resampled) >= 20:
        resampled = add_advanced_features(resampled)
        # تعقيم النتيجة (الدالة الآن لا تفعل شيئاً)
        resampled = sanitize_dataframe(resampled) or resampled
...
def fetch_tickers_with_retry(max_retries=2):
    # محاولة من WebSocket أولاً (التيكرات العامة)
    if ws_manager is not None:
...
def fetch_ohlcv_retry(symbol, timeframe='5m', limit=500, max_retries=2, ttl_seconds=60):
    """إصدار معدل: يستخدم WebSocket للـ 5m، والـ REST للأطر الأخرى (إصلاح 15 و 16)"""
    # للـ 5m نستخدم WebSocket
    if timeframe == '5m':
        if ws_manager is not None:
            df = ws_manager.get_klines(symbol, timeframe, limit=limit)
            if df is not None and not df.empty:
                return df
        # إذا فشل WebSocket، نستخدم REST كاحتياطي
        return fetch_ohlcv_retry_raw(symbol, timeframe, limit, max_retries)

    # للأطر الأخرى نستخدم REST مع كاش
    ...
    if df is not None and not df.empty:
        with _cache_lock:
            ...
            # التحقق من الحجم وحذف أقدم العناصر فوراً (منع تسرب الذاكرة)
...
# ======================= دالة add_advanced_features المعدلة (تحتوي على التنظيف) =======================
def add_advanced_features(df):
    ...
    # تنظيف البيانات (كما في الإصدار القديم v27.8)
    df = df.replace([np.inf, -np.inf], np.nan)
    df = df.ffill().bfill().fillna(0)
    return df.astype('float64')

def get_cached_features(symbol, timeframe, limit=500, ttl=30):
    """معدل لاستخدام fetch_ohlcv_retry الجديدة (إصلاح 17)"""
    cache_key = f"{symbol}_{timeframe}_{limit}"
    now = time.time()
    with _features_cache_lock:
        if cache_key in _features_cache:
            ...
            if (now - entry['timestamp']) < ttl:
                data = entry['data'].copy(deep=True) if not entry['data'].empty else pd.DataFrame()
                # تعقيم البيانات (الدالة الآن لا تفعل شيئاً)
                data = sanitize_dataframe(data) or data
                return data
            else:
    ...
    with _features_cache_lock:
        ...
        if len(_features_cache) > _features_cache_max:
            ...
            for key, _ in items[:int(_features_cache_max * 0.2)]:
...
def calculate_mtf_consensus_score(symbol, df_5m, df_1h, df_4h):
    """
    تحليل متعدد الأطر (MTF) لإعطاء درجة ثقة من 0 إلى 1.
    الأوزان: 5م 20%، 1س 35%، 4س 45%.
    """
    scores = []
...
def acquire_timeout(lock, timeout):
    ...
    try:
        ...
        # التحقق من أن القفل مملوك قبل محاولة تحريره
        if lock.locked():
            try:
                lock.release()
            except RuntimeError:
                pass  # تجاهل الخطأ إذا تم تحريره بالفعل
...
def test_gemini_api_key():
    ...
    try:
        ...
        # قائمة النماذج المفضلة حسب الأولوية (الأحدث أولاً)
        preferred = [
...
def _ask_ai_internal(symbol, df, current_price, sym_type, market_condition, ticker_data, threshold, desc):
    ...
    # الإصلاح 43: إزالة market_condition من الـ Prompt لتجنب التحيز
...
def dynamic_stop_loss_take_profit(entry, atr, side, regime, sym_type, max_sl=None):
    ...
    # حد أدنى لـ ATR = 1% من السعر (لمنع القيم الصفرية أو الصغيرة جداً)
    min_atr = entry * 0.01
    atr = max(atr, min_atr)
...
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        ...
        # استخدام WebSocket للحصول على البيانات بدلاً من إعادة التجميع
        df_confirm_1 = ws_manager.get_klines(symbol, '15m')
        df_confirm_2 = ws_manager.get_klines(symbol, '1h')
...
class SellingCommittee:
    ...
    def decide_sell(self, df_primary, symbol, pos, cur_price, market_condition='normal'):
        ...
        # استخدام WebSocket بدلاً من إعادة التجميع
        df_confirm_1 = ws_manager.get_klines(symbol, '15m')
        df_confirm_2 = ws_manager.get_klines(symbol, '1h')
...
class WhaleManipulationDetector:
    ...
    def detect_volume_spike(self, symbol, sym_type='normal'):
        try:
            # نستخدم 5m بدلاً من 1m لتقليل استدعاءات REST
...
def get_current_momentum(symbol, lookback_minutes):
    try:
        df = ws_manager.get_klines(symbol, '5m')
        df = sanitize_dataframe(df) or df
        ...
        try:
            ...
            for sym, pos in snapshot:
                ...
                if cur_price is None or cur_price <= 0:
                    # استخدام سعر الدخول إذا كان السعر غير صالح
                ...
                # استخدام ws_manager.get_klines مباشرة
                df = ws_manager.get_klines(sym, '5m')
                df = sanitize_dataframe(df) or df
                if df is None or df.empty:
                    continue
...
def analyze_and_trade():
    ...
    try:
        # استخدام with acquire_timeout لضمان تحرير القفل
        with acquire_timeout(_processing_lock, timeout=60):
            _analysis_failures = 0
            ...
            # تنظيف الكاشات القديمة (إصلاح 22)
            if not _initial_warmup_done and elapsed_since_start < STARTUP_DELAY_SECONDS + 30:
                ...
                for sym in BASE_SYMBOLS:
                    ws_manager.get_klines(sym, '5m')  # تهيئة WebSocket
                    time.sleep(0.5)
                _initial_warmup_done = True
                logger.info("✅ تم التهيئة. بدء التحليل العادي.")
                return

            # انتظار تهيئة WebSocket (إصلاح 14)
            if ws_manager is None or not ws_manager.initialized:
            ...
            # التعديل الثالث: توسيع نطاق all_syms لاستخدام التيكرات المحفوظة بدلاً من scanner.candidates
            ...
            for sym in active:
                ...
                if regime == 'trending_down':
                    continue
                # استخدام ws_manager.get_klines للحصول على الـ 5m مباشرة
                df_primary = ws_manager.get_klines(sym, '5m')
                df_primary = sanitize_dataframe(df_primary) or df_primary
                ...
                if FILTER_QUANTUM_ENABLED:
                    df_1h = ws_manager.get_klines(sym, '1h')
                    df_4h = ws_manager.get_klines(sym, '4h')
            ...
            for sym, sym_type, df_primary, regime in candidates_for_manipulation:
                ...
                if cur_price is None or cur_price <= 0:
                    continue
                # فلاتر الساعة و 4 ساعات باستخدام WebSocket مباشرة
        ...
        logger.warning("⚠️ مهلة الحصول على قفل التحليل (60 ثانية)، تخطي الدورة")
        force_unlock()
    except Exception as e:
        ...
        with _cache_lock:
            ...
            if len(items) > 400:
                for key, _ in items[:50]:
                    if key in _manipulation_cache:
                        del _manipulation_cache[key]
        # تنظيف كاشات WebSocket و OrderBookManipulationDetector
        if ws_manager:
            ws_manager.cleanup_ticker_cache(max_age=10)
            ws_manager.cleanup_orderbook_cache(max_age=2)
            ws_manager.cleanup_trades_cache(max_age=120)
            ws_manager.cleanup_all_tickers_cache(max_age=43200)  # 12 ساعة
            ws_manager.cleanup_klines_cache(max_symbols=100)
...
def sync_preemptive_stop_orders():
    while True:
        try:
            if TEST_MODE or PAPER_TRADING or not ENABLE_TRADING:
                time.sleep(300)  # زيادة الفترة إلى 5 دقائق لتقليل طلبات REST
                continue
            exchange = get_active_exchange()
            with _global_state_lock:
                symbols_to_check = list(open_positions.keys())
                        ...
                        if current_oid and current_oid not in order_map.values():
                            ...
                            if current_coin_balance < pos.remaining_size * 0.01:
                                ...
                                if total_sold_qty > 0:
                                    ...
                                    # الربح الإجمالي الفعلي من جميع عمليات البيع (بما في ذلك الجزئية السابقة)
                                    total_pnl = calculate_net_pnl(sym, pos.entry_price, avg_price, total_sold_qty,
                                                                  pos.side, pos.symbol_type)

                                    # الربح الذي تم تسجيله بالفعل من البيع الجزئي السابق (في close_partial)
                                    recorded_pnl = pos.closed_pnl
                                    # الربح الجديد الناتج عن هذا الإغلاق التلقائي (الجزء المتبقي)
                                    new_pnl = total_pnl - recorded_pnl

                                    # تحديث الإحصائيات بإضافة الربح الجديد فقط
                                    ...
                                    # تسجيل الصفقة في سجل التداول (مرة واحدة فقط)
                                    trade_logger.info(
                                    ...
                                    # إرسال إشعار تلغرام (الربح الكلي)
                                    ...
                                    # لم نجد صفقات بيع (نادر) – حذف المركز دون حساب ربح
                                    logger.warning(
        ...
        time.sleep(300)  # زيادة الفترة إلى 5 دقائق
                ...
                if data.get("ok"):
                    for upd in data.get("result", []):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------------------------------------------------
                            # 3. أوامر تشغيل/إيقاف الفلاتر الفردية (المفقودة من v27.7)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 4. أوامر ضبط عتبات الشراء واللجنة (المفقودة)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 5. أوامر ضبط فترة التبريد ومدة المسح (المفقودة)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 6. أوامر التبديل بين التداول الحقيقي والورقي (المفقودة)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 7. أوامر رفع/خفض عتبات الاشتباه والبيع (موجودة في v27.8 بالفعل)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 8. أوامر تعيين عتبات الخسارة الوقائية (موجودة في v27.8)
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 11. أوامر الذكاء الاصطناعي (تشغيل/إيقاف والتبديل بين النموذجين) - معدلة للتحقق
                            # ------------------------------------------------------------
                                ...
                                # التحول إلى أول نموذج متاح (نستخدم _detected_model لتحديد)
                                if _detected_model:
                                ...
                                if not GEMINI_API_KEY:
                                    ...
                                    try:
                                        ...
                                        models = genai.list_models()
                                        model_names = [m.name for m in models]
                                        # محاولة العثور على نموذج مختلف عن الحالي
                            ...
                            # ------------------------------------------------------------
                            # 12. أمر إعادة ضبط الفلاتر (معدل ليشمل الإعدادات الديناميكية)
                            # ------------------------------------------------------------
                                ...
                                CUSTOM_MAX_DAILY_TRADES = MAX_DAILY_TRADES
                                AI_ENABLED = False
                                # لا نغير _detected_model بل نتركه كما هو
                                send_telegram(
...
class MarketScanner:
    ...
    def scan(self):
        ...
        try:
            ...
            # إصلاح 31: تحديث اشتراكات WebSocket ديناميكياً
...
if __name__ == '__main__':
    ...
    # إنشاء ws_manager أولاً (قبل load_state)
    ws_manager = AdvancedWebSocketManager()
    scanner = MarketScanner()
    buying_committee = BuyingCommittee()

    # بدء WebSocket قبل تحميل الحالة (لضمان جاهزية ws_manager)
    ws_manager.start(list(BASE_SYMBOLS))
    logger.info("✅ تم بدء تشغيل AdvancedWebSocket Manager")
    time.sleep(5)  # انتظار تهيئة WebSocket

    # الآن تحميل الحالة (بعد بدء ws_manager)
    load_state()
    ...
    # حساب الزخم المتأخر للمراكز المفتوحة بعد بدء WebSocket (إصلاح 42)
    time.sleep(10)
    with _global_state_lock:
        for sym, pos in open_positions.items():
    ...
    logger.info(f"🚀 بدء البوت v28.1 (MTF + Full WebSocket) - الوضع: {mode_str}")
    send_telegram(
        f"🚀 بوت v28.1 (مع الفلتر الكمي MTF) يعمل – الوضع: {mode_str}\n✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n✅ تقرير يومي يعرض صافي الربح/الخسارة\n✅ التحويل بين التداول الورقي والحقيقي\n✅ أمر `منحنى` أو `رسم` لإرسال صورة بيانية لمنحنى الرصيد الكلي\n✅ أمر `عدد الصفقات المسموحة` للتحكم بالحد اليومي للصفقات\n✅ تحسينات كبيرة في الذاكرة ومنع التسريب\n✅ WebSocket خالص مع إعادة تشغيل تلقائي (شموع 5م، 1س، 4س، دفتر، صفقات، تيكرات)\n✅ فلتر كمي (MTF) بثلاثة أطر مع أوامر تشغيل/إيقاف وتعيين العتبة (`شغل فلتر الكم`, `اوقف فلتر الكم`, `عتبة الكم 0.60`)\n✅ مراقبة وإعادة تشغيل جميع الخيوط بما فيها WebSocket (مع حماية watchdog)\n✅ جلب صبور للرصيد الحقيقي (حتى 120 ثانية)\n✅ سرعة مراقبة المراكز 2 ثانية\n✅ التحكم بدرجة الاشتباه و عتبة البيع وعتبات الخسارة الوقائية\n✅ البيع التدريجي الصارم (بدون إغلاق مبكر بسبب المبلغ الصغير)\n✅ دعم أفضل لمنصة Render (منفذ 8080)\n✅ **إصلاح ذاتي كامل - لا حاجة لإعادة تشغيل خارجية**\n✅ **الوقف المسبق الموحد للعادي والميم: stop_loss_limit (مدعوم لجميع أزواج Spot)**")

    # تكوين Gemini مرة واحدة فقط عند بدء التشغيل
    if GEMINI_API_KEY:
        try:
            genai.configure(api_key=GEMINI_API_KEY)
