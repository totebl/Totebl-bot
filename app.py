...
class SimpleWebSocketManager:
    def __init__(self):
        ...
        # متغيرات لتحديث المرشحين عبر WebSocket
        self.last_candidates_update = 0
        self.candidates_update_interval = 600  # 10 دقائق
    ...
    def on_message(self, ws, message):
        try:
            ...
            with self.lock:
                ...
                # تحديث قائمة المرشحين من WebSocket كل 10 دقائق
                now = time.time()
    ...
    def _update_candidates_from_ws(self):
        """تحديث scanner.candidates باستخدام بيانات WebSocket (آلية احتياطية)"""
        global scanner
        if scanner is None:
            return
        ...
        try:
            ...
            for sym, data in tickers.items():
                ...
                if sym not in BASE_SYMBOLS_SET and FILTER_MARKET_CAP_ENABLED:
                    # استخدام القيمة السوقية المخزنة مؤقتاً أو تقدير تقريبي
            ...
            if filtered:
                ...
                # لا نغير next_scan_time حتى لا نلغي المسح المجدول
                logger.info(f"🔄 تم تحديث المرشحين عبر WebSocket ({len(scanner.candidates)} رمز)")
                # إرسال إشعار تلغرام إذا كانت القائمة جديدة ومختلفة عن السابقة
...
def analyze_and_trade():
    ...
    # ======== إضافة: إذا كنا نخرج من حالة الصفر (0 -> موجب)، نعيد ضبط المسح ========
    if CUSTOM_ACTIVE_SYMBOLS_LIMIT > 0 and _prev_active_symbols_limit == 0:
        ...
        global PAUSED, PAUSE_ANALYSIS, PAUSE_NEW_ENTRIES
        PAUSED = False
        PAUSE_ANALYSIS = False
        PAUSE_NEW_ENTRIES = False
        _prev_active_symbols_limit = CUSTOM_ACTIVE_SYMBOLS_LIMIT
    ...
    # ======== إضافة: التحقق من صلاحية قائمة المرشحين (احتياطي REST) ========
                    ...
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ========== الأوامر الديناميكية ==========
                                ...
                                if match:
                                    ...
                                    if 0 <= new_limit <= 200:
                                        ...
                                        # ======== إضافة: إعادة ضبط المسح عند الخروج من الصفر ========
                            ...
                            # ========== أمر إعادة الضبط ==========
                                ...
                                _prev_active_symbols_limit = ACTIVE_SYMBOLS_LIMIT
...
class MarketScanner:
    ...
    def scan(self):
        ...
        if not tickers:
            ...
            # لا نغير next_scan_time لتجنب التكرار السريع
            return
        filtered = []
        for sym, data in tickers.items():
