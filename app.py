...
# + التحديثات: get_actual_coin_balance, execute_market_close, force_market,
#   فحص الغبار، Throttling، Trailing Stop مخصص، أمر مزامنة، حفظ/استرجاع Trailing
# + تحديث Trailing Stop: تفعيل عند 2.2% / مسافة 1.4% / فحص فوري عند أول كسر
# + تحديث جديد: فلتر الذيل العلوي للشمعة الحالية 15د (طلب مستقل لحظي)
#   مع إظهار مسافة الذيل في رسالة الشراء (موجب/صفر/سالب)
# + تحديث Cooperative Timeout: الدورة تُنهي نفسها بعد 3 دقائق (قابل للتعديل حتى 230s)
#   مع إشعار تلغرام بعدد الرموز المفحوصة والناجحة ومدة الدورة
# + تحديث Execution Grace Period: فترة سماح 60s لتنفيذ الشراء بعد انتهاء التحليل
# ========================================================================
# 🆕 التحديثات الخمسة الجديدة (النسخة النهائية):
#   1) توسيع مسافة الذيل العلوي إلى 1.66% (0.0044 → 0.0166)
#   2) التنفيذ الفوري للشراء داخل نفس دورة التحليل (بدون انتظار)
#   3) حذف الفحص المكرر للتعرض (curr_exp) - الاعتماد على exp المحدَّث
#   4) تحديث الرصيد الخلفي كل 20-30 دقيقة بدلاً من 9-10 دقائق
#   5) تقليل مهلة Limit Order إلى 20 ثانية (بدل 45) ثم تحويل تلقائي إلى Market
#   + الخيار الآمن: الدورة تمتد إلى MAX_ANALYSIS + EXECUTION_GRACE (240s)
# ========================================================================
# 🆕 التحديث السادس: إضافة عدّادات تفصيلية للإشعار الختامي (9 عدّادات جديدة)
#   - rejected_regime, rejected_data, rejected_committee, rejected_score
#   - rejected_no_ticker, rejected_hour_candle, rejected_size, rejected_balance, rejected_liquidity
# ========================================================================
# 🆕 التحديث السابع (الجديد): 
#   1) فلتر جسم الشمعة الحالية (15د) - يرفض إذا كان ارتفاع الشمعة من افتتاحها > 4%
#   2) رفع حد السيولة اليومي من 2M$ إلى 4M$ (MIN_VOLUME_USD و MIN_24H_VOLUME_USD)
# ========================================================================
# 🆕 التحديث الثامن (الجديد جداً):
#   فلتر التوقيت الثلاثي (5/5/5): يرفض الصفقة إذا أعطى نموذج Timing
#   نفس القيمة (0.5) على الأطر الثلاثة معاً (5د + 15د + 1س)
#   مع إشعار فوري وتحكم كامل من تلغرام.
# ========================================================================
# 🆕 التحديث التاسع (نظام المراقبة قبل الشراء - Pending Watch):
#   - عند اجتياز الفرصة كل الفلاتر، لا يتم الشراء فوراً، بل تُوضع تحت المراقبة
#   - يُنفّذ الشراء فقط إذا تحرّك السعر +0.25% من سعر التحليل الأولي (تأكيد الاتجاه)
#   - يُلغى إذا انتهت المهلة (25 دقيقة) أو قفز السعر > 3% (منع المطاردة)
#   - تحكم كامل من تلغرام: حالة المراقبة / تعيين نسبة التفعيل / المهلة / سقف المطاردة
#   - مع إصلاح Deadlock في فحص التبريد (فصل الأقفال) + تنظيف كل 30 دقيقة
# ========================================================================
# 🆕 التحديث العاشر (الجديد): تسريع فحص التعرض في pending_watch_monitor
#   - استبدال get_total_equity() (طلب شبكي ~400ms) بقراءة من الذاكرة (~0ms)
#   - total_eq = bot_stats.last_balance + positions_value_cached
# ========================================================================
...
# ======== إضافة جلسة تلغرام الخاصة (تتجاوز الوكيل) ========
telegram_session = requests.Session()
telegram_session.trust_env = False   # تجاهل HTTP_PROXY و HTTPS_PROXY
# ==========================================================
...
# 🆕 التحديث الثاني: رفع الحد من 2M$ إلى 4M$
MIN_VOLUME_USD = float(os.environ.get('MIN_VOLUME_USD', 4_000_000))
...
MIN_24H_CHANGE_PERCENT = float(os.environ.get('MIN_24H_CHANGE_PERCENT', 5.0))
# 🆕 التحديث الثاني: رفع الحد من 2M$ إلى 4M$
MIN_24H_VOLUME_USD = float(os.environ.get('MIN_24H_VOLUME_USD', 4_000_000))
...
# ==========================================================================
# 🆕 فلتر الذيل العلوي للشمعة الحالية 15د (طلب مستقل لحظي)
# 🆕 التحديث 1: توسيع المسافة المسموحة إلى 1.66% (كانت 0.44%)
# ==========================================================================
MAX_UPPER_WICK_DISTANCE_PERCENT = float(os.environ.get('MAX_UPPER_WICK_DISTANCE_PERCENT', 0.0166))  # 🆕 1.66%
UPPER_WICK_FILTER_ENABLED = os.environ.get('UPPER_WICK_FILTER_ENABLED', 'true').lower() == 'true'
# ==========================================================================

# ==========================================================================
# 🆕 التحديث الأول: فلتر جسم الشمعة الحالية 15د
#   يرفض الشراء إذا كان ارتفاع الشمعة الحالية من افتتاحها > 4%
# ==========================================================================
MAX_CANDLE_BODY_PERCENT = float(os.environ.get('MAX_CANDLE_BODY_PERCENT', 0.04))  # 4%
CANDLE_BODY_FILTER_ENABLED = os.environ.get('CANDLE_BODY_FILTER_ENABLED', 'true').lower() == 'true'
# ==========================================================================

# ==========================================================================
# 🆕 فلتر التوقيت الثلاثي (5/5/5)
# ==========================================================================
TIMING_TRIPLE_FIVE_FILTER_ENABLED = os.environ.get('TIMING_TRIPLE_FIVE_FILTER_ENABLED', 'true').lower() == 'true'
TIMING_TRIPLE_FIVE_VALUE = float(os.environ.get('TIMING_TRIPLE_FIVE_VALUE', 0.5))
TIMING_TRIPLE_FIVE_TOLERANCE = float(os.environ.get('TIMING_TRIPLE_FIVE_TOLERANCE', 1e-6))
# ==========================================================================
...
TAKE_PROFIT_LEVELS_NORMAL = [0.015, 0.020, 0.025, 0.030, 0.035, 0.038, 0.045]
TAKE_PROFIT_LEVELS_MEME = [0.015, 0.020, 0.025, 0.035, 0.045, 0.050, 0.060]
...
MAX_ANALYSIS_CYCLE_SECONDS = 180   # مرحلة التحليل: 3 دقائق كحد أقصى (قابل للتعديل من تلغرام حتى 230s)
EXECUTION_GRACE_SECONDS = 60       # فترة سماح لتنفيذ الشراء بعد انتهاء التحليل
# ==========================================================================
# 🆕 نظام المراقبة قبل الشراء (Pending Watch)
# ==========================================================================
PENDING_WATCH_ENABLED = os.environ.get('PENDING_WATCH_ENABLED', 'true').lower() == 'true'
PENDING_WATCH_TRIGGER_PERCENT = float(os.environ.get('PENDING_WATCH_TRIGGER_PERCENT', 0.0025))   # 0.25%
PENDING_WATCH_TIMEOUT_SECONDS = int(os.environ.get('PENDING_WATCH_TIMEOUT_SECONDS', 1500))       # 25 دقيقة
PENDING_WATCH_MAX_CHASE_PERCENT = float(os.environ.get('PENDING_WATCH_MAX_CHASE_PERCENT', 0.03)) # 3%

_pending_watch = {}   # {symbol: {ref_price, created_at, size_usdt, atr, sym_type, ...}}
_pending_watch_lock = threading.Lock()
# ==========================================================================
...
# 🆕 التحديث 5: تقليل المهلة إلى 20 ثانية بدل 45 للدخول السريع ثم التحويل التلقائي إلى Market
LIMIT_TO_MARKET_TIMEOUT = int(os.environ.get('LIMIT_TO_MARKET_TIMEOUT', 20))
...
# ------------------- إعدادات استعادة المراكز المفقودة (جديدة) -------------------
MIN_RECOVER_POSITION_VALUE = float(os.environ.get('MIN_RECOVER_POSITION_VALUE', 4.0))  # أقل من 4$ يتم تجاهله
_recovery_lock = threading.Lock()  # قفل لمنع التكرار
PENDING_ORDER_TIMEOUT = 180  # إلغاء الأوامر المعلقة بعد 3 دقائق
...
def send_telegram(text):
    ...
    clean = clean.replace('&lt;code&gt;', '<code>').replace('&lt;/code&gt;', '</code>')
...
def send_telegram_photo(buf, caption=""):
    ...
    try:
        ...
        resp = telegram_session.post(url, files=files, data=data, timeout=30)
...
class Position:
    def __init__(self, symbol, side, size, entry, atr, sl, tp, sym_type, pred, conf, regime, tp_levels=None, ai_approved=False,
        ...
        self.trade_id = None
        self.manual_required_notified = False
        # ============ Trailing Stop مخصص (تحديث: 2.2% / 1.4%) ============
        self.trailing_active = False              # هل تم التفعيل؟
        self.trailing_peak_price = entry          # أعلى سعر بعد التفعيل
        self.trailing_last_check_time = None      # آخر فحص
        self.trailing_activation_profit = 0.022   # عتبة التفعيل (2.2%)
        self.trailing_drop_threshold = 0.014      # مسافة التتبع (1.4%)
        self.trailing_check_interval = 60         # ثوانٍ بين كل فحص متكرر
        self.trailing_triggered_pending = False   # هل كُسرت العتبة وينتظر فحص فوري؟
        # ==================================================================
...
# 🔥 إضافة الوكيل بشكل صريح إذا كانت متغيرات البيئة موجودة
http_proxy = os.environ.get('HTTP_PROXY') or os.environ.get('http_proxy')
https_proxy = os.environ.get('HTTPS_PROXY') or os.environ.get('https_proxy')
if http_proxy or https_proxy:
    primary_exchange.proxies = {
        'http': http_proxy or https_proxy,
        'https': https_proxy or http_proxy,
    }
    logger.info(f"✅ تم تعيين الوكيل لـ ccxt: {http_proxy or https_proxy}")
...
class SimpleWebSocketManager:
    ...
    def start(self):
        ...
        # قراءة الوكيل من متغيرات البيئة
        ...
        if proxy_url:
            ...
            if ':' in proxy_url_clean:
                ...
                self.proxy_port = int(parts[1]) if parts[1].isdigit() else 3128
            else:
                self.proxy_host = proxy_url_clean
                self.proxy_port = 3128
            logger.info(f"✅ سيتم استخدام وكيل WebSocket: {self.proxy_host}:{self.proxy_port}")
        ...
        while self.running:
            try:
                if self.proxy_host and self.proxy_port:
                    ...
                    self.ws.run_forever(ping_interval=30, ping_timeout=10, **proxy_options)
                else:
...
def execute_limit_order(symbol, side, size, price_ref, sym_type='normal'):
    ...
    for attempt in range(2):
        try:
            ...
            if "418" in error_msg or "banned" in error_msg:
            ...
            if "insufficient balance" in error_msg:
                logger.warning(f"⚠️ نقص رصيد لـ {symbol}، محاولة استعادة المراكز المفقودة...")
                recover_missing_positions()
                with _global_state_lock:
                    if symbol in open_positions:
                        logger.info(f"✅ تم استعادة {symbol}، لن نحاول الشراء مجدداً.")
                        return True, price_ref, size, "recovered"
            if attempt == 1:
                return False, None, 0, None
            continue
    return False, None, 0, None
...
# ======================================================================
# =========== التحديث 1 + 2: الدالتان الجديدتان ==========================
# ======================================================================
def get_actual_coin_balance(symbol):
    ...
    try:
        ...
        coin_data = balance.get(coin, {}) or {}
        free_amount = float(coin_data.get('free', 0.0) or 0.0)
        total_amount = float(coin_data.get('total', 0.0) or 0.0)
        amount = free_amount if free_amount > 0 else total_amount
        if amount <= 0:
            return 0.0, 0.0
        ticker = fetch_ticker_with_retry(symbol, max_retries=2)
        price = 0.0
        ...
        logger.info(f"📊 رصيد {symbol} الفعلي: {amount} (${value_usd:.4f})")
        return amount, value_usd
    except Exception as e:
        logger.error(f"❌ فشل جلب الرصيد الفعلي لـ {symbol}: {e}")
        return None, None


def execute_market_close(symbol, side, size, price_ref=0.0):
    """
    القيم المُرجعة:
    - (True, fill_price, filled_size, oid)     → نجاح
    - ("BELOW_MIN", value_usd, amount, None)   → قيمة < 5$ → تنبيه + حذف
    - (False, None, 0, None)                   → فشل نهائي
    """
    if _should_simulate():
        return True, price_ref, size, "sim"
    min_amt, max_amt, min_cost = get_amount_limits(symbol)
    ...
    exchange = get_active_exchange()
    try:
        requested_size = float(exchange.amount_to_precision(symbol, requested_size))
        size = requested_size
    except Exception as e:
        logger.warning(f"خطأ في تحويل الدقة لـ {symbol}: {e}")
    ...
    for attempt in range(MAX_RETRIES_CLOSE):
        try:
            ...
            if fill_price <= 0:
                ...
                fill_price = ticker['last'] if ticker else price_ref
            return True, fill_price, filled_size, order.get('id')
        except Exception as e:
            last_err = e
            err_low = str(e).lower()
            if ("insufficient" in err_low or "not enough" in err_low or "balance" in err_low):
                actual_amount, actual_value = get_actual_coin_balance(symbol)
                if actual_amount is None:
                    logger.warning(f"⚠️ فشل جلب الرصيد الفعلي لـ {symbol}")
                elif actual_value < 5.0:
                    return "BELOW_MIN", actual_value, actual_amount, None
                else:
                    sell_amount = min(actual_amount, requested_size)  # ← لا بيع أكثر من المطلوب
                    try:
                        new_size = float(exchange.amount_to_precision(symbol, sell_amount))
                        if new_size <= 0:
                            break
                        if side == 'sell':
                            order = exchange.create_market_sell_order(symbol, new_size)
                        else:
                            order = exchange.create_market_buy_order(symbol, new_size)
                        fill_price = float(order.get('average') or order.get('price') or 0)
                        filled_size = float(order.get('filled') or new_size)
                        if fill_price <= 0:
                            ticker = fetch_ticker_with_retry(symbol, max_retries=1)
                            fill_price = ticker['last'] if ticker else price_ref
                        return True, fill_price, filled_size, order.get('id')
                    except Exception as e2:
                        logger.error(f"❌ فشل الإغلاق بالحجم المقيّد: {e2}")
            if "418" in err_low or "banned" in err_low or "too many" in err_low:
                try:
                    retry_after = int(exchange.last_response_headers.get('Retry-After', 600))
                except Exception:
                    retry_after = 600
                time.sleep(retry_after + random.randint(0, 30))
            if attempt < MAX_RETRIES_CLOSE - 1:
...
# ===================== دوال استعادة المراكز المفقودة (جديدة) =====================
def recover_missing_positions():
    """
    تفحص الأرصدة في المنصة الحقيقية وتُنشئ Position لكل عملة
    غير USDT موجودة في المحفظة وليس لها مركز مسجل، بشرط أن تكون
    قيمة المركز أكبر من MIN_RECOVER_POSITION_VALUE (افتراضي 4$).
    تستخدم سعر الدخول من آخر صفقة شراء إن وجدت، وإلا تستخدم السعر الحالي.
    """
    global open_positions, bot_stats
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        logger.info("ℹ️ وضع المحاكاة/الورقي، لا حاجة لاستعادة المراكز المفقودة.")
        return

    # منع التكرار
    if not _recovery_lock.acquire(blocking=False):
        logger.info("⚠️ استعادة المراكز قيد التشغيل بالفعل، تخطي.")
        return
    ...
    try:
        ...
        try:
            ...
            logger.error(f"فشل جلب الأرصدة لاستعادة المراكز: {e}")
            return

        total_balance = balance.get('total', {})
        excluded = ['USDT', 'USDC', 'BUSD', 'TUSD', 'DAI']
        recovered_count = 0

        with _global_state_lock:
            for coin, amount in total_balance.items():
                if coin in excluded or amount <= 0:
                    continue
                symbol = f"{coin}/USDT"
                if symbol in open_positions:
                    continue  # المركز مسجل بالفعل

                # ---- تقدير قيمة المركز وتجاهل الصغير جداً ----
                try:
                    ticker = fetch_ticker_with_retry(symbol, max_retries=1)
                    if ticker and ticker.get('last'):
                        current_price = ticker['last']
                        position_value = amount * current_price
                        if position_value < MIN_RECOVER_POSITION_VALUE:
                            logger.info(f"تجاهل رصيد {symbol}: قيمته ${position_value:.2f} أقل من {MIN_RECOVER_POSITION_VALUE}$")
                            continue
                    else:
                        logger.warning(f"لا يمكن جلب السعر لـ {symbol}، تخطي الرصيد.")
                        continue
                except Exception as e:
                    logger.warning(f"فشل جلب السعر لـ {symbol} لتقدير القيمة: {e}")
                    continue
                # ------------------------------------------------

                # محاولة جلب سعر الدخول من آخر صفقة شراء
                entry_price = None
                try:
                    rest_rate_limiter.wait_if_needed(weight=10)
                    trades = exchange.fetch_my_trades(symbol, limit=1)
                    if trades and len(trades) > 0:
                        last_trade = trades[-1]
                        if last_trade['side'] == 'buy':
                            entry_price = float(last_trade['price'])
                            logger.info(f"تم العثور على سعر دخول من الصفقة الأخيرة لـ {symbol}: {entry_price}")
                except Exception as e:
                    logger.warning(f"فشل جلب آخر صفقة لـ {symbol}: {e}")

                if not entry_price:
                    # استخدام السعر الحالي كسعر دخول
                    try:
                        ticker = fetch_ticker_with_retry(symbol, max_retries=2)
                        if ticker and ticker.get('last'):
                            entry_price = ticker['last']
                            logger.info(f"استخدام السعر الحالي كسعر دخول لـ {symbol}: {entry_price}")
                        else:
                            logger.warning(f"لا يمكن تحديد سعر دخول لـ {symbol}، تخطي.")
                            continue
                    except Exception as e:
                        logger.warning(f"فشل جلب السعر الحالي لـ {symbol}: {e}")
                        continue

                # تحديد نوع العملة
                sym_type = classify_symbol(symbol, ticker_data={'last': entry_price})

                # حجم المركز = الرصيد كاملاً (نفترض أنه صفقة واحدة)
                size = amount
                ...
                sl_price, tp_price = dynamic_stop_loss_take_profit(entry_price, atr_val, 'buy', 'neutral', sym_type, max_sl)
                ...
                recovered_count += 1
                send_telegram(f"🔄 تم استعادة مركز مفقود: {symbol} - الكمية: {size:.6f} - سعر الدخول: {entry_price:.8f} - القيمة: ${position_value:.2f}")
                logger.info(f"✅ تم استعادة المركز المفقود لـ {symbol} (قيمته ${position_value:.2f})")

            if recovered_count > 0:
                save_state()
                send_telegram(f"✅ تم استعادة {recovered_count} مركز(ات) مفقودة.")
            else:
                logger.info("ℹ️ لا توجد مراكز مفقودة للاستعادة.")

    finally:
        _recovery_lock.release()

def periodic_position_recovery():
    """
    خيط خلفي يقوم بتشغيل recover_missing_positions كل 30 دقيقة
    لضمان مزامنة المراكز مع المحفظة الفعلية.
    """
    # انتظر 5 دقائق قبل البدء لتجنب التزاحم مع بدء التشغيل
    time.sleep(300)
    while True:
        try:
            if not PAPER_TRADING and not TEST_MODE and ENABLE_TRADING:
                logger.info("🔄 تشغيل الاستعادة الدورية للمراكز المفقودة (كل 30 دقيقة)...")
                send_telegram("🔄 تشغيل الاستعادة الدورية للمراكز المفقودة (كل 30 دقيقة)...")
                recover_missing_positions()
        except Exception as e:
            logger.error(f"خطأ في الاستعادة الدورية: {e}")
        # انتظر 30 دقيقة (1800 ثانية) بين كل استعادة
        time.sleep(1800)
...
# ======================================================================
# =========== التحديث 5,6,7,8,9: close_partial المحدثة بالكامل =========
# ======================================================================
def close_partial(symbol, percent, price, reason, pnl_usdt=None, force_market=False):
    ...
    with _global_state_lock:
        ...
        # ============ فحص الغبار أولاً (سريع) ============
        try:
            current_pos_value = pos.remaining_size * price if price > 0 else 0.0
            if 0 < current_pos_value < 5.0:
                logger.info(f"🚫 {symbol}: قيمة ${current_pos_value:.4f} < 5$ → حذف متوازن")
                if not getattr(pos, 'manual_required_notified', False):
                    pos.manual_required_notified = True
                    send_telegram(
                        f"🔴🔴🔴🔴🔴\n"
                        f"⚠️ <b>مركز محذوف تلقائياً</b>\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
                        f"🔸 الرمز: <code>{symbol}</code>\n"
                        f"🔸 الكمية: <code>{pos.remaining_size:.8f}</code>\n"
                        f"🔸 القيمة: <b>${current_pos_value:.4f}</b>\n"
                        f"🔸 السبب: القيمة أقل من حد التداول الفوري (5$).\n"
                        f"✅ تم حذف المركز من البوت.\n"
                        f"⚖️ PnL = 0 (متوازن).\n"
                        f"ℹ️ يمكنك بيع الغبار يدوياً من Binance."
                    )
                _local_pending_symbols.discard(symbol)
                _exchange_pending_symbols.discard(symbol)
                with _cooldown_lock:
            ...
            logger.warning(f"⚠️ فشل فحص الغبار لـ {symbol}: {e}")
        # ==================================================

        # ============ Throttling ============
        if pos.last_fail_time is not None:
            elapsed = (datetime.now() - pos.last_fail_time).total_seconds()
            threshold = 300 if getattr(pos, 'manual_required_notified', False) else 60
            if elapsed < threshold:
                logger.debug(f"⏭️ تخطي {symbol}: آخر فشل قبل {elapsed:.0f}s")
                return
        # =====================================
    ...
    try:
        ...
        try:
            if not is_sim:
                ...
                # === 1) محاولات أمر الحد (تُتخطى عند force_market=True) ===
                if not force_market:
                    for attempt in range(MAX_RETRIES_CLOSE):
                        ws_data = ws_manager.get_ticker(symbol)
                        if ws_data and ws_data.get('price', 0) > 0:
                        ...
                        success, fill_price, filled_size, _ = execute_limit_close(
                            symbol, close_side, close_size, curr, pos.symbol_type,
                ...
                # === 2) أمر السوق ===
                if not success:
                    m_result = execute_market_close(symbol, close_side, close_size, price)

                    # ============ حالة 1: قيمة < 5$ ============
                    if m_result[0] == "BELOW_MIN":
                        actual_value = m_result[1]
                        actual_amount = m_result[2]
                        logger.info(f"🚫 {symbol}: ${actual_value:.4f} < 5$ → حذف متوازن")
                        if not getattr(pos, 'manual_required_notified', False):
                            pos.manual_required_notified = True
                            send_telegram(
                                f"🔴🔴🔴🔴🔴\n"
                                ...
                                f"━━━━━━━━━━━━━━━━━━━━\n"
                                f"🔸 الرمز: <code>{symbol}</code>\n"
                                f"🔸 الكمية الفعلية: <code>{actual_amount:.8f}</code>\n"
                                f"🔸 القيمة: <b>${actual_value:.4f}</b>\n"
                                f"🔸 السبب: القيمة أقل من 5$.\n"
                                f"✅ تم حذف المركز من البوت.\n"
                                f"⚖️ PnL = 0 (متوازن)."
                            )
                        fill_price = price
                        filled_size = pos.remaining_size
                        close_size = pos.remaining_size
                        success = True
                        pnl_net = 0.0

                    # ============ حالة 2: نجاح الإغلاق ============
                    elif m_result[0] is True:
                        success = True
                        fill_price = m_result[1]
                        filled_size = m_result[2]
                        if price > 0:
                            slippage = abs(fill_price - price) / price
                            max_slippage = (MAX_SLIPPAGE_EMERGENCY_MEME
                                            if pos.symbol_type == 'meme'
                                            else MAX_SLIPPAGE_EMERGENCY_NORMAL)
            ...
            with _global_state_lock:
                ...
                if pos.remaining_size <= 1e-8:
                    ...
                    # else: pnl_net == 0 → إغلاق متوازن لا يُحسب ربحاً ولا خسارة
...
# ======================================================================
# =========== التحديث 10: close_all_positions بـ force_market ==========
# ======================================================================
def close_all_positions(reason="أمر إداري"):
    ...
    for sym in syms:
        ...
        if price:
            close_partial(sym, 1.0, price, reason, force_market=True)
            time.sleep(0.5)
            with _global_state_lock:
...
# ======================================================================
# =========== التحديث 12: retry_stuck_positions بـ force_market ========
# ======================================================================
    ...
    while True:
        try:
            ...
            for sym, pos in stuck.items():
                ...
                close_partial(sym, 1.0, cur_price, f"إعادة محاولة #{pos.retry_count+1} لإغلاق عالق", force_market=True)
                time.sleep(1)
            with _global_state_lock:
...
# ======================================================================
# =========== التحديث 11 + 13: monitor_positions المحدثة ================
# ======================================================================
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # ============ التحديث 13: Trailing Stop مخصص ============
                # 1) تفعيل عند ربح 2.2%
                if not pos.trailing_active and profit >= pos.trailing_activation_profit:
                    pos.trailing_active = True
                    pos.trailing_peak_price = cur_price
                    pos.trailing_last_check_time = datetime.now()
                    send_telegram(
                        f"🚀 <b>تفعيل مراقبة السعر (Trailing)</b>\n"
                        f"🔸 الرمز: <code>{sym}</code>\n"
                        f"🔸 الربح الحالي: {profit:.2%}\n"
                        f"🔸 سعر الذروة: {cur_price:.8f}\n"
                        f"ℹ️ سيتم البيع الكامل إذا انخفض السعر 1.4% عن الذروة "
                        f"وأصبح أقل من إغلاق آخر شمعة 15د."
                    )

                # 2) منطق Trailing إذا كان مفعلاً
                if pos.trailing_active:
                    if cur_price > pos.trailing_peak_price:
                        pos.trailing_peak_price = cur_price

                    if pos.trailing_peak_price > 0:
                        drop = (pos.trailing_peak_price - cur_price) / pos.trailing_peak_price
                    else:
                        drop = 0.0

                    if drop >= pos.trailing_drop_threshold:
                        # ==== الفحص الفوري في نفس الدورة عند أول كسر للعتبة ====
                        if not pos.trailing_triggered_pending:
                            pos.trailing_triggered_pending = True
                            should_check = True
                            logger.info(f"⚡ {sym}: أول كسر لعتبة التتبع ({drop:.2%}) - فحص فوري")
                        else:
                            # بعدها نلتزم بـ 60 ثانية بين كل فحص لتجنب حظر API
                            elapsed = (datetime.now() - pos.trailing_last_check_time).total_seconds() \
                                      if pos.trailing_last_check_time else 999
                            should_check = elapsed >= pos.trailing_check_interval
                        # =====================================================

                        if should_check:
                            pos.trailing_last_check_time = datetime.now()
                            df_15m_trail = fetch_ohlcv_retry_raw(sym, '15m', limit=3)
                            if not df_15m_trail.empty and len(df_15m_trail) >= 2:
                                last_closed_close = float(df_15m_trail['close'].iloc[-2])
                                if cur_price < last_closed_close:
                                    trail_reason = (
                                        f"🎯 Trailing Stop (ربح سابق {profit:.2%})\n"
                                        f"انخفض السعر {drop:.2%} عن الذروة {pos.trailing_peak_price:.8f}\n"
                                        f"السعر الحالي {cur_price:.8f} < إغلاق 15د {last_closed_close:.8f}"
                                    )
                                    close_partial(sym, 1.0, cur_price, trail_reason, force_market=True)
                                    continue
                                else:
                                    logger.info(f"⏸️ {sym}: Trailing - السعر ≥ إغلاق 15د، إعادة الفحص بعد {pos.trailing_check_interval}s")
                            else:
                                logger.warning(f"⚠️ {sym}: فشل جلب شمعة 15د لفحص Trailing")
                    else:
                        # السعر ارتد فوق العتبة → إعادة تعيين للسماح بفحص فوري مرة أخرى عند الكسر القادم
                        if pos.trailing_triggered_pending:
                            pos.trailing_triggered_pending = False
                            pos.trailing_last_check_time = None
                            logger.debug(f"🔄 {sym}: ارتد السعر فوق عتبة التتبع، إعادة تعيين الحالة")
                # ============================================================
                ...
                if pos.crash_monitor_start is None:
                    ...
                    if elapsed_seconds <= 120 and drop >= 0.04:
                        close_partial(sym, 1.0, cur_price, "💥 انهيار مفاجئ >4% خلال دقيقتين (إغلاق كامل)", force_market=True)
                        continue
                ...
                if pos.stop_loss:
                    ...
                    if (pos.side=='buy' and cur_price <= sl) or (pos.side=='sell' and cur_price >= sl):
                        close_partial(sym, 1.0, cur_price, "🛑 وقف خسارة", force_market=True)
                        continue
                
                if profit <= -0.06:
                    close_partial(sym, 1.0, cur_price, "💥 أقصى خسارة 6%", force_market=True)
                    continue
...
# 🆕 فلتر الذيل العلوي للشمعة الحالية 15د (طلب مستقل لحظي)
# ==========================================================================
def check_upper_wick_filter(symbol, current_price=None):
    """
    يفحص الذيل العلوي للشمعة الحالية (15د) بطلب مستقل حديث (بدون كاش).
    
    Returns:
        (ok: bool, message: str, distance: float or None)
        - ok: True = اسمح، False = ارفض
        - distance: (candle_high - current_price) / current_price
            * موجب: السعر تحت رأس الذيل (يوجد ذيل علوي)
            * صفر : السعر عند رأس الذيل بالضبط
            * سالب: السعر فوق رأس الذيل (اختراق حقيقي)
    """
    try:
        # ⭐ طلب مستقل حديث - بدون كاش (limit=3 لتقليل الحمل)
        df = fetch_ohlcv_retry_raw(symbol, '15m', limit=3, max_retries=2)
        if df is None or df.empty:
            return True, "بيانات غير كافية", None

        # ⭐ آخر صف = الشمعة الحالية (غير المغلقة)
        candle_high = float(df['high'].iloc[-1])

        # السعر الحالي من WebSocket أولاً (الأحدث)
        ...
        if current_price <= 0 or candle_high <= 0:
            return True, "سعر غير صالح", None

        # ⭐ حساب المسافة دائماً (حتى لو الفلتر معطل)
        distance = (candle_high - current_price) / current_price

        # إذا الفلتر معطل → اسمح دائماً لكن أعرض المسافة
        if not UPPER_WICK_FILTER_ENABLED:
            return True, "الفلتر معطل", distance

        if distance > MAX_UPPER_WICK_DISTANCE_PERCENT:
            return False, (
                f"🪶 ذيل علوي: رأس ({candle_high:.8f}) ← السعر ({current_price:.8f}) "
                f"= {distance:.2%} > {MAX_UPPER_WICK_DISTANCE_PERCENT:.2%}"
            ), distance
        return True, f"الذيل {distance:.2%} مقبول", distance
    except Exception as e:
        logger.debug(f"خطأ في check_upper_wick_filter لـ {symbol}: {e}")
        return True, "تعذر الفحص - تم السماح", None
# ==========================================================================

# ==========================================================================
# 🆕 التحديث الأول: فلتر جسم الشمعة الحالية 15د
# ==========================================================================
def check_candle_body_filter(symbol, current_price=None):
    """
    يفحص جسم الشمعة الحالية (15د) - يجب ألا يتجاوز ارتفاعها 4% من الافتتاح.

    Returns:
        (ok: bool, message: str, body_percent: float or None)
        - ok: True = اسمح، False = ارفض
        - body_percent: (current_price - candle_open) / candle_open
    """
    try:
        # طلب مستقل حديث - بدون كاش
        df = fetch_ohlcv_retry_raw(symbol, '15m', limit=3, max_retries=2)
        if df is None or df.empty:
            return True, "بيانات غير كافية", None
        ...
        if candle_open <= 0:
            return True, "سعر افتتاح غير صالح", None
        ...
        # حساب نسبة جسم الشمعة
        body_percent = (current_price - candle_open) / candle_open

        if not CANDLE_BODY_FILTER_ENABLED:
            return True, "الفلتر معطل", body_percent

        if body_percent > MAX_CANDLE_BODY_PERCENT:
            return False, (
                f"🔥 جسم الشمعة كبير: افتتاح ({candle_open:.8f}) ← سعر ({current_price:.8f}) "
                f"= {body_percent:+.2%} > {MAX_CANDLE_BODY_PERCENT:.2%}"
            ), body_percent
        return True, f"جسم الشمعة {body_percent:+.2%} مقبول", body_percent
    except Exception as e:
        logger.debug(f"خطأ في check_candle_body_filter لـ {symbol}: {e}")
        return True, "تعذر الفحص - تم السماح", None
# ==========================================================================
...
def execute_full_trade(symbol, side, size, price, atr, sl, tp, sym_type, pred, conf, regime, ai_approved=False, approval_reason="",
                        scores_5m=None, scores_15m=None, scores_1h=None,
                        weighted_score_5m=None, weighted_score_15m=None, weighted_score_1h=None,
                        final_score=None, upper_wick_distance=None,
                        reserved_by_caller=False):   # 🆕 الإصلاح #1
    ...
    try:
        ...
        # 🆕 الإصلاح #1: إذا حجز المُتصل الرمز مسبقاً، تجاوز فحص pending
    ...
    try:
        ...
        if _should_simulate():
            ...
            if upper_wick_distance is not None:
                wick_display = f"🪶 مسافة الذيل العلوي: {upper_wick_distance:+.2%}"
            else:
                wick_display = "🪶 مسافة الذيل العلوي: غير متاح"
...
# ==========================================================================
# 🆕 مراقب الفرص المُعلّقة (Pending Watch Monitor)
# ✅ مُدمج مع الإصلاح #1 (ترتيب الأقفال) والإصلاح #3 (تنظيف _local_pending)
# ✅ الإصلاح النهائي: فصل قفل التبريد لمنع Deadlock + تنظيف كل 30 دقيقة
# 🆕 التحديث العاشر: تسريع فحص التعرض (قراءة من الذاكرة بدل الشبكة)
# ==========================================================================
def pending_watch_monitor():
    """يراقب الرموز وينفذ الشراء عند +0.25% | يلغي عند > 3% أو انتهاء المهلة"""
    fails_tracker = {}
    last_cleanup = time.time()
    CLEANUP_INTERVAL_SECONDS = 1800   # 🆕 30 دقيقة (كان 300 ثانية)
    
    while True:
        try:
            with _pending_watch_lock:
                items = list(_pending_watch.items())

            if not items:
                time.sleep(0.5)   # 🆕 راحة أكبر عند عدم وجود عمل (3 رموز/يوم فقط)
                continue

            # تنظيف دوري لـ fails_tracker (كل 30 دقيقة)
            now_t = time.time()
            if now_t - last_cleanup > CLEANUP_INTERVAL_SECONDS:
                with _pending_watch_lock:
                    active_syms = set(_pending_watch.keys())
                fails_tracker = {s: c for s, c in fails_tracker.items() if s in active_syms}
                last_cleanup = now_t

            now = time.time()
            all_ws_tickers = ws_manager.get_all_tickers()
            with ws_manager.lock:
                ws_ts_snapshot = {k: v.get('timestamp', 0) for k, v in ws_manager.ticker_cache.items()}

            for sym, info in items:
                # 1) فحص الإيقاف العام
                if _is_paused() or PAUSE_NEW_ENTRIES:
                    continue

                elapsed = now - info['created_at']

                # 2) انتهاء المهلة
                if elapsed > PENDING_WATCH_TIMEOUT_SECONDS:
                    with _pending_watch_lock:
                        _pending_watch.pop(sym, None)
                    fails_tracker.pop(sym, None)
                    logger.warning(f"⏰ {sym}: انتهت مهلة المراقبة ({elapsed:.0f}s)")
                    send_telegram(
                        f"⏰ <b>انتهت مهلة المراقبة</b>\n"
                        f"🔸 <code>{sym}</code> لم يتحرك +{PENDING_WATCH_TRIGGER_PERCENT:.2%} "
                        f"خلال {PENDING_WATCH_TIMEOUT_SECONDS // 60} دقيقة\n"
                        f"❌ تم إلغاء الفرصة."
                    )
                    continue

                # 3) جلب السعر (WS أولاً، REST احتياطياً)
                ws_data = all_ws_tickers.get(sym)
                ws_age = now - ws_ts_snapshot.get(sym, 0)

                if ws_data and ws_data.get('last', 0) > 0 and ws_age < 5.0:
                    cur_price = ws_data['last']
                    fails_tracker[sym] = 0
                else:
                    last_rest = info.get('last_rest_attempt', 0)
                    if now - last_rest < 3.0:
                        continue
                    info['last_rest_attempt'] = now
                    ticker = fetch_ticker_with_retry(sym, max_retries=1)
                    if not ticker or not ticker.get('last'):
                        fails_tracker[sym] = fails_tracker.get(sym, 0) + 1
                        if fails_tracker[sym] == 10:
                            send_telegram(f"⚠️ {sym}: فشل جلب السعر 10 مرات متتالية")
                        continue
                    cur_price = ticker['last']
                    fails_tracker[sym] = 0
                ...
                # 4) حساب نسبة الحركة
                move_pct = (cur_price / info['ref_price']) - 1

                # 4أ) 🚫 منع المطاردة
                if move_pct > PENDING_WATCH_MAX_CHASE_PERCENT:
                    with _pending_watch_lock:
                        _pending_watch.pop(sym, None)
                    fails_tracker.pop(sym, None)
                    logger.info(f"🚫 {sym}: قفزة +{move_pct:.2%} > {PENDING_WATCH_MAX_CHASE_PERCENT:.2%} - إلغاء")
                    send_telegram(
                        f"🚫 <b>إلغاء (منع المطاردة)</b>\n"
                        f"🔸 <code>{sym}</code>\n"
                        f"🔸 القفزة: <b>+{move_pct:.2%}</b> > {PENDING_WATCH_MAX_CHASE_PERCENT:.2%}\n"
                        f"ℹ️ لا نطارد القمم المفاجئة."
                    )
                    continue

                # 4ب) شرط التفعيل
                if move_pct < PENDING_WATCH_TRIGGER_PERCENT:
                    continue

                # ══════════════════════════════════════════════════════════
                # 🆕 الإصلاح #1: دمج الأقفال لمنع حالة السباق
                # ══════════════════════════════════════════════════════════
                with _global_state_lock:
                    if sym in _local_pending_symbols or sym in _exchange_pending_symbols or sym in open_positions:
                        with _pending_watch_lock:
                            _pending_watch.pop(sym, None)
                        fails_tracker.pop(sym, None)
                        continue
                    _local_pending_symbols.add(sym)
                    with _pending_watch_lock:      # ← داخل نفس الـ with لمنع السباق
                        _pending_watch.pop(sym, None)
                # ══════════════════════════════════════════════════════════

                logger.info(f"⚡ {sym}: تفعيل (+{move_pct:.2%}) - جاري التنفيذ")
                trigger_detected_at = time.time()

                # 6) فحص الحد اليومي
                with _global_state_lock:
                    if _daily_trades_count >= CUSTOM_MAX_DAILY_TRADES:
                        _local_pending_symbols.discard(sym)
                        send_telegram(f"⛔ {sym}: الحد اليومي ({CUSTOM_MAX_DAILY_TRADES}) - إلغاء")
                        continue

                # ══════════════════════════════════════════════════════════
                # 7) فحص التبريد ✅ الإصلاح النهائي: فصل القفلين لمنع Deadlock
                #    الترتيب الرسمي في المشروع: _global_state_lock → _cooldown_lock
                #    لذلك نُغلق _cooldown_lock أولاً (استعلام فقط) ثم نفتحه قبل
                #    استخدام _global_state_lock.
                # ══════════════════════════════════════════════════════════
                with _cooldown_lock:
                    is_cooling = (
                        sym in _symbol_cooldown_until
                        and datetime.now() < _symbol_cooldown_until[sym]
                    )
                if is_cooling:
                    with _global_state_lock:
                        _local_pending_symbols.discard(sym)
                    logger.info(f"⏭️ {sym}: تبريد نشط - إلغاء")
                    continue
                # ══════════════════════════════════════════════════════════
                ...
                if free_bal < info['size_usdt']:
                    with _global_state_lock:
                        _local_pending_symbols.discard(sym)
                    send_telegram(f"⚠️ {sym}: رصيد غير كافٍ (${free_bal:.2f}) - إلغاء")
                    continue

                # 9) فحص التعرض الكلي (🆕 قراءة من الذاكرة - أسرع)
                with _global_state_lock:
                    pos_list = list(open_positions.items())
                    pending_others = len(_local_pending_symbols) - 1
                curr_exp = 0.0
                for s, p in pos_list:
                    ws_d = ws_manager.get_ticker(s)
                    pr = ws_d['price'] if ws_d and ws_d.get('price', 0) > 0 else p.entry_price
                    curr_exp += p.remaining_size * pr
                # قيمة المراكز المفتوحة فقط (بدون الرموز المعلقة)
                positions_value_cached = curr_exp
                curr_exp += pending_others * info['size_usdt']
                # 🆕 total_eq من الذاكرة = الرصيد الحر المخزّن + قيمة المراكز
                with _global_state_lock:
                    total_eq = bot_stats.last_balance + positions_value_cached
                max_exp = total_eq * CUSTOM_MAX_EXPOSED_PERCENT
                if curr_exp + info['size_usdt'] > max_exp:
                    with _global_state_lock:
                        _local_pending_symbols.discard(sym)
                    send_telegram(f"⛔ {sym}: تجاوز التعرض - إلغاء")
                    continue

                # 10) إعادة حساب الحجم
                size = info['size_usdt'] / cur_price
                min_amt, max_amt, min_cost = get_amount_limits(sym)
                if size < min_amt or (min_cost and size * cur_price < min_cost):
                    with _global_state_lock:
                        _local_pending_symbols.discard(sym)
                    send_telegram(f"⚠️ {sym}: حجم أقل من الحد الأدنى - إلغاء")
                    continue
                if max_amt and size > max_amt:
                    size = max_amt

                # 11) إعادة حساب SL/TP من سعر التفعيل
                max_sl = MAX_SL_PERCENT_MEME if info['sym_type'] == 'meme' else MAX_SL_PERCENT_NORMAL
                new_sl, new_tp = dynamic_stop_loss_take_profit(
                    cur_price, info['atr'], 'buy', info['regime'], info['sym_type'], max_sl
                )

                approval_reason = (
                    info['reason'] +
                    f"\n⚡ <b>تفعيل المراقبة:</b> +{move_pct:.2%} "
                    f"({info['ref_price']:.8f} → {cur_price:.8f})\n"
                    f"🏷️ النوع: {info['sym_type']}"
                )

                # 12) التنفيذ
                succ, actual_size = execute_full_trade(
                    sym, 'buy', size, cur_price, info['atr'],
                    new_sl, new_tp, info['sym_type'],
                    info['avg_score'], info['avg_score'], info['regime'],
                    ai_approved=True,
                    approval_reason=approval_reason,
                    scores_5m=info['scores_5m'],
                    scores_15m=info['scores_15m'],
                    scores_1h=info['scores_1h'],
                    weighted_score_5m=info['weighted_5m'],
                    weighted_score_15m=info['weighted_15m'],
                    weighted_score_1h=info['weighted_1h'],
                    final_score=info['final_score'],
                    upper_wick_distance=info['wick_distance'],
                    reserved_by_caller=True
                )

                latency_ms = (time.time() - trigger_detected_at) * 1000

                if succ:
                    send_telegram(
                        f"⚡ <b>تم تفعيل الشراء (تأكيد الاتجاه)</b>\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
                        f"🔸 <code>{sym}</code>\n"
                        f"🔸 المرجع: <code>{info['ref_price']:.8f}</code>\n"
                        f"🔸 التفعيل: <code>{cur_price:.8f}</code> (+{move_pct:.2%})\n"
                        f"🔸 SL/TP: <code>{new_sl:.8f}</code> / <code>{new_tp:.8f}</code>\n"
                        f"⚡ زمن التنفيذ: {latency_ms:.0f}ms"
                    )
                    logger.info(f"⚡ {sym}: شراء ناجح (latency={latency_ms:.0f}ms)")
                else:
                    # ══════════════════════════════════════════════════════════
                    # 🆕 الإصلاح #3: تنظيف احتياطي _local_pending عند الفشل
                    # ══════════════════════════════════════════════════════════
                    with _global_state_lock:
                        _local_pending_symbols.discard(sym)
                    # ══════════════════════════════════════════════════════════
                    send_telegram(
                        f"❌ <b>فشل التنفيذ بعد التفعيل</b>\n"
                        f"🔸 <code>{sym}</code> @ {cur_price:.8f}\n"
                        f"ℹ️ تم تحرير الحجز تلقائياً. راجع bot.log."
                    )
                    logger.warning(f"❌ {sym}: فشل التنفيذ بعد التفعيل - تم تنظيف _local_pending")

        except Exception as e:
            logger.error(f"خطأ في pending_watch_monitor: {e}")
            generate_error_report("فشل_خيط", "مراقبة", f"pending_watch_monitor: {e}", traceback.format_exc())
        time.sleep(0.5)   # 🆕 0.5s بدل 0.2s (رموز قليلة → لا حاجة للسرعة القصوى)
# ==========================================================================
...
def analyze_and_trade():
    ...
    try:
        ...
        # 🆕 تتبع وقت الدورة — للإنهاء التعاوني
        ...
        # 🆕 استبعد رموز المراقبة المعلقة من التحليل
        with _pending_watch_lock:
            forbidden = forbidden.union(set(_pending_watch.keys()))

        # ==================================================================
        # 🆕 التحديث 2: التنفيذ الفوري - تجهيز متغيرات التنفيذ قبل الحلقة
        # ==================================================================
        pos_usdt = total_eq * POSITION_SIZE_PERCENT
        rem_bal = free_bal
        exp = curr_exp
        ...
        analyzed_count = 0        # 🆕 عدّاد الرموز التي دخلت التحليل الفعلي
        approved_count = 0        # 🆕 اجتازت كل الفلاتر الأساسية
        executed_count = 0        # 🆕 تم شراؤها فعلياً
        watching_count = 0        # 🆕 الإصلاح #2: عدّاد الرموز تحت المراقبة
        rejected_wick = 0         # 🆕 مرفوضة بسبب الذيل العلوي
        skipped_limits = 0        # 🆕 مرفوضة بسبب حدود/تعرض/رصيد
        # 🆕 التحديث السادس: عدّادات تشخيصية جديدة (لتوضيح أسباب الرفض):
        rejected_regime = 0       # 🆕 اتجاه هابط
        rejected_data = 0         # 🆕 بيانات ناقصة (df_15m)
        rejected_committee = 0    # 🆕 لجنة الشراء رفضت
        rejected_score = 0        # 🆕 عتبة النتيجة/الحد الأعلى
        rejected_no_ticker = 0    # 🆕 لا يوجد سعر (ticker)
        rejected_hour_candle = 0  # 🆕 فلتر شمعة الساعة
        rejected_size = 0         # 🆕 حجم الصفقة صغير
        rejected_balance = 0      # 🆕 رصيد غير كافٍ (قبل الذيل)
        rejected_liquidity = 0    # 🆕 فلتر السيولة
        rejected_candle_body = 0  # 🆕 التحديث الأول: عدّاد جسم الشمعة كبير
        rejected_timing_triple = 0  # 🆕 فلتر التوقيت 5/5/5

        for sym in active:
            # 🆕 فحص وقت الدورة - إنهاء تعاوني (مع فترة سماح التنفيذ - الخيار الآمن)
            if time.time() - cycle_start_time > (MAX_ANALYSIS_CYCLE_SECONDS + EXECUTION_GRACE_SECONDS):
                logger.warning(f"⏱️ الدورة تجاوزت {MAX_ANALYSIS_CYCLE_SECONDS + EXECUTION_GRACE_SECONDS}s - إنهاء تعاوني (فحص {analyzed_count} رمز، تنفيذ {executed_count})")
                break
            ...
            # 🆕 زيادة العدّاد - الرمز دخل التحليل الفعلي
            analyzed_count += 1
            
            regime = detect_market_regime(sym)
            if regime == 'trending_down':
                rejected_regime += 1         # 🆕 عدّاد: اتجاه هابط
                continue
            ...
            if df_15m.empty or len(df_15m) < 50:
                rejected_data += 1           # 🆕 عدّاد: بيانات ناقصة
                continue
            ...
            if dec != 'buy':
                rejected_committee += 1      # 🆕 عدّاد: لجنة الشراء رفضت
                continue
            ...
            if (sym_type == 'meme' and final_score < SCALP_MIN_PROFIT) or (sym_type != 'meme' and final_score < STRENGTH_THRESHOLD):
                rejected_score += 1          # 🆕 عدّاد: عتبة النتيجة
                continue
            if final_score > UPPER_THRESHOLD_GLOBAL:
                rejected_score += 1          # 🆕 عدّاد: تجاوز الحد الأعلى
                continue

            # ==========================================================
            # 🆕 فلتر التوقيت الثلاثي: رفض 5/5/5 على (5د + 15د + 1س)
            # ==========================================================
            if TIMING_TRIPLE_FIVE_FILTER_ENABLED:
                t_5m = scores_5m.get('timing')
                t_15m = scores_15m.get('timing')
                t_1h = scores_1h.get('timing')
                if t_5m is not None and t_15m is not None and t_1h is not None:
                    def _is_five(x):
                        return abs(x - TIMING_TRIPLE_FIVE_VALUE) < TIMING_TRIPLE_FIVE_TOLERANCE
                    if _is_five(t_5m) and _is_five(t_15m) and _is_five(t_1h):
                        rejected_timing_triple += 1
                        logger.info(
                            f"⏭️ رفض {sym}: اجتماع 5/5/5 على Timing "
                            f"(5د={t_5m:.3f}, 15د={t_15m:.3f}, 1س={t_1h:.3f})"
                        )
                        send_telegram(
                            f"⏭️ <b>رفض (فلتر التوقيت الثلاثي)</b>\n"
                            f"━━━━━━━━━━━━━━━━━━━━\n"
                            f"🔸 الرمز: <code>{sym}</code>\n"
                            f"🔸 عضو التوقيت (Timing) أعطى <b>5 / 5 / 5</b>\n"
                            f"   • 5 دقائق: <code>{t_5m:.3f}</code>\n"
                            f"   • 15 دقيقة: <code>{t_15m:.3f}</code>\n"
                            f"   • 1 ساعة: <code>{t_1h:.3f}</code>\n"
                            f"ℹ️ السبب: اجتماع ثلاث خمسات = إشارة توقيت ضعيفة/محايدة."
                        )
                        continue
            # ==========================================================
            ...
            min_amt, max_amt, min_cost = get_amount_limits(sym)
            if size < min_amt:
                rejected_size += 1           # 🆕 عدّاد: حجم أقل من الحد الأدنى
                continue
            if max_amt and size > max_amt: size = max_amt
            if min_cost and size * cur_price < min_cost:
                rejected_size += 1           # 🆕 عدّاد: قيمة أقل من الحد الأدنى
                continue
            if free_bal < size * cur_price:
                rejected_balance += 1        # 🆕 عدّاد: رصيد غير كافٍ
                continue
            if FILTER_LIQUIDITY_ENABLED:
                liquidity_ok, liq_msg = check_liquidity(sym, size * cur_price, sym_type)
                if not liquidity_ok:
                    rejected_liquidity += 1  # 🆕 عدّاد: سيولة غير كافية
            ...
            # ============================================================
            # 🆕 التحديث 2: التنفيذ الفوري - بدون أي انتظار
            # ============================================================
            with _global_state_lock:
                if _daily_trades_count >= CUSTOM_MAX_DAILY_TRADES:
                    logger.info("⛔ تم الوصول للحد اليومي للصفقات، إيقاف التنفيذ الفوري")
                    break
                if sym in open_positions or sym in _local_pending_symbols or sym in _exchange_pending_symbols:
                    continue

            # ⭐ فحص الذيل العلوي اللحظي (طلب مستقل قبل الشراء مباشرة)
            wick_ok, wick_msg, wick_distance = check_upper_wick_filter(sym, cur_price)
            if not wick_ok:
                rejected_wick += 1
                logger.info(f"⏭️ رفض فوري {sym}: {wick_msg}")
                send_telegram(f"⏭️ رفض فوري {sym} (ذيل علوي): {wick_msg}")
                continue

            # ⭐ التحديث الأول: فحص جسم الشمعة اللحظي (جديد)
            body_ok, body_msg, body_percent = check_candle_body_filter(sym, cur_price)
            if not body_ok:
                rejected_candle_body += 1
                logger.info(f"⏭️ رفض فوري {sym}: {body_msg}")
                send_telegram(f"⏭️ رفض فوري {sym} (جسم شمعة): {body_msg}")
                continue

            # ⭐ فحوصات التنفيذ النهائية (بالأسعار الحالية)
            sz = pos_usdt / cur_price
            min_amt2, max_amt2, min_cost2 = get_amount_limits(sym)
            if sz < min_amt2:
                skipped_limits += 1
                continue
            if max_amt2 and sz > max_amt2: sz = max_amt2
            if min_cost2 and sz * cur_price < min_cost2:
                skipped_limits += 1
                continue
            if exp + pos_usdt > max_exp2:
                logger.info(f"⛔ {sym}: تجاوز التعرض الأقصى (exp={exp:.0f} + {pos_usdt:.0f} > {max_exp2:.0f})")
                skipped_limits += 1
                continue
            if rem_bal < pos_usdt:
                logger.info(f"⛔ {sym}: رصيد غير كافٍ ({rem_bal:.2f} < {pos_usdt:.2f})")
                skipped_limits += 1
                continue
            ...
            # 🆕 نظام المراقبة قبل الشراء (Pending Watch)
            # ================================================================
            if PENDING_WATCH_ENABLED:
                with _pending_watch_lock:
                    if sym in _pending_watch:
                        continue
                    _pending_watch[sym] = {
                        'ref_price': cur_price,
                ...
                trigger_price = cur_price * (1 + PENDING_WATCH_TRIGGER_PERCENT)
                logger.info(f"👁️ {sym}: تحت المراقبة (مرجع={cur_price:.8f}, هدف={trigger_price:.8f})")
                send_telegram(
                    f"👁️ <b>تحت المراقبة قبل التنفيذ</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━\n"
                    f"🔸 الرمز: <code>{sym}</code> ({sym_type})\n"
                    f"🔸 المرجع: <code>{cur_price:.8f}</code>\n"
                    f"🔸 الهدف: <code>{trigger_price:.8f}</code> (+{PENDING_WATCH_TRIGGER_PERCENT:.2%})\n"
                    f"🔸 سقف المطاردة: +{PENDING_WATCH_MAX_CHASE_PERCENT:.2%}\n"
                    f"🔸 الحجم المخطط: ~${pos_usdt:.2f}\n"
                    f"⏰ المهلة: {PENDING_WATCH_TIMEOUT_SECONDS // 60} دقيقة"
                )
                forbidden.add(sym)
                watching_count += 1     # 🆕 الإصلاح #2
            else:
                # الوضع القديم: تنفيذ فوري مباشر
                succ, actual_size = execute_full_trade(
                    sym, 'buy', sz, cur_price, atr_val,
                    sl_price, tp_price, sym_type, avg_score, avg_score, regime,
                    ai_approved=True,
                    approval_reason=reason,
                    scores_5m=scores_5m, scores_15m=scores_15m, scores_1h=scores_1h,
                    weighted_score_5m=weighted_5m, weighted_score_15m=weighted_15m,
                    weighted_score_1h=weighted_1h, final_score=final_score,
                    upper_wick_distance=wick_distance
                )
                if succ:
                    executed_count += 1
                    forbidden.add(sym)
                    ...
                    if new_rem is None:
                        logger.error("⚠️ فشل تحديث الرصيد بعد الصفقة. إيقاف التنفيذ الفوري.")
                        send_telegram("⚠️ فشل تحديث الرصيد بعد الصفقة. تم إيقاف تنفيذ المزيد من الفرص في هذه الدورة.")
                        break
                    rem_bal = new_rem
                    exp += actual_size * cur_price
                    logger.info(f"✅ تنفيذ فوري: {sym} | حجم={actual_size:.6f} | سعر={cur_price:.8f}")
                else:
                    skipped_limits += 1

        # ============================================================
        # 🆕 إشعار إغلاق دورة التحليل (مع عدّادات تفصيلية - التحديث السادس + السابع + الثامن)
        # ============================================================
        if analyzed_count > 0:
            cycle_duration = time.time() - cycle_start_time
            timed_out = cycle_duration >= (MAX_ANALYSIS_CYCLE_SECONDS + EXECUTION_GRACE_SECONDS)
            status_icon = "⏱️ انتهت بالتايم آوت" if timed_out else "✅ انتهت طبيعياً"
            skipped = len(active) - analyzed_count
            send_telegram(
                f"📊 <b>انتهت دورة التحليل (تنفيذ فوري)</b> — {status_icon}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"🔍 <b>رموز تم فحصها:</b> {analyzed_count} / {len(active)}\n"
                f"⏭️ <b>تم تخطيها (مراكز/تبريد/تايم آوت):</b> {skipped}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"📋 <b>تفصيل أسباب الرفض:</b>\n"
                f"   🔻 اتجاه هابط: {rejected_regime}\n"
                f"   📊 بيانات ناقصة: {rejected_data}\n"
                f"   🗳️ لجنة الشراء: {rejected_committee}\n"
                f"   🎯 عتبة النتيجة/الحد الأعلى: {rejected_score}\n"
                f"   💱 سعر غير متوفر: {rejected_no_ticker}\n"
                f"   🕐 فلتر شمعة الساعة: {rejected_hour_candle}\n"
                f"   📏 حجم الصفقة: {rejected_size}\n"
                f"   💰 رصيد غير كافٍ: {rejected_balance}\n"
                f"   💧 سيولة غير كافية: {rejected_liquidity}\n"
                f"   🔥 جسم شمعة كبير: {rejected_candle_body}\n"
                f"   ⏰ توقيت 5/5/5 (3 أطر): {rejected_timing_triple}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"✅ <b>اجتازت الفلاتر الأساسية:</b> {approved_count}\n"
                f"👁️ <b>تحت المراقبة (Pending Watch):</b> {watching_count}\n"
                f"🛒 <b>تم شراؤها فوراً:</b> {executed_count}\n"
                f"🪶 <b>مرفوضة (ذيل علوي):</b> {rejected_wick}\n"
                f"⚙️ <b>مرفوضة (حدود/تعرض نهائي):</b> {skipped_limits}\n"
                f"⏱️ <b>مدة الدورة:</b> {cycle_duration:.1f}s"
            )
        # ============================================================
    except TimeoutError:
...
def sync_pending_orders():
    ...
    try:
        ...
        for order in orders:
            ...
            # إلغاء الأوامر المعلقة التي تزيد عن PENDING_ORDER_TIMEOUT (180 ثانية)
            if (now_ts - order_time).total_seconds() > PENDING_ORDER_TIMEOUT and sym not in open_positions:
                try:
                    rest_rate_limiter.wait_if_needed(weight=2)
                    exchange.cancel_order(order['id'], sym)
                    logger.info(f"✅ تم إلغاء أمر معلق قديم لـ {sym} (عمره > {PENDING_ORDER_TIMEOUT} ثانية)")
                    send_telegram(f"🔄 تم إلغاء أمر معلق لـ {sym} (كان يحجز الرصيد)")
                except Exception as e:
...
def self_heartbeat():
    ...
    while True:
        try:
            # استخدم جلسة منفصلة لتجاهل الوكيل للـ localhost
            local_session = requests.Session()
            local_session.trust_env = False
            local_session.get(f"http://localhost:{port}/ping", timeout=5)
        except:
            pass
        time.sleep(300)
...
def background_balance_updater():
    ...
    while True:
        ...
        # 🆕 التحديث 4: تحديث الرصيد الخلفي كل 20-30 دقيقة بدل 9-10 دقائق (تقليل استهلاك API)
        sleep_duration = random.uniform(1200, 1800)
    ...
    while True:
        try:
            with _global_state_lock:
                ...
                daily_trades = _daily_trades_count
                total_equity = get_total_equity()
            report = (f"<b>📊 تقرير دوري (كل 4 ساعات)</b>\n🕒 الوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n💰 الرصيد الكلي: ${total_equity:.2f}\n📈 إجمالي الربح الصافي: ${total_pnl:.2f}\n📊 صفقات اليوم: {daily_trades}\n🔓 مراكز مفتوحة: {positions_count}\n⏸️ متوقف: {'نعم' if paused else 'لا'}\n🛑 عتبات الخسارة الوقائية:\n   - بيع 25% عند {STOP_LOSS_PARTIAL_1_PERCENT:.2%}\n   - بيع 33% عند {STOP_LOSS_PARTIAL_2_PERCENT:.2%}\n   - بيع كامل عند {STOP_LOSS_FULL_PERCENT:.2%}\n⚖️ أوزان الأطر: 5م={WEIGHT_5M:.2f}% | 15د={WEIGHT_15M:.2f}% | 1س={WEIGHT_1H:.2f}%\n🔺 <b>الحد الأعلى الموحد للثقة:</b> {UPPER_THRESHOLD_GLOBAL:.2f}\n📏 <b>حجم الصفقة (نسبة ثابتة):</b> {POSITION_SIZE_PERCENT:.1%}\n🪶 <b>فلتر الذيل العلوي:</b> {'✅ مفعل' if UPPER_WICK_FILTER_ENABLED else '❌ معطل'} (مسافة {MAX_UPPER_WICK_DISTANCE_PERCENT:.2%})\n🔥 <b>فلتر جسم الشمعة:</b> {'✅ مفعل' if CANDLE_BODY_FILTER_ENABLED else '❌ معطل'} (حد {MAX_CANDLE_BODY_PERCENT:.2%})\n⏰ <b>فلتر التوقيت الثلاثي:</b> {'✅ مفعل' if TIMING_TRIPLE_FIVE_FILTER_ENABLED else '❌ معطل'}\n👁️ <b>المراقبة قبل الشراء:</b> {'✅ مفعل' if PENDING_WATCH_ENABLED else '❌ معطل'} (+{PENDING_WATCH_TRIGGER_PERCENT:.2%} | سقف {PENDING_WATCH_MAX_CHASE_PERCENT:.2%})\n⏱️ <b>وقت التحليل:</b> {MAX_ANALYSIS_CYCLE_SECONDS}s | <b>فترة سماح التنفيذ:</b> {EXECUTION_GRACE_SECONDS}s\n⚡ <b>مهلة Limit قبل Market:</b> {LIMIT_TO_MARKET_TIMEOUT}s")
...
# ======================================================================
# =========== التحديث 14: telegram_polling (يحتوي أمر مزامنة) ==========
# ======================================================================
def telegram_polling():
    ...
    global PAUSED, last_analysis_time
    global UPPER_WICK_FILTER_ENABLED, MAX_UPPER_WICK_DISTANCE_PERCENT
    global CANDLE_BODY_FILTER_ENABLED, MAX_CANDLE_BODY_PERCENT  # 🆕 التحديث الأول
    global MAX_ANALYSIS_CYCLE_SECONDS   # 🆕
    global EXECUTION_GRACE_SECONDS      # 🆕
    global TIMING_TRIPLE_FIVE_FILTER_ENABLED, TIMING_TRIPLE_FIVE_VALUE  # 🆕 فلتر التوقيت الثلاثي
    global PENDING_WATCH_ENABLED, PENDING_WATCH_TRIGGER_PERCENT, PENDING_WATCH_TIMEOUT_SECONDS, PENDING_WATCH_MAX_CHASE_PERCENT
    ...
    while True:
        try:
            ...
            try:
                resp = telegram_session.get(url, params=params, timeout=35)
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ========== حالة الفلاتر ==========
                                ...
                                status = (f"📊 <b>حالة الفلاتر والقيم الحالية</b>\n"
                                          ...
                                          f"👁️ <b>المراقبة قبل الشراء:</b> {'✅ مفعل' if PENDING_WATCH_ENABLED else '❌ معطل'} "
                                          f"(+{PENDING_WATCH_TRIGGER_PERCENT:.2%} | سقف {PENDING_WATCH_MAX_CHASE_PERCENT:.2%} | "
                                          f"مهلة {PENDING_WATCH_TIMEOUT_SECONDS // 60}د)\n"
                                          f"🪶 <b>فلتر الذيل العلوي (15د):</b> {'✅ مفعل' if UPPER_WICK_FILTER_ENABLED else '❌ معطل'} (مسافة {MAX_UPPER_WICK_DISTANCE_PERCENT:.2%})\n"
                                          f"🔥 <b>فلتر جسم الشمعة (15د):</b> {'✅ مفعل' if CANDLE_BODY_FILTER_ENABLED else '❌ معطل'} (حد {MAX_CANDLE_BODY_PERCENT:.2%})\n"
                                          f"⏰ <b>فلتر التوقيت الثلاثي (5/5/5):</b> {'✅ مفعل' if TIMING_TRIPLE_FIVE_FILTER_ENABLED else '❌ معطل'}\n"
                                          f"⏱️ <b>وقت انتهاء دورة التحليل:</b> {MAX_ANALYSIS_CYCLE_SECONDS} ثانية\n"
                                          f"➕ <b>فترة سماح التنفيذ:</b> {EXECUTION_GRACE_SECONDS} ثانية\n"
                                          f"🧮 <b>الميزانية الكلية للدورة:</b> {MAX_ANALYSIS_CYCLE_SECONDS + EXECUTION_GRACE_SECONDS} ثانية\n"
                                          f"⚡ <b>مهلة Limit Order قبل Market:</b> {LIMIT_TO_MARKET_TIMEOUT} ثانية\n"
                                          f"━━━━━━━━━━━━━━━━━━━━\n"
                            ...
                            # ========== التحديث 14: أمر مزامنة المراكز ==========
                            elif text == "مزامنة" or text == "زامن" or text == "مزامنة المراكز":
                                with _global_state_lock:
                                    syms_to_sync = list(open_positions.keys())
                                if not syms_to_sync:
                                    send_telegram("📭 لا توجد مراكز للمزامنة.")
                                else:
                                    send_telegram(f"🔄 جاري مزامنة {len(syms_to_sync)} مركز...")
                                    synced = 0; deleted = 0; failed = 0
                                    lines = ["<b>🔄 تقرير المزامنة</b>", "━━━━━━━━━━━━━━━━━━━━"]
                                    for sym in syms_to_sync:
                                        try:
                                            actual_amount, actual_value = get_actual_coin_balance(sym)
                                            if actual_amount is None:
                                                lines.append(f"⚠️ {sym}: فشل جلب الرصيد")
                                                failed += 1
                                                continue
                                            with _global_state_lock:
                                            ...
                                            # حالة 1: القيمة < 5$ → حذف
                                            if actual_value < 5.0:
                                                lines.append(f"🗑️ {sym}: ${actual_value:.4f} < 5$ → محذوف")
                                                with _global_state_lock:
                                                    if sym in open_positions:
                                            ...
                                            # حالة 2: فرق > 1% → تحديث
                                            elif abs(actual_amount - old_size) / max(old_size, 1e-9) > 0.01:
                                                with _global_state_lock:
                                                    if sym in open_positions:
                                                        open_positions[sym].remaining_size = actual_amount
                                                        open_positions[sym].total_size = actual_amount
                                                lines.append(f"✅ {sym}: {old_size:.6f} → {actual_amount:.6f} (${actual_value:.2f})")
                                                synced += 1

                                            # حالة 3: مطابق
                                            else:
                                                lines.append(f"☑️ {sym}: مطابق ({actual_amount:.6f} | ${actual_value:.2f})")
                                        except Exception as e:
                                            lines.append(f"❌ {sym}: {str(e)[:60]}")
                                            failed += 1
                                    lines.append("━━━━━━━━━━━━━━━━━━━━")
                                    lines.append(f"📊 مُحدّث={synced} | محذوف={deleted} | فشل={failed}")
                                    if deleted > 0:
                                        lines.append("🔴🔴🔴🔴🔴")
                                    save_state()
                                    send_telegram("\n".join(lines))
                            ...
                            # ========== 🆕 تحكم في وقت انتهاء الدورة ==========
                            elif text.startswith('تعيين وقت انتهاء الدورة') or text.startswith('مدة الدورة'):
                                match = re.search(r'(\d+)', text)
                                if match:
                                    new_val = int(match.group(1))
                                    # الحد الأقصى المحسوب: 300s (LOCK_TIMEOUT) - 60s (EXECUTION_GRACE) - 10s (هامش) = 230s
                                    if 60 <= new_val <= 230:
                                        MAX_ANALYSIS_CYCLE_SECONDS = new_val
                                        save_state()
                                        send_telegram(
                                            f"✅ تم تعيين وقت انتهاء دورة التحليل إلى <b>{new_val} ثانية</b>\n"
                                            f"➕ فترة سماح التنفيذ: {EXECUTION_GRACE_SECONDS}s\n"
                                            f"⏱️ الميزانية الكلية: {new_val + EXECUTION_GRACE_SECONDS}s\n"
                                            f"ℹ️ الحد الأقصى: 230s (لضمان أن التحليل + التنفيذ < 300s = وقت التحرير القسري)"
                                        )
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 60 و 230 ثانية\n(سبب الحد الأقصى 230: تجنّب التحرير القسري عند 300s مع فترة سماح التنفيذ 60s)")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين وقت انتهاء الدورة 180`")
                            ...
                            # ========== 🆕 التحديث الأول: أوامر فلتر جسم الشمعة ==========
                            elif text.startswith('تعيين حد جسم الشمعة'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_val = float(match.group(1)) / 100.0
                                    if 0.001 <= new_val <= 0.20:
                                        MAX_CANDLE_BODY_PERCENT = new_val
                                        save_state()
                                        send_telegram(
                                            f"✅ تم تعيين حد جسم الشمعة إلى {new_val:.2%}\n"
                                            f"ℹ️ سيتم رفض أي صفقة يكون فيها ارتفاع الشمعة الحالية (15د) "
                                            f"من سعر افتتاحها أكبر من {new_val:.2%}"
                                        )
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.1% و 20%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين حد جسم الشمعة 4%`")
                            elif text == 'شغل فلتر جسم الشمعة':
                                CANDLE_BODY_FILTER_ENABLED = True
                                save_state()
                                send_telegram(
                                    f"🟢 تم تفعيل فلتر جسم الشمعة\n"
                                    f"📏 الحد الحالي: {MAX_CANDLE_BODY_PERCENT:.2%}"
                                )
                            elif text == 'أوقف فلتر جسم الشمعة':
                                CANDLE_BODY_FILTER_ENABLED = False
                                save_state()
                                send_telegram("🔴 تم إيقاف فلتر جسم الشمعة (لن يتم فحص جسم الشمعة قبل الشراء)")

                            # ========== 🆕 أوامر فلتر التوقيت الثلاثي (5/5/5) ==========
                            elif text == 'شغل فلتر التوقيت الثلاثي':
                                TIMING_TRIPLE_FIVE_FILTER_ENABLED = True
                                save_state()
                                send_telegram("🟢 تم تفعيل فلتر التوقيت الثلاثي (رفض 5/5/5).")
                            elif text == 'أوقف فلتر التوقيت الثلاثي':
                                TIMING_TRIPLE_FIVE_FILTER_ENABLED = False
                                save_state()
                                send_telegram("🔴 تم إيقاف فلتر التوقيت الثلاثي.")
                            elif text == 'حالة فلتر التوقيت الثلاثي' or text == 'فلتر التوقيت الثلاثي':
                                send_telegram(
                                    f"📊 <b>حالة فلتر التوقيت الثلاثي</b>\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                    f"🔘 الحالة: {'✅ مفعل' if TIMING_TRIPLE_FIVE_FILTER_ENABLED else '❌ معطل'}\n"
                                    f"📏 الدرجة المُراقَبة: {TIMING_TRIPLE_FIVE_VALUE:.2f}\n"
                                    f"🎯 قاعدة الرفض: Timing = 5/5/5 على (5د + 15د + 1س) معًا\n"
                                    f"✅ مقبول: 5/6/3 | 5/5/6 | 6/6/5 | 5/3/3 | 7/5/3 ...\n"
                                    f"❌ مرفوض: 5/5/5 فقط"
                                )
                            # ==========================================================

                            elif text == 'حالة فلتر جسم الشمعة' or text == 'فلتر جسم الشمعة':
                                send_telegram(
                                    f"📊 <b>حالة فلتر جسم الشمعة</b>\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                    f"🔘 الحالة: {'✅ مفعل' if CANDLE_BODY_FILTER_ENABLED else '❌ معطل'}\n"
                                    f"📏 الحد الأقصى لارتفاع جسم الشمعة: {MAX_CANDLE_BODY_PERCENT:.2%}\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                    f"ℹ️ يرفض الدخول إذا كان ارتفاع الشمعة الحالية (15د) "
                                    f"من سعر افتتاحها أكبر من {MAX_CANDLE_BODY_PERCENT:.2%}\n"
                                    f"⚙️ للتحكم:\n"
                                    f"• `تعيين حد جسم الشمعة 4%`\n"
                                    f"• `شغل فلتر جسم الشمعة`\n"
                                    f"• `أوقف فلتر جسم الشمعة`"
                                )
                            # ==============================================
                            
                            # ========== 🆕 أوامر فلتر الذيل العلوي ==========
                            elif text.startswith('تعيين مسافة الذيل العلوي'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_val = float(match.group(1)) / 100.0
                                    if 0.0005 <= new_val <= 0.05:
                                        MAX_UPPER_WICK_DISTANCE_PERCENT = new_val
                                        save_state()
                                        send_telegram(
                                            f"✅ تم تعيين حد مسافة الذيل العلوي إلى {new_val:.2%}\n"
                                            f"ℹ️ سيتم رفض أي صفقة تكون فيها المسافة بين رأس ذيل الشمعة الحالية (15د) "
                                            f"والسعر الحالي أكبر من {new_val:.2%}"
                                        )
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.05% و 5%")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين مسافة الذيل العلوي 1.66%`")
                            elif text == 'شغل فلتر الذيل العلوي':
                                UPPER_WICK_FILTER_ENABLED = True
                                save_state()
                                send_telegram(
                                    f"🟢 تم تفعيل فلتر الذيل العلوي\n"
                                    f"📏 المسافة الحالية المسموحة: {MAX_UPPER_WICK_DISTANCE_PERCENT:.2%}"
                                )
                            elif text == 'أوقف فلتر الذيل العلوي':
                                UPPER_WICK_FILTER_ENABLED = False
                                save_state()
                                send_telegram("🔴 تم إيقاف فلتر الذيل العلوي (لن يتم فحص الذيل قبل الشراء)")
                            elif text == 'حالة فلتر الذيل العلوي' or text == 'فلتر الذيل العلوي':
                                send_telegram(
                                    f"📊 <b>حالة فلتر الذيل العلوي</b>\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                    f"🔘 الحالة: {'✅ مفعل' if UPPER_WICK_FILTER_ENABLED else '❌ معطل'}\n"
                                    f"📏 مسافة الذيل العلوي المسموحة: {MAX_UPPER_WICK_DISTANCE_PERCENT:.2%}\n"
                                    f"━━━━━━━━━━━━━━━━━━━━\n"
                                    f"ℹ️ يرفض الدخول إذا كانت المسافة بين رأس ذيل الشمعة الحالية (15د) "
                                    f"والسعر الحالي أكبر من {MAX_UPPER_WICK_DISTANCE_PERCENT:.2%}\n"
                                    f"⚙️ للتحكم:\n"
                                    f"• `تعيين مسافة الذيل العلوي 1.66%`\n"
                                    f"• `شغل فلتر الذيل العلوي`\n"
                                    f"• `أوقف فلتر الذيل العلوي`"
                                )
                            # ==============================================
                            ...
                            # ========== 🆕 أوامر نظام المراقبة قبل الشراء ==========
                            elif text == 'حالة المراقبة' or text == 'عرض المراقبة':
                                with _pending_watch_lock:
                                    items = list(_pending_watch.items())
                                if not items:
                                    send_telegram("📭 لا توجد رموز تحت المراقبة حالياً.")
                                else:
                                    lines = [f"<b>👁️ الرموز تحت المراقبة ({len(items)})</b>",
                                             "━━━━━━━━━━━━━━━━━━━━"]
                                    now_ts = time.time()
                                    for sym, info in items:
                                        remaining = PENDING_WATCH_TIMEOUT_SECONDS - (now_ts - info['created_at'])
                                        trigger = info['ref_price'] * (1 + PENDING_WATCH_TRIGGER_PERCENT)
                                        ws_data = ws_manager.get_ticker(sym)
                                        cur_p = ws_data['price'] if ws_data and ws_data.get('price', 0) > 0 else 0
                                        lines.append(
                                            f"🔸 <code>{sym}</code>\n"
                                            f"   مرجع: {info['ref_price']:.8f}\n"
                                            f"   هدف: {trigger:.8f}\n"
                                            f"   الحالي: {cur_p:.8f}\n"
                                            f"   متبقي: {remaining/60:.1f} دقيقة"
                                        )
                                    send_telegram("\n".join(lines))
                            elif text == 'شغل المراقبة قبل الشراء':
                                PENDING_WATCH_ENABLED = True
                                save_state()
                                send_telegram(
                                    f"🟢 تم تفعيل المراقبة قبل الشراء\n"
                                    f"📏 نسبة التفعيل: +{PENDING_WATCH_TRIGGER_PERCENT:.2%}\n"
                                    f"🚫 سقف المطاردة: +{PENDING_WATCH_MAX_CHASE_PERCENT:.2%}\n"
                                    f"⏰ المهلة: {PENDING_WATCH_TIMEOUT_SECONDS // 60} دقيقة"
                                )
                            elif text == 'أوقف المراقبة قبل الشراء':
                                PENDING_WATCH_ENABLED = False
                                with _pending_watch_lock:
                                    _pending_watch.clear()
                                save_state()
                                send_telegram("🔴 تم إيقاف المراقبة (العودة للتنفيذ الفوري)")
                            elif text.startswith('تعيين نسبة تفعيل المراقبة'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_val = float(match.group(1)) / 100.0
                                    if 0.0005 <= new_val <= 0.05:
                                        PENDING_WATCH_TRIGGER_PERCENT = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تعيين نسبة تفعيل المراقبة إلى {new_val:.2%}")
                                    else:
                                        send_telegram("⚠️ القيمة بين 0.05% و 5%")
                                else:
                                    send_telegram("⚠️ استخدم: `تعيين نسبة تفعيل المراقبة 0.25%`")
                            elif text.startswith('تعيين مهلة المراقبة'):
                                match = re.search(r'(\d+)\s*دقيقة', text)
                                if match:
                                    mins = int(match.group(1))
                                    if 1 <= mins <= 60:
                                        PENDING_WATCH_TIMEOUT_SECONDS = mins * 60
                                        save_state()
                                        send_telegram(f"✅ تم تعيين مهلة المراقبة إلى {mins} دقيقة")
                                    else:
                                        send_telegram("⚠️ القيمة بين 1 و 60 دقيقة")
                                else:
                                    send_telegram("⚠️ استخدم: `تعيين مهلة المراقبة 25 دقيقة`")
                            elif text.startswith('تعيين سقف المطاردة'):
                                match = re.search(r'(\d+(?:\.\d+)?)\s*%?', text)
                                if match:
                                    new_val = float(match.group(1)) / 100.0
                                    if 0.005 <= new_val <= 0.20:
                                        PENDING_WATCH_MAX_CHASE_PERCENT = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تعيين سقف المطاردة إلى {new_val:.2%}")
                                    else:
                                        send_telegram("⚠️ القيمة بين 0.5% و 20%")
                                else:
                                    send_telegram("⚠️ استخدم: `تعيين سقف المطاردة 3%`")
                            elif text == 'إلغاء المراقبة':
                                with _pending_watch_lock:
                                    count = len(_pending_watch)
                                    _pending_watch.clear()
                                send_telegram(f"🧹 تم إلغاء {count} رمز من قائمة المراقبة.")
                            # ==========================================================
                            ...
                            # ========== أمر إعادة الضبط ==========
                                ...
                                buying_committee.apply_multiplier(0.92)
                                ...
                                CUSTOM_MAX_HOLD_MINUTES_BEFORE_TP = 55
                                PAUSE_NEW_ENTRIES = False
                                ...
                                FIFTH_MODEL_FILTER_ENABLED = True
                                SIXTH_MODEL_FILTER_MODEL = 'rule'
                                SIXTH_MODEL_FILTER_THRESHOLD = 0.47
                                SIXTH_MODEL_FILTER_TIMEFRAME = '15m'
                                SIXTH_MODEL_FILTER_ENABLED = True
                                ...
                                UPPER_WICK_FILTER_ENABLED = True
                                MAX_UPPER_WICK_DISTANCE_PERCENT = 0.0166   # 1.66%
                                CANDLE_BODY_FILTER_ENABLED = True          # 🆕 التحديث الأول
                                MAX_CANDLE_BODY_PERCENT = 0.04             # 🆕 التحديث الأول
                                TIMING_TRIPLE_FIVE_FILTER_ENABLED = True   # 🆕 فلتر التوقيت الثلاثي
                                TIMING_TRIPLE_FIVE_VALUE = 0.5             # 🆕 فلتر التوقيت الثلاثي
                                MAX_ANALYSIS_CYCLE_SECONDS = 180   # 🆕
                                PENDING_WATCH_ENABLED = True
                                PENDING_WATCH_TRIGGER_PERCENT = 0.0025
                                PENDING_WATCH_TIMEOUT_SECONDS = 1500
                                PENDING_WATCH_MAX_CHASE_PERCENT = 0.03
                                with _pending_watch_lock:
                                    _pending_watch.clear()
                                _prev_active_symbols_limit = CUSTOM_ACTIVE_SYMBOLS_LIMIT
...
def thread_watchdog():
    essential_threads = {
        ...
        'periodic_position_recovery': periodic_position_recovery,
        'pending_watch_monitor': pending_watch_monitor  # 🆕
    }
...
# ======================================================================
# =========== التحديث 15: save_state مع متغيرات Trailing ==============
# ======================================================================
def save_state():
    if not _global_state_lock.acquire(timeout=5):
        ...
        return
    try:
        # 🆕 الإصلاح #2: سناب شوت آمن لقائمة المراقبة
        with _pending_watch_lock:
            _pw_snapshot = {sym: dict(info) for sym, info in _pending_watch.items()}
        ...
        for sym, p in open_positions.items():
            ...
            pos_dict[sym] = {
                ...
                'trade_id': p.trade_id,
                # ============ التحديث 15: حفظ Trailing ============
                'manual_required_notified': getattr(p, 'manual_required_notified', False),
                'trailing_active': getattr(p, 'trailing_active', False),
                'trailing_peak_price': getattr(p, 'trailing_peak_price', p.entry_price),
                'trailing_last_check_time': p.trailing_last_check_time.isoformat() if getattr(p, 'trailing_last_check_time', None) else None,
                'trailing_activation_profit': getattr(p, 'trailing_activation_profit', 0.022),
                'trailing_drop_threshold': getattr(p, 'trailing_drop_threshold', 0.014),
                'trailing_check_interval': getattr(p, 'trailing_check_interval', 60),
                'trailing_triggered_pending': getattr(p, 'trailing_triggered_pending', False),
                # ====================================================
        ...
        data = {
            ...
            'filters': {
                ...
                'upper_wick_filter_enabled': UPPER_WICK_FILTER_ENABLED,
                'max_upper_wick_distance_percent': MAX_UPPER_WICK_DISTANCE_PERCENT,
                'candle_body_filter_enabled': CANDLE_BODY_FILTER_ENABLED,   # 🆕 التحديث الأول
                'max_candle_body_percent': MAX_CANDLE_BODY_PERCENT,          # 🆕 التحديث الأول
                'timing_triple_five_filter_enabled': TIMING_TRIPLE_FIVE_FILTER_ENABLED,  # 🆕 فلتر التوقيت الثلاثي
                'timing_triple_five_value': TIMING_TRIPLE_FIVE_VALUE,                     # 🆕 فلتر التوقيت الثلاثي
                'max_analysis_cycle_seconds': MAX_ANALYSIS_CYCLE_SECONDS,   # 🆕
                'execution_grace_seconds': EXECUTION_GRACE_SECONDS,          # 🆕
                # 🆕 نظام المراقبة قبل الشراء
                'pending_watch_enabled': PENDING_WATCH_ENABLED,
                'pending_watch_trigger_percent': PENDING_WATCH_TRIGGER_PERCENT,
                'pending_watch_timeout_seconds': PENDING_WATCH_TIMEOUT_SECONDS,
                'pending_watch_max_chase_percent': PENDING_WATCH_MAX_CHASE_PERCENT,
            },
            'custom_settings': {
            ...
            # 🆕 الإصلاح #2: استخدام السناب شوت
            'pending_watch': {
                sym: {k: v for k, v in info.items() if k != 'last_rest_attempt'}
                for sym, info in _pw_snapshot.items()
            }
        }
...
# ======================================================================
# =========== التحديث 16: load_state مع متغيرات Trailing ===============
# ======================================================================
def load_state():
    ...
    try:
        for sym, pdata in data.get('open_positions', {}).items():
            ...
            # ============ التحديث 16: استرجاع متغيرات Trailing ============
            pos.manual_required_notified = pdata.get('manual_required_notified', False)
            pos.trailing_active = pdata.get('trailing_active', False)
            pos.trailing_peak_price = pdata.get('trailing_peak_price', pdata.get('entry_price', 0))
            trail_time = pdata.get('trailing_last_check_time')
            pos.trailing_last_check_time = datetime.fromisoformat(trail_time) if trail_time else None
            # === إجبار القيم الجديدة (تجاهل ما هو محفوظ) ===
            pos.trailing_activation_profit = 0.022   # 2.2%
            pos.trailing_drop_threshold = 0.014      # 1.4%
            pos.trailing_check_interval = 60
            pos.trailing_triggered_pending = False   # إعادة تعيين عند التشغيل
            # ============================================================
        ...
        UPPER_WICK_FILTER_ENABLED = filters.get('upper_wick_filter_enabled', True)
        MAX_UPPER_WICK_DISTANCE_PERCENT = filters.get('max_upper_wick_distance_percent', 0.0166)
        CANDLE_BODY_FILTER_ENABLED = filters.get('candle_body_filter_enabled', True)   # 🆕 التحديث الأول
        MAX_CANDLE_BODY_PERCENT = filters.get('max_candle_body_percent', 0.04)          # 🆕 التحديث الأول
        TIMING_TRIPLE_FIVE_FILTER_ENABLED = filters.get('timing_triple_five_filter_enabled', True)  # 🆕 فلتر التوقيت الثلاثي
        TIMING_TRIPLE_FIVE_VALUE = filters.get('timing_triple_five_value', 0.5)                      # 🆕 فلتر التوقيت الثلاثي
        MAX_ANALYSIS_CYCLE_SECONDS = filters.get('max_analysis_cycle_seconds', 180)   # 🆕
        EXECUTION_GRACE_SECONDS = filters.get('execution_grace_seconds', 60)          # 🆕
        PENDING_WATCH_ENABLED = filters.get('pending_watch_enabled', True)
        PENDING_WATCH_TRIGGER_PERCENT = filters.get('pending_watch_trigger_percent', 0.0025)
        PENDING_WATCH_TIMEOUT_SECONDS = filters.get('pending_watch_timeout_seconds', 1500)
        PENDING_WATCH_MAX_CHASE_PERCENT = filters.get('pending_watch_max_chase_percent', 0.03)
        ...
        # 🆕 استرجاع قائمة المراقبة (فقط إذا كانت حديثة < 10 دقائق)
        try:
            state_age_local = time.time() - os.path.getmtime(STATE_FILE)
            if PENDING_WATCH_ENABLED and state_age_local < 600:
                pw_data = data.get('pending_watch', {})
                with _pending_watch_lock:
                    _pending_watch.clear()
                    restored = 0
                    for sym, info in pw_data.items():
                        if time.time() - info.get('created_at', 0) < PENDING_WATCH_TIMEOUT_SECONDS:
                            info['last_rest_attempt'] = 0
                            _pending_watch[sym] = info
                            restored += 1
                if restored > 0:
                    logger.info(f"✅ تم استرجاع {restored} رمز لقائمة المراقبة")
                    send_telegram(f"✅ تم استرجاع {restored} رمز إلى قائمة المراقبة.")
            else:
                logger.info("ℹ️ الحالة قديمة (>10 دقائق) أو المراقبة معطلة - لم يتم استرجاع قائمة المراقبة")
        except Exception as e:
            logger.warning(f"فشل استرجاع قائمة المراقبة: {e}")

        # ========== استعادة المراكز المفقودة عند بدء التشغيل (جديد) ==========
        if not PAPER_TRADING and not TEST_MODE and ENABLE_TRADING:
            recover_missing_positions()
    except Exception as e:
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v29.0 (نسخة مبسطة + التحديثات) - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v29.0 (النسخة المبسطة بعد التنظيف + التحديثات) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"🆕 التحديثات النهائية (10):\n"
                  f"   1) 🪶 مسافة الذيل العلوي: 1.66% (كانت 0.44%)\n"
                  f"   2) 🚀 التنفيذ الفوري: شراء داخل نفس دورة التحليل بدون انتظار\n"
                  f"   3) 🧹 حذف الفحص المكرر للتعرض (تنظيف)\n"
                  f"   4) 🔄 تحديث الرصيد الخلفي: 20-30 دقيقة (كان 9-10 دقائق)\n"
                  f"   5) ⚡ مهلة Limit Order: {LIMIT_TO_MARKET_TIMEOUT}s ثم تحويل تلقائي إلى Market\n"
                  f"   6) 📋 إشعار ختامي مع عدّادات تفصيلية لأسباب الرفض\n"
                  f"   7) 🔥 فلتر جسم الشمعة: حد {MAX_CANDLE_BODY_PERCENT:.2%}\n"
                  f"   8) ⏰ فلتر التوقيت الثلاثي: رفض 5/5/5 على (5د+15د+1س)\n"
                  f"   9) 👁️ نظام المراقبة قبل الشراء (Pending Watch):\n"
                  f"      • تفعيل الشراء عند +{PENDING_WATCH_TRIGGER_PERCENT:.2%}\n"
                  f"      • سقف المطاردة: +{PENDING_WATCH_MAX_CHASE_PERCENT:.2%}\n"
                  f"      • المهلة: {PENDING_WATCH_TIMEOUT_SECONDS // 60} دقيقة\n"
                  f"   10) ⚡ تسريع فحص التعرض في pending_watch_monitor (قراءة من الذاكرة بدل الشبكة ~0ms)\n"
                  f"   ⚠️ ملاحظة: تم رفع حد السيولة اليومي إلى ${MIN_24H_VOLUME_USD:,.0f} (كان 2M$)\n"
                  f"   • ⏱️ الدورة الآمنة: التحليل={MAX_ANALYSIS_CYCLE_SECONDS}s + التنفيذ={EXECUTION_GRACE_SECONDS}s = {MAX_ANALYSIS_CYCLE_SECONDS + EXECUTION_GRACE_SECONDS}s\n"
                  f"\n👁️ <b>المراقبة قبل الشراء:</b> {'✅ مفعلة' if PENDING_WATCH_ENABLED else '❌ معطلة'}\n"
                  f"   • نسبة التفعيل: +{PENDING_WATCH_TRIGGER_PERCENT:.2%} (موحّدة)\n"
                  f"   • سقف المطاردة: +{PENDING_WATCH_MAX_CHASE_PERCENT:.2%}\n"
                  f"   • المهلة: {PENDING_WATCH_TIMEOUT_SECONDS // 60} دقيقة\n"
                  f"   • الأمر للتحكم: <code>حالة المراقبة</code>")
