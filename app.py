...
ACTIVE_SYMBOLS_LIMIT = int(os.environ.get('ACTIVE_SYMBOLS_LIMIT', 15))   # تم التعديل من 20 إلى 15
...
LOCK_TIMEOUT_SECONDS = 720   # تم التعديل من 660 إلى 720 ثانية (12 دقيقة)
_manipulation_cache = {}
...
def get_total_equity():
    # للتأكد من أن الوضع الورقي يحسب إجمالي الرصيد = رصيد حر + قيمة المراكز المفتوحة
    ...
    for attempt in range(3):
        try:
            if PAPER_TRADING:
                # في الورقي: الرصيد الحر هو bot_stats.last_balance
                with _global_state_lock:
                    free = bot_stats.last_balance
            else:
                # في الحقيقي: نجلب الرصيد من Binance
                bal = exchange.fetch_balance()
                free = bal.get('USDT', {}).get('free', 0.0)
            # حساب قيمة المراكز المفتوحة بسعر السوق الحالي (نفس الكود للحقيقي والورقي)
            with _global_state_lock:
...
primary_exchange = ccxt.binance({
    ...
    'rateLimit': 3000,   # تم التعديل من 2000 إلى 3000
    'timeout': 60000,
    'options': {
        'defaultType': 'spot',
        'recvWindow': 60000,
...
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=500, max_retries=4):
    time.sleep(0.1)   # إضافة تأخير بسيط بين الطلبات لتجنب حظر IP
    exchange = get_active_exchange()
    last_err = ""
    attempt = 0
        ...
        try:
            ...
            for sym, pos in snapshot:
                ...
                if sell_dec:
                    ...
                    continue
                if pos.trailing_activated and pos.trailing_stop is not None:
                    # تحديث فقط إذا زاد الوقف المتحرك بنسبة >= 1% عن آخر قيمة مرسلة
                    if pos.last_trailing_stop_sent is None:
                        should_update = True
                    else:
                        # نسبة الزيادة في قيمة الوقف نفسه (وليس نسبة من سعر الدخول)
                        increase_ratio = (pos.trailing_stop - pos.last_trailing_stop_sent) / pos.last_trailing_stop_sent
                        should_update = increase_ratio >= 0.01   # 1% أو أكثر
                    if should_update:
                ...
                if profit <= -0.06:
                    ...
                    continue
            time.sleep(3)   # تم التعديل من 1.5 إلى 3 ثوانٍ
        except Exception as e:
...
def analyze_and_trade():
    ...
    if _processing_lock.locked():
        ...
        if lock_age > 300:
            logger.critical(f"🚨 قفل _processing_lock محتجز لمدة {lock_age:.0f} ثانية! محاولة تحرير قسري...")
            try:
                _processing_lock.release()
                logger.info("✅ تم تحرير القفل القسري")
            except Exception as e:
    ...
    try:
        ...
        # ========== التعديل النهائي ==========
        # استخدم get_total_equity() في جميع الحالات
        ...
        if _processing_lock.locked():
            ...
            if elapsed > LOCK_TIMEOUT_SECONDS:
                ...
                try:
                    ...
                    else: logger.error("❌ فشل الحصول على _global_state_lock لحفظ الحالة - قد تفقد بعض البيانات.")
                except Exception as e: logger.error(f"خطأ أثناء حفظ الحالة: {e}")
                sys.exit(1)
...
def background_analyzer():
    ...
    while True:
        analyze_and_trade()
        time.sleep(100)   # تم التعديل من 60 إلى 100 ثانية
...
def thread_watchdog():
    ...
    while True:
        try:
            time.sleep(5)   # مراقبة كل 5 ثوانٍ بدلاً من 30
            ...
            if not ws_manager.is_alive():
                ...
                # حلقة إعادة محاولة قسرية (5 مرات، كل 7 ثوانٍ)
                for attempt in range(1, 6):
                    logger.info(f"محاولة إعادة تشغيل WebSocket Manager رقم {attempt}/5")
                    ws_manager.restart()   # هذه الدالة توقف القديم وتشغل الجديد
                    time.sleep(7)          # انتظر 7 ثوانٍ بين المحاولات
            ...
            if _processing_lock.locked():
                lock_age = time.time() - _last_processing_lock_released
                if lock_age > 720:   # تم التعديل من 600 إلى 720 ثانية
                    logger.critical(f"🚨 قفل _processing_lock عالق لمدة {lock_age:.0f} ثانية! حفظ الحالة والخروج.")
                    send_telegram(f"🚨 قفل التحليل عالق لمدة {lock_age:.0f} ثانية - إعادة تشغيل البوت.")
                    save_state()
                    os._exit(1)
        except Exception as e:
