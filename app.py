    ...
    try:
        ...
        for sym in active:
            ...
            if succ:
                executed_count += 1
                forbidden.add(sym)
                # تحديث الرصيد والتعرض فوراً للرمز التالي
                ...
                logger.info(f"✅ تنفيذ فوري: {sym} | حجم={actual_size:.6f} | سعر={cur_price:.8f} | متبقي من الدورة={len(active)-analyzed_count}")
            else:
                skipped_limits += 1
...
def thread_watchdog():
    essential_threads = {
        ...
        'periodic_position_recovery': periodic_position_recovery  # <-- الخيط الجديد
    }
