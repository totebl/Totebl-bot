# ========================================================================
# بوت التداول المتطور v25.0 - تحكم كامل عن طريق تلغرام
# - إضافة أوامر تلغرام للتحكم بجميع الفلاتر والعتبات ديناميكياً
# - إمكانية تشغيل/إيقاف: التلاعب، السيولة، القيمة السوقية، حجم 24h، تغير 24h، شمعة الساعة
# - التحكم بفترة المسح، فترة التبريد، عتبات لجنة الشراء (رفع/خفض بنسبة)
# - التحكم بعتبتي الشراء العامة (عادي/ميم)
# - حفظ جميع الإعدادات واستعادتها بعد إعادة التشغيل
# ========================================================================
...
<head><meta charset="UTF-8"><title>بوت v25.0</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v25.0 – تحكم كامل عبر تلغرام</h1>
<div id="stats"></div>
...
@csrf.exempt
def ping():
    return jsonify({"status":"pong","version":"25.0"})
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v25.0 – تحكم كامل عبر تلغرام", "version":"25.0"})
...
@csrf.exempt
def health():
    return jsonify({"healthy":True,"paused":_is_paused(),"open":len(open_positions),"version":"25.0"})
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v25.0 - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v25.0 يعمل – الوضع: {mode_str}\n✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n✅ أوامر: اوقف/شغل فلتر (التلاعب، السيولة، القيمة السوقية، حجم 24h، تغير 24h، شمعة الساعة)\n✅ ضبط مدة المسح، التبريد، عتبات الشراء")
