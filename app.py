# ========================================================================
# بوت التداول المتطور v26.1 - تحكم كامل عن طريق تلغرام + رصيد حقيقي
# تم التعديل لحل مشاكل تسرب الذاكرة واستقرار التشغيل على Render
# التعديلات: تنظيف الكاشات غير المحدودة، تقليل حدود الكاش، نسخ سطحي، تقليل الخيوط، تنظيف أسرع
# ========================================================================
...
ACTIVE_SYMBOLS_LIMIT = int(os.environ.get('ACTIVE_SYMBOLS_LIMIT', 12))   # تم التخفيض من 20 إلى 12
...
_ohlcv_cache_max = 100   # تم التخفيض من 150 إلى 100
_features_cache = OrderedDict()
_features_cache_max = 80   # تم التخفيض من 200 إلى 80
    ...
    with _cache_lock:
        if cache_key in _ohlcv_cache:
            ...
            if (now - entry['timestamp']) < ttl_seconds:
                # استخدام نسخة سطحية بدلاً من العميقة لتوفير الذاكرة
                return entry['data'].copy(deep=False) if not entry['data'].empty else pd.DataFrame()
            else:
...
def get_cached_features(symbol, timeframe, limit=500, ttl=30):
    ...
    if df_f.empty:
        ...
        return pd.DataFrame()
    with _features_cache_lock:
        _features_cache[cache_key] = {'data': df_f.copy(deep=False), 'timestamp': now}
        if len(_features_cache) > _features_cache_max:
            _features_cache.popitem(last=False)
    return df_f.copy(deep=False)
            ...
            if resp.status_code == 200:
                ...
                if cap > 0:
                    with _market_cap_cache_lock:
                        ...
                        # منع النمو غير المحدود: احتفظ فقط بآخر 500 رمز
                        if len(_market_cap_cache) > 500:
                            items = sorted(_market_cap_cache.items(), key=lambda x: x[1]['timestamp'])
                            for i in range(min(100, len(items))):
                                del _market_cap_cache[items[i][0]]
                    return cap
            else:
                time.sleep(1)
        except Exception as e:
...
def get_real_balance_usdt():
    ...
    if _balance_failure_paused:
        if now - _balance_failure_start_time < 120:
            logger.debug("⚠️ مؤقت فشل الرصيد لا يزال نشطًا، لن نحاول جلب الرصيد الآن")
            return None
        else:
            _balance_failure_paused = False
            _balance_retry_count = 0
            logger.info("🔄 انتهت فترة التهدئة، سنحاول جلب الرصيد مجددًا")
    exchange = get_active_exchange()
    for attempt in range(3):
        try:
            bal = exchange.fetch_balance()
            ...
            logger.warning(f"محاولة {attempt+1}/3 لجلب الرصيد فشلت: {repr(e)}")
            if attempt == 2:
                with _state_lock:
                    if not _balance_failure_paused:
                        ...
                        _set_paused(True)
                        send_telegram(f"⚠️ فشل جلب الرصيد: {str(e)[:100]}. تم الإيقاف المؤقت. سأحاول مجددًا بعد دقيقتين.")
                        generate_error_report("فشل_اتصال", "رصيد", f"فشل get_real_balance_usdt: {repr(e)}")
            time.sleep(1)
    return None
...
<head><meta charset="UTF-8"><title>بوت v26.1</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v26.1 – تحكم كامل + إصلاح أخطاء المحاكاة</h1>
<div id="stats"></div>
...
def update_preemptive_stop_loss(symbol, pos, new_stop_price):
    ...
    if new_oid:
        ...
        logger.debug(f"⚠️ فشل تحديث الوقف المسبق لـ {symbol}")
...
def manual_resume():
    global daily_loss_cooldown_until, _daily_loss_tracker, _balance_failure_paused, _balance_retry_count, _last_successful_balance_time
    with _global_state_lock:
        _set_paused(False)
        daily_loss_cooldown_until = None
        _balance_failure_paused = False
...
def analyze_and_trade():
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:   # تم التخفيض من 10 إلى 4
        ...
        if lock_acquired:
            ...
            _last_processing_lock_released = time.time()
        # جمع القمامة بعد التحليل للمساعدة في استقرار الذاكرة
        gc.collect()
...
def cache_cleanup_thread():
    while True:
        time.sleep(600)  # تم التخفيض من 3600 إلى 600 (كل 10 دقائق)
        now = time.time()
        # تنظيف _ohlcv_cache (احتفظ بآخر 50 عنصراً)
        with _cache_lock:
            if len(_ohlcv_cache) > 50:
                keys_to_delete = list(_ohlcv_cache.keys())[:len(_ohlcv_cache)-50]
                for k in keys_to_delete:
                    del _ohlcv_cache[k]
        # تنظيف _features_cache (احتفظ بآخر 50 عنصراً)
        with _features_cache_lock:
            if len(_features_cache) > 50:
                keys_to_delete = list(_features_cache.keys())[:len(_features_cache)-50]
                for k in keys_to_delete:
                    del _features_cache[k]
            else:
                for key in list(_features_cache.keys()):
                    if now - _features_cache[key]['timestamp'] > 300:
                        del _features_cache[key]
        # تنظيف _orderbook_cache بالكامل
        with _orderbook_cache_lock:
            _orderbook_cache.clear()
        # تنظيف _manipulation_cache بالكامل
        with _cache_lock:
            _manipulation_cache.clear()
        # تنظيف _trades_cache
        with _trades_cache_lock:
            _trades_cache.clear()
        # تنظيف _amount_limits_cache و _amount_limits_cache_time (تسرب ذاكرة)
        with _state_lock:
            for sym in list(_amount_limits_cache.keys()):
                if now - _amount_limits_cache_time.get(sym, 0) > 7200:  # أقدم من ساعتين
                    del _amount_limits_cache[sym]
                    del _amount_limits_cache_time[sym]
        # تنظيف _market_cap_cache (احتفظ بآخر 500 عنصر)
        with _market_cap_cache_lock:
            if len(_market_cap_cache) > 500:
                items = sorted(_market_cap_cache.items(), key=lambda x: x[1]['timestamp'])
        ...
        # تنظيف نموذج EMR
        if 'buying_committee' in globals():
            with buying_committee.models['emr']._cache_lock:
                if len(buying_committee.models['emr']._result_cache) > 200:
                    items = sorted(buying_committee.models['emr']._result_cache.items(), key=lambda x: x[1][1])
                    for sym, _ in items[:50]:
                        del buying_committee.models['emr']._result_cache[sym]
        # جمع القمامة
        gc.collect()
        logger.debug("🧹 تم التنظيف القوي للكاشات وجمع القمامة")
                            ...
                            if text == "توقف عن التداول":
                                ...
                                if not ENABLE_TRADING:
                                    ...
                                    if real_bal is not None:
                                        ...
                                        send_telegram("⚠️ فشل جلب الرصيد الحقيقي فورًا. سأستمر في المحاولة كل دقيقتين. تأكد من إعدادات API.")
                                    save_state()
                                else:
...
@csrf.exempt
def ping():
    return jsonify({"status":"pong","version":"26.1"})
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v26.1 – تحكم كامل عبر تلغرام + رصيد فوري + إصلاح أخطاء المحاكاة", "version":"26.1"})
...
@csrf.exempt
def health():
    return jsonify({"healthy":True,"paused":_is_paused(),"open":len(open_positions),"version":"26.1"})
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v26.1 - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v26.1 يعمل – الوضع: {mode_str}\n✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n✅ تقرير يومي يعرض صافي الربح/الخسارة\n✅ التحويل بين التداول الورقي والحقيقي: `تداول حقيقي` / `تداول ورقي`\n✅ إصلاح أخطاء 'فشل تحديث الوقف المسبق' في وضع المحاكاة\n✅ تحسينات كبيرة لإدارة الذاكرة ومنع التسرب")
