# ========================================================================
# بوت التداول المتطور v26.3 - نسخة مبسطة (بدون لجنة بيع، بدون فحص تلاعب/حيتان/دفاتر)
# مهلات الشبكة قابلة للتعديل عبر متغيرات البيئة
# ========================================================================
...
# --------------------------- مهلات الشبكة (قابلة للتعديل) ---------------------------
CCXT_TIMEOUT = int(os.environ.get('CCXT_TIMEOUT', 60000))          # مللي ثانية
FETCH_TICKERS_TIMEOUT = int(os.environ.get('FETCH_TICKERS_TIMEOUT', 45))  # ثواني
FETCH_OHLCV_TIMEOUT = int(os.environ.get('FETCH_OHLCV_TIMEOUT', 30))      # ثواني
FETCH_OHLCV_TOTAL_TIMEOUT = int(os.environ.get('FETCH_OHLCV_TOTAL_TIMEOUT', 60))  # ثواني
...
FILTER_MANIPULATION_ENABLED = False   # تم تعطيله نهائياً
FILTER_LIQUIDITY_ENABLED = False      # تم تعطيله نهائياً
...
primary_exchange = ccxt.binance({
    ...
    'rateLimit': 2000,
    'timeout': CCXT_TIMEOUT,        # استخدام المتغير القابل للتعديل
    'options': {
        'defaultType': 'spot',
        'recvWindow': 60000,
...
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=500, max_retries=2):
    ...
    timeout_total = FETCH_OHLCV_TOTAL_TIMEOUT
    for attempt in range(max_retries):
...
def fetch_tickers_with_retry(max_retries=2):
    ...
    with _global_state_lock:
        if bot_stats.last_valid_tickers:
            ...
            logger.warning(f"⚠️ استخدام بيانات مخزنة (عمرها {age:.0f} ثانية)")
...
def get_market_condition():
    try:
        ticker = fetch_ticker_with_retry('BTC/USDT')
        if ticker and ticker.get('percentage', 0) > 2:
            return 'good'
        elif ticker and ticker.get('percentage', 0) < -2:
            return 'bad'
        return 'normal'
    except:
        return 'normal'
...
<head><meta charset="UTF-8"><title>بوت v26.3</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v26.3 – مهلات قابلة للتعديل + نسخة مبسطة</h1>
<div id="stats"></div>
...
def load_state():
    ...
    try:
        ...
        FILTER_MANIPULATION_ENABLED = filters.get('manipulation', False)
        FILTER_LIQUIDITY_ENABLED = filters.get('liquidity', False)
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # Stop loss
                if pos.stop_loss:
                    sl_price = pos.stop_loss * (0.998 if pos.side == 'buy' else 1.002)
                    if (pos.side == 'buy' and cur_price <= sl_price) or (pos.side == 'sell' and cur_price >= sl_price):
                        close_partial(sym, 1.0, cur_price, "🛑 Stop Loss")
                        continue

                # Trailing stop
                config = get_scalp_config(pos.symbol_type)
                if profit >= config['trailing_activation'] and pos.trailing_stop:
                    if (pos.side == 'buy' and cur_price <= pos.trailing_stop) or (pos.side == 'sell' and cur_price >= pos.trailing_stop):
                        close_partial(sym, 1.0, cur_price, "🏃 Trailing Stop")
                        continue
                ...
                # Additional protective sell at -1.5% and -2% (partial)
                if profit < 0:
                    if profit <= -0.015 and not pos.sold_at_15:
                        close_partial(sym, 0.25, cur_price, f"⚠️ بيع وقائي: خسارة {abs(profit):.2%} (بيع 25%)")
                        pos.sold_at_15 = True
                        save_state()
                ...
                # Crash detection within 2 minutes
                ...
                # Time-based exit if first target not hit after 1 hour
                if profit > 0.011:
...
def analyze_and_trade():
    ...
    if not _processing_lock.acquire(timeout=1):
        logger.warning("⚠️ تخطي التحليل: _processing_lock محتجز من دورة سابقة")
        return

    try:
        with _global_state_lock:
            snapshot_positions = list(open_positions.items())
            snapshot_pending_local = set(_local_pending_symbols)
            snapshot_pending_ex = set(_exchange_pending_symbols)
            snapshot_last_close = dict(last_close_time)
            daily_trades_date = _daily_trades_date
            daily_loss_tracker = _daily_loss_tracker
            daily_trades_count = _daily_trades_count
            paused = _is_paused()
            bot_last_balance = bot_stats.last_balance
        _processing_lock.release()
    except Exception as e:
        _processing_lock.release()
        logger.error(f"خطأ في نسخ الحالة: {e}")
        return
    ...
    if market_cond == 'good':
        ...
        max_allowed_exposure = bot_last_balance * 0.70
    else:
        position_percent = MAX_POSITION_PERCENT
        max_allowed_exposure = bot_last_balance * MAX_EXPOSED_PERCENT

    max_pos_usdt = bot_last_balance * position_percent
    ...
    all_syms = list(set(BASE_SYMBOLS + scanner.candidates + [sym for sym, _ in snapshot_positions]))
    ...
    forbidden = set([sym for sym, _ in snapshot_positions]) | snapshot_pending_local | snapshot_pending_ex

    candidates = []
    for sym in active:
        if sym in forbidden:
            continue
        last_close_time_sym = snapshot_last_close.get(sym)
        if last_close_time_sym and (datetime.now() - last_close_time_sym).total_seconds() < cooldown_hours * 3600:
            continue
        ...
        candidates.append((sym, sym_type, df_primary, regime))

    pending_buys = []
    for sym, sym_type, df_primary, regime in candidates:
    ...
    # إعادة الحصول على القفل لتنفيذ الصفقات فقط
    if not _processing_lock.acquire(timeout=2):
        logger.warning("⚠️ فشل الحصول على القفل لتنفيذ الصفقات")
        return

    try:
        with _global_state_lock:
            free_bal = bot_stats.last_balance
            total_eq = bot_stats.last_balance
            max_exp2 = total_eq * (0.70 if market_cond == 'good' else MAX_EXPOSED_PERCENT)
            curr_exp = sum(p.remaining_size * all_tickers.get(p.symbol, {}).get('last', p.entry_price) for sym, p in open_positions.items())
        ...
        for opp in pending_buys:
            ...
            if opp['symbol'] in open_positions or opp['symbol'] in _local_pending_symbols or opp['symbol'] in _exchange_pending_symbols:
                continue
            pos_usdt = total_eq * (MAX_POSITION_PERCENT_GOOD if market_cond == 'good' else MAX_POSITION_PERCENT)
    ...
    while True:
        ...
        if _processing_lock.locked():
            ...
            if elapsed > LOCK_TIMEOUT_SECONDS:
                logger.critical(f"❗ قفل _processing_lock معلق لمدة {elapsed:.0f} ثانية! محاولة حفظ الحالة ثم الخروج.")
...
@csrf.exempt
def ping():
    return jsonify({"status":"pong","version":"26.3"})
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v26.3 – مهلات قابلة للتعديل + نسخة مبسطة", "version":"26.3"})
...
@csrf.exempt
def health():
    return jsonify({"healthy":True,"paused":_is_paused(),"open":len(open_positions),"version":"26.3"})
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v26.3 - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v26.3 يعمل – الوضع: {mode_str}\n✅ مهلات الشبكة قابلة للتعديل (CCXT_TIMEOUT, FETCH_TICKERS_TIMEOUT, FETCH_OHLCV_TIMEOUT, FETCH_OHLCV_TOTAL_TIMEOUT)\n✅ نسخة مبسطة بدون فحص تلاعب/حيتان/دفاتر وبدون لجنة بيع\n✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n✅ التحويل بين التداول الورقي والحقيقي: `تداول حقيقي` / `تداول ورقي`")
