# ========================================================================
# بوت التداول المتطور v27.0 - حل جذري لمشكلة تعليق القفل + تحسين الأداء
# ========================================================================
...
primary_exchange = ccxt.binance({
    ...
    'rateLimit': 2000,
    'timeout': 15000,   # تم التخفيض من 60000 إلى 15000 مللي ثانية
    'options': {
        'defaultType': 'spot',
        'recvWindow': 60000,
...
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=500, max_retries=2):
...
def fetch_ohlcv_persistent(symbol, timeframe, limit=500, max_attempts=3, retry_interval=5):
...
def get_total_equity():
    ...
    try:
        with acquire_timeout(_global_state_lock, timeout=2):
            return bot_stats.last_balance
    except TimeoutError:
        logger.warning("⚠️ تعذر الحصول على _global_state_lock في get_total_equity، نعيد القيمة المخزنة")
        return bot_stats.last_balance
...
<head><meta charset="UTF-8"><title>بوت v27.0</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v27.0 – حل جذري لتعليق القفل</h1>
<div id="stats"></div>
...
# ----------------------------------------
# الحل الجذري: analyze_and_trade مع تحرير القفل قبل الاتصالات الشبكية
# ----------------------------------------
def analyze_and_trade():
    global bot_stats, _daily_loss_tracker, _daily_trades_count, _daily_trades_date
    global _daily_winning_trades, _daily_losing_trades, daily_loss_cooldown_until
    global _initial_warmup_done, _last_processing_lock_released
    global _last_market_condition, _last_market_condition_time
    global buying_committee, scanner

    if not _processing_lock.acquire(timeout=1):
        logger.warning("⚠️ تخطي التحليل: القفل محتجز من دورة سابقة (تجاوز 1 ثانية)")
        return

    try:
        # نسخ الحالة الضرورية تحت القفل
        with _state_lock:
            today = datetime.now().date()
            is_paused = _is_paused()
            if is_paused:
                if daily_loss_cooldown_until and datetime.now() >= daily_loss_cooldown_until:
                    _set_paused(False)
                    daily_loss_cooldown_until = None
                    _daily_loss_tracker = 0.0
                    _daily_trades_count = 0
            ...
            current_free_bal = bot_stats.last_balance
            total_eq = current_free_bal
            positions_snapshot = list(open_positions.items())
            pending_local = set(_local_pending_symbols)
            pending_ex = set(_exchange_pending_symbols)
            last_close_dict = dict(last_close_time)
            daily_trades = _daily_trades_count
            max_daily = MAX_DAILY_TRADES
            daily_loss = _daily_loss_tracker
            exposed = 0.0
            for sym, p in positions_snapshot:
                # نستخدم سعر مخزن مؤقتاً أو سعر الدخول لحساب تقريبي
                price = p.entry_price
                exposed += p.remaining_size * price
            pending_exp = get_pending_exposure_estimate()
            curr_exp = exposed + pending_exp

        # تحرير القفل الرئيسي - كل عمليات الشبكة ستتم بدونه
        _processing_lock.release()

        elapsed_since_start = time.time() - _bot_start_time
        if elapsed_since_start < STARTUP_DELAY_SECONDS:
            logger.info(f"⏳ انتظار {STARTUP_DELAY_SECONDS - elapsed_since_start:.0f} ثانية قبل التحليل")
            return

        if not _initial_warmup_done:
            for sym in BASE_SYMBOLS:
                for tf in [TIMEFRAMES['primary'], TIMEFRAMES['confirm_1'], TIMEFRAMES['confirm_2']]:
                    get_cached_features(sym, tf, limit=200, ttl=120)
                    time.sleep(0.3)
        ...
        # الرموز النشطة
        all_syms = list(set(BASE_SYMBOLS + scanner.candidates + [sym for sym, _ in positions_snapshot]))
        active = all_syms[:ACTIVE_SYMBOLS_LIMIT]

        # جلب جميع الأسعار
        all_tickers = fetch_tickers_with_retry()
        if not all_tickers:
            logger.warning("⚠️ لا توجد بيانات أسعار، تخطي التحليل")
            return

        forbidden = set([sym for sym, _ in positions_snapshot]) | pending_local | pending_ex

        candidates_for_manipulation = []
        for sym in active:
            if sym in forbidden:
                continue
            last_close_sym = last_close_dict.get(sym)
            if last_close_sym and (datetime.now() - last_close_sym).total_seconds() < cooldown_hours * 3600:
                continue
            regime = detect_market_regime(sym)
            if regime == 'trending_down':
                continue
            df_primary = fetch_ohlcv_persistent(sym, TIMEFRAMES['primary'], limit=200, max_attempts=2, retry_interval=5)
            if df_primary.empty or len(df_primary) < 30:
                continue
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for future in as_completed(futures, timeout=30):
                sym = futures[future]
                try:
                    is_manip, score, m_type = future.result(timeout=5)
        ...
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            if FILTER_MANIPULATION_ENABLED:
                is_manip, score, m_type = manipulation_results.get(sym, (False, 0.0, "missing"))
                if is_manip:
                    continue

            dec, buy_score, conf = buying_committee.decide(df_primary, sym, sym_type, market_cond)
            if dec != 'buy':
                continue
            if sym_type == 'meme' and buy_score < SCALP_MIN_PROFIT:
                continue
            if sym_type != 'meme' and buy_score < STRENGTH_THRESHOLD:
                continue
            ...
            if FILTER_HOUR_CANDLE_ENABLED:
                df_1h = fetch_ohlcv_persistent(sym, '1h', limit=2, max_attempts=2)
                if df_1h.empty or len(df_1h) < 2 or cur_price <= df_1h['close'].iloc[-2]:
                    continue
            if FILTER_4H_CANDLE_ENABLED:
                df_4h = fetch_ohlcv_persistent(sym, '4h', limit=2, max_attempts=2)
                if df_4h.empty or len(df_4h) < 2 or cur_price <= df_4h['close'].iloc[-2]:
                    continue
            ...
            if FILTER_LIQUIDITY_ENABLED:
                liq_ok, _ = check_liquidity(sym, size * cur_price, sym_type)
                if not liq_ok:
                    continue
            ...
            pending_buys.append({
                'symbol': sym, 'price': cur_price, 'size': size, 'atr': atr_val,
                'sl': sl_price, 'tp': tp_price, 'type': sym_type,
                'score': buy_score, 'conf': conf, 'regime': regime
            })

        if not pending_buys:
            return

        pending_buys.sort(key=lambda x: x['score'], reverse=True)

        # إعادة الحصول على القفل للتنفيذ (قصير جداً)
        if not _processing_lock.acquire(timeout=2):
            logger.warning("⚠️ فشل الحصول على القفل لتنفيذ الصفقات، سيتم تخطي هذه الدورة")
            return

        try:
            # تحديث الرصيد والتعرض قبل التنفيذ
            free_bal = current_free_bal
            if not PAPER_TRADING and ENABLE_TRADING and not TEST_MODE:
                real_bal = get_real_balance_usdt()
                if real_bal is not None:
                    free_bal = real_bal
            with _state_lock:
                total_eq2 = bot_stats.last_balance
                max_exp2 = total_eq2 * (0.70 if market_cond == 'good' else MAX_EXPOSED_PERCENT)
                # إعادة حساب التعرض الحالي بدقة
                exposed2 = 0.0
                for sym, p in open_positions.items():
                    price = all_tickers.get(sym, {}).get('last', p.entry_price)
                    exposed2 += p.remaining_size * price
                curr_exp2 = exposed2 + get_pending_exposure_estimate()

            for opp in pending_buys:
                ...
                if min_cost2 and sz * opp['price'] < min_cost2:
                    continue
                if curr_exp2 + pos_usdt > max_exp2:
                    continue
                if free_bal < pos_usdt:
                    continue
                succ, actual = execute_full_trade(opp['symbol'], 'buy', sz, opp['price'], opp['atr'],
                                                  opp['sl'], opp['tp'], opp['type'], opp['score'],
                                                  opp['conf'], opp['regime'])
                if succ:
                    free_bal -= actual * opp['price']
                    curr_exp2 += actual * opp['price']
                    _daily_trades_count += 1
        finally:
            _processing_lock.release()
    except Exception as e:
    ...
    while True:
        ...
        with _state_lock:
            for sym in list(_amount_limits_cache.keys()):
                if now - _amount_limits_cache_time.get(sym, 0) > 7200:
...
@csrf.exempt
def ping():
    return jsonify({"status":"pong","version":"27.0"})
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v27.0 – حل جذري لمشكلة التعليق + تحكم كامل عبر تلغرام", "version":"27.0"})
...
@csrf.exempt
def health():
    return jsonify({"healthy":True,"paused":_is_paused(),"open":len(open_positions),"version":"27.0"})

# --------------------------- خيط مراقبة الخيوط الأساسية (بدون lock_health_monitor) ---------------------------
def thread_watchdog():
    essential_threads = {
