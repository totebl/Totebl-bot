...
import zipfile  # <--- إضافة جديدة لدعم الملفات المضغوطة
...
MARKETS_CACHE_FILE = "data/markets_cache.json"   # <-- الملف الجديد لحفظ الأسواق
...
# ==================== دوال جديدة لحفظ وتحميل الأسواق محلياً وإرسال الملفات ====================
...
# ==================== التعديل الرئيسي: دالة تحميل الأسواق من ZIP أو JSON ====================
def load_markets_from_cache():
    """تحميل الأسواق من الملف المحلي (يدعم JSON و ZIP)"""
    # أولاً: محاولة تحميل الملف المضغوط
    zip_path = MARKETS_CACHE_FILE + ".zip"
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                if json_files:
                    ...
                    logger.info("✅ تم تحميل الأسواق من الكاش المضغوط (data/markets_cache.zip)")
                    return True
                else:
                    logger.warning("⚠️ ملف ZIP لا يحتوي على ملف JSON")
        except Exception as e:
            logger.warning(f"⚠️ فشل تحميل الكاش المضغوط: {e}")

    # ثانياً: محاولة تحميل الملف العادي (JSON)
    if not os.path.exists(MARKETS_CACHE_FILE):
        return False
    try:
        ...
        primary_exchange.markets_by_id = data['markets_by_id']
        primary_exchange.symbols = data['symbols']
        logger.info("✅ تم تحميل الأسواق من الكاش المحلي (data/markets_cache.json)")
        return True
    except Exception as e:
        logger.warning(f"⚠️ فشل تحميل الكاش المحلي: {e}")
        return False
# =============================================================================================

def save_markets_cache():
    """حفظ الأسواق الحالية في ملف محلي (JSON عادي)"""
    try:
        data = {
        ...
        with open(MARKETS_CACHE_FILE, 'w') as f:
            json.dump(data, f)
        logger.info("💾 تم حفظ الأسواق في الكاش المحلي (data/markets_cache.json)")
    except Exception as e:
                    ...
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ==================== أمر تلغرام الجديد ====================
                                ...
                                # محاولة إرسال الملف المضغوط إن وجد، وإلا إرسال JSON
                                zip_path = MARKETS_CACHE_FILE + ".zip"
                                if os.path.exists(zip_path):
                                    send_telegram_file(zip_path, "📂 ملف الأسواق المضغوط (data/markets_cache.zip)")
                                elif os.path.exists(MARKETS_CACHE_FILE):
                                    send_telegram_file(MARKETS_CACHE_FILE, "📂 ملف الأسواق المخزن (data/markets_cache.json)")
                                else:
...
if __name__ == '__main__':
    ...
    if not os.path.exists(MARKETS_CACHE_FILE + ".zip") and not os.path.exists(MARKETS_CACHE_FILE):
        save_markets_cache()
    # ========================================================================
