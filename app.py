...
class Position:
    def __init__(self, symbol, side, size, entry, atr, sl, tp, sym_type, pred, conf, regime, tp_levels=None, ai_approved=False,
        ...
        self.trailing_activation_profit = 0.03    # عتبة التفعيل (3%)
        self.trailing_drop_threshold = 0.01       # نسبة الانخفاض (1%)
            ...
            for sym, pos in snapshot:
                ...
                if not pos.trailing_active and profit >= pos.trailing_activation_profit:
                    ...
                    send_telegram(
                        ...
                        f"ℹ️ سيتم البيع الكامل إذا انخفض السعر 1% عن الذروة "
    ...
    try:
        ...
        for sym, p in open_positions.items():
            ...
            pos_dict[sym] = {
                ...
                'trailing_activation_profit': getattr(p, 'trailing_activation_profit', 0.03),
                'trailing_drop_threshold': getattr(p, 'trailing_drop_threshold', 0.01),
...
def load_state():
    ...
    try:
        for sym, pdata in data.get('open_positions', {}).items():
            ...
            pos.trailing_activation_profit = pdata.get('trailing_activation_profit', 0.03)
            pos.trailing_drop_threshold = pdata.get('trailing_drop_threshold', 0.01)
...
if __name__ == '__main__':
    ...
    send_telegram(f"🚀 بوت v29.0 (النسخة المبسطة بعد التنظيف + التحديثات) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"   • Trailing Stop مخصص (تفعيل عند +3%)\n"
