# ========================================================================
# بوت التداول v27.12 – إصدار محسن بالكامل يعتمد على 3 أطر زمنية (5م، 15د، 1س) مع أوزان ديناميكية
# التغييرات الجوهرية:
#   - استبدال التقطيع الثلاثي (نوافذ متداخلة) بجمع حقيقي لثلاثة أطر زمنية (5م، 15د، 1س)
#   - حساب المتوسط المرجح للأعضاء الخمسة باستخدام أوزان قابلة للتعديل لكل إطار
#   - المضاعف يُستخرج من إطار 5 دقائق بدلاً من 1 ساعة (لزيادة الحساسية)
#   - نظام بيع جديد يعتمد على متوسط 5م + 15د (كل 60 ثانية) بدلاً من 5م فقط
#   - عرض درجات النماذج الخمسة على كل إطار في رسالة الشراء للشفافية
#   - أوامر تلغرام جديدة للتحكم بالأوزان وعتبة البيع
...
# ======================== تم تحديث SELL_WEIGHTS إلى النسب الذهبية ========================
SELL_WEIGHTS = {
    'rule': 0.05,
    'emr': 0.05,
    'cfhm': 0.20,
...
# ======================== تم تحديث القيم الافتراضية إلى النسب الذهبية ========================
WEIGHT_5M = 15.0
WEIGHT_15M = 30.0
WEIGHT_1H = 55.0
...
<head><meta charset="UTF-8"><title>بوت v27.12 (Spot + 3 أطر)</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v27.12 (Spot Edition – 3 Timeframes)</h1>
<div id="stats"></div>
...
class BuyingCommittee:
    def __init__(self):
        ...
        # ======================== تم تحديث الأوزان الافتراضية إلى النسب الذهبية ========================
        self.weights = {
            'rule': 0.05,
            'emr': 0.05,
            'cfhm': 0.20,
                    ...
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ------------------- أمر ضبط أوزان النماذج الخمسة دفعة واحدة (جديد) -------------------
                                ...
                                if not matches:
                                    send_telegram("⚠️ لم يتم العثور على أسماء وقيم صحيحة.\nالصيغة المطلوبة: `تعيين أوزان النماذج Rule=5 EMR=5 CFHM=20 Timing=55 VWAP=15`")
                                    continue
                            ...
                            # ------------------------------------------------------------
                                ...
                                PAUSE_ANALYSIS = False
                                # إعادة ضبط أوزان النماذج إلى القيم الذهبية
                                buying_committee.weights = {'rule':0.05, 'emr':0.05, 'cfhm':0.20, 'timing':0.55, 'vwap_obv':0.15}
                                SELL_WEIGHTS = {'rule':0.05, 'emr':0.05, 'cfhm':0.20, 'timing':0.55, 'vwap_obv':0.15}
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية (مع أوزان النماذج الذهبية: 5%،5%،20%،55%،15% وأوزان الأطر 15%،30%،55%)")
                                save_state()
    ...
    try:
        ...
        data = {
            ...
            'weights': {
                ...
                '15m': WEIGHT_15M,
                '1h': WEIGHT_1H,
                'models': buying_committee.weights  # حفظ أوزان النماذج
            }
        }
...
def load_state():
    ...
    try:
        ...
        WEIGHT_5M = weights.get('5m', 15.0)
        WEIGHT_15M = weights.get('15m', 30.0)
        WEIGHT_1H = weights.get('1h', 55.0)
        # استعادة أوزان النماذج
        model_weights = weights.get('models', {'rule':0.05, 'emr':0.05, 'cfhm':0.20, 'timing':0.55, 'vwap_obv':0.15})
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v27.12 (Spot + Gemini AI + تبريد ذكي + 3 أطر حقيقية + أوزان ديناميكية) - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v27.12 (الإصدار المحسن بـ 3 أطر زمنية 5م، 15د، 1س مع أوزان ديناميكية) يعمل – الوضع: {mode_str}\n"
