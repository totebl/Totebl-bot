...
# ======================================================================
# ========== نظام البيع الجديد (يعتمد على متوسط 5م + 15د) ==========
SELL_15M_THRESHOLD = float(os.environ.get('SELL_15M_THRESHOLD', 0.25))   # ✅ 0.25
...
class BuyingCommittee:
    ...
    def decide(self, df_5m, df_15m, df_1h, symbol, sym_type='normal', market_condition='normal'):
        ...
        if avg > thresh and avg >= min_conf:
            return 'buy', avg, avg, f"✅ موافقة: المتوسط={avg:.3f} | العتبة={thresh:.3f}", avg_scores
        else:
            if avg < min_conf:
            ...
            return 'neutral', avg, avg, f"❌ رفض: {main_reason}", avg_scores
...
# ======================= محرك التوصيات الذكي (جديد) =======================
def calculate_suggested_weights():
    """
    توصية ديناميكية باستخدام التصنيف الهرمي (بدلاً من معامل الارتباط الخطي).
    - تعمل مع 4 صفقات كحد أدنى.
    - تحلل آخر 400 صفقة كحد أقصى.
    - توزع الأوزان ديناميكياً مع احترام الحدود: 
      الأطر: الأول 40-70%، الثاني 20-40%، الثالث 1-5%.
      الأعضاء: الأول 40-50%، الثاني 20-30%، الثالث 10-15%، الباقي 0-5%.
    - تنتج أوامر تلغرام جاهزة للتطبيق.
    """
    log_file = "data/trade_log.csv"
    if not os.path.exists(log_file) or os.path.getsize(log_file) < 100:
        return None, "⚠️ سجل التداول فارغ أو غير موجود. شغّل البوت أولاً."

    try:
        df = pd.read_csv(log_file, 
                         names=['symbol','side','entry','exit','size','pnl',
                                'reason','pred','conf','regime','sym_type','ai_approved'],
                         header=None, encoding='utf-8')
    except Exception as e:
        return None, f"❌ فشل قراءة سجل التداول: {e}"

    # الحد الأدنى: 4 صفقات
    if len(df) < 4:
        return (f"<b>📊 توصية أولية (عدد الصفقات: {len(df)})</b>\n"
                f"⚠️ البيانات لا تزال قليلة. يُنصح باستخدام الأوزان الافتراضية:\n"
                f"📨 <code>تعيين أوزان النماذج Rule=10 EMR=5 CFHM=10 Timing=55 VWAP=20</code>\n"
                f"📨 <code>تعيين أوزان الأطر 50 1 49</code>"), None

    # اقتصار التحليل على آخر 400 صفقة
    df = df.tail(400)

    # ============================================================
    # 1. استخراج درجات الأطر والأعضاء من عمود reason
    # ============================================================
    frame_data = {'5m': {'scores': [], 'pnls': []},
                  '15m': {'scores': [], 'pnls': []},
                  '1h': {'scores': [], 'pnls': []}}
    
    model_data = {m: {'scores': [], 'pnls': []} for m in ['rule','emr','cfhm','timing','vwap_obv']}

    # أنماط الاستخراج (لإطارات 5م، 15د، 1س)
    frame_patterns = {
        '5m': r"📊 إطار 5 دقائق:\n\s+Rule=([\d.]+)\s*\|\s*EMR=([\d.]+)\s*\|\s*CFHM=([\d.]+)\s*\|\s*Timing=([\d.]+)\s*\|\s*VWAP=([\d.]+)",
        '15m': r"📊 إطار 15 دقيقة:\n\s+Rule=([\d.]+)\s*\|\s*EMR=([\d.]+)\s*\|\s*CFHM=([\d.]+)\s*\|\s*Timing=([\d.]+)\s*\|\s*VWAP=([\d.]+)",
        '1h': r"📊 إطار 1 ساعة:\n\s+Rule=([\d.]+)\s*\|\s*EMR=([\d.]+)\s*\|\s*CFHM=([\d.]+)\s*\|\s*Timing=([\d.]+)\s*\|\s*VWAP=([\d.]+)"
    }
    ...
    for idx, row in df.iterrows():
        ...
        for frame, pattern in frame_patterns.items():
            match = re.search(pattern, reason, re.DOTALL)
            if match:
                vals = list(map(float, match.groups()))
                avg_score = sum(vals) / len(vals)  # متوسط درجات الأعضاء الخمسة لهذا الإطار
                frame_data[frame]['scores'].append(avg_score)
                frame_data[frame]['pnls'].append(pnl)

        # استخراج درجات الأعضاء (من قسم المتوسط الحسابي)
        model_match = re.search(r"📈 المتوسط الحسابي .*:\n\s+Rule=([\d.]+)\s*\|\s*EMR=([\d.]+)\s*\|\s*CFHM=([\d.]+)\s*\|\s*Timing=([\d.]+)\s*\|\s*VWAP=([\d.]+)", reason, re.DOTALL)
        if model_match:
            vals = list(map(float, model_match.groups()))
            names = ['rule', 'emr', 'cfhm', 'timing', 'vwap_obv']
            for i, name in enumerate(names):
                model_data[name]['scores'].append(vals[i])
                model_data[name]['pnls'].append(pnl)

    # ============================================================
    # 2. حساب أداء كل إطار (معامل ارتباط بسيط)
    # ============================================================
    def calculate_performance(data):
        ...
        if len(scores) > 4 and np.std(scores) > 0.001 and np.std(pnls) > 0.001:
    ...
    for frame, data in frame_data.items():
        frame_performance[frame] = calculate_performance(data)

    # ============================================================
    # 3. توزيع الأوزان ديناميكياً على الأطر (مع احترام الحدود 40-70، 20-40، 1-5)
    # ============================================================
    sorted_frames = sorted(frame_performance.items(), key=lambda x: x[1], reverse=True)

    frame_weights = {}
    if len(sorted_frames) >= 3 and sum([x[1] for x in sorted_frames]) > 0.001:
        perf1, perf2, perf3 = sorted_frames[0][1], sorted_frames[1][1], sorted_frames[2][1]
        total_perf = perf1 + perf2 + perf3
        
        if total_perf > 0:
            # حساب النسب المئوية الأولية (تطبيع)
            raw_p1 = perf1 / total_perf
            raw_p2 = perf2 / total_perf
            raw_p3 = perf3 / total_perf
            
            # توزيع الوزن الإجمالي (100%) ديناميكياً مع احترام الحدود
            # الخطوة 1: تحديد الوزن الأولي للإطار الأول (بين 40% و 70%)
            w1 = 40 + (raw_p1 - (1/3)) * 90
            w1 = max(40, min(70, w1))
            
            # الخطوة 2: تحديد الوزن للإطار الثاني (بين 20% و 40%)
            ratio_2_3 = perf2 / (perf2 + perf3 + 0.001)
            w2 = 20 + ratio_2_3 * 20
            w2 = max(20, min(40, w2))
            
            # الخطوة 3: الوزن الثالث هو المتبقي (بين 1% و 5%)
            w3 = 100 - w1 - w2
            # ضمان أن w3 بين 1 و 5
            if w3 < 1:
                w3 = 1
                w2 -= (1 - w3)
                if w2 < 20:
            ...
            frame_weights = {
                sorted_frames[0][0]: w1,
                sorted_frames[1][0]: w2,
                sorted_frames[2][0]: w3
            }
        else:
            frame_weights = {sorted_frames[0][0]: 50, sorted_frames[1][0]: 30, sorted_frames[2][0]: 20}
    else:
        # إذا لم تكن البيانات كافية، نرجع الأوزان الافتراضية
        return (f"<b>📊 توصية مؤقتة (عدد الصفقات: {len(df)})</b>\n"
                f"⚠️ البيانات غير كافية للتصنيف الهرمي. يُنصح باستخدام الأوزان الافتراضية:\n"
                f"📨 <code>تعيين أوزان الأطر 50 1 49</code>"), None

    # ============================================================
    # 4. تقييم أداء الأعضاء (على الإطار الأفضل فقط)
    # ============================================================
    best_frame = sorted_frames[0][0]
    member_scores = {'rule': [], 'emr': [], 'cfhm': [], 'timing': [], 'vwap_obv': []}
    member_pnls = {'rule': [], 'emr': [], 'cfhm': [], 'timing': [], 'vwap_obv': []}
    
    pattern_best = frame_patterns[best_frame]
    for idx, row in df.iterrows():
        reason = row['reason']
        if not isinstance(reason, str):
            continue
        ...
        if pd.isna(pnl):
            continue
        match = re.search(pattern_best, reason, re.DOTALL)
        if match:
            vals = list(map(float, match.groups()))
            names = ['rule', 'emr', 'cfhm', 'timing', 'vwap_obv']
            for i, name in enumerate(names):
                member_scores[name].append(vals[i])
                member_pnls[name].append(pnl)

    member_performance = {}
    for name in member_scores:
        if len(member_scores[name]) > 4:
            corr = np.corrcoef(member_scores[name], member_pnls[name])[0, 1]
            member_performance[name] = max(0, corr if not np.isnan(corr) else 0)
        else:
            member_performance[name] = 0.0

    # ترتيب الأعضاء تنازلياً وتوزيع الأوزان
    sorted_members = sorted(member_performance.items(), key=lambda x: x[1], reverse=True)
    member_weights = {}
    if len(sorted_members) >= 3 and sum([x[1] for x in sorted_members]) > 0.001:
        total_mem_perf = sum([x[1] for x in sorted_members[:3]])
        if total_mem_perf > 0:
            w1 = min(50, max(40, (sorted_members[0][1] / total_mem_perf) * 50))
            w2 = min(30, max(20, (sorted_members[1][1] / total_mem_perf) * 30))
            w3 = min(15, max(10, (sorted_members[2][1] / total_mem_perf) * 15))
            remaining = 100 - w1 - w2 - w3
            member_weights = {
                sorted_members[0][0]: w1,
                sorted_members[1][0]: w2,
                sorted_members[2][0]: w3,
                sorted_members[3][0]: max(0, remaining - 5) if len(sorted_members) > 3 else 0,
                sorted_members[4][0]: 0 if len(sorted_members) > 3 else remaining
            }
        else:
            for i, (name, _) in enumerate(sorted_members[:3]):
                member_weights[name] = 30 if i == 0 else (25 if i == 1 else 20)
    else:
        # استخدام الأوزان الافتراضية
        member_weights = {'rule': 10, 'emr': 5, 'cfhm': 10, 'timing': 55, 'vwap_obv': 20}

    # ============================================================
    # 5. بناء رسالة التوصية النهائية
    # ============================================================
    result_msg = f"<b>📊 توصية الأوزان المثلى (تحليل آخر {len(df)} صفقة)</b>\n\n"
    
    result_msg += f"<b>⏱️ أوزان الأطر (5م، 15د، 1س):</b>\n"
    for frame, weight in frame_weights.items():
        result_msg += f"   {frame}: {weight:.1f}%\n"
    
    tg_frames = f"تعيين أوزان الأطر {int(frame_weights.get('5m', 0))} {int(frame_weights.get('15m', 0))} {int(frame_weights.get('1h', 0))}"
    result_msg += f"\n📨 <code>{tg_frames}</code>\n\n"
    
    result_msg += f"<b>🤖 أوزان النماذج (على الإطار الأفضل {best_frame}):</b>\n"
    name_map = {'rule':'Rule','emr':'EMR','cfhm':'CFHM','timing':'Timing','vwap_obv':'VWAP_OBV'}
    for name in ['rule', 'emr', 'cfhm', 'timing', 'vwap_obv']:
        weight = member_weights.get(name, 0)
        result_msg += f"   {name_map[name]}: {weight:.1f}%\n"
    
    tg_models = f"تعيين أوزان النماذج Rule={int(member_weights.get('rule', 0))} EMR={int(member_weights.get('emr', 0))} CFHM={int(member_weights.get('cfhm', 0))} Timing={int(member_weights.get('timing', 0))} VWAP={int(member_weights.get('vwap_obv', 0))}"
    result_msg += f"\n📨 <code>{tg_models}</code>"
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # ===== نظام بيع 15 دقيقة (يعتمد على متوسط 5 و15 دقيقة) =====
                now = time.time()
                ...
                if now - last_check >= SELL_15M_CHECK_INTERVAL:
                    try:
                        ...
                        # حساب المتوسط الحسابي لكل نموذج
                        avg_scores = {}
                        for model in SELL_WEIGHTS.keys():
                            avg_scores[model] = (scores_5m[model] + scores_15m[model]) / 2.0
                        score_15m = (
                            avg_scores['rule'] * SELL_WEIGHTS['rule'] +
                            avg_scores['emr'] * SELL_WEIGHTS['emr'] +
                            avg_scores['cfhm'] * SELL_WEIGHTS['cfhm'] +
                            avg_scores['timing'] * SELL_WEIGHTS['timing'] +
                            avg_scores['vwap_obv'] * SELL_WEIGHTS['vwap_obv']
                        )
                        if score_15m < SELL_15M_THRESHOLD:
                            scores_str = (
                                f"Rule={avg_scores['rule']:.3f}, "
                                f"Hybrid={avg_scores['emr']:.3f}, "
                                f"CFHM={avg_scores['cfhm']:.3f}, "
                                f"Timing={avg_scores['timing']:.3f}, "
                                f"VWAP_OBV={avg_scores['vwap_obv']:.3f}"
                            )
                            reason = (
                                f"📉 15m Committee (avg 5m+15m): {score_15m:.3f} < {SELL_15M_THRESHOLD:.2f}\n"
                                f"📊 الدرجات: {scores_str}"
                            )
                        ...
                        logger.warning(f"⚠️ فشل فحص 15 دقيقة لـ {sym}: {e}")
                        _last_15m_sell_check[sym] = now
...
# --------------------------- دالة التحليل الرئيسية (معدلة لـ 3 أطر) ---------------------------
def analyze_and_trade():
    ...
    try:
        ...
        for sym, sym_type, df_5m, df_15m, df_1h, regime in candidates_for_manipulation:
            ...
            dec, avg_score, conf, _, avg_scores = buy_comm.decide(df_5m, df_15m, df_1h, sym, sym_type, market_cond)
            if dec != 'buy':
                continue
            ...
            scores_5m = buy_comm._get_scores(df_5m)
            scores_15m = buy_comm._get_scores(df_15m)
            scores_1h = buy_comm._get_scores(df_1h)
...
# ------------------- الدالة المعدلة sync_preemptive_stop_orders (الحل المبكر مع تجنب ازدواجية الأرباح) -------------------
def sync_preemptive_stop_orders():
    while True:
        ...
        time.sleep(60)   # يمكن تغييرها إلى 15 لتسريع الاكتشاف
                ...
                if data.get("ok"):
                    for upd in data.get("result",[]):
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ======================== تم تحديث أمر حالة الفلاتر لعرض أوزان النماذج ========================
                                ...
                                status = (f"📊 <b>حالة الفلاتر والقيم الحالية</b>\n━━━━━━━━━━━━━━━━━━━━\n"
                                          ...
                                          f"📉 <b>عتبة بيع 15 دقيقة (متوسط 5م+15د):</b> {SELL_15M_THRESHOLD:.2%}\n"
                                          f"━━━━━━━━━━━━━━━━━━━━\n"
                            ...
                            # ------------------------------------------------------------
                            # 16. أمر تعيين عتبة بيع 15 دقيقة (معدل)
                            # ------------------------------------------------------------
                                ...
                                if numbers:
                                    ...
                                    if 0.10 <= new_val <= 0.90:
                                        SELL_15M_THRESHOLD = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تعيين عتبة بيع 15 دقيقة إلى {new_val:.2f}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.10 و 0.90")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين عتبة بيع 15 دقيقة 0.42`")
...
if __name__ == '__main__':
    ...
    send_telegram(f"🚀 بوت v28.0 (الإصدار المحسن بـ 3 أطر زمنية 5م، 15د، 1س مع أوزان ديناميكية وتوصيات ذكية) يعمل – الوضع: {mode_str}\n"
                  ...
                  f"✅ **أمر `توصية بالأوزان` لتحليل آخر 100 صفقة واقتراح الأوزان المثلى للنماذج والأطر باستخدام معامل الارتباط**\n"
