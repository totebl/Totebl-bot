...
class HybridMomentumRSIModel:
    ...
    def update(self, symbol, df):
        ...
        # ============================================================
        # 2. حساب مؤشر القوة النسبية (RSI) - الوزن 30%
        # ============================================================
        if 'rsi' in df.columns and not pd.isna(df['rsi'].iloc[-1]):
            rsi_value = df['rsi'].iloc[-1]
            rsi_score = rsi_value / 100.0
        else:
            rsi_score = 0.5
...
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        ...
        # تطبيق الأوزان المرجحة (4 نماذج الآن)
        if sym_type == 'meme':
            weights = {'rule': 0.15, 'emr': 0.30, 'cfhm': 0.30, 'timing': 0.25}
        else:
            weights = {'rule': 0.30, 'emr': 0.25, 'cfhm': 0.25, 'timing': 0.20}
