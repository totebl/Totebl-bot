# ========================================================================
# بوت التداول المتطور v24.52 - النسخة النهائية الموسعة (قيم معدلة)
# التعديلات المطبقة:
# - الإصلاحات 1-7 السابقة
# - فلتر الشمعة الصاعدة على 4 ساعات
# - توسيع شروط الماسح وفحص السيولة وتبريد أقل
# ========================================================================

import subprocess, sys, os, logging, time, warnings
import logging.handlers
from datetime import datetime, timedelta, timezone
from collections import defaultdict, deque, OrderedDict
from contextlib import contextmanager
import threading, atexit, signal, importlib, re, traceback
...
# --------------------------- إعدادات التداول الأساسية (مع التعديلات) ---------------------------
...
COOLDOWN_HOURS_BAD = float(os.environ.get('COOLDOWN_HOURS_BAD', 0.5))  # معدل: من 1.0 إلى 0.5
...
STRENGTH_THRESHOLD = float(os.environ.get('STRENGTH_THRESHOLD', 0.25))  # معدل: من 0.30 إلى 0.25
SCALP_MIN_PROFIT = float(os.environ.get('SCALP_MIN_PROFIT', 0.20))       # معدل: من 0.25 إلى 0.20
...
ACTIVE_SYMBOLS_LIMIT = int(os.environ.get('ACTIVE_SYMBOLS_LIMIT', 20))
MAX_DAILY_TRADES = int(os.environ.get('MAX_DAILY_TRADES', 30))
BASE_COOLDOWN_HOURS = float(os.environ.get('BASE_COOLDOWN_HOURS', 0.5))   # معدل: من 1.0 إلى 0.5

# تم تخفيف شروط الماسح الأولي
MIN_24H_CHANGE_PERCENT = float(os.environ.get('MIN_24H_CHANGE_PERCENT', 0.5))   # من 1.0 إلى 0.5
MIN_24H_VOLUME_USD = float(os.environ.get('MIN_24H_VOLUME_USD', 75_000))        # من 100_000 إلى 75_000
MIN_MARKET_CAP_USD = float(os.environ.get('MIN_MARKET_CAP_USD', 1_500_000))     # من 2_000_000 إلى 1_500_000
...
DAILY_LOSS_MODE = os.environ.get('DAILY_LOSS_MODE', 'absolute')
MAX_STUCK_RETRIES = int(os.environ.get('MAX_STUCK_RETRIES', 10))

TRAILING_ACTIVATION_NORMAL = float(os.environ.get('TRAILING_ACTIVATION_NORMAL', 0.015))
TRAILING_ACTIVATION_MEME = float(os.environ.get('TRAILING_ACTIVATION_MEME', 0.020))
TRAILING_DISTANCE_NORMAL = float(os.environ.get('TRAILING_DISTANCE_NORMAL', 0.012))
TRAILING_DISTANCE_MEME = float(os.environ.get('TRAILING_DISTANCE_MEME', 0.010))
...
PREPLACED_SL_PERCENT_NORMAL = 0.03
PREPLACED_SL_PERCENT_MEME = 0.06
...
TAKE_PROFIT_PERCENTS_NORMAL = [0.10, 0.15, 0.15, 0.15, 0.10, 0.50, 1.00]
TAKE_PROFIT_LEVELS_MEME = [0.015, 0.022, 0.030, 0.041, 0.051, 0.066, 0.080]
TAKE_PROFIT_PERCENTS_MEME = [0.10, 0.15, 0.15, 0.15, 0.10, 0.50, 1.00]

LOCK_TIMEOUT_SECONDS = 480
...
# --------------------------- متغيرات الإصلاحات الجديدة ---------------------------
_last_loss_time = {}
LOSS_COOLDOWN_MINUTES = int(os.environ.get('LOSS_COOLDOWN_MINUTES', 1))  # معدل: من 30 إلى 1

_pending_symbols_time = {}
PENDING_TIMEOUT_SECONDS = int(os.environ.get('PENDING_TIMEOUT_SECONDS', 3600))  # ساعة
...
def generate_error_report(error_type, component, details="", traceback_str=""):
    ...
    with _global_state_lock:
        ...
        paused = _is_paused()
        pending_local = list(_local_pending_symbols)
        pending_ex = list(_exchange_pending_symbols)
    ...
    for name, info in _essential_threads.items():
        t = info['thread']
        threads_status[name] = "حي" if t.is_alive() else "ميت"
...
def send_critical_error(msg, tb):
    clean_msg = escape_html(msg[:200])
    clean_tb = escape_html(tb)
    logger.error(f"حرج: {msg}\n{tb}")
    safe_text = f"<b>⚠️ خطأ حرج</b>\n<pre>{clean_msg}\n{clean_tb[:500]}</pre>"
    send_telegram(safe_text)
    generate_error_report("خطأ_حرج", "تحليل", msg, tb)
...
def load_markets_with_retry(max_retries=5):
    for attempt in range(max_retries):
        try:
            ...
            if 'BTC/USDT' in primary_exchange.markets:
                ...
                raise Exception("BTC/USDT غير موجود في الأسواق المحملة")
        except Exception as e:
            logger.error(f"⚠️ فشل تحميل الأسواق (محاولة {attempt+1}/{max_retries}): {e}")
            if attempt < max_retries - 1:
                wait = (attempt + 1) * 10
                logger.info(f"⏳ إعادة المحاولة بعد {wait} ثانية...")
                time.sleep(wait)
            else:
                logger.critical("❌ فشل تحميل الأسواق بعد كل المحاولات.")
                send_telegram("❌ فشل تحميل الأسواق بعد 5 محاولات.")
                return False
    return False

if not load_markets_with_retry():
    send_telegram("⚠️ البوت يعمل بدون بيانات أسواق صحيحة.")

# --------------------------- دوال الجلب الأساسية (بدون تغيير) ---------------------------
_orderbook_cache = {}
    ...
    try:
        ...
        with _orderbook_cache_lock:
            ...
            if len(_orderbook_cache) > _ORDERBOOK_CACHE_MAX:
                oldest_key = min(_orderbook_cache.items(), key=lambda x: x[1]['timestamp'])[0]
                del _orderbook_cache[oldest_key]
        return ob
    except Exception as e:
...
def fetch_ohlcv_retry_raw(symbol, timeframe, limit=500, max_retries=4):
    ...
    while attempt < max_retries:
        try:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(exchange.fetch_ohlcv, symbol, timeframe, actual_limit)
                bars = future.result(timeout=FETCH_OHLCV_TIMEOUT)
            ...
            last_err = f"مهلة {FETCH_OHLCV_TIMEOUT} ثانية"
            attempt += 1
            if attempt < max_retries:
                wait = 2**attempt + 1
                logger.warning(f"⏳ مهلة لـ {symbol} {timeframe}، إعادة محاولة بعد {wait} ثانية")
                time.sleep(wait)
        except Exception as e:
            last_err = str(e)
            ...
            if hasattr(e, 'headers') and e.headers.get('Retry-After'):
                ...
                logger.warning(f"⏳ Binance يطلب الانتظار {wait} ثانية")
                send_telegram(f"⚠️ <b>تجاوز معدل الطلبات</b>\nرمز: {symbol}\nالانتظار: {wait} ثانية")
                time.sleep(wait)
            else:
    ...
    generate_error_report("فشل_بيانات", "جلب_الشموع", f"فشل جلب {symbol} {timeframe}")
    return pd.DataFrame()
    ...
    with _global_state_lock:
        if bot_stats.last_valid_tickers:
            ...
            if age < STALE_DATA_MAX_AGE:
                logger.warning(f"⚠️ استخدام بيانات قديمة (عمرها {age:.0f} ثانية) كحل احتياطي")
                tickers_copy = dict(bot_stats.last_valid_tickers)
                for sym in tickers_copy:
                    if tickers_copy[sym].get('percentage') is None:
                        tickers_copy[sym]['percentage'] = 0.0
                    if tickers_copy[sym].get('quoteVolume') is None:
                        tickers_copy[sym]['quoteVolume'] = 0.0
                    if tickers_copy[sym].get('last') is None:
                        tickers_copy[sym]['last'] = 0.0
                return tickers_copy
            else:
                logger.error(f"❌ بيانات السوق قديمة جداً ({age:.0f} ثانية) > {STALE_DATA_MAX_AGE}")
                generate_error_report("فشل_بيانات", "جلب_الأسعار", f"بيانات قديمة جداً: {age:.0f}")
    return {}
...
def get_market_cap_from_coingecko(symbol, ticker_data=None):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            logger.warning(f"CoinGecko فشل لـ {symbol}: {e}")
...
# --------------------------- دوال التوازن والرصيد (بالإصلاح 2) ---------------------------
def get_total_equity():
    # الإصلاح رقم 7: في الوضع الورقي، أضف قيمة المراكز المفتوحة
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            cash = bot_stats.last_balance
            positions_value = 0.0
            for sym, pos in open_positions.items():
                ticker = fetch_ticker_with_retry(sym)
                price = ticker['last'] if ticker else pos.entry_price
                positions_value += pos.remaining_size * price
            total = cash + positions_value
            # تحديث last_balance بالقيمة الإجمالية لأغراض التقارير (اختياري)
...
def get_real_balance_usdt():
    ...
    for attempt in range(3):
        try:
            ...
            logger.warning(f"محاولة {attempt+1} لجلب الرصيد فشلت: {e}")
            time.sleep(1)
    now = time.time()
    with _state_lock:
        if _last_successful_balance_time == 0 or (now - _last_successful_balance_time) > 120:
            if not _balance_failure_paused:
                _balance_failure_paused = True
                _balance_failure_start_time = now
                _set_paused(True)
...
def _get_balance_no_pause(max_retries=3):
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
            return bot_stats.last_balance
    exchange = get_active_exchange()
...
def _safe_fetch_balance_after_trade(initial_delay=1.0, max_delay=300):
    """
    محاولات لا نهائية لجلب الرصيد مع تأخير متزايد.
    ترسل تنبيهاً كل 5 دقائق عند الفشل، وعند النجاح ترسل تنبيهاً بالاستعادة.
    لا توقف البوت أبداً.
    """
    if PAPER_TRADING or TEST_MODE or not ENABLE_TRADING:
        with _global_state_lock:
    ...
    attempt = 0
    last_alert_time = 0
    alert_interval = 300  # 5 دقائق
    
    while True:
        bal = _get_balance_no_pause()
        if bal is not None and bal > 0:
            with _global_state_lock:
                old_balance = bot_stats.last_balance
                bot_stats.last_balance = bal
            if attempt > 0:
                send_telegram(f"✅ تم استعادة الرصيد بعد انقطاع. الرصيد الحالي: {bal:.2f} USDT (كان قديماً: {old_balance:.2f})")
            return bal
        
        attempt += 1
        now = time.time()
        if now - last_alert_time >= alert_interval:
            send_telegram(f"⚠️ فشل جلب الرصيد (محاولة #{attempt}). سيتم إعادة المحاولة تلقائياً. التداول مستمر.")
            last_alert_time = now
            generate_error_report("تحذير_رصيد", "تحديث", f"فشل جلب الرصيد {attempt} مرة")
        
        time.sleep(delay)
        delay = min(delay * 1.5, max_delay)  # تأخير تصاعدي يصل إلى 5 دقائق
...
<head><meta charset="UTF-8"><title>بوت v24.52</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v24.52 – ماسح موسع</h1>
<div id="stats"></div>
        ...
        try:
            ...
            while time.time() - start < timeout:
                try:
                    ...
                    logger.debug(f"خطأ في جلب حالة الأمر {oid}: {e}")
                time.sleep(0.2)
            if filled>0:
                try:
                    exchange.cancel_order(oid, symbol)
                except Exception as e:
                    logger.warning(f"فشل إلغاء الأمر {oid}: {e}")
                return True, avg_price, filled, oid
            else:
                try:
                    exchange.cancel_order(oid, symbol)
            ...
            if "Post only" in str(e) and attempt == 0 and POST_ONLY_ORDERS:
                logger.info(f"⚠️ رفض postOnly لـ {symbol}، إعادة محاولة بدون")
                continue
...
def place_preemptive_stop_loss(symbol, size, entry_price, sym_type, stop_price_override=None):
    ...
    if stop_price_override is not None:
        ...
        sl_percent = PREPLACED_SL_PERCENT_MEME if sym_type == 'meme' else PREPLACED_SL_PERCENT_NORMAL
    ...
    try:
        ...
        logger.info(f"✅ أمر وقف مسبق (stop_loss_limit) لـ {symbol} ID: {oid}")
        return oid
    except Exception as e:
        logger.error(f"فشل stop_loss_limit لـ {symbol}: {e}")
        try:
            order = exchange.create_market_sell_order(symbol, size)
            logger.warning(f"⚠️ تم استخدام أمر سوق طارئ لـ {symbol}")
            return order.get('id')
...
def update_preemptive_stop_loss(symbol, pos, new_stop_price):
    # تستخدم عند تحديث الوقف المتحرك أو بعد البيع الجزئي
    ...
    if new_oid:
        ...
        logger.warning(f"⚠️ فشل تحديث الوقف المسبق لـ {symbol}")
        generate_error_report("فشل_أمر", "أوامر", f"فشل تحديث الوقف المسبق لـ {symbol}")
    ...
    for attempt in range(3):
        try:
            ...
            if total < pos.remaining_size * 0.01:
                logger.warning(f"⚠️ المركز {symbol} لم يعد موجوداً في المنصة. سيتم حذفه بدون تسجيل خسارة وهمية.")
                cancel_preemptive_stop_loss(symbol, pos.preemptive_sl_order_id)
                send_telegram(f"ℹ️ تم حذف المركز {symbol} تلقائياً لأنه غير موجود في المحفظة (تم إغلاقه يدوياً).")
                return False
            return True
        except Exception as e:
                ...
                with _global_state_lock:
                    ...
                    if current_oid and current_oid not in order_map.values():
                        ...
                        if pos.trailing_activated and pos.trailing_stop is not None:
                            ...
                            sl_percent = PREPLACED_SL_PERCENT_MEME if pos.symbol_type == 'meme' else PREPLACED_SL_PERCENT_NORMAL
                            new_stop = pos.entry_price * (1 - sl_percent)
                        ...
                        if new_oid:
                            ...
                            logger.error(f"❌ فشل إنشاء وقف مسبق جديد لـ {sym} أثناء المزامنة")
...
class EMRModel:
    ...
    def _get_dominant_cycle(self, prices):
        ...
        try:
            ...
            logger.debug(f"خطأ في تحويل FFT: {e}")
            return None,None,None
...
class CFHMModel:
    ...
    def calculate_scores(self, df):
        if len(df) < max(self.flow_win, self.mom_win):
            return None, None
...
class EngulfingModel:
    def get_sell_vote_percent(self, df, cur_price, pos):
        ...
        try:
            ...
            logger.debug(f"خطأ في engulfing: {e}")
            return 0.0
...
class LiquidityExitModel:
    def get_sell_vote_percent(self, symbol, cur_price):
        try:
            ...
            logger.debug(f"خطأ في liquidity: {e}")
            return 0.0
...
class MomentumDecayModel:
    def get_sell_vote(self, df, pos, cur_price):
        if pos.initial_momentum is None or pos.entry_momentum_time is None or pos.initial_momentum == 0:
            return 0.0
...
class BuyingCommittee:
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        ...
        try:
            ...
            logger.debug(f"EMR فشل لـ {symbol}: {e}")
            buy_pct_emr = None
        ...
        cfg = config.get(market_condition, config['normal'])
        ...
        adjusted = []
        for v in valid:
            adj = max(0.0, min(1.0, v * multiplier))
            adjusted.append(adj)
...
class SellingCommittee:
    ...
    def decide_sell(self, df_primary, symbol, pos, cur_price, market_condition='normal'):
        ...
        try:
            ...
            logger.debug(f"EMR فشل في البيع لـ {symbol}: {e}")
            sell_pct_emr = None
        ...
        for v in raw_base:
            if v is not None:
                base_valid.append(max(0.0, min(1.0, v * multiplier)))
        other_votes = [
...
def fetch_recent_trades(symbol, limit=50, ttl=10):
    ...
    try:
        ...
        logger.debug(f"فشل جلب الصفقات لـ {symbol}: {e}")
    return []
...
class OrderBookManipulationDetector:
    ...
    def _cleanup_old_entries(self, now, ttl=300):
        ...
        for sym in list(self.order_snapshots.keys()):
            if now - self.order_snapshots[sym].get('timestamp', 0) > ttl:
    ...
    def detect_spoofing(self, symbol, current_ob, sym_type='normal', window_seconds=10, min_orders=5):
        ...
        if symbol not in self.order_snapshots:
            self.order_snapshots[symbol] = {
                'timestamp': now,
                'bids': current_ob.get('bids', [])[:20],
        ...
        if sym_type == 'meme':
            size_multiplier = SPOOFING_ORDER_SIZE_MULTIPLIER_MEME
        else:
            size_multiplier = SPOOFING_ORDER_SIZE_MULTIPLIER_NORMAL
        ...
        self.spoofing_history[symbol].append({
            ...
            'large_orders': total_large_orders,
            'cancelled': large_orders_cancelled
        })
    ...
    def detect_fake_liquidity(self, symbol, current_ob, sym_type='normal'):
        ...
        if sym_type == 'meme':
            wall_multiplier = FAKE_WALL_SIZE_MULTIPLIER_MEME
        else:
            wall_multiplier = FAKE_WALL_SIZE_MULTIPLIER_NORMAL
        ...
        for price, size in prev_large_bids:
            found = any(is_same_price_level(price, cp, 0.005) for cp, cs in curr_large_bids)
            if not found:
                vanished_bids += 1
        vanished_asks = 0
        for price, size in prev_large_asks:
            found = any(is_same_price_level(price, cp, 0.005) for cp, cs in curr_large_asks)
            if not found:
                vanished_asks += 1
        ...
        self.wall_history[symbol].append({
            'time': now,
...
class WhaleManipulationDetector:
    ...
    def detect_whale_trade(self, symbol, sym_type='normal', current_ob=None):
        ...
        if sym_type == 'meme':
            min_whale_usd = WHALE_MIN_TRADE_USD_MEME
        else:
            min_whale_usd = WHALE_MIN_TRADE_USD_NORMAL
        recent_whale_trades = []
        whale_count = 0
        ...
        for trade in trades:
            ...
            if amount_usd >= min_whale_usd:
                whale_count += 1
                total_whale_volume += amount_usd
                recent_whale_trades.append({
                    'time': trade['timestamp'] / 1000 if isinstance(trade['timestamp'], (int, float)) else now,
    ...
    def detect_volume_spike(self, symbol, sym_type='normal'):
        try:
            ...
            threshold = WHALE_VOLUME_SPIKE_MULTIPLIER
            if sym_type == 'meme':
            ...
            logger.debug(f"خطأ في كشف ارتفاع الحجم لـ {symbol}: {e}")
            return False, 0.0

order_book_detector = OrderBookManipulationDetector()
whale_detector = WhaleManipulationDetector()
_volume_history = defaultdict(lambda: deque(maxlen=50))
            ...
            if is_spoofing:
                ...
                if is_fake_liq:
                    ...
                    if suspicion_score >= MANIPULATION_REJECT_THRESHOLD:
                        result = (True, suspicion_score, "suspicious")
                    else:
...
# --------------------------- دوال حفظ واستعادة الحالة (مع تعديل طفيف) ---------------------------
def save_state():
    ...
    try:
        ...
        data = {
            ...
            'bot_stats':{
                ...
                'equity_curve':bot_stats.equity_curve[-1000:],
...
def load_state():
    ...
    try:
        ...
        bot_stats.equity_curve = stats.get('equity_curve',[])
...
# --------------------------- دوال التداول (مع الإصلاحات 1 و 4) ---------------------------
...
def cancel_stop_loss_safely(symbol, order_id, max_attempts=3):
    """دالة مساعدة لإلغاء الوقف مع التأكد من الإلغاء"""
    if not order_id:
        return True
    for attempt in range(max_attempts):
        try:
            ...
            if order['status'] == 'canceled':
                logger.info(f"✅ تم إلغاء الوقف {order_id} لـ {symbol}")
                return True
        except Exception as e:
            logger.warning(f"محاولة {attempt+1} لإلغاء الوقف {order_id} فشلت: {e}")
            time.sleep(0.5)
    logger.error(f"❌ فشل إلغاء الوقف {order_id} لـ {symbol} بعد {max_attempts} محاولات")
    return False
...
def close_partial(symbol, percent, price, reason, pnl_usdt=None):
    ...
    try:
        ...
        try:
            ...
            if not is_sim:
                new_balance = _safe_fetch_balance_after_trade()
                ...
                with _global_state_lock:
                    ...
                    logger.debug(f"تحديث الرصيد الورقي: +{sale_proceeds:.4f} USDT -> الرصيد الجديد: {bot_stats.last_balance:.2f}")
            ...
            with _global_state_lock:
                if DAILY_LOSS_MODE == 'net':
                    _daily_loss_tracker = max(0, _daily_loss_tracker - pnl_net) if pnl_net > 0 else _daily_loss_tracker + abs(pnl_net)
                else:
                    if pnl_net < 0:
                ...
                sell_msg = (f"{direction} <b>بيع جزئي</b> {symbol}\n"
                            f"النسبة: {percent*100:.0f}% | الحجم: {filled_size:.6f}\n"
                            f"السعر: {fill_price:.8f} | الربح الصافي: {pnl_net:+.4f} USDT ({pnl_percent:+.2f}%)\n"
                            f"السبب: {reason}")
                send_telegram(sell_msg)

                # إصلاح رقم 4: تسجيل وقت الخسارة الكاملة
                if pos.remaining_size <= 1e-8 and pnl_net < 0:
                    _last_loss_time[symbol] = datetime.now()
                ...
                if pos.remaining_size <= 1e-8:
                    ...
                    if not is_sim:
                        # إصلاح رقم 1: إلغاء الوقف القديم أولاً ثم إنشاء جديد
                        if pos.preemptive_sl_order_id:
                            cancel_stop_loss_safely(symbol, pos.preemptive_sl_order_id)
                            pos.preemptive_sl_order_id = None
                            time.sleep(0.3)
                        new_stop = None
                        if pos.trailing_activated and pos.trailing_stop is not None:
                            ...
                            new_stop = pos.entry_price * (1 - sl_percent)
                        if new_stop:
                            new_oid = place_preemptive_stop_loss(symbol, pos.remaining_size, pos.entry_price, pos.symbol_type, stop_price_override=new_stop)
                            if new_oid:
                                pos.preemptive_sl_order_id = new_oid
                                pos.last_trailing_stop_sent = new_stop
                            else:
                                logger.error(f"❌ فشل إنشاء وقف جديد لـ {symbol} بعد البيع الجزئي")
                                send_telegram(f"⚠️ مركز {symbol} أصبح بدون وقف بعد البيع الجزئي!")
                save_state()
        finally:
            with _global_state_lock:
...
def retry_stuck_positions():
    while True:
        try:
            with _global_state_lock:
                stuck = {sym: pos for sym, pos in open_positions.items()
                         if pos.last_fail_time is not None and pos.retry_count < MAX_STUCK_RETRIES}
...
def get_current_momentum(symbol, lookback_minutes):
    try:
        ...
        logger.debug(f"خطأ في حساب الزخم الحالي لـ {symbol}: {e}")
        return None
    ...
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # ========== نظام البيع المبكر المنفصل (بدون زخم) ==========
                # بيع 50% عند خسارة 1.5%
                if profit <= -0.015:
                    close_partial(sym, 0.5, cur_price, f"⚠️ وقاية: خسارة {abs(profit):.2%} (بيع 50%)")
                    continue
                
                # بيع 100% عند خسارة 3%
                if profit <= -0.03:
                    close_partial(sym, 1.0, cur_price, f"💀 وقاية: خسارة {abs(profit):.2%} (إغلاق كامل)")
                    continue
                # =======================================================
                ...
                if sell_dec:
                    close_partial(sym, 1.0, cur_price, f"🗳️ قرار لجنة البيع (متوسط {avg:.2%})")
                    continue

                if pos.trailing_activated and pos.trailing_stop is not None:
                    if pos.last_trailing_stop_sent is None or abs(pos.trailing_stop - pos.last_trailing_stop_sent) / pos.entry_price > 0.001:
                        # إصلاح رقم 1: إلغاء القديم ثم إنشاء جديد
                        if pos.preemptive_sl_order_id:
                            cancel_stop_loss_safely(sym, pos.preemptive_sl_order_id)
                            pos.preemptive_sl_order_id = None
                            time.sleep(0.3)
                        new_oid = place_preemptive_stop_loss(sym, pos.remaining_size, pos.entry_price, pos.symbol_type, stop_price_override=pos.trailing_stop)
                        if new_oid:
                            pos.preemptive_sl_order_id = new_oid
                            pos.last_trailing_stop_sent = pos.trailing_stop
                        else:
                            logger.error(f"❌ فشل تحديث الوقف المتحرك لـ {sym}")
                ...
                for idx, (target, pct) in enumerate(tp_levels_copy):
                    if (pos.side == 'buy' and cur_price >= target * (1 - tp_tolerance)) or (pos.side == 'sell' and cur_price <= target * (1 + tp_tolerance)):
                        ...
                        if idx >= 5 or pos.remaining_size * pct * cur_price < 5.0:
...
# --------------------------- دالة فحص السيولة (الإصلاح رقم 3) ---------------------------
def check_liquidity(symbol, size_usd, sym_type='normal'):
    """
    فحص سيولة صارم:
    - يرفض الصفقة إذا تجاوز حجمها النسبة المئوية المحددة من العمق (رفض قاطع).
    - النسب: 30% للعادي، 40% للميم (قابلة للتعديل عبر متغيرات البيئة).
    - يرفض أيضاً عند: لا دفتر، سعر صفر، فارق كبير، عمق صفر.
    """
    ob = fetch_orderbook_with_cache(symbol, limit=10)
    if not ob:
    ...
    if spread > max_spread:
        return False, f"فارق سعر كبير: {spread:.4f} > {max_spread}"
    ...
    if min_depth == 0:
        return False, "العمق صفر"
    
    # نسب رفض صارمة (معدلة)
    ratio_normal = float(os.environ.get('LIQUIDITY_RATIO_NORMAL', 0.30))  # من 0.25 إلى 0.30
    ratio_meme = float(os.environ.get('LIQUIDITY_RATIO_MEME', 0.40))      # من 0.35 إلى 0.40
    ratio_limit = ratio_meme if sym_type == 'meme' else ratio_normal
    
    if size_usd > min_depth * ratio_limit:
        logger.warning(f"❌ رفض صفقة {symbol}: حجم {size_usd:.0f} > {ratio_limit*100:.0f}% من العمق ({min_depth:.0f})")
        return False, f"حجم الصفقة كبير جداً بالنسبة للعمق ({size_usd/min_depth:.1%} > {ratio_limit:.0%})"
    
    return True, "السيولة كافية"

# --------------------------- دالة تنفيذ الصفقة (الإصلاح رقم 1: إنشاء الوقف بعد ثانيتين) ---------------------------
def execute_full_trade(symbol, side, size, price, atr, sl, tp, sym_type, pred, conf, regime):
    ...
    try:
        ...
        _pending_symbols_time[symbol] = time.time()   # للإصلاح 6
    finally:
        _global_state_lock.release()
    ...
    try:
        ...
        # ---------- الوضع الحقيقي ----------
        # تنفيذ أمر الشراء
        success, fill_price, filled_size, order_id = execute_limit_order(symbol, side, size, price, sym_type)
        if not success or filled_size <= 0:
            return False, 0
        
        # إنشاء كائن المركز بدون وقف
        pos = Position(symbol, side, filled_size, fill_price, atr, sl, tp, sym_type, pred, conf, regime)
        pos.preemptive_sl_order_id = None
        with _global_state_lock:
            open_positions[symbol] = pos
        
        # جدولة إنشاء الوقف بعد ثانيتين في خيط منفصل
        def create_stop_loss_after_delay():
            time.sleep(2)
            with _global_state_lock:
                current_pos = open_positions.get(symbol)
                if not current_pos or current_pos.preemptive_sl_order_id is not None:
                    return
            stop_id = place_preemptive_stop_loss(symbol, filled_size, fill_price, sym_type)
            if stop_id:
                with _global_state_lock:
                    current_pos = open_positions.get(symbol)
                    if current_pos:
                        current_pos.preemptive_sl_order_id = stop_id
                        # تعيين سعر الوقف المبدئي
                        sl_percent = PREPLACED_SL_PERCENT_MEME if sym_type == 'meme' else PREPLACED_SL_PERCENT_NORMAL
                        current_pos.last_trailing_stop_sent = fill_price * (1 - sl_percent)
                save_state()
                logger.info(f"✅ تم إنشاء وقف مسبق لـ {symbol} بعد ثانيتين من الشراء")
            else:
                logger.error(f"❌ فشل إنشاء الوقف بعد الشراء لـ {symbol}")
                send_telegram(f"⚠️ مركز {symbol} بدون وقف خسارة! تدخل يدوي مطلوب.")
        
        threading.Thread(target=create_stop_loss_after_delay, daemon=True).start()
...
# --------------------------- دالة التحليل الرئيسية (مع الفلاتر) ---------------------------
def analyze_and_trade():
    ...
    lock_acquired = False
    try:
        lock_acquired = _processing_lock.acquire(timeout=15)
        if not lock_acquired:
            logger.warning("⚠️ تخطي التحليل: لم نتمكن من الحصول على _processing_lock بعد 15 ثانية")
            return
        
        # تنظيف الرموز المعلقة القديمة (الإصلاح 6)
        cleanup_stale_pending_symbols()
        ...
        if not _initial_warmup_done and elapsed_since_start < STARTUP_DELAY_SECONDS + 30:
            logger.info("🔥 تحميل الكاش الأولي للرموز الأساسية (BTC,ETH,BNB,SOL)...")
        ...
        if elapsed_since_start < (STARTUP_DELAY_SECONDS + STARTUP_REDUCED_LIMIT_DURATION):
            ...
            logger.debug(f"⏳ فترة التهدئة: استخدام {effective_limit} رمزاً فقط بدلاً من {ACTIVE_SYMBOLS_LIMIT}")
            ...
            if _daily_trades_date != today:
                if _daily_trades_count > 0:
                    ...
                    report = (
                        ...
                        f"📈 الصفقات: {_daily_trades_count} | ✅ ربح: {_daily_winning_trades} | ❌ خسارة: {_daily_losing_trades}\n"
                        f"🏆 نسبة النجاح: {wr:.1f}%\n"
                        f"💰 خسارة اليوم: ${_daily_loss_tracker:.2f}\n"
        ...
        total_eq = get_total_equity() if not TEST_MODE and not PAPER_TRADING and ENABLE_TRADING else bot_stats.last_balance
        free_bal = get_real_balance_usdt() if not TEST_MODE and not PAPER_TRADING else bot_stats.last_balance
        ...
        try:
            ...
            logger.warning(f"فشل الحصول على حالة السوق: {e}")
            market_cond = 'normal'
        with _market_condition_lock:
        ...
        if market_cond == 'good':
            position_percent = MAX_POSITION_PERCENT_GOOD
        else:
        ...
        if market_cond == 'good':
            max_allowed_exposure = total_eq * 0.70
        else:
            max_allowed_exposure = total_eq * MAX_EXPOSED_PERCENT

        buy_comm = BuyingCommittee()
        with _global_state_lock:
        ...
        # ========== تعديل فلتر 4 ساعات ==========
        # قائمة لتتبع آخر مرة تم إرسال إشعار فشل لكل رمز
        _last_4h_fail_time = getattr(analyze_and_trade, '_last_4h_fail_time', {})
        ...
        for sym in active:
            ...
            if last_close_time.get(sym) and (datetime.now() - last_close_time[sym]).total_seconds() < cooldown_hours * 3600:
                continue
            # الإصلاح رقم 4: منع الشراء بعد الخسارة (مدة التبريد معدلة إلى دقيقة واحدة)
            last_loss = _last_loss_time.get(sym)
            if last_loss and (datetime.now() - last_loss).total_seconds() < LOSS_COOLDOWN_MINUTES * 60:
                continue
            
            # ---------- الفلتر الجديد: مقارنة السعر الحالي مع إغلاق آخر شمعة 4 ساعات ----------
            def get_last_4h_close_with_retry(symbol, max_retries=3):
                """جلب آخر شمعة مغلقة على 4 ساعات مع إعادة المحاولات"""
                for attempt in range(max_retries):
                    try:
                        df = fetch_ohlcv_retry_raw(symbol, '4h', limit=2, max_retries=1)
                        if df is not None and len(df) >= 2:
                            # آخر شمعة مغلقة هي التي قبل الحالية (index -2)
                            prev_candle = df.iloc[-2]
                            close_prev = prev_candle['close']
                            return close_prev, None
                        elif df is not None and len(df) == 1:
                            # حالة نادرة: فقط شمعة واحدة متاحة، نستخدمها
                            close_prev = df.iloc[-1]['close']
                            return close_prev, None
                        else:
                            error_msg = f"بيانات غير كافية (عدد الشموع: {len(df) if df is not None else 0})"
                            if attempt == max_retries - 1:
            ...
            close_prev_4h, error = get_last_4h_close_with_retry(sym, max_retries=3)
            if close_prev_4h is None:
                # فشل جلب البيانات - إرسال إشعار تلغرام (مرة كل 30 دقيقة لكل رمز)
                now = time.time()
                last_fail = _last_4h_fail_time.get(sym, 0)
                if now - last_fail > 1800:  # 30 دقيقة
                    send_telegram(f"⚠️ <b>فشل جلب بيانات 4 ساعات</b>\nالرمز: {sym}\nالخطأ: {error}\nتم رفض الرمز احتياطاً.")
                    _last_4h_fail_time[sym] = now
                    # حفظ المتغير للاستدعاءات القادمة
                    analyze_and_trade._last_4h_fail_time = _last_4h_fail_time
                logger.warning(f"🚫 رفض {sym}: فشل جلب بيانات 4 ساعات - {error}")
                continue
            
            # الحصول على السعر الحالي من التيكر
            current_price = all_tickers.get(sym, {}).get('last')
            if current_price is None or current_price <= 0:
                logger.warning(f"🚫 رفض {sym}: السعر الحالي غير متوفر")
                continue
            
            # المقارنة الأساسية: السعر الحالي > إغلاق آخر شمعة 4 ساعات → اتجاه صاعد
            if current_price <= close_prev_4h:
                logger.info(f"🚫 رفض {sym}: اتجاه هابط (السعر الحالي {current_price:.8f} <= إغلاق آخر 4 ساعات {close_prev_4h:.8f})")
                continue
            
            # إذا وصلنا هنا، الاتجاه صاعد
            logger.debug(f"✅ {sym}: اتجاه صاعد (السعر {current_price:.8f} > {close_prev_4h:.8f})")
            # ------------------------------------------------
            
            regime = detect_market_regime(sym)
            if regime == 'trending_down':
                continue
            df_primary = get_cached_features(sym, TIMEFRAMES['primary'], limit=500, ttl=30)
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.warning(f"مهلة كشف التلاعب لـ {sym} بعد {MANIPULATION_TIMEOUT} ثانية")
                    manipulation_results[sym] = (False, 0.0, "timeout")
                except Exception as e:
                    logger.warning(f"خطأ في نتيجة كشف التلاعب لـ {sym}: {e}")
        ...
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            ...
            if is_manipulated:
                logger.info(f"🚫 رفض الشراء لـ {sym}: تم اكتشاف تلاعب ({manip_type}) درجة {suspicion_score:.2%}")
            ...
            # فحص السيولة (الإصلاح رقم 3 مع النسب الجديدة)
        ...
        if pending:
            ...
            for opp in pending:
                with _global_state_lock:
                    if _daily_trades_count >= MAX_DAILY_TRADES:
                        break
                ...
                if market_cond == 'good':
                    max_exp2 = total_eq2 * 0.70
                else:
                    max_exp2 = total_eq2 * MAX_EXPOSED_PERCENT
                if market_cond == 'good':
                    pos_usdt = total_eq2 * MAX_POSITION_PERCENT_GOOD
                else:
                    pos_usdt = total_eq2 * MAX_POSITION_PERCENT
                ...
                succ, actual_size = execute_full_trade(opp['symbol'], 'buy', sz, opp['price'], opp['atr'],
                                                       ...
                                                       opp['conf'], opp['regime'])
                if succ:
                    new_rem = _safe_fetch_balance_after_trade()
                    if new_rem is None:
        ...
        send_critical_error("خطأ في التحليل", traceback.format_exc())
    finally:
        if lock_acquired:
            _processing_lock.release()
            _last_processing_lock_released = time.time()

# --------------------------- دالة تنظيف الرموز المعلقة (الإصلاح 6) ---------------------------
def cleanup_stale_pending_symbols():
    now = time.time()
    with _global_state_lock:
        stale = [sym for sym, ts in _pending_symbols_time.items() if now - ts > PENDING_TIMEOUT_SECONDS]
        for sym in stale:
            _local_pending_symbols.discard(sym)
            _exchange_pending_symbols.discard(sym)
            if sym in _pending_symbols_time:
                del _pending_symbols_time[sym]
        if stale:
            logger.info(f"🧹 تم مسح {len(stale)} رمزاً معلقاً قديماً (أكثر من {PENDING_TIMEOUT_SECONDS//60} دقيقة): {stale}")

# --------------------------- بقية دوال الخلفية والخيوط (بدون تغيير) ---------------------------
def health_monitor():
    stuck_start = None
    last_alert_time = 0
    while True:
        ...
        if _processing_lock.locked():
            if stuck_start is None:
                ...
                if elapsed > 480:
                    if time.time() - last_alert_time > 1800:
                        logger.critical("⚠️ قفل التحليل عالق لأكثر من 8 دقائق! قد يحتاج تدخل يدوي.")
                        send_telegram("⚠️ تحذير: قفل التحليل محتجز منذ 8 دقائق، يرجى إعادة تشغيل البوت يدوياً إذا تكرر.")
                        generate_error_report("خطأ_حرج", "خيوط", "قفل التحليل عالق لأكثر من 8 دقائق")
    ...
    while True:
        ...
        if _processing_lock.locked():
            ...
            if elapsed > LOCK_TIMEOUT_SECONDS:
                logger.critical(f"❗ قفل _processing_lock معلق لمدة {elapsed:.0f} ثانية! (أكثر من {LOCK_TIMEOUT_SECONDS} ثانية) محاولة حفظ الحالة ثم الخروج.")
                send_telegram(f"🚨 قفل التحليل عالق لمدة {elapsed:.0f} ثانية - حفظ الحالة والخروج لإعادة التشغيل (الآلية الاحتياطية).")
                try:
                    if _global_state_lock.acquire(timeout=3):
                        try:
                            save_state()
                            logger.info("✅ تم حفظ الحالة قبل الخروج.")
                        finally:
                            _global_state_lock.release()
                    else:
                        logger.error("❌ فشل الحصول على _global_state_lock لحفظ الحالة - قد تفقد بعض البيانات.")
                except Exception as e:
                    logger.error(f"خطأ أثناء حفظ الحالة: {e}")
                sys.exit(1)
        else:
            last_release = now
...
def sync_pending_orders():
    ...
    try:
        ...
        logger.warning(f"فشل مزامنة الأوامر: {e}. سيتم الاحتفاظ بالبيانات القديمة لمدة دقيقة واحدة.")
        pass
    ...
    while True:
        ...
        with _cache_lock:
            for key in list(_ohlcv_cache.keys()):
                if now - _ohlcv_cache[key]['timestamp'] > 60:
                    del _ohlcv_cache[key]
        with _features_cache_lock:
            for key in list(_features_cache.keys()):
                if now - _features_cache[key]['timestamp'] > 30:
                    del _features_cache[key]
        with _orderbook_cache_lock:
            for key in list(_orderbook_cache.keys()):
                if now - _orderbook_cache[key]['timestamp'] > 5:
                    del _orderbook_cache[key]
        with _cache_lock:
            for key in list(_manipulation_cache.keys()):
                if now - _manipulation_cache[key]['timestamp'] > _MANIPULATION_CACHE_TTL * 2:
                    del _manipulation_cache[key]
        logger.debug("🧹 تنظيف الذاكرة المؤقتة")
                                ...
                                if not positions:
                                    ...
                                    for sym, pos in positions:
                                        ...
                                        lines.append(f"{direction} {sym} ({pos.side})\n"
                                                     f"   الكمية: {pos.remaining_size:.6f}\n"
                                                     f"   السعر الحالي: {cur_price:.8f}\n"
                                                     f"   سعر الدخول: {pos.entry_price:.8f}\n"
                                                     f"   الربح/الخسارة: {pnl:+.2f} USDT ({pnl_percent:+.2f}%)\n"
                                                     f"   القيمة: ${value:.2f}")
    ...
    while True:
        try:
            with _global_state_lock:
                ...
                total_equity = get_total_equity()
            report = (
                f"<b>📊 تقرير دوري (كل 4 ساعات)</b>\n"
                f"🕒 الوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                f"💰 الرصيد الكلي: ${total_equity:.2f}\n"
                f"📈 إجمالي الربح الصافي: ${total_pnl:.2f}\n"
                f"📊 صفقات اليوم: {daily_trades}\n"
                f"🔓 مراكز مفتوحة: {positions_count}\n"
                f"⏸️ متوقف: {'نعم' if paused else 'لا'}\n"
            )
    ...
    def scan(self):
        ...
        for sym, data in tickers.items():
            ...
            if vol is None:
                vol = 0.0
            # شروط الماسح الموسعة
            if chg < MIN_24H_CHANGE_PERCENT or vol < max(MIN_24H_VOLUME_USD, MIN_VOLUME_USD):
                continue
            if sym not in BASE_SYMBOLS_SET:
                cap = get_market_cap_from_coingecko(sym, data)
                if cap < MIN_MARKET_CAP_USD:
                    continue
            score = min(chg/8,6) + min(np.log10(max(vol,1))/5,5)
            hl = data.get('high',1)/max(data.get('low',1),0.001)
            score += min((hl-1)*40,3)
...
@csrf.exempt
def ping():
    return jsonify({"status":"pong","version":"24.52"})

@app.route('/wakeup')
def wakeup():
    return jsonify({"status":"awake","paused":_is_paused(),"open":len(open_positions)})

@app.route('/positions_report')
@require_auth
def positions_report():
    with _global_state_lock:
        ...
        for sym,pos in open_positions.items():
            ticker = fetch_ticker_with_retry(sym)
            price = ticker['last'] if ticker else pos.entry_price
            pnl = (price-pos.entry_price)*pos.remaining_size if pos.side=='buy' else (pos.entry_price-price)*pos.remaining_size
...
@app.route('/')
def home():
    return jsonify({"status":"بوت v24.52 – ماسح موسع (حجم 75k, تغير 0.5%, كاب 1.5م)", "version":"24.52"})
...
@require_auth
def api_stats():
    total = bot_stats.total_trades
    wr = (bot_stats.winning_trades/total*100) if total>0 else 0
    positions = []
    with _global_state_lock:
        for sym,pos in open_positions.items():
            ticker = fetch_ticker_with_retry(sym)
            price = ticker['last'] if ticker else pos.entry_price
            profit = ((price-pos.entry_price)/pos.entry_price*100) if pos.side=='buy' else ((pos.entry_price-price)/pos.entry_price*100)
            positions.append({'symbol':sym,'side':pos.side,'type':pos.symbol_type,'profit':round(profit,2)})
    total_equity = get_total_equity()
    return jsonify({
        'total_pnl':bot_stats.total_pnl_usdt,
        'winning_trades':bot_stats.winning_trades,
...
def analyze():
    analyze_and_trade()
    return jsonify({"status":"تم التحليل"})

@app.route('/pause', methods=['POST'])
@require_auth
def pause():
    _set_paused(True)
    send_telegram("⏸️ تم إيقاف البوت")
    save_state()
    return jsonify({"status":"متوقف"})

@app.route('/resume', methods=['POST'])
@require_auth
def resume():
    manual_resume()
    return jsonify({"status":"مستأنف"})

@app.route('/scan')
@require_auth
def force_scan():
    scanner.scan()
    return jsonify({"candidates":scanner.candidates})

@app.route('/health')
@csrf.exempt
def health():
    return jsonify({"healthy":True,"paused":_is_paused(),"open":len(open_positions),"version":"24.52"})

# --------------------------- خيط مراقبة الأقفال وإعادة التشغيل ---------------------------
def thread_watchdog():
    essential_threads = {
    ...
    while True:
        ...
        for name in list(_essential_threads.keys()):
            ...
            t = info['thread']
            if not t.is_alive():
                logger.warning(f"⚠️ خيط {name} توقف. جارٍ إعادة التشغيل...")
                send_telegram(f"🔄 خيط {name} توقف وإعادة تشغيله تلقائياً.")
                generate_error_report("فشل_خيط", "خيوط", f"الخيط {name} توقف وأعيد تشغيله")
                new_t = threading.Thread(target=info['target'], daemon=True, name=name)
                new_t.start()
                _essential_threads[name]['thread'] = new_t

def graceful_shutdown(signum=None, frame=None):
    logger.info("إيقاف آمن...")
    save_state()
    time.sleep(2)
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    if not BINANCE_API_KEY or not BINANCE_SECRET_KEY:
        ...
        sys.exit(1)
    if not TEST_MODE and not PAPER_TRADING and ENABLE_TRADING:
        try:
            test_ex = ccxt.binance({'apiKey':BINANCE_API_KEY,'secret':BINANCE_SECRET_KEY})
            if BINANCE_SANDBOX:
                test_ex.set_sandbox_mode(True)
            test_ex.fetch_balance()
            logger.info("✅ التحقق من API ناجح")
        except Exception as e:
            logger.critical(f"❌ فشل التحقق من API: {e}")
            sys.exit(1)
    scanner = MarketScanner()
    load_state()
    sync_pending_orders()
    mode_str = "محاكاة" if TEST_MODE else ("ورقي" if PAPER_TRADING else ("Testnet" if BINANCE_SANDBOX else "حقيقي"))
    logger.info(f"🚀 بدء البوت v24.52 - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v24.52 يعمل – الوضع: {mode_str}\n✅ الماسح: أقل تغير 0.5% | أقل حجم 75k$ | كاب 1.5م | تحليل 20 رمزاً\n✅ فحص السيولة: 30% عادي / 40% ميم\n✅ تبريد أقل (0.5 ساعة، خسارة دقيقة واحدة)\n✅ فلتر الشمعة الصاعدة على 4 ساعات\n✅ نظام شك موحد (متوسط حسابي) عتبة 65%\n✅ درجة الحيتان تقدمية (0.3 لكل حوت)\n✅ إزالة الرفض المنفرد للحيتان\n✅ أمر تلغرام 'صفقاتي'")
