...
# =====================================================================
# 2. كلاس ProxyRotator (إدارة وتناوب البروكسيات)
# =====================================================================
...
class ProxyRotator:
    ...
    def _test_proxy(self, proxy):
        """اختبار بروكسي عبر إرسال طلب ping إلى Binance"""
        try:
    ...
    def _fetch_and_test_proxies(self):
        """جلب البروكسيات من جميع المصادر واختبارها"""
        logger.info("🔄 جلب البروكسيات من جميع المصادر...")
        all_new_proxies = fetch_all_free_proxies()
        if not all_new_proxies:
            logger.warning("⚠️ لم يتم العثور على أي بروكسي من أي مصدر.")
            return False
        
        test_limit = min(30, len(all_new_proxies))
        working = []
        logger.info(f"🔍 جاري اختبار {test_limit} بروكسي...")
        for proxy in all_new_proxies[:test_limit]:
            if self._test_proxy(proxy):
                working.append(proxy)
                logger.debug(f"✅ بروكسي يعمل: {proxy}")
            else:
                logger.debug(f"❌ بروكسي معطل: {proxy}")
        ...
        with self.lock:
            ...
            if working:
                ...
                logger.info(f"✅ تم العثور على {len(working)} بروكسي صالح. البروكسي الحالي: {self.current_proxy}")
                return True
            else:
                logger.warning("⚠️ لم يتم العثور على أي بروكسي صالح من بين المصادر.")
                return False

    def get_proxy(self):
        """إعادة بروكسي صالح للاستخدام."""
        with self.lock:
            now = time.time()
            if (now - self.last_fetch_time > 600) or not self.working_proxies:
                logger.info("⏳ انتهت صلاحية القائمة أو فارغة، جاري التحديث...")
                self._fetch_and_test_proxies()
                self.last_fetch_time = now
            
            if not self.working_proxies:
                all_new = fetch_all_free_proxies()
                if all_new:
                    self.all_proxies = all_new
                    self.working_proxies = all_new[:10]
                    self.current_index = 0
                    self.current_proxy = self.working_proxies[0] if self.working_proxies else None
                    logger.warning(f"⚠️ استخدام {len(self.working_proxies)} بروكسي بدون اختبار (حل طوارئ)")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "418" in str(e) or "banned" in str(e):
                ...
                continue
            if "429" in str(e):
                logger.info(f"🔄 طلب مكثف، التبديل إلى بروكسي آخر...")
                continue
...
def fetch_tickers_with_retry(max_retries=2):
    ...
    for attempt in range(max_retries):
        try:
            ...
            rest_rate_limiter.wait_if_needed(weight=20)  # وزن كبير لأنها تجلب جميع الرموز
