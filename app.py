# ========================================================================
# بوت التداول v27.10 – إصدار مستقر معدل لـ Binance Spot + Google Gemini AI
# التغييرات: إضافة نموذج VWAP+OBV كعضو خامس في لجنة الشراء مع التقطيع الثلاثي للشموع
# جميع الميزات الأخرى محفوظة كما هي
# ========================================================================
...
# ========== تم إلغاء متغيرات التبريد القديمة واستبدالها بالتبريد الذكي ==========
# BASE_COOLDOWN_HOURS, COOLDOWN_HOURS_GOOD, COOLDOWN_HOURS_BAD تم إلغاؤها
...
MEME_MOMENTUM_CHECK_MINUTES = int(os.environ.get('MEME_MOMENTUM_CHECK_MINUTES', 15))  # تم التعديل إلى 15 لتتناسب مع الإطار 15 دقيقة
MEME_MAX_NO_PROFIT_HOLD_MINUTES = int(os.environ.get('MEME_MAX_NO_PROFIT_HOLD_MINUTES', 30))
LIMIT_ORDER_SLIPPAGE = float(os.environ.get('LIMIT_ORDER_SLIPPAGE', 0.003))   # تم التعديل إلى 0.3%
...
POST_ONLY_ORDERS = os.environ.get('POST_ONLY_ORDERS', 'false').lower() == 'true'  # تم التعطيل افتراضياً لضمان التنفيذ السريع
...
# SELL_DECISION_THRESHOLD = float(...)  # تم حذفه
...
USE_1H_MULTIPLIER = True   # تفعيل المضاعف المستند إلى إطار 1 ساعة (قابل للتعديل عبر تلغرام)
...
# ========== نظام بيع 5 دقائق (لجنة الشراء على المراكز المفتوحة) ==========
SELL_5M_THRESHOLD = float(os.environ.get('SELL_5M_THRESHOLD', 0.40))  # افتراضي 0.40
# ========== أوزان لجنة البيع (نظام 5 دقائق فقط، لا تؤثر على الشراء) ==========
SELL_WEIGHTS = {
    'rule': 0.15,
    'emr': 0.15,
    'cfhm': 0.39,
...
# ============================================================
SELL_5M_CHECK_INTERVAL = 40  # كل 40 ثانية
_last_5m_sell_check = {}  # تخزين آخر وقت فحص لكل رمز
_5m_check_lock = threading.Lock()
# ============================================================
...
# ========== التعديل الرئيسي: مبادلة الإطارات الزمنية ==========
...
class RateLimiter:
    def __init__(self, max_weight=1200, period=60):
...
<head><meta charset="UTF-8"><title>بوت v27.10 (معدل لـ Spot)</title></head>
<body style="background:#0a0e27;color:#e0e0e0;font-family:sans-serif;padding:20px;">
<h1>🤖 بوت التداول v27.10 (Spot Edition)</h1>
<div id="stats"></div>
...
class VWAP_OBV_Model:
    ...
    1. VWAP: متوسط السعر المرجح بالحجم (السعر العادل).
    2. OBV: حجم التداول التراكمي (هل الحجم يدعم الاتجاه؟).
    """
    def get_score(self, df):
        if len(df) < 10:
            return 0.5  # درجة محايدة

        # 1. حساب VWAP إن لم يكن موجوداً
        if 'vwap' not in df.columns:
            typical_price = (df['high'] + df['low'] + df['close']) / 3
            df['vwap'] = (typical_price * df['volume']).cumsum() / df['volume'].cumsum()

        # 2. حساب OBV إن لم يكن موجوداً
        ...
        # 3. درجة VWAP: كلما كان السعر أقل من VWAP كانت الدرجة أفضل
        current_price = df['close'].iloc[-1]
        vwap = df['vwap'].iloc[-1]
        if vwap > 0:
            vwap_ratio = current_price / vwap
            if vwap_ratio < 0.98:
                vwap_score = 1.0   # ممتاز (السعر أقل من القيمة العادلة)
            elif vwap_ratio < 1.00:
                vwap_score = 0.7   # جيد (السعر حول القيمة العادلة)
            elif vwap_ratio < 1.02:
                vwap_score = 0.3   # ضعيف (السعر أعلى قليلاً)
            else:
                vwap_score = 0.0   # سيء جداً (السعر مرتفع جداً)
        else:
            vwap_score = 0.5

        # 4. درجة OBV: يجب أن يكون الاتجاه صاعداً
        if 'obv_trend' in df.columns and not pd.isna(df['obv_trend'].iloc[-1]):
            obv_score = 1.0 if df['obv_trend'].iloc[-1] > 0 else 0.2
        else:
            obv_score = 0.5

        # الدرجة النهائية = متوسط VWAP و OBV
        return (vwap_score + obv_score) / 2

# ======================= النموذج المتقدم الأول (القاعدة + أنماط الشموع + التباعد) =======================
class AdvancedRuleModel:
    """
    نموذج متقدم يحل محل النموذج القاعدي (Rule).
    يجمع بين: القاعدة الكلاسيكية + أنماط الشموع + التباعد السعري.
    جميع الحسابات تعتمد على بيانات الشموع والمؤشرات المحسوبة مسبقاً.
    """
    def __init__(self):
        pass
    ...
    def get_signal_percent(self, df):
        ...
        macd_diff = df['macd_diff'].iloc[-1] if 'macd_diff' in df else 0
        macd_signal = df['macd_signal'].iloc[-1] if 'macd_signal' in df else 0
        bb_upper = df['bb_upper'].iloc[-1] if 'bb_upper' in df else close*1.02
        bb_lower = df['bb_lower'].iloc[-1] if 'bb_lower' in df else close*0.98
        bb_width = df['bb_width'].iloc[-1] if 'bb_width' in df else 0.02
        vol_ratio = df['volume_ratio'].iloc[-1] if 'volume_ratio' in df else 1.0
        high20 = df['high'].iloc[-20:].max() if len(df)>=20 else close
        low20 = df['low'].iloc[-20:].min() if len(df)>=20 else close
        ...
        # 2. RSI (مع علاقة غير خطية لتجنب القمم)
        if rsi < 30:
            rsi_score = 0.8  # منطقة تشبع بيعي قوية
        elif rsi < 45:
            rsi_score = 0.6 + (rsi-30)/75  # 0.6 إلى 0.8
        elif rsi < 55:
            rsi_score = 0.5  # محايد
        elif rsi < 70:
            rsi_score = 0.4 - (rsi-55)/75  # 0.4 إلى 0.2
        else:
            rsi_score = 0.1  # تشبع شرائي شديد

        # 3. MACD
        macd_bull = 0.2 if macd_diff > macd_signal else 0.0
        ...
        # 5. البولينجر باند (السعر بالنسبة للحدود)
        if bb_width > 0.02:  # تقلب كافٍ
            if close < bb_lower:
                bb_score = 0.2  # قاع
            elif close > bb_upper:
                bb_score = 0.0  # قمة
        ...
        # 6. نسبة الحجم
        vol_score = min(0.2, (vol_ratio - 0.8) * 0.2) if vol_ratio > 0.8 else 0.0

        # 7. الموقع بالنسبة للقمة والقاع (آخر 20 شمعة)
        range_20 = high20 - low20 if high20 != low20 else 1
        pos = (close - low20) / range_20
        if pos < 0.3:
            pos_score = 0.15  # قريب من القاع
        elif pos < 0.6:
            pos_score = 0.10
        else:
            pos_score = 0.0  # قريب من القمة

        # === التحسينات الإضافية (التباعد والأنماط) ===
        div_buy, div_sell = self._detect_divergence(df)
        pat_buy, pat_sell = self._candle_patterns(df)

        buy_score = trend_bull + rsi_score*0.15 + macd_bull + adx_bull + bb_score + vol_score + pos_score + div_buy + pat_buy
        sell_score = 1.0 - buy_score + div_sell + pat_sell  # نضيف تأثير البيع مع إعادة التوازن

        buy_score = max(0.0, min(1.0, buy_score))
        sell_score = max(0.0, min(1.0, sell_score))
        return buy_score, sell_score

# ======================= النموذج المتقدم الثاني (الزخم والتدفق المالي) =======================
class AdvancedMomentumFlowModel:
    """
    نموذج متقدم للزخم والتدفق المالي (بديل EMR)
    يحلل: الزخم المُكيَّف، MFI، OBV مع التباعد، التسارع، ومرشح ADX.
    جميع الحسابات تعتمد فقط على بيانات الشموع والمؤشرات المحسوبة مسبقاً.
    """
    def __init__(self):
        pass
    ...
    def _detect_obv_divergence(self, df):
        """
        كشف تباعد بين السعر و OBV (آخر 20 شمعة)
        إشارة شراء: سعر يصنع قاع أقل لكن OBV يصنع قاع أعلى.
        إشارة بيع: سعر يصنع قمة أعلى لكن OBV يصنع قمة أقل.
        """
    ...
    def update(self, symbol, df):
        ...
        # استخراج المؤشرات المحسوبة مسبقاً من الـ DataFrame
        ...
        stoch_k = df['stoch_rsi_k'].iloc[-1] if 'stoch_rsi_k' in df else 50
        vol_ratio = df['volume_ratio'].iloc[-1] if 'volume_ratio' in df else 1.0

        # ===================== 1. الزخم المُكيَّف مع ATR + التسارع =====================
        if len(df) >= 11:
            mom5 = (df['close'].iloc[-1] - df['close'].iloc[-6]) / df['close'].iloc[-6] if df['close'].iloc[-6] != 0 else 0
            mom10 = (df['close'].iloc[-1] - df['close'].iloc[-11]) / df['close'].iloc[-11] if df['close'].iloc[-11] != 0 else 0
            # الزخم المُكيَّف (متوسط مرجح بين 5 و 10 شموع)
            adj_mom = (mom5 * 0.6 + mom10 * 0.4) / (atr_pct + 0.001)
            
            # التسارع: الفرق بين الزخم قصير المدى والزخم طويل المدى
            acceleration = (mom5 - mom10) / (atr_pct + 0.001)
        else:
            adj_mom = 0
            acceleration = 0

        # تحويل الزخم إلى درجة 0-1 باستخدام دالة سيجمويد
        mom_score = 1 / (1 + np.exp(-adj_mom * 2.5))
        mom_score = max(0.0, min(1.0, mom_score))
        
        # تحويل التسارع إلى مكافأة أو عقوبة (تصل إلى 0.15)
        accel_bonus = max(-0.15, min(0.15, acceleration * 0.5))

        # ===================== 2. MFI (مؤشر تدفق الأموال) =====================
        if mfi < 20:
            mfi_score = 0.8
        elif mfi < 40:
            mfi_score = 0.6 + (mfi - 20) / 100  # 0.6 → 0.8
        elif mfi < 60:
            mfi_score = 0.5
        elif mfi < 80:
            mfi_score = 0.4 - (mfi - 60) / 100  # 0.4 → 0.2
        else:
            mfi_score = 0.1

        # ===================== 3. RSI مع مرشح اتجاهي =====================
        if adx > 25:
            # في اتجاه قوي، نثق بالـ RSI
            if rsi < 30:
                rsi_score = 0.8
            elif rsi < 45:
                rsi_score = 0.6
            ...
            # في سوق عرضي، نكون أكثر حذراً (نتجه نحو المنتصف)
            if rsi < 40:
                rsi_score = 0.6
            elif rsi < 60:
                rsi_score = 0.5
        ...
        # ===================== 4. OBV (اتجاه الحجم) مع التباعد =====================
        obv_score = 0.0
        if obv_trend > 0:
            obv_score += 0.15
        
        # كشف التباعد بين السعر و OBV
        obv_div_buy, obv_div_sell = self._detect_obv_divergence(df)
        ...
        if vol_ratio > 1.5:
            ...
            vol_score = -0.05  # حجم ضعيف يقلل الثقة

        # ===================== 7. الدمج النهائي (أوزان ديناميكية حسب ADX) =====================
        if adx > 25:
            # في الاتجاه: نعطي وزن أكبر للزخم و RSI
            weights = {'mom': 0.35, 'rsi': 0.25, 'mfi': 0.15, 'obv': 0.10, 'stoch': 0.05, 'vol': 0.10}
        else:
            # في التذبذب: نعطي وزن أكبر لـ MFI و OBV (للكشف عن التراكم/التوزيع)
            weights = {'mom': 0.25, 'rsi': 0.20, 'mfi': 0.20, 'obv': 0.15, 'stoch': 0.10, 'vol': 0.10}

        # حساب الدرجة الخام
        buy_score = (mom_score * weights['mom'] +
                     rsi_score * weights['rsi'] +
                     mfi_score * weights['mfi'] +
                     (0.5 + obv_score) * weights['obv'] +
                     (0.5 + stoch_score) * weights['stoch'] +
                     (0.5 + vol_score) * weights['vol'])
        
        # إضافة التسارع (يضاف للشراء إذا كان إيجابياً، وللبيع إذا كان سلبياً)
        buy_score += accel_bonus * 0.5
        
        # إضافة تباعد OBV
        buy_score += obv_div_buy
        sell_bonus = obv_div_sell

        # ضبط الدرجات ضمن النطاق 0-1
        buy_score = max(0.0, min(1.0, buy_score))
        sell_score = max(0.0, min(1.0, 1.0 - buy_score + sell_bonus))
        sell_score = max(0.0, min(1.0, sell_score))
...
# ======================= نموذج توقيت الدخول الجديد (العضو المصوت الرابع) =======================
...
# --------------------------- دالة مساعدة لحساب المضاعف من درجة 1 ساعة ---------------------------
def get_multiplier_from_score(score_1h):
    """
    تستخرج المضاعف بناءً على درجة إطار 1 ساعة وفق الجدول المحدد.
    """
    if score_1h < 0.40:
        return 0.75
    elif score_1h < 0.43:
...
# --------------------------- لجنة الشراء (محسّنة مع العضو الرابع والخامس) - تم تعديل الأوزان وإزالة المضاعف القديم ---------------------------
class BuyingCommittee:
    ...
    def calculate_weighted_average(self, df, symbol, sym_type='normal', market_condition='normal'):
        """
        تحسب المتوسط المرجح للدرجات (بدون مضاعف) باستخدام نفس النماذج والأوزان.
        تعيد قاموساً يحتوي على:
        - 'avg_scores': قاموس بمتوسطات النماذج الخمسة.
        - 'weighted_avg': المتوسط المرجح النهائي.
        - 'rule_emr_sum': مجموع rule + emr (للتحقق من العتبة).
        """
        total_len = len(df)
        if total_len < 20:
            df_full = df
            ...
            df_full = df
            n_half = max(20, total_len // 2)
            n_quarter = max(20, total_len // 4)
            df_half = df.iloc[-n_half:]
            df_quarter = df.iloc[-n_quarter:]

        def _get_scores(df_local):
            scores = {}
            rule_buy, _ = self.models['rule'].get_signal_percent(df_local)
            scores['rule'] = rule_buy if rule_buy is not None else 0.5
            try:
                emr_res = self.models['emr'].update(symbol, df_local)
                scores['emr'] = emr_res['buy'] if emr_res else 0.5
            except:
                scores['emr'] = 0.5
            cfhm_buy, _ = self.models['cfhm'].calculate_scores(df_local)
            scores['cfhm'] = cfhm_buy if cfhm_buy is not None else 0.5
            scores['timing'] = self.models['timing'].get_entry_score(df_local)
            scores['vwap_obv'] = self.models['vwap_obv'].get_score(df_local)
            return scores

        scores_full = _get_scores(df_full)
        scores_half = _get_scores(df_half)
        scores_quarter = _get_scores(df_quarter)

        avg_scores = {}
        for model in ['rule', 'emr', 'cfhm', 'timing', 'vwap_obv']:
            avg_scores[model] = (scores_full[model] + scores_half[model] + scores_quarter[model]) / 3.0

        # ===== الأوزان الجديدة (Rule=15%, EMR=15%, CFHM=28%, Timing=28%, VWAP_OBV=14%) =====
        weights = {'rule': 0.15, 'emr': 0.15, 'cfhm': 0.28, 'timing': 0.28, 'vwap_obv': 0.14}
        weighted_avg = 0.0
        for model, w in weights.items():
            weighted_avg += avg_scores[model] * w
    ...
    def decide(self, df_primary, symbol, sym_type='normal', market_condition='normal'):
        """
        تقرر لجنة الشراء بناءً على الإطار الأساسي فقط (بدون مضاعف).
        تستخدم calculate_weighted_average للحصول على المتوسطات وتطبق العتبات.
        تُعيد القرار، المتوسط المرجح، الثقة، السبب، وقاموس avg_scores.
        """
        global RULE_EMR_SUM_UPPER, RULE_EMR_SUM_LOWER, RULE_EMR_SUM_FILTER_ENABLED
        
        res = self.calculate_weighted_average(df_primary, symbol, sym_type, market_condition)
        ...
        # بناء سبب القرار مع الدرجات
        adjusted_scores = [avg_scores['rule'], avg_scores['emr'], avg_scores['cfhm'], avg_scores['timing'], avg_scores['vwap_obv']]
        reason_base = (f"الدرجات (متوسط 3 نوافذ): {', '.join([f'{v:.3f}' for v in adjusted_scores])}")

        if avg > thresh and avg >= min_conf:
            return 'buy', avg, avg, f"✅ موافقة: {reason_base}", avg_scores
        else:
            if avg < min_conf:
            ...
            return 'neutral', avg, avg, f"❌ رفض: {main_reason} | {reason_base}", avg_scores

# --------------------------- لجنة البيع (تم حذفها بالكامل) ---------------------------
# تم إزالة كلاس SellingCommittee كما هو مطلوب.
...
def monitor_positions():
    ...
    global TRAILING_DISTANCE_PERCENT, SELL_5M_THRESHOLD, SELL_5M_CHECK_INTERVAL, _last_5m_sell_check
    while True:
        try:
            ...
            for sym, pos in snapshot:
                ...
                # 4️⃣ نظام بيع 5 دقائق (لجنة الشراء على المراكز المفتوحة)
                now = time.time()
                last_check = _last_5m_sell_check.get(sym, 0)
                if now - last_check >= SELL_5M_CHECK_INTERVAL:
                    try:
                        # محاولة جلب 500 شمعة من إطار 5 دقائق مع إعادة محاولة واحدة
                        df_5m = None
                        for attempt in range(2):
                            df_5m = get_cached_features(sym, '5m', limit=500, ttl=60)
                            if not df_5m.empty and len(df_5m) >= 50:
                                break
                            if attempt == 0:
                                logger.info(f"⏳ إعادة محاولة جلب 5m لـ {sym} (المحاولة 2)")
                                time.sleep(1)
                        
                        # إذا فشل الجلب أو البيانات غير كافية → نتجاوز الفحص
                        if df_5m is None or df_5m.empty or len(df_5m) < 50:
                            logger.debug(f"⚠️ تخطي فحص 5 دقائق لـ {sym} (بيانات غير كافية)")
                            _last_5m_sell_check[sym] = now
                            continue
                        ...
                        # تمرير البيانات على لجنة الشراء (نفس طريقة حساب المضاعف)
                        res = buying_committee.calculate_weighted_average(
                            df_5m, 
                            sym, 
                            pos.symbol_type, 
                            current_market
                        ...
                        # حساب درجة البيع بأوزان خاصة (لا تؤثر على لجنة الشراء)
                        score_5m = (
                        ...
                        # المقارنة بالعتبة المحددة (إذا كانت أقل → بيع كامل)
                        if score_5m < SELL_5M_THRESHOLD:
                            # بناء رسالة البيع مع عرض الدرجات الخمس
                            scores_str = (
                            ...
                            reason = (
                                f"📉 5m Committee: {score_5m:.3f} < {SELL_5M_THRESHOLD:.2f}\n"
                                f"📊 الدرجات: {scores_str}"
                            )
                            close_partial(sym, 1.0, cur_price, reason)
                            _last_5m_sell_check[sym] = now
                            continue  # انتقل إلى الرمز التالي
                        
                        # تحديث وقت الفحص بعد التنفيذ الناجح
                        _last_5m_sell_check[sym] = now
                        
                    except Exception as e:
                        logger.warning(f"⚠️ فشل فحص 5 دقائق لـ {sym}: {e}")
                        _last_5m_sell_check[sym] = now  # منع تكرار المحاولات الفاشلة
...
# --------------------------- دالة التحليل الرئيسية (معدلة لاستخدام المضاعف الجديد وعرض الدرجات) ---------------------------
    ...
    try:
        ...
        for sym in active:
            ...
            if sym in open_positions:
                ...
                ttl_used = 120
            df_primary = get_cached_features(sym, TIMEFRAMES['primary'], limit=500, ttl=ttl_used)
        ...
        for sym, sym_type, df_primary, regime in candidates_for_manipulation:
            ...
            # ===== حساب score_15m و score_1h باستخدام الدالة الجديدة =====
            res_15m = buy_comm.calculate_weighted_average(df_primary, sym, sym_type, market_cond)
            score_15m = res_15m['weighted_avg']
            avg_scores_15m = res_15m['avg_scores']
            
            # جلب إطار 1 ساعة
            df_1h = get_cached_features(sym, '1h', limit=500, ttl=ttl_used)
            if df_1h.empty or len(df_1h) < 50:
                # إذا تعذر جلب 1 ساعة، نستخدم مضاعف افتراضي 0.8 (لأن البيانات غير مؤكدة)
                final_score = score_15m
                multiplier = 0.8
                avg_scores_1h = None
            else:
                res_1h = buy_comm.calculate_weighted_average(df_1h, sym, sym_type, market_cond)
                score_1h = res_1h['weighted_avg']
                avg_scores_1h = res_1h['avg_scores']
                # ===== تعديل المضاعف بناءً على مجموع (Rule+EMR) على 1 ساعة =====
                rule_emr_sum_1h = res_1h['rule_emr_sum']
                if rule_emr_sum_1h < 1.3:
                    multiplier = 0.8
                else:
                    multiplier = get_multiplier_from_score(score_1h)
                # =============================================================
                if USE_1H_MULTIPLIER:
                    final_score = score_15m * multiplier
                else:
                    final_score = score_15m
            
            # ===== التحقق من عتبة لجنة الشراء باستخدام final_score =====
            dec, _, _, _, _ = buy_comm.decide(df_primary, sym, sym_type, market_cond)
            if dec != 'buy':
                continue
            
            # ===== التحقق من عتبة النوع (ميم/عادية) =====
            ...
            # بناء سبب القبول مع الدرجات من كلا الإطارين
            d1_15, d2_15, d3_15, d4_15, d5_15 = avg_scores_15m['rule'], avg_scores_15m['emr'], avg_scores_15m['cfhm'], avg_scores_15m['timing'], avg_scores_15m['vwap_obv']
            if avg_scores_1h is not None:
                d1_1h, d2_1h, d3_1h, d4_1h, d5_1h = avg_scores_1h['rule'], avg_scores_1h['emr'], avg_scores_1h['cfhm'], avg_scores_1h['timing'], avg_scores_1h['vwap_obv']
                reason = (f"✅ موافقة: المتوسط={final_score:.3f} | العتبة={thresh:.3f} | المضاعف={multiplier:.2f}x | "
                          f"\nدرجات 15 دقيقة (متوسط 3 نوافذ): {d1_15:.3f}, {d2_15:.3f}, {d3_15:.3f}, {d4_15:.3f}, {d5_15:.3f}"
                          f"\nدرجات 1 ساعة (متوسط 3 نوافذ): {d1_1h:.3f}, {d2_1h:.3f}, {d3_1h:.3f}, {d4_1h:.3f}, {d5_1h:.3f}")
            else:
                reason = (f"✅ موافقة: المتوسط={final_score:.3f} | العتبة={thresh:.3f} | المضاعف={multiplier:.2f}x | "
                          f"\nدرجات 15 دقيقة (متوسط 3 نوافذ): {d1_15:.3f}, {d2_15:.3f}, {d3_15:.3f}, {d4_15:.3f}, {d5_15:.3f}")

            extra = final_score * 100 * 0.35 + (score_1h if 'score_1h' in locals() else final_score) * 30 + (2 if sym_type == 'meme' else 0)
            pending.append({
                'symbol': sym,
                'score': extra,
                'price': cur_price,
...
def background_balance_updater():
    ...
    while True:
        if ENABLE_TRADING and not TEST_MODE and not PAPER_TRADING:
            ...
            get_total_equity()
        time.sleep(120)  # تم التمديد إلى 120 ثانية
...
# --------------------------- دالة telegram_polling المعدلة (مع إضافة أوامر التحكم بالمضاعف) ---------------------------
def telegram_polling():
    global RULE_EMR_SUM_THRESHOLD, RULE_EMR_SUM_UPPER, RULE_EMR_SUM_LOWER, RULE_EMR_SUM_FILTER_ENABLED
    global _last_telegram_update_id
    ...
    global TRAILING_DISTANCE_PERCENT, SELL_5M_THRESHOLD
    global PAUSE_ANALYSIS
                        ...
                        if msg and str(msg["chat"]["id"]) == str(TELEGRAM_CHAT_ID):
                            ...
                            # ===== تم حذف أمر 'عتبة البيع' =====
                                ...
                                status = (f"📊 <b>حالة الفلاتر والقيم الحالية</b>\n━━━━━━━━━━━━━━━━━━━━\n"
                                          ...
                                          f"🔁 <b>المضاعف (1 ساعة):</b> {'✅ مفعل' if USE_1H_MULTIPLIER else '❌ معطل'}\n"
                                          f"📊 <b>نسبة تتبع الأرباح (الوقف المتحرك):</b> {TRAILING_DISTANCE_PERCENT:.2%}\n"
                                          f"📉 <b>عتبة بيع 5 دقائق (للمراكز المفتوحة):</b> {SELL_5M_THRESHOLD:.2%}\n"
                                          f"━━━━━━━━━━━━━━━━━━━━\n"
                            ...
                            # ------------------------------------------------------------
                            # 8. أوامر رفع/خفض عتبات الاشتباه (تم حذف أوامر البيع)
                            # ------------------------------------------------------------
                            ...
                            # تم حذف أوامر رفع/خفض عتبة البيع
                            
                            # ------------------------------------------------------------
                            # 9. أوامر تعيين عتبات الخسارة الوقائية
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 16. أمر تعيين عتبة بيع 5 دقائق (معدل)
                            # ------------------------------------------------------------
                            elif text.startswith('تعيين عتبة بيع'):
                                numbers = re.findall(r'(\d+(?:\.\d+)?)', text)
                                if numbers:
                                    new_val = float(numbers[-1])
                                    if 0.10 <= new_val <= 0.90:
                                        SELL_5M_THRESHOLD = new_val
                                        save_state()
                                        send_telegram(f"✅ تم تعيين عتبة بيع 5 دقائق إلى {new_val:.2f}")
                                    else:
                                        send_telegram("⚠️ القيمة يجب أن تكون بين 0.10 و 0.90")
                                else:
                                    send_telegram("⚠️ الصيغة غير صحيحة. استخدم: `تعيين عتبة بيع 5 دقائق 0.35`")

                            # ------------------------------------------------------------
                            # 17. أمر إعادة ضبط الفلاتر (محدث)
                            # ------------------------------------------------------------
                                ...
                                # إعادة ضبط التتبع وبيع 5 دقائق
                                TRAILING_DISTANCE_PERCENT = 0.01
                                SELL_5M_THRESHOLD = 0.40
                                # إعادة ضبط PAUSE_ANALYSIS إلى False (تشغيل التحليل)
                                PAUSE_ANALYSIS = False
                                send_telegram("🟢 تم إعادة ضبط جميع الفلاتر والإعدادات الديناميكية إلى القيم الافتراضية الأصلية (الذكاء الاصطناعي معطل، النموذج الافتراضي Flash، نظام التبريد الذكي مفعّل بـ 12 ساعة للرابح و 5 دقائق للخاسر، وعتبات النموذجين: الأعلى 1.5 والأدنى 1.3، وتحديث الوقف مفعّل، نسبة تتبع الأرباح 1%، عتبة بيع 5 دقائق 0.40، التحليل مفعّل)")
                                save_state()
                            
                            # ------------------------------------------------------------
                            # 18. أمر رسم منحنى الرصيد الكلي
                            # ------------------------------------------------------------
                            ...
                            # ------------------------------------------------------------
                            # 19. أوامر إيقاف دخول الصفقات الجديدة
                            # ------------------------------------------------------------
                            ...
                            # ============================================================
                            # 20. أوامر تشغيل/إيقاف المضاعف
                            # ============================================================
                            elif text == 'شغل المضاعف':
                                USE_1H_MULTIPLIER = True
                                save_state()
                                send_telegram("✅ تم **تفعيل** المضاعف المستند إلى إطار 1 ساعة.\nسيتم ضرب درجة الثقة النهائية بالمضاعف المستخلص من تحليل الإطار التأكيدي.")
                            
                            elif text == 'أوقف المضاعف':
                                USE_1H_MULTIPLIER = False
                                save_state()
                                send_telegram("⛔ تم **إيقاف** المضاعف المستند إلى إطار 1 ساعة.\nستُستخدم درجة الثقة النهائية كما هي من الإطار الأساسي (15 دقيقة) دون أي تعديل.")
    ...
    try:
        ...
        data = {
            ...
            'filters': {
                ...
                'sell_5m_threshold': SELL_5M_THRESHOLD,
            },
            'custom_settings': {
...
def load_state():
    ...
    global FILTER_MANIPULATION_ENABLED, FILTER_LIQUIDITY_ENABLED, FILTER_MARKET_CAP_ENABLED
    global FILTER_VOLUME_24H_ENABLED, FILTER_CHANGE_24H_ENABLED, FILTER_HOUR_CANDLE_ENABLED
    # FILTER_4H_CANDLE_ENABLED محذوف
    ...
    try:
        ...
        # FILTER_4H_CANDLE_ENABLED تم حذفه
        ...
        # تم حذف SELL_DECISION_THRESHOLD
        ...
        SELL_5M_THRESHOLD = filters.get('sell_5m_threshold', 0.40)
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 بدء البوت v27.10 (Spot + Gemini AI + تبريد ذكي + VWAP/OBV + تقطيع ثلاثي) - الوضع: {mode_str}")
    send_telegram(f"🚀 بوت v27.10 (الإصدار المعدل لـ Binance Spot + Gemini AI + تبريد ذكي + VWAP/OBV + تقطيع ثلاثي) يعمل – الوضع: {mode_str}\n✅ تحكم كامل بالفلاتر والعتبات عبر تلغرام\n✅ أمر 'رصيدي' لعرض الرصيد الحر + قيمة المراكز\n✅ تقرير يومي يعرض صافي الربح/الخسارة\n✅ التحويل بين التداول الورقي والحقيقي: `تداول حقيقي` / `تداول ورقي`\n✅ أمر `منحنى` أو `رسم` لإرسال صورة بيانية لمنحنى الرصيد الكلي\n✅ أمر `عدد الصفقات المسموحة` للتحكم بالحد اليومي للصفقات\n✅ تحسينات كبيرة في الذاكرة ومنع التسريب\n✅ WebSocket خالص مع إعادة تشغيل تلقائي\n✅ مراقبة وإعادة تشغيل جميع الخيوط بما فيها WebSocket (مع حماية watchdog)\n✅ جلب صبور للرصيد الحقيقي (حتى 120 ثانية)\n✅ سرعة مراقبة المراكز 1 ثانية\n✅ التحكم بدرجة الاشتباه: `رفع عتبة الاشتباه 70%` أو `خفض عتبة الاشتباه 50%`\n✅ التحكم بعتبات الخسارة الوقائية:\n   - `تعيين عتبة خسارة 25% 0.9`\n   - `تعيين عتبة خسارة 33% 1.5`\n   - `تعيين عتبة خسارة 100% 2.0`\n   - `عتبات الخسارة` لعرض القيم الحالية\n✅ البيع التدريجي الصارم (بدون إغلاق مبكر بسبب المبلغ الصغير)\n✅ نسب البيع: 15%, 15%, 15%, 15%, 20%, 50%, 100%\n✅ دعم أفضل لمنصة Render (منفذ 8080)\n✅ **إصلاح ذاتي كامل - لا حاجة لإعادة تشغيل خارجية**\n✅ **الوقف المسبق الموحد للعادي والميم: stop_loss_limit (مدعوم لجميع أزواج Spot)**\n✅ **خيط auto_recovery خفيف (بدون إعادة تحميل الأسواق) - مع منطق الانتظار 12 دقيقة ثم 3 محاولات قبل الاسترداد**\n✅ **تحسين حساب قيمة المراكز المفتوحة في التداول الورقي والحقيقي مع فاصل زمني 5 ثوانٍ وإعادة محاولات واعتماد على WebSocket**\n✅ **إصلاح تقلص حجم الصفقات المتزامنة (تم تعديل حساب total_eq2 مرة واحدة قبل حلقة الفرص)**\n✅ **التحكم الديناميكي بعدد الرموز وحجم التعرض وحجم الصفقة حسب حالة السوق عبر أوامر تلغرام**\n✅ **منع البيع الجزئي الصغير جداً (أقل من min_cost) تلقائياً**\n✅ **ترقيم أهداف الربح بشكل متسلسل (الهدف الأول، الثاني، ...)**\n✅ **إضافة جميع أوامر v27.7 المفقودة (تشغيل/إيقاف الفلاتر الفردية، تعديل عتبات الشراء، فترة التبريد، مدة المسح)**\n✅ **إضافة أمر تلغرام للتحكم بعدد الصفقات اليومية المسموحة**\n✅ **تحسين تنفيذ الأوامر: هامش ±0.3%، مهلة 10 ثوانٍ، فاصل تحقق 1 ثانية، تحول إلى أمر سوق تلقائي مع إعادة جلب الحالة قبل الإلغاء**\n✅ **إصلاح خطير: استخدام stop_loss_limit بدلاً من stop_loss لضمان التوافق مع سوق Spot**\n✅ **إضافة فلتر Google Gemini مع نموذجين (Flash و Flash-Lite) والتحكم بهما عبر تلغرام**\n✅ **تحسينات شاملة للذكاء الاصطناعي: مهلة 65 ثانية، إعادة محاولة واحدة للأخطاء المؤقتة، إشعارات القبول والرفض مع السبب، اختبار المفتاح عند بدء التشغيل، استخدام بيانات كاملة، تسجيل الموافقة في سجل التداول**\n✅ **فلتر أداء X يوم (جديد) مع تحكم كامل بعدد الأيام والعتبة عبر تلغرام**\n✅ **إيقاف دخول الصفقات الجديدة (بدون إغلاق المراكز المفتوحة) عبر أمر `توقف عن دخول الصفقات`**\n✅ **تحسين حالة السوق باستخدام نسبة الرابحين/الخاسرين (Advance/Decline Ratio)**\n✅ **تغيير نموذج التأكيد في لجنة الشراء من rule إلى emr**\n✅ **زيادة فترة الزخم في النموذج القاعدي من 6 إلى 12 شمعة**\n✅ **إضافة أوزان مرجحة للنماذج في لجنة الشراء (أوزان مختلفة للـ meme والعادي)**\n✅ **حل مشكلة البداية الباردة لنموذج EMR بإرجاع 0.5 بدلاً من None**\n✅ **إضافة سبب القبول إلى رسالة الشراء على تلغرام**\n✅ **التحكم الديناميكي بفترة الانتظار قبل البيع الكامل عبر أمر `فترة الانتظار X دقيقة`**\n✅ **إضافة نموذج توقيت الدخول (EntryTimingModel) كعضو مصوت رابع في لجنة الشراء**\n✅ **استبدال نموذج EMR القديم بالنموذج الهجين HybridMomentumRSIModel**\n✅ **تعديل علاقة RSI في النموذج الهجين إلى علاقة غير خطية لتجنب التفاؤل المفرط في القمم**\n✅ **تعديل أوزان لجنة الشراء إلى: CFHM 30%، Rule 10%، Hybrid 15%، Timing 30%، VWAP_OBV 15%**\n✅ **نظام تبريد ذكي جديد (قابل للتعديل عبر تلغرام): 12 ساعة للصفقة الرابحة، 5 دقائق للصفقة الخاسرة**\n✅ **أوامر تلغرام للتحكم بالتبريد: `تعيين تبريد الرابح X ساعة` و `تعيين تبريد الخاسر X دقيقة` و `عرض التبريد`**\n✅ **إضافة نموذج VWAP+OBV كعضو مصوت خامس في لجنة الشراء بوزن 15%**\n✅ **تطبيق التقطيع الثلاثي للشموع (كامل، نصف، ربع) مع حساب المتوسط الحسابي للدرجات**\n✅ **التقطيع الديناميكي يتكيف مع أي عدد من الشموع (حد أدنى 20 شمعة)**\n✅ **عرض الدرجة الخامسة في رسالة الشراء مع الإشارة إلى أنها متوسط 3 نوافذ**\n✅ **عتبات مجموع النموذجين (Rule+EMR) كحد أدنى وحد أعلى مع تحكم كامل عبر تلغرام**\n✅ **التحكم بتحديث الوقف المسبق الحقيقي (تشغيل/إيقاف، نسبة التفعيل، ونسبة التنفيذ) عبر تلغرام**\n✅ **مضاعف جديد مستند إلى إطار 1 ساعة مع تحكم عبر تلغرام: `شغل المضاعف` / `أوقف المضاعف**\n✅ **نظام تتبع الأرباح الفوري (الوقف المتحرك) يُفعّل من اللحظة الأولى بنسبة قابلة للتعديل عبر تلغرام**\n✅ **نظام بيع 5 دقائق للمراكز المفتوحة باستخدام لجنة الشراء وإعادة محاولة جلب الشموع وعرض الدرجات في رسالة البيع**\n✅ **أمر `تصفير التبريد` لمسح كل التبريدات**\n✅ **أمر `إيقاف التحليل` و `تشغيل التحليل` للتحكم في التحليل والمسح دون التأثير على المراكز المفتوحة**\n✅ **تم حذف فلتر شمعة 4 ساعات بالكامل**")
